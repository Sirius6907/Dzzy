...
const BOT_ABOUT: &str =
    "A tiny non-AI Dzzy reference bot that replies to !countdown and countdown-style !fib.";
...
impl Config {
    fn from_env() -> Result<Self> {
        let relay_url =
            std::env::var("DZZY_RELAY_URL").unwrap_or_else(|_| DEFAULT_RELAY_URL.to_string());
        let channel_id = required_env("DZZY_CHANNEL_ID")?;
        let bot_keys = Keys::parse(&required_env("DZZY_BOT_PRIVATE_KEY")?)
            .context("DZZY_BOT_PRIVATE_KEY must be an nsec or hex private key")?;

        let auth_mode =
            std::env::var("DZZY_BOT_AUTH_MODE").unwrap_or_else(|_| "standalone".to_string());
        let owner_auth_tag = match auth_mode.as_str() {
            "standalone" => None,
            "owner-attested" => {
                let tag_json = match std::env::var("DZZY_AUTH_TAG") {
                    Ok(value) if !value.trim().is_empty() => value,
                    _ => {
                        let owner_keys = Keys::parse(&required_env("DZZY_OWNER_PRIVATE_KEY")?)
                            .context("DZZY_OWNER_PRIVATE_KEY must be an nsec or hex private key")?;
                        dzzy_sdk::nip_oa::compute_auth_tag(&owner_keys, &bot_keys.public_key(), "")?
                    }
                };

                let owner = dzzy_sdk::nip_oa::verify_auth_tag(&tag_json, &bot_keys.public_key())
                    .context("DZZY_AUTH_TAG is not valid for DZZY_BOT_PRIVATE_KEY")?;
                eprintln!("owner-attested auth tag verified; owner={}", owner.to_hex());
                Some(dzzy_sdk::nip_oa::parse_auth_tag(&tag_json)?)
            }
            other => {
                bail!("DZZY_BOT_AUTH_MODE must be 'standalone' or 'owner-attested', got {other:?}")
            }
        };
...
async fn publish_profile(ws: &mut Ws, config: &Config) -> Result<()> {
    let builder = dzzy_sdk::builders::build_profile(
...
async fn maybe_reply(
    ...
    let builder = dzzy_sdk::builders::build_message(
