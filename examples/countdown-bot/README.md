...
A tiny non-AI Dzzy bot example.

The bot is deliberately boring and algorithmic: it listens to one Dzzy channel
...
It demonstrates that Dzzy participants do not have to be LLM agents. Any
...
members list and in Dzzy's mention autocomplete.
...
```bash
DZZY_RELAY_URL=ws://localhost:3000 \
DZZY_CHANNEL_ID=<channel-uuid> \
DZZY_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
DZZY_BOT_AUTH_MODE=standalone \
...
relay. This reuses the same owner-attestation credential path that Dzzy agents
...
DZZY_CHANNEL_ID=<channel-uuid> \
DZZY_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
DZZY_OWNER_PRIVATE_KEY=<owner-or-agent-nsec-or-hex-secret> \
DZZY_BOT_AUTH_MODE=owner-attested \
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```
...
```bash
DZZY_AUTH_TAG='["auth","<owner-pubkey>","","<sig>"]' \
DZZY_BOT_AUTH_MODE=owner-attested \
# plus DZZY_RELAY_URL, DZZY_CHANNEL_ID, DZZY_BOT_PRIVATE_KEY
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```

Relay requirements for this path:

- `DZZY_REQUIRE_RELAY_MEMBERSHIP=true` on closed relays.
- `DZZY_ALLOW_NIP_OA_AUTH=true` so owner-attested non-member bot keys can be
  admitted.
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The Dzzy UI adds that tag when the bot is selected from
