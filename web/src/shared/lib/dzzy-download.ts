export const DZZY_RELEASES_URL = "https://github.com/block/dzzy/releases";
const DZZY_RELEASES_API_URL =
  "https://api.github.com/repos/block/dzzy/releases?per_page=10";
const CACHE_KEY = "dzzy.latestDownload.v1";
const CACHE_TTL_MS = 60 * 60 * 1000;

export type DzzyDownloadPlatform = {
...
): DzzyDownloadPlatform["operatingSystem"] {
...
function normalizeArchitecture(
  value: string,
): DzzyDownloadPlatform["architecture"] {
...
export async function detectDzzyDownloadPlatform(
  navigatorValue: Navigator,
): Promise<DzzyDownloadPlatform> {
  const userAgentData = (
...
function assetPattern(platform: DzzyDownloadPlatform): RegExp | undefined {
...
export function selectDzzyDownloadUrl(
  releases: GitHubRelease[],
  platform: DzzyDownloadPlatform,
): string | undefined {
...
export async function resolveDzzyDownloadUrlForPlatform(
  platform: DzzyDownloadPlatform,
): Promise<string> {
  try {
  ...
  try {
    const response = await fetch(DZZY_RELEASES_API_URL, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!response.ok) return DZZY_RELEASES_URL;
    const url = selectDzzyDownloadUrl(
      (await response.json()) as GitHubRelease[],
      platform,
    );
    if (!url) return DZZY_RELEASES_URL;
    try {
    ...
    return DZZY_RELEASES_URL;
  }
}

export async function resolveDzzyDownloadUrl(): Promise<string> {
  return resolveDzzyDownloadUrlForPlatform(
    await detectDzzyDownloadPlatform(navigator),
  );
}
