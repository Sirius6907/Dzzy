import dzzyAppIcon from "@/assets/app-icon@3x.png";
import { claimInviteInBrowser } from "@/features/invite/invite-api";
import {
  DZZY_RELEASES_URL,
  type DzzyDownloadPlatform,
  detectDzzyDownloadPlatform,
  resolveDzzyDownloadUrlForPlatform,
} from "@/shared/lib/dzzy-download";
...
export function InvitePage({ code }: { code: string }) {
  ...
  const [downloadUrl, setDownloadUrl] = React.useState(DZZY_RELEASES_URL);
  ...
  React.useEffect(() => {
    let active = true;
    detectDzzyDownloadPlatform(navigator).then(async (platform) => {
      if (!active) return;
      if (
      ...
      const url = await resolveDzzyDownloadUrlForPlatform(platform);
  ...
  const openInvite = async () => {
    ...
    try {
      ...
      window.location.href = `dzzy://join?${query.toString()}`;
    } finally {
      setOpening(false);
    }
  };
    ...
    >
      <div className="w-full max-w-xl space-y-4">
        <div className="flex w-full flex-col items-center rounded-3xl bg-white px-6 py-10 sm:px-12 sm:py-12">
          ...
          >
            <img alt="Dzzy" className="h-full w-full" src={dzzyAppIcon} />
          </div>
          ...
          <div className="mt-9 w-full max-w-md space-y-2">
            ...
            {policy === null ? (
              ...
              >
                <a
                  href={`dzzy://join?relay=${encodeURIComponent(relay)}&code=${encodeURIComponent(code)}`}
                >
                  Accept invite in Dzzy
                </a>
              </Button>
            ) : (
              <Button
