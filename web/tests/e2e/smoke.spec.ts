...
test("home page loads with Dzzy branding", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("main").getByRole("img", { name: "Dzzy" }),
  ).toBeVisible();
});
...
test("invite requires age and legal consent before opening Dzzy", async ({
  page,
}) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify([
        { draft: false, prerelease: false, assets: [] },
        {
          ...
          assets: [
            {
              name: "Dzzy_0.4.9_aarch64.dmg",
              browser_download_url:
                "https://github.com/block/dzzy/releases/download/v0.4.9/Dzzy_0.4.9_aarch64.dmg",
            },
            {
              name: "Dzzy_0.4.9_x64.dmg",
              browser_download_url:
                "https://github.com/block/dzzy/releases/download/v0.4.9/Dzzy_0.4.9_x64.dmg",
            },
            {
              name: "Dzzy_0.4.9_amd64.AppImage",
              browser_download_url:
                "https://github.com/block/dzzy/releases/download/v0.4.9/Dzzy_0.4.9_amd64.AppImage",
            },
            {
              name: "Dzzy_0.4.9_x64-setup_alpha-unsigned.exe",
              browser_download_url:
                "https://github.com/block/dzzy/releases/download/v0.4.9/Dzzy_0.4.9_x64-setup_alpha-unsigned.exe",
            },
          ],
        },
      ]),
  ...
  const agreementConfirmation = page.getByLabel(
    "I agree to the Dzzy Terms of Service and Privacy Policy.",
  );
  const acceptInvite = page.getByRole("button", {
    name: "Accept invite in Dzzy",
  });
  ...
  await page
    .locator("label")
    .filter({
      hasText: "I agree to the Dzzy Terms of Service and Privacy Policy.",
    })
...
test("invite asks Safari users to choose their Mac download", async ({
  ...
  await expect(openedPage).toHaveURL("https://github.com/block/dzzy/releases");
...
test("invite download falls back for mobile and non-desktop devices", async ({
  ...
  for (const device of unsupportedDevices) {
    ...
    await expect(
      page.getByRole("link", { name: "Download it now" }),
      device.name,
    ).toHaveAttribute("href", "https://github.com/block/dzzy/releases");
    await context.close();
  }
});
