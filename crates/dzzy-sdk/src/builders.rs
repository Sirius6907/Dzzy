...
///
/// Shortcodes are case-insensitive in Dzzy's relay-global set; lowercase
...
/// Build a Dzzy-native delete event (kind 9005).
...
/// Build a Dzzy-native delete event (kind 9005) with optional moderation metadata.
...
pub const CUSTOM_EMOJI_SET_D_TAG: &str = "dzzy:custom-emoji";
...
/// "dzzy:custom-emoji")`. Replaces the caller's prior set. The workspace
...
pub fn build_update_channel(
    ...
    if name
        .map(dzzy_core::channel::canonical_channel_name)
...
mod tests {
    ...
    fn repo_announcement_with_tags_preserves_metadata_and_canonicalizes_d() {
        let tags = vec![
            ...
            Tag::parse(["dzzy-channel", "channel-id"]).unwrap(),
        ...
        assert!(has_tag(&ev, "dzzy-channel", "channel-id"));
    ...
    fn git_patch_rejects_whitespace_only_content() {
        // Regression: a failed `git format-patch | dzzy patches send
