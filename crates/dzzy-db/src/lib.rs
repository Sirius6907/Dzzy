#![deny(unsafe_code)]
#![warn(missing_docs)]
//! dzzy-db — Postgres event store for Dzzy.
//!
//! ## Design invariants
...
pub mod partition;
/// Dzzy product-feedback sidecar persistence.
...
use dzzy_core::{CommunityId, StoredEvent};
...
impl Default for DbConfig {
    ...
    fn default() -> Self {
        Self {
            database_url: "postgres://dzzy:dzzy_dev@localhost:5432/dzzy".to_string(), // sadscan:disable np.postgres.1
...
impl Db {
    ...
    /// `dzzy.created_at_floor` GUC — this is what makes the replica fence
    ...
    ///
    /// `arm_floor_guard` sets the `dzzy.created_at_floor` session GUC on
        ...
        if arm_floor_guard {
            options = options.after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    sqlx::query("SELECT set_config('dzzy.created_at_floor', $1, false)")
    ...
    /// the migration decision. On a relay with `DZZY_AUTO_MIGRATE` off, the
    ...
    /// request/connection error. dzzy-db only reads the durable host map.
    ...
    /// fully-formed [`dzzy_core::tenant::TenantContext`] — host included — to
    ...
    ///
    /// Used by the runtime conformance read-seam emitters in `dzzy-relay`:
    ...
    pub async fn replace_addressable_event(
        ...
        let kind_i32 = dzzy_core::kind::event_kind_i32(event);
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![dzzy_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = dzzy_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
    ...
    /// ordering watermark. Dzzy mesh status coordinates also hard-delete their
    ...
    pub async fn replace_parameterized_event(
        ...
        let read_state_t_tag_count = event
            ...
            .count();
        let is_nip_rs = kind_i32 == dzzy_core::kind::KIND_READ_STATE as i32
            && d_tag_count == 1
            && has_exact_d_tag
        ...
        let is_dzzy_mesh_status = kind_i32 == dzzy_core::kind::KIND_BOOKMARK_SET as i32
            && d_tag.starts_with("dzzy-mesh-member-status:")
            && event.tags.iter().any(|tag| {
                let parts = tag.as_slice();
                parts.len() == 2 && parts[0] == "k" && parts[1] == "dzzy-mesh-status"
            });
        let hard_delete_superseded = is_nip_rs || is_dzzy_mesh_status;
        ...
        if existing.is_some() {
            if is_nip_rs {
                ...
                sqlx::query("SELECT set_config('dzzy.nip_rs_hard_delete', 'on', true)")
                    .execute(&mut *tx)
                    .await?;
            }
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://dzzy:dzzy_dev@localhost:5432/dzzy";
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let old = EventBuilder::new(Kind::Custom(dzzy_core::kind::KIND_READ_STATE as u16), "old")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old");
        let new = EventBuilder::new(Kind::Custom(dzzy_core::kind::KIND_READ_STATE as u16), "new")
            .tags(tags)
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let d_tag = "dzzy-mesh-member-status:owner-test";
        let tags = vec![
            Tag::parse(["d", d_tag]).expect("d tag"),
            Tag::parse(["k", "dzzy-mesh-status"]).expect("k tag"),
        ];
        let base = Timestamp::now().as_secs();
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(dzzy_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
    ...
    async fn duplicate_nip_rs_discriminator_tags_keep_legacy_retention() {
        ...
        for (case, tags) in [
            ...
            let old = EventBuilder::new(
                Kind::Custom(dzzy_core::kind::KIND_READ_STATE as u16),
                format!("{case}-old"),
            )
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
    ...
    async fn nip_rs_hard_delete_fence_fails_closed_and_scopes_opt_in_to_transaction() {
        ...
        for commit in [true, false] {
            ...
            let value: String =
                sqlx::query_scalar("SELECT set_config('dzzy.nip_rs_hard_delete', 'on', true)")
                    .fetch_one(&mut *tx)
                    .await
            ...
            let leaked: Option<String> = sqlx::query_scalar(
                "SELECT NULLIF(current_setting('dzzy.nip_rs_hard_delete', true), '')",
            )
            .fetch_one(&mut *conn)
            .await
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        let base = Timestamp::now().as_secs();
        let a = EventBuilder::new(Kind::Custom(dzzy_core::kind::KIND_READ_STATE as u16), "A")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign A");
        let x = EventBuilder::new(Kind::Custom(dzzy_core::kind::KIND_READ_STATE as u16), "X")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign X");
        let b = EventBuilder::new(Kind::Custom(dzzy_core::kind::KIND_READ_STATE as u16), "B")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 2))
            .sign_with_keys(&keys)
            .expect("sign B");
        let c = EventBuilder::new(Kind::Custom(dzzy_core::kind::KIND_READ_STATE as u16), "C")
            .tags(tags)
        ...
        async fn legacy_insert(
            ...
            .bind(dzzy_core::kind::KIND_READ_STATE as i32)
    ...
    async fn created_at_floor_guard_aborts_old_channel_rows_at_commit() {
        ...
        let insert_raw = |ev: nostr::Event, channel_id: Option<Uuid>| {
            ...
            async move {
                ...
                sqlx::query("SELECT set_config('dzzy.created_at_floor', $1, true)")
    ...
    async fn armed_pool_rejects_old_channel_inserts_through_public_api() {
        ...
        let effective: String = sqlx::query_scalar("SHOW dzzy.created_at_floor")
            .fetch_one(&db.pool)
            .await
    ...
    /// `DZZY_AUTO_MIGRATE` off: an armed GUC with no enforcing trigger must
    ...
    async fn fence_probe_refuses_to_start_without_verified_floor_guard() {
        ...
        // Sabotage B: trigger dropped entirely (the DZZY_AUTO_MIGRATE=off /
