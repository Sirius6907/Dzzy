//! `dzzy-test-cli` — Manual testing CLI for the Dzzy relay.
//!
//! # Usage
//!
//! ```text
//! dzzy-test-cli [OPTIONS]
//!
//! Options:
...
//! Send a message:
//! ```text
//! dzzy-test-cli --channel my-channel --send "Hello, Dzzy!"
//! ```
//!
//! Subscribe and watch events:
//! ```text
//! dzzy-test-cli --channel my-channel --subscribe
//! ```
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "dzzy_test_client=debug".to_string())
                .as_str(),
        )
        .init();
    ...
    let keys = match std::env::var("DZZY_PRIVATE_KEY") {
        Ok(sk) => Keys::parse(&sk).expect("invalid DZZY_PRIVATE_KEY"),
        Err(_) => Keys::generate(),
    };
...
async fn run_send(url: &str, keys: &Keys, channel: &str, message: &str, kind: u16) {
    ...
    let mut client = match DzzyTestClient::connect(url, keys).await {
        Ok(c) => c,
        Err(e) => {
...
async fn run_subscribe(url: &str, keys: &Keys, channel: &str, kind: u16) {
    ...
    loop {
        match client.recv_event(Duration::from_secs(30)).await {
            ...
            Ok(_) => {}
            Err(dzzy_test_client::TestClientError::Timeout) => {
                // Keep waiting.
            }
            Err(e) => {
                eprintln!("Error: {e}");
...
fn print_help() {
    println!(
        r#"dzzy-test-cli — Manual testing CLI for the Dzzy relay
...
EXAMPLES:
    ...
    dzzy-test-cli --url ws://relay.example.com --channel test --subscribe
"#
    );
}
