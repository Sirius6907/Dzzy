//! End-to-end acceptance tests for Dzzy shared compute.
//!
//! These tests require a membership-gated dzzy-relay and a mesh-enabled desktop
...
//! | 5 | dropped member → typed auth failure reaches lastError | runbook (desktop harness) | dzzy-agent `-32001` unit |
...
const KIND_DZZY_MESH_MEMBER_STATUS: u16 = 30003;
const MESH_STATUS_D_TAG_PREFIX: &str = "dzzy-mesh-member-status:";
const MESH_STATUS_TYPE: &str = "dzzy-mesh-status";
...
fn mesh_status_filter() -> Filter {
    Filter::new()
        .kind(Kind::Custom(KIND_DZZY_MESH_MEMBER_STATUS))
...
async fn trust_member_reads_mesh_status() {
    ...
    let mut client = DzzyTestClient::connect(&url, &member)
        .await
    ...
    let status = events
        .iter()
        .find(|e| {
            e.kind == Kind::Custom(KIND_DZZY_MESH_MEMBER_STATUS) && e.pubkey == member.public_key()
        })
...
///
/// Requires a relay with `DZZY_REQUIRE_RELAY_MEMBERSHIP=true` and a published
...
async fn trust_nonmember_read_denied() {
    ...
    let mut client = match DzzyTestClient::connect(&url, &stranger).await {
        Ok(c) => c,
    ...
    let leaked = events
        .iter()
        .any(|e| e.kind == Kind::Custom(KIND_DZZY_MESH_MEMBER_STATUS));
    assert!(
        !leaked,
...
/// B running a mesh client + a launched dzzy-agent pointed at B's local
...
/// split route. Auto-split is mesh runtime behavior (no Dzzy code); this row
