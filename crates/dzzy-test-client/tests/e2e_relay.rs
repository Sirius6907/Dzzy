//! End-to-end integration tests for the Dzzy relay.
//!
...
use base64::Engine;
use dzzy_test_client::{DzzyTestClient, RelayMessage, TestClientError};
...
fn test_owner_keys() -> Keys {
    std::env::var("DZZY_TEST_OWNER_PRIVATE_KEY")
        .ok()
...
async fn e2e_db_pool() -> sqlx::Pool<sqlx::Postgres> {
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://dzzy:dzzy_dev@localhost:5432/dzzy".to_string());
...
async fn test_connect_and_authenticate() {
    ...
    let client = DzzyTestClient::connect(&url, &keys)
        .await
...
async fn test_client_submitted_nip43_membership_snapshots_are_rejected() {
    ...
    let mut ws = DzzyTestClient::connect(&url, &keys).await.expect("connect");
    let ok = ws
...
async fn test_send_event_and_receive_via_subscription() {
    ...
    let mut client_a = DzzyTestClient::connect(&url, &keys_a)
        .await
    ...
    let mut client_b = DzzyTestClient::connect(&url, &keys_b)
        .await
...
async fn test_large_event_frame_below_configured_limit_is_accepted() {
    ...
    let mut client = DzzyTestClient::connect(&url, &keys).await.expect("connect");
...
async fn test_unauthenticated_rejected() {
    ...
    let mut client = DzzyTestClient::connect_unauthenticated(&url)
        .await
...
async fn test_multiple_concurrent_clients() {
    ...
    let mut clients: Vec<DzzyTestClient> =
        futures_util::future::try_join_all(keys.iter().map(|k| DzzyTestClient::connect(&url, k)))
            .await
...
async fn test_pubkey_mismatch_rejected() {
    ...
    let mut client = DzzyTestClient::connect(&url, &keys_a)
        .await
...
async fn test_nip29_put_user_default_policy_allows() {
    ...
    let mut ws = DzzyTestClient::connect(&url, &channel_owner_keys)
        .await
...
async fn test_unarchive_emits_member_added_notification() {
    ...
    let mut ws = DzzyTestClient::connect(&url, &owner_keys)
        .await
...
async fn test_nip29_put_user_self_add_bypasses_policy() {
    ...
    // Connect as agent.
    let mut ws = DzzyTestClient::connect(&url, &agent_keys)
        .await
...
async fn test_nip29_standard_client_flow() {
    ...
    let mut client = DzzyTestClient::connect(&url, &keys)
        .await
...
async fn test_membership_notification_emitted_on_add() {
    ...
    let mut agent_client = DzzyTestClient::connect(&url, &agent_keys)
        .await
        .expect("connect as agent");
...
async fn create_private_channel_ws(client: &mut DzzyTestClient, keys: &Keys) -> String {
...
async fn test_private_channel_any_member_can_invite() {
    ...
    let mut owner_client = DzzyTestClient::connect(&url, &owner_keys)
        .await
        .expect("connect as owner");
    ...
    let mut member_client = DzzyTestClient::connect(&url, &member_keys)
        .await
...
async fn test_private_channel_non_member_cannot_invite() {
    ...
    let mut outsider_client = DzzyTestClient::connect(&url, &outsider_keys)
        .await
...
async fn test_reply_ingest_pushes_live_thread_summary() {
    ...
    async fn recv_summary(client: &mut DzzyTestClient) -> nostr::Event {
        loop {
            match client
                .recv_event(Duration::from_secs(5))
                .await
