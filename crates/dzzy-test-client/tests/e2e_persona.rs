...
async fn test_persona_shared_read_gate_foreign_sees_only_shared() {
    ...
    let mut author = DzzyTestClient::connect(&url, &author_keys)
        .await
    ...
    let mut foreign = DzzyTestClient::connect(&url, &foreign_keys)
        .await
...
async fn test_persona_count_excludes_foreign_unshared() {
    ...
    // dzzy-ws-client parses this into RelayMessage::Count.
