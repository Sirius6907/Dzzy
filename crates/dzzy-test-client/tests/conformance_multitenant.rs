...
//! cargo test -p dzzy-test-client --test conformance_multitenant -- --ignored
//! ```
//!
...
mod row_zero_host_binding {
    ...
    /// properties of it (see channel: `dzzy-relay-rewrite`, 2026-06-27):
    ///
    ///   * [`super::channels_membership::same_channel_uuid_in_two_communities_is_isolated`]
    ///     (Mari, `dzzy-db`) asserts **coexistence**: a channel UUID that exists
...
mod nip11_relay_info {
    ...
    /// (`crates/dzzy-relay/src/nip11.rs::_RELAY_INFO_BUILD_STATIC_INPUT_FENCE`):
...
// ---------------------------------------------------------------------------
// API tokens and NIP-98 replay (Sami — dzzy-auth)
// ---------------------------------------------------------------------------
mod api_tokens_nip98_replay {
    ...
    /// in `dzzy-relay`** — there is no route by which a client can bind a token
    /// to a community over HTTP, so the "mint in A, present to B" precondition
    /// has no entry point.
    ///
    /// Verified on PR head:
    ///   * `crates/dzzy-relay/src/router.rs:52-79` — full route list is `/`,
    ...
    ///   * `crates/dzzy-relay/src/api/` directory is `{bridge, events, media,
    ...
    ///     `crates/dzzy-relay/src/api/media.rs:638`, which extracts the
    ///     `X-Auth-Token: dzzy_*` header and looks up
    ...
    /// contract (its deps are `dzzy-ws-client`/`reqwest`/`tokio-tungstenite`/
    /// `s3` — no `sqlx`, no `dzzy-db`).
    ///
    ...
    ///
    ///   1. `crates/dzzy-db/src/api_token.rs:425
    ...
    ///   2. `crates/dzzy-db/src/api_token.rs:488
    ...
    fn token_minted_in_a_does_not_authorize_in_b() {
        ...
        // If anyone adds a `/tokens` route to `dzzy-relay`, this row's shape
    ...
    /// in `crates/dzzy-relay/src/api/bridge.rs:79` (return `Ok(())` before
    ...
    ///
    /// The replay key shape is `dzzy:{community}:nip98:{event_id_hex}` (see
    /// `crates/dzzy-auth/src/nip98_replay.rs:103 nip98_replay_key`). The
    ...
    /// (`crates/dzzy-auth/src/nip98_replay.rs:163
    ...
    /// "simplifying" the key to just `dzzy:nip98:{event_id}`, or
    ...
    /// (relay-membership is open under `DZZY_REQUIRE_AUTH_TOKEN=false`).
    #[tokio::test]
    #[ignore]
    ...
    /// the pattern in `crates/dzzy-auth/src/nip98.rs`; kept local to this
...
// ---------------------------------------------------------------------------
// Membership / allowlist / archived identities (Sami — dzzy-auth)
// ---------------------------------------------------------------------------
...
mod users_profiles_nip05 {
    ...
    use dzzy_test_client::DzzyTestClient;
    ...
    /// relay's ingest side-effect at `crates/dzzy-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// also syncs the parsed fields into `users` via
    /// `update_user_profile(tenant.community(), pubkey, ...)`.
    async fn publish_kind0(client: &mut DzzyTestClient, keys: &Keys, content_json: &str) -> String {
    ...
    /// bridge endpoint; with `DZZY_REQUIRE_AUTH_TOKEN=false` the dev-mode
    ...
    /// `crates/dzzy-relay/src/api/nip05.rs::extract_domain` — strips the
    ...
    /// `crates/dzzy-db/src/user.rs:140` (`UPDATE users SET ... WHERE
    /// community_id = $N AND pubkey = $M`) AND the event-storage fence at
    /// `crates/dzzy-db/src/event.rs::get_events_by_ids` (or the equivalent
    ...
    async fn same_pubkey_distinct_profiles_in_two_communities() {
        ...
        let mut client_a = DzzyTestClient::connect(&ws_a, &keys)
            .await
            .expect("connect A");
        let _id_a = publish_kind0(&mut client_a, &keys, &content_a).await;

        let mut client_b = DzzyTestClient::connect(&ws_b, &keys)
            .await
            .expect("connect B");
    ...
    /// `crates/dzzy-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// validates the handle via `crates/dzzy-relay/src/api/nip05.rs::canonicalize_nip05`
    ...
    /// from `crates/dzzy-db/src/user.rs::get_user_by_nip05` (line 185 form
    ...
    async fn same_nip05_local_part_on_two_hosts_is_independent() {
        ...
        let mut client_a = DzzyTestClient::connect(&ws_a, &keys_a)
            .await
            .expect("connect A");
        let _ = publish_kind0(&mut client_a, &keys_a, &content_a).await;

        let mut client_b = DzzyTestClient::connect(&ws_b, &keys_b)
            .await
            .expect("connect B");
...
// ---------------------------------------------------------------------------
// Feed read-side isolation (Perci — dzzy-db/relay bridge)
// ---------------------------------------------------------------------------
mod feed_read_side_isolation {
    ...
    /// `crates/dzzy-db/src/feed.rs`; this row is the wire-level conformance
...
// ---------------------------------------------------------------------------
// Channels and channel membership (Mari — dzzy-db)
// ---------------------------------------------------------------------------
mod channels_membership {
    ...
    /// `crates/dzzy-relay/src/handlers/side_effects.rs::handle_create_group`.
    ...
    /// `DZZY_REQUIRE_AUTH_TOKEN=false` recipe). Returns the events as their
    ...
    /// `crates/dzzy-db/src/event.rs::query_events` (the non-p-tag branch,
...
mod workflows {
    ...
    /// the REST bridge (`POST /events`). In dev mode (`DZZY_REQUIRE_AUTH_TOKEN
    ...
    /// `crates/dzzy-relay/src/handlers/command_executor.rs:703`
    ...
    /// (`crates/dzzy-workflow/src/lib.rs` — "approval gates not yet implemented,
    ...
    /// minting), **not** dzzy-db — the scoping fence it will exercise is
    /// already landed.
    #[tokio::test]
    #[ignore]
...
// ---------------------------------------------------------------------------
// Search / FTS (Quinn — dzzy-search)
// ---------------------------------------------------------------------------
mod search_fts {
    use super::*;

    use dzzy_test_client::{DzzyTestClient, RelayMessage};
    ...
    ///   - `crates/dzzy-search/src/query.rs::search` lines 160-161 (the FTS
    ///     `WHERE community_id = $ctx` predicate); and
    ///   - `crates/dzzy-db/src/event.rs::get_events_by_ids` lines 870-872
...
// ---------------------------------------------------------------------------
// Redis pub/sub, presence, typing, cache invalidation (Max — dzzy-pubsub)
// ---------------------------------------------------------------------------
mod pubsub_presence_typing {
    ...
    async fn publish_presence(client: &mut DzzyTestClient, keys: &Keys, status: &str) {
    ...
    async fn subscribe_typing(client: &mut DzzyTestClient, sub_id: &str, channel_id: &str) {
        let filter = Filter::new()
    ...
    /// Obligation: keys are `dzzy:{community}:…`; cross-node fan-out never
    ...
    ///    → `dzzy_pubsub::get_presence_bulk`). A's query must return only A's
    ///    status and B's only B's. This bites the Redis key format in
    ///    `crates/dzzy-pubsub/src/presence.rs::presence_key`, which must include
    ///    `ctx.community()` (`dzzy:{community}:presence:{pubkey}`). Same pubkey
    ...
    ///     `dzzy-pubsub/src/presence.rs::presence_key` (shared Redis key). The B
...
// ---------------------------------------------------------------------------
// Media / Blossom / S3 (Perci — dzzy-media)
// ---------------------------------------------------------------------------
mod media_blossom {
    use super::*;
...
// ---------------------------------------------------------------------------
// Git hosting / NIP-34 / object storage (Perci — dzzy-media/git)
// ---------------------------------------------------------------------------
mod git_hosting {
    use super::*;
...
// ---------------------------------------------------------------------------
// Audit log and observability (Dawn — dzzy-audit)
// ---------------------------------------------------------------------------
mod audit_log {
    ...
    //! `crates/dzzy-relay/src/router.rs` (the route list is `/`, `/info`,
    ...
    //! `dzzy_audit::AuditService::{verify_chain, get_entries}`, which are
    //! operator-internal (consumed by `dzzy-admin`). `crates/dzzy-audit/src/
    ...
    //! (its deps are `dzzy-ws-client`/`reqwest`/`tokio-tungstenite`/`s3` — no
    //! `sqlx`, no `dzzy-audit`), and a DB-direct read can never catch a
    ...
    //!      `dzzy_audit::service::tests::chains_are_independent_per_community`
    ...
    //!      `dzzy_relay::handlers::event::tests::
    ...
    //!   2. **Errors don't leak** —
    //!      `dzzy_audit::error::tests::audit_error_text_carries_no_community_id_or_constraint`
    //!      asserts no `AuditError` variant's rendered text embeds a
    //!      `community_id`, constraint name, or cross-community object id.
    //!
    //! Substrate on PR head: `crates/dzzy-audit/src/entry.rs` keys `AuditEntry`
