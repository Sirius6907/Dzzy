...
//!   cargo test -p dzzy-test-client --test e2e_git -- --ignored --nocapture
//! ```
...
/// Run `git` with the Dzzy credential helper and isolated config.
...
/// Run `git` with the Dzzy credential helper and isolated config. Asserts the
...
impl GitS3Probe {
    fn from_env() -> Self {
        ...
        let bucket = std::env::var("DZZY_GIT_S3_BUCKET")
            ...
            .unwrap_or_else(|_| "dzzy-media".to_string());
    ...
    fn pointer_key(owner: &str, repo: &str) -> String {
        ...
        if let Ok(community) = std::env::var("DZZY_E2E_GIT_COMMUNITY_ID") {
...
async fn git_concurrent_push_one_wins_and_repo_recovers() {
    ...
    let tmp = tempdir_named("dzzy-e2e-git-concurrent");
    let url = format!("{}/git/{}/{}", relay_http_url(), owner_hex, repo);
