...
fn build_bootstrap(cwd: &Path, shell_hint: &str) -> String {
    ...
    let dzzy_hint =
        if std::env::var("DZZY_RELAY_URL").is_ok() && std::env::var("DZZY_PRIVATE_KEY").is_ok() {
            "\nDzzy relay configured. Run `dzzy --help` to see available commands.\n"
        } else {
            ""
        };
    format!(
        "Working directory: {}\n\
         Detected stack: {}\n\
         Shell: {shell_hint} (set DZZY_SHELL to override) — write command strings in that shell's syntax.\n\
...
pub async fn run(
    ...
    // DZZY_PRIVATE_KEY is intentionally inherited — the dzzy CLI needs it.
...
///
/// The resolver supports `DZZY_SHELL=cmd`/`pwsh` (explicit operator overrides
...
/// broken, so the resolver only needs to honor DZZY_SHELL. The probe logic and
...
fn resolve_bash(_path_env: &str) -> Result<(PathBuf, String), String> {
    // Honor DZZY_SHELL on Unix so power users can opt into zsh or another shell.
    if let Some(raw) = std::env::var_os("DZZY_SHELL") {
...
///   1. `DZZY_SHELL` env override — explicit operator choice, any shell (cmd,
...
fn resolve_bash(path_env: &str) -> Result<(PathBuf, String), String> {
    ...
    Err(
        "Git for Windows (Git Bash) is required but was not found. Checked \\
         DZZY_SHELL, GIT_BASH, bash.exe and git.exe on PATH, the standard Git install locations, \\
         and HKLM/HKCU\\\\SOFTWARE\\\\GitForWindows. Git's \"Cmd\" PATH option adds \\
         Git\\\\cmd\\\\git.exe but not Git\\\\bin\\\\bash.exe; Dzzy normally derives Git Bash from that git.exe. \\
         Install it from https://git-scm.com/download/win and select \"Git from the command line \\
         and also from 3rd-party software\", then relaunch Dzzy. You can also set \\
         DZZY_SHELL to a shell executable."
            .into(),
    )
}
...
mod windows_resolver_tests {
    ...
    // Process-global env mutation guard: tests that mutate DZZY_SHELL,
    ...
    #[test]
    fn dzzy_shell_override_wins_over_everything() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // DZZY_SHELL pointing at a real file must be returned without probing
        // the standard Git-for-Windows locations or PATH.
        let dir = tempdir().expect("tempdir");
        let fake_bash = dir.path().join("my-bash.exe");
        touch(&fake_bash);
        // Temporarily set DZZY_SHELL; clean up after the test.
        env::set_var("DZZY_SHELL", &fake_bash);
        let result = resolve_bash("");
        env::remove_var("DZZY_SHELL");
        let (resolved, _name) = result.expect("DZZY_SHELL override should resolve");
        assert_eq!(resolved, fake_bash);
    }

    #[test]
    fn dzzy_shell_override_skipped_when_path_absent() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // If DZZY_SHELL points at a non-existent path the resolver must fall
        // through rather than returning a dead path.
        env::set_var("DZZY_SHELL", r"C:\does\not\exist\bash.exe");
        ...
        if let Ok((resolved, _)) = result {
            assert_ne!(
                ...
                r"C:\does\not\exist\bash.exe",
                "non-existent DZZY_SHELL must not be returned"
            );
        }
        // An Err is also acceptable (no Git installed on test host).
    }

    /// Explicit DZZY_SHELL bare name resolves through PATH and uses NO System32
    /// exclusion — cmd/pwsh live in System32 legitimately.
    #[test]
    fn dzzy_shell_explicit_bare_name_resolves_from_system32() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // Simulate cmd.exe living in a dir that would be excluded by the WSL guard.
        // The explicit DZZY_SHELL branch must NOT skip System32.
        ...
        // for bash.exe, but must NOT skip it for an explicit DZZY_SHELL).
        let path_env = env::join_paths([sys32.path().to_path_buf()]).expect("join");
        env::set_var("DZZY_SHELL", "cmd");
        ...
        let (resolved, name) =
            result.expect("explicit DZZY_SHELL=cmd should resolve even from System32-like dir");
    ...
    fn implicit_bash_scan_still_skips_system32() {
        ...
        // Without an explicit DZZY_SHELL, the fallback scan must skip it.
        let sys32 = tempdir().expect("sys32");
        touch(&sys32.path().join("bash.exe"));

        let path_env = env::join_paths([sys32.path().to_path_buf()]).expect("join");
        // No DZZY_SHELL — trigger the implicit bash fallback scan.
        env::remove_var("DZZY_SHELL");
    ...
    /// Verify that constructing SharedState with DZZY_SHELL set produces a
    ...
    fn shared_state_bootstrap_hint_matches_resolved_shell() {
        ...
        touch(&fake_pwsh);
        env::set_var("DZZY_SHELL", &fake_pwsh);
    ...
    /// F3: DZZY_SHELL bare command name (e.g. "pwsh") resolved through PATH.
    /// When pwsh.exe is on PATH, resolve_bash must return it and report "pwsh".
    #[test]
    fn dzzy_shell_bare_name_resolved_through_path_when_present() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        let dir = tempdir().expect("tempdir");
        let fake_pwsh = dir.path().join("pwsh.exe");
        touch(&fake_pwsh);
        ...
        env::set_var("DZZY_SHELL", "pwsh");
        let result = resolve_bash(path_env.to_str().expect("utf8"));
        env::remove_var("DZZY_SHELL");

        let (resolved, name) = result.expect("bare DZZY_SHELL=pwsh should resolve from PATH");
    ...
    /// F3: DZZY_SHELL bare command name absent from PATH → fall through, do not
    /// report pwsh as the active shell.
    #[test]
    fn dzzy_shell_bare_name_absent_from_path_falls_through() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // Set DZZY_SHELL to a command that won't be on any real PATH.
        env::set_var("DZZY_SHELL", "dzzy-shell-does-not-exist-xyz");
        let result = resolve_bash("");
        env::remove_var("DZZY_SHELL");
        if let Ok((resolved, name)) = result {
            assert_ne!(
                resolved.file_name().and_then(|n| n.to_str()).unwrap_or(""),
                "dzzy-shell-does-not-exist-xyz.exe",
                "absent DZZY_SHELL must not be returned as the resolved path"
            );
            assert_ne!(
                name, "dzzy-shell-does-not-exist-xyz",
                "absent DZZY_SHELL must not be reported as the shell name"
            );
        }
    ...
    fn git_cmd_on_path_resolves_sibling_git_bash() {
        ...
        let old_dzzy_shell = env::var_os("DZZY_SHELL");
        ...
        match old_dzzy_shell {
            Some(value) => env::set_var("DZZY_SHELL", value),
            None => env::remove_var("DZZY_SHELL"),
        }
        match old_git_bash {
