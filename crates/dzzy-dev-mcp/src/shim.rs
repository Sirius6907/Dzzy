...
///
/// Shell children receive `path_env`, `git_env`, and `DZZY_PRIVATE_KEY` (for
/// the dzzy CLI). `NOSTR_PRIVATE_KEY` is removed from the process env after
...
impl Shim {
    pub fn install() -> std::io::Result<Self> {
        let dir = tempfile::Builder::new().prefix("dzzy-dev-mcp-").tempdir()?;
...
fn write_keyfile(shim_dir: &Path, raw: &str) -> Option<KeyInfo> {
    ...
    let keys = match nostr::Keys::parse(raw) {
        ...
        Err(e) => {
            eprintln!(
                "dzzy-dev-mcp: warning: NOSTR_PRIVATE_KEY is set but invalid ({e}); \
    ...
    if write_keyfile_atomic(&keyfile, raw.as_bytes()).is_err() {
        eprintln!(
            "dzzy-dev-mcp: warning: failed to write nostr keyfile; git auth/signing disabled"
        );
        return None;
    }
    let keyfile_path = match keyfile.to_str() {
        ...
        None => {
            eprintln!(
                "dzzy-dev-mcp: warning: tempdir path is not valid UTF-8; git auth/signing disabled"
            );
            return None;
        }
    };
...
/// Format: `<hex_pubkey>@<relay_host>` (e.g., `ab12...cd@relay.dzzy.dev`).
/// Falls back to `<hex_pubkey>@dzzy` if no relay URL is configured.
fn derive_git_email(pubkey_hex: &str) -> String {
    let host = std::env::var("DZZY_RELAY_URL")
        .ok()
        .and_then(|url| {
...
/// via dzzy-agent (which clears env), the base is always 0 — composition only
...
fn build_git_env(info: &KeyInfo) -> Vec<(String, String)> {
    ...
    let entries: Vec<(&str, String)> = vec![
        ...
        ("user.email", email),
        // Nostr credential helper is additive — it silently declines non-Dzzy
        ...
        // Required: Dzzy relay verifies NIP-98 against the full repo-root URL.
