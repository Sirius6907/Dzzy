...
fn sign_media_get_auth(keys: &nostr::Keys, authority: &str) -> Result<String, String> {
    ...
    let event = EventBuilder::new(Kind::from(24242), "Get dzzy-media")
        .tags(tags)
        .sign_with_keys(keys)
        .map_err(|e| e.to_string())?;
    Ok(format!(
...
fn server_authority(url: &reqwest::Url) -> Option<String> {
    let authority = dzzy_core::tenant::relay_url_authority(url.as_str());
...
/// media and `DZZY_PRIVATE_KEY` is available; `None` otherwise.
///
...
fn relay_media_get_auth(url: &reqwest::Url) -> Option<String> {
    let relay = std::env::var("DZZY_RELAY_URL").ok()?;
    let relay = reqwest::Url::parse(&relay).ok()?;
    if !is_relay_media_url(url, &relay) {
        return None;
    }
    let key = std::env::var("DZZY_PRIVATE_KEY").ok()?;
    let keys = match nostr::Keys::parse(&key) {
        Ok(k) => k,
        Err(e) => {
            tracing::warn!("DZZY_PRIVATE_KEY invalid; fetching relay media unauthenticated: {e}");
            return None;
        }
    };
...
/// `DZZY_RELAY_URL` + `DZZY_PRIVATE_KEY` are configured.
async fn fetch_url(url: &str) -> Result<Vec<u8>, ErrorData> {
    ...
    if auth.is_some() {
        // Do not forward Dzzy auth headers to a redirect target. reqwest strips
        ...
        if matches!(status.as_u16(), 401 | 403) && !authed {
            return Err(invalid_params(format!(
                "fetch {url} returned HTTP {status} — this relay requires authenticated media \
                 reads; set DZZY_PRIVATE_KEY (and DZZY_RELAY_URL) to a member identity"
            )));
        }
        return Err(invalid_params(format!(
