...
//! `dzzy-relay` / `dzzy-pubsub`. Fixed-window counter algorithm.
//!
...
///
/// The Redis-backed production implementation lives in `dzzy-relay` / `dzzy-pubsub`.
...
/// key is community-prefixed (`dzzy:{community}:ratelimit:{pubkey}:{suffix}`). The
...
/// `dzzy:{community}:ratelimit:{pubkey_hex}:{suffix}`.
///
...
/// Redis key for IP-based rate limit: `dzzy:ratelimit:ip:{ip}:conn`.
///
/// Operator-global by design — see [`RateLimiter`] docs.
pub fn ip_rate_limit_key(ip: &IpAddr) -> String {
    format!("dzzy:ratelimit:ip:{}:conn", ip)
}
...
mod tests {
    ...
    fn rate_limit_key_includes_community_prefix() {
        ...
        let expected_prefix = format!("dzzy:{}:ratelimit:", ctx.community());
        assert!(
    ...
    fn ip_rate_limit_key_format() {
        ...
        assert_eq!(ip_rate_limit_key(&ip), "dzzy:ratelimit:ip:192.168.1.1:conn");
    }
