...
//!
//! Defines [`ChannelAccessChecker`] so `dzzy-auth` can enforce access
//! without depending on `dzzy-db` directly.
...
///
/// Implemented by the database layer (`dzzy-db`) in production. The `dzzy-auth`
