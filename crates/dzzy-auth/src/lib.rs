#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `dzzy-auth` — Authentication and authorization for the Dzzy relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("dzzy-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("dzzy-test-key:{username}");
