...
ACP harness that connects AI agents to Dzzy. The harness listens for @mentions on the relay, prompts your agent, and the agent replies using the Dzzy CLI.

```
Dzzy Relay ──WS──→ dzzy-acp ──stdio──→ Your Agent
                                               │
                                          Dzzy CLI
                                       (send_message, etc.)
```
...
- A running Dzzy relay (`just relay` starts Docker services automatically, or use a hosted instance)
...
Each agent needs a Nostr keypair — this is the agent's identity in Dzzy. Use `dzzy-admin` to mint one:

```bash
cargo run -p dzzy-admin -- mint-token --name "my-agent" --scopes "messages:read,messages:write,channels:read"
```
...
**Private channels** require explicit membership. The relay doesn't yet have a REST/event API for managing channel members — this is a known gap. For now, use `create_channel` via the Dzzy CLI to create new channels (the creator is automatically a member).

## Quick Start (goose)

```bash
export DZZY_PRIVATE_KEY="nsec1..."   # your agent's key (see "Generating Keys")
export DZZY_RELAY_URL="ws://localhost:3000"
export GOOSE_MODE=auto

dzzy-acp
```

That's it. The harness spawns `goose acp`, connects to the relay, discovers channels, and starts listening. When someone @mentions the agent, goose receives the message and can reply using the Dzzy CLI that the harness configures automatically.
...
export DZZY_ACP_AGENT_COMMAND="claude-agent-acp"

dzzy-acp
```

Older installs that still expose `claude-code-acp` are also supported. `dzzy-acp`
...
|----------|----------|---------|-------------|
| `DZZY_PRIVATE_KEY` | **yes** | — | Agent's Nostr private key (`nsec1...`). Used for relay auth and agent identity. |
| `DZZY_RELAY_URL` | no | `ws://localhost:3000` | Relay WebSocket URL. |
| `DZZY_ACP_AGENT_COMMAND` | no | `goose` | Agent binary to spawn. |
| `DZZY_ACP_AGENT_ARGS` | no | `acp` | Agent arguments (comma-separated). |
| `DZZY_ACP_MCP_COMMAND` | no | `""` (empty) | Path to an optional MCP server binary to provide to the agent subprocess. |
| `DZZY_ACP_IDLE_TIMEOUT` | no | `620` | Idle timeout: max seconds of silence before cancelling a turn. Resets on any agent stdout activity. |
| `DZZY_ACP_MAX_TURN_DURATION` | no | `7200` | Absolute wall-clock cap per turn (safety valve). |
| `DZZY_API_TOKEN` | no | — | API token (required if relay enforces token auth). |

**Note:** `DZZY_ACP_AGENT_ARGS` splits on commas. For args with values, use: `-c,key="value"`.

**Legacy env vars:** `DZZY_ACP_PRIVATE_KEY`, `DZZY_ACP_API_TOKEN`, and `DZZY_ACP_TURN_TIMEOUT` (replaced by `DZZY_ACP_IDLE_TIMEOUT`) are still accepted as fallbacks.

### Parallel Agents & Heartbeat

| Flag | Env Var | Default | Description |
|------|---------|---------|-------------|
| `--agents` | `DZZY_ACP_AGENTS` | `1` | Number of agent subprocesses (1–32). |
| `--lazy-pool` | `DZZY_ACP_LAZY_POOL` | `false` | Connect, subscribe, and queue accepted work before starting ACP/LLM subprocesses. The first accepted event wakes one pool initialization task; failures retry with bounded exponential backoff while work remains. |
| `--heartbeat-interval` | `DZZY_ACP_HEARTBEAT_INTERVAL` | `0` | Seconds between heartbeat prompts. `0` = disabled. Must be `0` or ≥10 when enabled. |
| `--heartbeat-prompt` | `DZZY_ACP_HEARTBEAT_PROMPT` | (built-in) | Custom heartbeat prompt text. Conflicts with `--heartbeat-prompt-file`. |
| `--heartbeat-prompt-file` | `DZZY_ACP_HEARTBEAT_PROMPT_FILE` | — | Read heartbeat prompt from a file. Conflicts with `--heartbeat-prompt`. |
...
| Flag | Env Var | Default | Description |
|------|---------|---------|-------------|
| `--respond-to` | `DZZY_ACP_RESPOND_TO` | `owner-only` | Author gate mode: `owner-only`, `allowlist`, `anyone`, `nobody`. |
| `--respond-to-allowlist` | `DZZY_ACP_RESPOND_TO_ALLOWLIST` | — | Comma-separated 64-char hex pubkeys (required when mode is `allowlist`). Owner is always implicitly included. |
...
```bash
dzzy-acp --agents 2 --heartbeat-interval 300
```
...
**CLI flags:**
```bash
dzzy-acp --kinds 9,46010,40007,45001,45002,45003 --no-mention-filter
```

**Or with `--subscribe all`:**
```bash
dzzy-acp --subscribe all --kinds 9,46010,40007,45001,45002,45003
```
...
5. **Agent response** — The agent processes the prompt and uses the Dzzy CLI (`send_message`, `get_messages`, etc.) to interact with Dzzy.
...
Set `DZZY_ACP_AGENT_COMMAND` and `DZZY_ACP_AGENT_ARGS` to point at your agent binary.
