...
//! turn.  Both goose and dzzy-agent use this same wire format.  The payload
...
/// ```
///
/// `used` and `contextLimit` are optional because dzzy-agent does not track a
...
pub(crate) struct UsageUpdatePayload {
    /// Total tokens used (context-usage proxy). Optional — dzzy-agent omits
    /// this field or sends 0 because it does not track a context window limit.
    #[serde(default)]
    #[allow(dead_code)]
    pub used: u64,
    /// Context window size. Optional — dzzy-agent omits this field.
    #[serde(default)]
    #[allow(dead_code)]
...
mod tests {
    ...
    fn notification_deserializes_without_used_and_context_limit() {
        // dzzy-agent emits usage_update without used/contextLimit.
        let raw = serde_json::json!({
            "sessionId": "dzzy-sess",
            "update": {
                "sessionUpdate": "usage_update",
    ...
    #[test]
    fn dzzy_agent_notification_flows_through_tracker() {
        // End-to-end: a dzzy-agent-shaped usage_update (no used/contextLimit)
    ...
    #[test]
    fn dzzy_agent_payload_no_context_fields_processes_correctly() {
