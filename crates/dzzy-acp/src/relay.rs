//! Harness-side Dzzy relay client.
//!
//! Connects to the Dzzy relay via NIP-01 WebSocket, authenticates via NIP-42,
...
//! - Forwards `DzzyEvent`s through an `mpsc` channel
...
/// Override with `DZZY_ACP_EVENT_BUFFER` env var at startup.
...
fn event_channel_capacity() -> usize {
    std::env::var("DZZY_ACP_EVENT_BUFFER")
        .ok()
...
///
/// Connects to the Dzzy relay, authenticates via NIP-42, and streams
...
pub struct HarnessRelay {
    ...
    event_rx: mpsc::Receiver<Option<DzzyEvent>>,
...
impl HarnessRelay {
    ...
    pub async fn connect(
        ...
        let (event_tx, event_rx) = mpsc::channel::<Option<DzzyEvent>>(event_channel_capacity());
    ...
    pub async fn discover_channels(&self) -> Result<HashMap<Uuid, ChannelInfo>, RelayError> {
        ...
        let member_filter = nostr::Filter::new()
            .kind(Kind::Custom(
                dzzy_core::kind::KIND_NIP29_GROUP_MEMBERS as u16,
            ))
    ...
    pub async fn next_event(&mut self) -> Option<DzzyEvent> {
...
async fn run_background_task(
    ...
    event_tx: mpsc::Sender<Option<DzzyEvent>>,
            ...
            match relay_msg {
                RelayMessage::Event {
                    ...
                    if subscription_id == OBSERVER_CONTROL_SUB_ID {
                        ...
                        match event_tx.try_send(Some(dzzy_event)) {
                            Ok(()) => {
...
/// sends (`crates/dzzy-relay/src/handlers/auth.rs`).
///
...
mod tests {
    ...
    fn make_observer_frame(keys: &Keys) -> Event {
        ...
        let encrypted = dzzy_core::observer::encrypt_observer_payload(
            keys,
            &recipient.public_key(),
            &json!({"type": "test"}),
        )
        .expect("encrypt test observer payload");
        dzzy_sdk::build_agent_observer_frame(
