...
pub struct AcpClient {
    ...
    ///
    /// Both goose and dzzy-agent emit `session_info_update` with this field;
    ...
    /// run clears (goose/dzzy-agent emit `activeRunId: null` at end of turn).
    ...
    /// deltas. Both goose and dzzy-agent emit this notification; goose gates
    /// on client capability advertisement, dzzy-agent emits unconditionally.
...
///
/// Returns `Some(json_string)` when `has_generated_codex_config` is true (Dzzy injected a
...
pub(crate) fn build_codex_config_env(
    ...
    // Without an explicit Dzzy-generated overlay signal, skip the merge entirely.
...
fn build_client_capabilities() -> serde_json::Value {
    serde_json::json!({
        // Signal to ACP adapters that Dzzy can hand users to terminal-native
        // auth flows. Adapters decide which auth methods to expose; Dzzy does
...
impl AcpClient {
    ...
    pub async fn spawn(
        ...
        // Per-persona env vars (e.g., GOOSE_PROVIDER, DZZY_AGENT_PROVIDER).
    ...
    /// `["/cmd args", "<dzzy context>"]` instead of one wrapped block.
    ...
    ///
    /// Both goose and dzzy-agent emit `session_info_update`; other agents
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        match update_type {
            ...
            "session_info_update" => {
                // Both goose and dzzy-agent emit `session_info_update` with
                ...
                // alongside it at the params level. Goose and dzzy-agent both
...
mod tests {
    ...
    async fn goose_system_prompt_request_uses_append_contract() {
        ...
        assert_eq!(received["params"]["key"], "dzzy");
    ...
    /// `SessionInfoUpdate` schema), matching what goose and dzzy-agent
    /// emit on the wire.
    ...
    fn build_codex_config_env_generated_only_single_entry_with_signal_true_merges_with_parent() {
        // No persona: Dzzy injects one CODEX_CONFIG; signal=true.
    ...
    fn build_codex_config_env_persona_only_signal_false_returns_none() {
        // Persona set CODEX_CONFIG; Dzzy did not inject a generated overlay (signal=false).
