...
/// Redis key prefix for Dzzy-scoped pub/sub topics and keys.
pub const DZZY_PREFIX: &str = "dzzy";
    ...
    pub fn redis_channel(&self) -> String {
        match self.topic {
            EventTopic::Channel(channel_id) => {
                format!("{DZZY_PREFIX}:{}:channel:{channel_id}", self.community_id)
            }
            EventTopic::Global => format!("{DZZY_PREFIX}:{}:global", self.community_id),
        }
    }
...
mod tests {
    ...
    fn channel_key_includes_community_and_channel() {
        ...
        assert_eq!(
            ...
            format!("dzzy:{}:channel:{channel_id}", ctx.community())
        );
    }
    ...
    fn global_key_includes_community() {
        ...
        assert_eq!(global_key(&ctx), format!("dzzy:{}:global", ctx.community()));
    }
    ...
    fn parses_channel_topic() {
        ...
        let channel_id = Uuid::from_u128(0xbbbb);
        let raw = format!("dzzy:{community_id}:channel:{channel_id}");
    ...
    fn parses_global_topic() {
        let community_id = CommunityId::from_uuid(Uuid::from_u128(0xaaaa));
        let raw = format!("dzzy:{community_id}:global");
    ...
    fn rejects_malformed_or_wrong_prefix_topics() {
        for raw in [
            "",
            "not-dzzy:00000000-0000-0000-0000-00000000aaaa:global",
            "dzzy:not-a-uuid:global",
            "dzzy:00000000-0000-0000-0000-00000000aaaa",
            "dzzy:00000000-0000-0000-0000-00000000aaaa:global:extra",
            "dzzy:00000000-0000-0000-0000-00000000aaaa:channel",
            "dzzy:00000000-0000-0000-0000-00000000aaaa:channel:not-a-uuid",
            "dzzy:00000000-0000-0000-0000-00000000aaaa:presence:abc",
        ] {
            assert!(
