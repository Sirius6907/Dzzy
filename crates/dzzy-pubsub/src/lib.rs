#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `dzzy-pubsub` — Redis pub/sub fan-out, presence tracking, and typing indicators.
//!
//! # Architecture
//!
//! ```text
...
//!         └── dynamic SUBSCRIBE dzzy:{community}:channel:{id} / dzzy:{community}:global
...
pub struct ChannelEvent {
    ...
    pub community_id: dzzy_core::CommunityId,
...
/// Central pub/sub manager for a Dzzy relay instance.
...
impl PubSubManager {
    ...
    /// `dzzy:{community}:channel:{id}` / `dzzy:{community}:global`, and
