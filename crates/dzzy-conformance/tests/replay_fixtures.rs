...
use dzzy_conformance::checker::{check_trace, Scenario};
use dzzy_conformance::transitions::TransitionError;
use dzzy_conformance::{
...
/// `expected_trace` byte-exactly. Run with `DZZY_CONFORMANCE_UPDATE=1`
...
fn assert_fixture_matches(name: &str, expected_trace: &[TraceStep]) {
    ...
    if std::env::var("DZZY_CONFORMANCE_UPDATE").is_ok() {
    ...
    let actual = fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "fixture {} missing or unreadable ({e}); run with \
             DZZY_CONFORMANCE_UPDATE=1 to create it",
            path.display()
        )
    });
    ...
    assert_eq!(
        ...
        "committed fixture {} drifted from the typed builder; run with \
         DZZY_CONFORMANCE_UPDATE=1 to refresh if the change is intentional",
        name
    );
