//! Dzzy V2 kind number registry.
//!
//! This module is the authoritative source for Dzzy kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by Dzzy today.
...
/// Dzzy custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: Dzzy command kinds (Pure Nostr plan)
...
/// `crates/dzzy-search/tests/fts_integration.rs`).
///
...
/// Dzzy product feedback submission. Accepted at ingest, sidecarred to the
...
// Dzzy community moderation commands (mod-signed, processed like 9030-series:
...
/// Dzzy: Set the workspace profile (icon). Admin/owner-signed command.
...
// Not using NIP-90 kinds (5000–6999) — Dzzy requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the Dzzy workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a Dzzy command kind that requires transactional execution.
...
/// Safe: all Dzzy kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all Dzzy kind constants fit in nostr's u16-backed Kind.
