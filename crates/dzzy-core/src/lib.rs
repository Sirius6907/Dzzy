#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `dzzy-core` — zero-I/O foundation types for the Dzzy relay.
//!
//! Provides [`StoredEvent`], filter matching, kind constants, and event
//! verification. All other Dzzy crates depend on this one.
...
pub mod git_perms;
/// Dzzy kind number registry — custom event type constants.
pub mod kind;
