...
//!
//! This module defines the core data types for the Dzzy git permission system.
//! The permission model: channel role = repo role; `dzzy-protect` tags on
...
/// Maximum number of `dzzy-protect` tags per repo.
...
/// A single protection rule parsed from a `dzzy-protect` tag on kind:30617.
///
/// Format: `["dzzy-protect", "<ref-pattern>", "<rule>", ...]`
...
/// Errors from parsing a `dzzy-protect` tag.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl fmt::Display for RuleParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TooFewValues => write!(f, "dzzy-protect tag needs pattern + at least one rule"),
...
/// Parse a single `dzzy-protect` tag into a `ProtectionRule`.
///
/// Tag format: `["dzzy-protect", "<pattern>", "<rule1>", "<rule2>", ...]`
/// The first element ("dzzy-protect") should already be stripped — pass
/// the remaining values starting with the pattern.
/// Parse a single `dzzy-protect` tag (simple API, discards unknown rules).
...
/// Parse a single `dzzy-protect` tag, returning unknown rules for logging.
...
/// Parse all `dzzy-protect` tags from a kind:30617 event's tag list.
///
/// Returns an error if any `dzzy-protect` tag is structurally malformed.
...
pub fn parse_protection_tags(tags: &[Vec<String>]) -> Result<ParsedProtection, RuleParseError> {
    ...
    for tag in tags {
        if tag.first().map(|s| s.as_str()) != Some("dzzy-protect") {
            continue;
        }
...
/// Built-in default minimum role for an operation when no `dzzy-protect` tag matches.
