...
mod tests {
    ...
    fn static_keys_build_client_with_configured_region() {
        let storage = MediaStorage::new(&storage_config("dzzy_dev", "dzzy_dev_secret"))
    ...
    fn partial_static_keys_are_rejected() {
        let err = match MediaStorage::new(&storage_config("dzzy_dev", "")) {
        ...
        let err = match MediaStorage::new(&storage_config("", "dzzy_dev_secret")) {
            Ok(_) => panic!("partial static creds must not silently use credential chain"),
            Err(err) => err,
        };
        assert!(
