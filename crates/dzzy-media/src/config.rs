...
pub struct MediaConfig {
    ...
    /// `DZZY_MEDIA_UPLOAD_RECORDS=true`.
    #[serde(default)]
...
impl MediaConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.upload_ip_header.is_some() && !self.upload_records_enabled {
            return Err(
                "DZZY_MEDIA_UPLOAD_IP_HEADER is set but DZZY_MEDIA_UPLOAD_RECORDS is not \
        ...
        if self.upload_port_header.is_some() && self.upload_ip_header.is_none() {
            return Err(
                "DZZY_MEDIA_UPLOAD_PORT_HEADER is set without DZZY_MEDIA_UPLOAD_IP_HEADER — \
        ...
        for (name, value) in [
            ("DZZY_MEDIA_UPLOAD_IP_HEADER", &self.upload_ip_header),
            ("DZZY_MEDIA_UPLOAD_PORT_HEADER", &self.upload_port_header),
        ] {
            if let Some(h) = value {
...
mod tests {
    ...
    fn valid_config() -> MediaConfig {
        MediaConfig {
            ...
            s3_bucket: "dzzy-media".to_string(),
