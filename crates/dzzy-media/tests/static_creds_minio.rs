...
//!
//! Run it against the docker-compose MinIO (creds `dzzy_dev`/`dzzy_dev_secret`,
//! bucket `dzzy-media`, endpoint `http://localhost:9000`):
//!
//! ```bash
//! docker compose up -d minio minio-init
//! cargo test -p dzzy-media --test static_creds_minio -- --ignored
//! ```
//!
//! Overridable via `DZZY_S3_ENDPOINT` / `DZZY_S3_ACCESS_KEY` /
//! `DZZY_S3_SECRET_KEY` / `DZZY_S3_BUCKET`.

use dzzy_media::config::MediaConfig;
use dzzy_media::storage::MediaStorage;
