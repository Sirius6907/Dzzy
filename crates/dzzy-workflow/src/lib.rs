#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `dzzy-workflow` — Workflow engine for Dzzy.
//!
...
pub struct WorkflowEngine {
    ...
    pub(crate) workflow_cache:
        moka::sync::Cache<(CommunityId, Uuid), Arc<Vec<dzzy_db::workflow::WorkflowRecord>>>,
}
...
/// Build a [`executor::TriggerContext`] from a [`dzzy_core::StoredEvent`].
///
...
pub fn build_trigger_context(event: &dzzy_core::StoredEvent) -> executor::TriggerContext {
    let kind_u32 = event_kind_u32(&event.event);
...
fn trigger_matches_event(trigger: &TriggerDef, kind_u32: u32) -> bool {
    use dzzy_core::kind::{KIND_REACTION, KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_DIFF};
    match trigger {
...
"#;
    ...
    fn trigger_matches_stream_message() {
        ...
        assert!(trigger_matches_event(
            &trigger,
            dzzy_core::kind::KIND_STREAM_MESSAGE
        ));
        assert!(!trigger_matches_event(
            &trigger,
    ...
    fn schedule_trigger_never_matches_events() {
        ...
        assert!(!trigger_matches_event(
            &trigger,
            dzzy_core::kind::KIND_WORKFLOW_TRIGGERED
        ));
    }
    ...
    fn workflow_execution_kinds_do_not_match_any_trigger() {
        ...
        for kind in dzzy_core::kind::KIND_WORKFLOW_TRIGGERED
            ..=dzzy_core::kind::KIND_WORKFLOW_APPROVAL_DENIED
        {
            assert!(
    ...
    fn make_message_event() -> dzzy_core::StoredEvent {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "hello world")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        dzzy_core::StoredEvent::new(event, Some(Uuid::new_v4()))
    }

    /// Create a reaction event with an `e` tag pointing to a target message.
    fn make_reaction_event() -> (dzzy_core::StoredEvent, String) {
    ...
    fn build_trigger_context_no_channel_id() {
        ...
        let stored = dzzy_core::StoredEvent::new(event, None);
        let ctx = build_trigger_context(&stored);
    ...
    fn test_build_trigger_context_reaction_multiple_e_tags() {
        ...
        let stored = dzzy_core::StoredEvent::new(event, Some(Uuid::new_v4()));
        let ctx = build_trigger_context(&stored);
