...
//! (OPS metadata) and tells Dzzy where to find personas, hooks, and MCP
//! config.
//!
//! ```json
//! {
...
pub struct Engines {
    /// Semver range the Dzzy runtime must satisfy (e.g. `">=0.9.0"`).
    #[serde(skip_serializing_if = "Option::is_none", alias = "dzzy")]
    pub dzzy: Option<String>,
}
...
/// validator issues advisory warnings for Dzzy-unknown keys.
...
mod tests {
    ...
    fn parse_full_manifest() {
        ...
        assert_eq!(m.engines.unwrap().dzzy.as_deref(), Some(">=0.9.0"));
