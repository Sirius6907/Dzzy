...
for deployment in Dzzy. It is a **superset of the [Open Plugin Spec](https://open-plugin-spec.org)**
...
1. **Portable** — zip file or git repo; no Dzzy tooling required to inspect
...
4. **Harness-honest** — explicit about what the agent runtime does vs. what dzzy-acp does
...
and Dzzy-specific extensions live alongside the OPS fields at the top level. Since the Open
...
```json
{
  ...
  "version": "1.2.0",
  "description": "A four-agent security review team for Dzzy.",
  "author": "Meadow Engineering",
  "license": "MIT",
  "homepage": "https://github.com/example/meadow-security-team",
  "keywords": ["security", "code-review", "dzzy"],
  "engines": {
    "dzzy": ">=0.9.0"
  },
  "personas": [
...
- **Dzzy**: reads both OPS fields and the Dzzy-specific fields; `personas` is authoritative.
- **Version negotiation**: `engines.dzzy` specifies minimum required Dzzy version; dzzy-acp
  rejects packs requiring a newer version.
- **Extension mechanism**: Dzzy-specific fields sit at the top level of `plugin.json` alongside
  OPS fields. No OPS core field is overloaded.
- **`defaults`**: ignored entirely by OPS consumers. dzzy-acp resolves it at deploy time before
...
└── my-pack-1.2.0.dzzypack.sha256  # Checksum (required for zip distribution)
```
...
# === Behavioral Config (Dzzy-specific) ===
subscribe:
  - "#security-reviews"
  - "#code-reviews"
triggers:
...
| `display_name` | string | ✅ | Human-readable name shown in Dzzy UI. |
...
dzzy-acp assembles the agent's context from two distinct prompt layers before sending each
...
```
[Base]
<platform orientation — injected by dzzy-acp>
...
The `[Base]` layer is compiled into dzzy-acp and is **identical for every agent**. It covers:

| Content | Purpose |
|---------|---------|
| Platform identity | Tells the agent it is running inside Dzzy and what that means |
...
Pack authors do not write or configure the `[Base]` layer — it is maintained by the Dzzy team
and updated in dzzy-acp releases.

**Disabling or customizing the base layer**: Set `DZZY_ACP_NO_BASE_PROMPT` to omit the `[Base]`
section entirely. To replace the compiled-in default with custom content, set
`DZZY_ACP_BASE_PROMPT_FILE` to a file path — dzzy-acp reads it at startup and uses it instead.
...
- That the agent is running inside Dzzy (covered by `[Base]`)
...
> **Note**: `.agents/skills/` is dzzy-acp's canonical skill location. The other paths shown
...
$AGENT_CWD/.agents/skills/<skill-name>/SKILL.md   ← dzzy-acp uses this one
```
...
dzzy-acp process. dzzy-acp determines what value to pass as `NewSessionRequest.cwd` in this
order:

1. The `AGENT_CWD` environment variable on the dzzy-acp process, if set.
2. `std::env::current_dir()` as a fallback.
3. If both fail, dzzy-acp logs an error and **refuses to start**.
...
is **not overwritten**. This allows operators to pin custom skill versions. dzzy-acp **must log
...
dzzy-acp lists available skills in the user message prefix so the agent knows what's available.
...
by the agent runtime. `dzzy pack validate` warns on skill name mismatches but does not yet
...
dzzy-acp merges them and passes the result via the ACP protocol — no filesystem placement required.
...
When implemented, all `env` values will be scanned for `${VAR_NAME}`. dzzy-acp will resolve
from the process environment **before** passing to the agent runtime. Unresolved variables will
cause a startup error.

### Delivery

dzzy-acp passes the merged config via `NewSessionRequest.mcp_servers`. **No `.mcp.json` is written to the agent's working directory.**
...
in the pack. dzzy-acp appends it to the persona prompt in the user message prefix.

dzzy-acp appends `instructions.md` to the persona prompt in the user message prefix (see
...
Hooks are shell commands fired by dzzy-acp at agent lifecycle points. **Agent runtimes typically have no hook system**
...
Hooks run as the dzzy-acp user; working directory is `$AGENT_CWD`; agent env vars are available.
...
that must be implemented directly in dzzy-acp's dispatch loop, not via a hook.
...
`on_stop` fires on normal exit and on handled errors. It **will not fire** if dzzy-acp crashes
...
**Hooks are NOT agent runtime features.** They are implemented entirely in dzzy-acp. Bypassing
dzzy-acp means no hooks fire.
...
Dzzy conversations. These are all Dzzy-specific — the agent runtime has no awareness of them. They sit
...
In this spec, **"deploy time"** means when dzzy-acp loads the pack and constructs per-persona
session configurations — typically at dzzy-acp process startup. For git-based packs, this occurs
each time dzzy-acp starts and reads the installed pack directory.

When dzzy-acp resolves the effective configuration for a persona, it applies this order (highest
wins):

```
1. Operator env vars           — e.g. GOOSE_MODEL, GOOSE_PROVIDER (agent-runtime-specific)
                                 already set in the parent process environment
2. Desktop UI per-agent        — overrides set in the Dzzy desktop app per-agent settings
...
5. Built-in defaults           — dzzy-acp's hardcoded fallback values
```

dzzy-acp resolves levels 3–5 at deploy time (when the pack is loaded and sessions are
constructed). Levels 1–2 are applied at runtime and are outside the pack's control.

**Level 1 — Operator env vars**: If the operator has already set env vars for model, provider, temperature, or context limit in the parent process environment, dzzy-acp MUST NOT override them with pack/persona values. dzzy-acp only injects env vars for fields that are NOT already set in the parent environment. This ensures operators can always override pack configuration.

**Implementation**: when constructing the child process environment, dzzy-acp checks
...
| `temperature` | float | `0.7` | Provider-dependent (typically 0.0–2.0). dzzy-acp passes through without range validation; `dzzy pack validate` checks type only (must be a number), not range. | Passed as env var to agent runtime. |
...
**Unknown keys** in `defaults` (in `plugin.json`) are **validation warnings** in `dzzy pack
...
dzzy-acp logs a `WARN` and ignores unknown manifest keys, remaining fail-soft:
...
The `#` prefix in `subscribe` entries is a **display convention only**. Channel names in the Dzzy
relay are stored and queried **without** the `#` prefix. dzzy-acp strips the leading `#` before
making any relay API calls. `"#security-reviews"` and `"security-reviews"` are equivalent in this
field.

### Env Var Mapping

dzzy-acp resolves pack defaults and per-persona overrides (precedence levels 3–5) into a single
...
dzzy-acp translates persona behavioral config fields to agent configuration via environment
...
If `model` is omitted from both the persona frontmatter and `defaults`, dzzy-acp does not set
`GOOSE_PROVIDER` or `GOOSE_MODEL`, and the agent runtime uses its configured operator default.

> **Implementation note**: `AcpClient::spawn` accepts per-persona env vars via the `extra_env` parameter. dzzy-acp checks `std::env::var(key)` before injecting each var — if the parent environment already has the key set, injection is skipped (operator precedence, level 1).
...
> Field Schema table lists dzzy-acp's built-in fallbacks (precedence level 5). If `defaults` is
...
All fields are consumed entirely by dzzy-acp. None are passed to the agent runtime directly — they are projected as env vars or used by the harness's subscription/dispatch logic.
...
A pack is distributed as a `.dzzypack` file (zip archive):

```bash
dzzy pack validate ./my-pack
dzzy pack ./my-pack --output my-pack-1.2.0.dzzypack
dzzy install ./my-pack-1.2.0.dzzypack
dzzy install https://example.com/releases/my-pack-1.2.0.dzzypack
```

#### Pack Integrity (Required)

Zip packs **must** ship with `<pack-name>-<version>.dzzypack.sha256` containing `sha256sum`
output (`<hex-digest>  <filename>`). dzzy-acp **must** verify before installation and refuse on
...
```json
{
  "com.example.meadow-security-team": {
    "source": "https://example.com/releases/my-pack-1.2.0.dzzypack",
    "sha256": "a3f1c2d4e5b6...",
    "version": "1.2.0",
...
```bash
dzzy install github:example/meadow-security-team
dzzy install github:example/meadow-security-team@v1.2.0
dzzy install git+https://gitlab.example.com/team/pack.git
```
...
A Dzzy-hosted registry and in-app browser for discovering and installing packs. API-compatible
with OPS registries. Details TBD.

### Installed Pack Location

Installed packs live at `~/.dzzy/packs/<pack-id>/`. dzzy-acp reads packs from this location
at agent startup.

### Desktop App Import

The Dzzy desktop app can import persona packs via the Import button:
...
|-----------|----------------|-----------|-------------------|
| Skills | Copy at deploy time (planned) | dzzy-acp will copy `skills/` → `$AGENT_CWD/.agents/skills/` | ✅ Yes (only one) |
| MCP servers | ACP protocol | `NewSessionRequest.mcp_servers` | ❌ No |
| Persona prompt | User message prefix | `[System]` block prepended to user message text by dzzy-acp | ❌ No |
| Pack instructions | User message prefix | Appended to `[System]` block in user message text | ❌ No |
| Lifecycle hooks | Harness internal | dzzy-acp fires shell commands directly | ❌ No |
| Model/provider | Child process env vars | Agent-runtime-specific env vars (e.g. `GOOSE_PROVIDER`, `GOOSE_MODEL`) | ❌ No |
| Behavioral config | Harness internal | dzzy-acp subscription + dispatch logic | ❌ No |
| Pack defaults (`defaults`) | Harness internal | Resolved at deploy time by dzzy-acp into per-persona effective config; never passed to the agent runtime directly | ❌ No |

> **Pack defaults are resolved at deploy time**, not at runtime. When dzzy-acp loads a pack and
...
dzzy-acp's `format_prompt()` in `queue.rs` prepends a `[System]` block to the **user message
text** before sending it to the agent runtime. This is a **dzzy-acp feature, not an agent runtime
...
harness-side interpolation is implemented, dzzy-acp will resolve them from the process
...
- **Phase 1 (zip)**: Packs **must** ship with `<pack-name>-<version>.dzzypack.sha256` containing
  `sha256sum` output (`<hex-digest>  <filename>`). dzzy-acp **must** verify before installation
  and refuse on mismatch.
- **Phase 2 (git)**: `pack.lock` pins the resolved commit SHA; dzzy-acp verifies on install.
- **Phase 3 (registry)**: Registry signatures TBD.

### Hook Execution

Hooks run with dzzy-acp's privileges — significant attack surface. Only install packs from
trusted sources. Review all hook commands before installing. Consider sandboxing dzzy-acp
(container, restricted user) for untrusted packs. dzzy-acp should display hook commands before
...
### From V6 (dzzy-namespaced) Format

Field mapping from V6 `.persona.md` to current `.persona.md`:

| V6 location | Current location |
|---|---|
| `dzzy.model` | `model` (top-level frontmatter) |
| `dzzy.temperature` | `temperature` (top-level frontmatter) |
| `dzzy.max_context_tokens` | `max_context_tokens` (top-level frontmatter) |
| `dzzy.subscribe` | `subscribe` (top-level frontmatter) |
| `dzzy.respond_to` | `triggers` (top-level frontmatter) |
| `dzzy.thread_replies` | `thread_replies` (top-level frontmatter) |
| `dzzy.broadcast_replies` | `broadcast_replies` (top-level frontmatter) |
| `plugin.json` → `dzzy.defaults` | `plugin.json` → `defaults` (top-level) |
| `plugin.json` → `dzzy.personas` | `plugin.json` → `personas` (top-level) |
| `plugin.json` → `dzzy.pack_instructions` | `plugin.json` → `pack_instructions` (top-level) |
| `plugin.json` → `dzzy.mcp_config` | `plugin.json` → `mcp_config` (top-level) |
| `plugin.json` → `dzzy.hooks_config` | `plugin.json` → `hooks_config` (top-level) |
...
5. Run `dzzy pack validate ./my-pack`

### Backward Compatibility

The V6 namespaced `dzzy:` block format is not supported. Only the current flat top-level fields format is accepted. The `respond_to` key is accepted as a legacy alias for `triggers` in both persona frontmatter and `plugin.json` defaults.
...
1. **`session/set_model` as env var alternative**: The ACP runtime implements `on_set_model()` (ACP
   unstable feature). dzzy-acp could call `session/set_model` after `session/new` to set the
...
2. **`CONTEXT_FILE_NAMES` env var**: The goose agent runtime supports this env var to control which filenames are
   scanned for hints. Should dzzy-acp set this to include pack-specific filenames? Deferred
   pending use case.
...
`dzzy pack init` scaffolding; hot reload of skills/instructions; skill marketplace; pack dependencies; agent-to-agent handoff within a pack.
...
| PF-2 | `dzzy pack validate` CLI: **Implemented.** Schema-validates `plugin.json`; checks `.persona.md` required identity fields; validates behavioral config fields; warns on unknown keys and skill name mismatches. Remaining: verify `skills:` and `hooks:` paths exist; error on `SKILL.md` missing `name:` or `description:`. | `dzzy-cli` / `dzzy-admin` |
| PF-3 | Skill collision warning: emit `WARN` when a pack skill is skipped because a skill with the same load key already exists in `.agents/skills/`. | dzzy-acp skill copy logic |
| PF-4 | `$AGENT_CWD` resolution: determine `NewSessionRequest.cwd` from (1) `AGENT_CWD` env var, (2) `std::env::current_dir()`, (3) error and refuse to start. | dzzy-acp startup / session init |
| PF-5 | Skill parse failure warning: emit `WARN` when `parse_skill_content` returns `None` (missing `name:`, missing `description:`, or malformed frontmatter). Currently the agent runtime silently skips. dzzy-acp should pre-validate during skill copy. | dzzy-acp skill copy logic |
| PF-6 | Per-subprocess env var injection: **Implemented.** `AcpClient::spawn` accepts `extra_env: &[(String, String)]` injected via `Command::env()`. dzzy-acp checks `std::env::var(key)` before injecting — operator env vars take precedence (level 1). | `dzzy-acp/src/acp.rs` `AcpClient::spawn()` |
