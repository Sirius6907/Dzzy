...
//! - Dzzy-agent personas emit DZZY_AGENT_MODEL, DZZY_AGENT_PROVIDER
...
use dzzy_persona::resolve::resolve_pack;
...
#[test]
fn resolve_pack_dzzy_agent_persona_emits_dzzy_agent_vars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
...
"#,
    ...
    assert_eq!(
        env.get("DZZY_AGENT_MODEL"),
        Some(&"gpt-4o"),
        "should emit DZZY_AGENT_MODEL=gpt-4o"
    );
    assert_eq!(
        env.get("DZZY_AGENT_PROVIDER"),
        Some(&"openai"),
        "should emit DZZY_AGENT_PROVIDER=openai"
    );

    // Must NOT contain GOOSE_* keys
    assert!(
        !env.contains_key("GOOSE_MODEL"),
        "dzzy-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "dzzy-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    // Dzzy-agent persona
    fs::write(
        root.join("agents/dzzy-bot.persona.md"),
        r#"---
name: "dzzy-bot"
display_name: "Dzzy Bot"
description: "A dzzy-agent runtime bot"
runtime: "dzzy-agent"
model: "openai:gpt-4o"
---
You are a dzzy bot.
"#,
    ...
    let dzzy = pack
        .personas
        .iter()
        .find(|p| p.name == "dzzy-bot")
        .expect("dzzy-bot should exist");
    ...
    assert!(
        !goose_env.contains_key("DZZY_AGENT_MODEL"),
        "goose persona must not emit DZZY_AGENT_MODEL"
    );
    assert!(
        !goose_env.contains_key("DZZY_AGENT_PROVIDER"),
        "goose persona must not emit DZZY_AGENT_PROVIDER"
    );

    // Dzzy-agent persona gets DZZY_AGENT_* env vars
    let dzzy_env: std::collections::HashMap<_, _> = dzzy
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(dzzy_env.get("DZZY_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(dzzy_env.get("DZZY_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !dzzy_env.contains_key("GOOSE_MODEL"),
        "dzzy-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !dzzy_env.contains_key("GOOSE_PROVIDER"),
        "dzzy-agent persona must not emit GOOSE_PROVIDER"
    );
}
