...
impl Config {
    ...
    pub fn from_map(e: &HashMap<String, String>) -> Result<Self, ConfigError> {
        ...
        let grant_keys = parse_keyring(e, "DZZY_PUSH_GRANT_KEYS")?;
        let token_keys = parse_keyring(e, "DZZY_PUSH_TOKEN_KEYS")?;
        if grant_keys.iter().any(|grant| {
            token_keys
                .iter()
                .any(|token| grant.id == token.id || grant.key == token.key)
        }) {
            return Err(ConfigError::Invalid("DZZY_PUSH_TOKEN_KEYS"));
        }
        let public_delivery_url = req(e, "DZZY_PUSH_PUBLIC_DELIVERY_URL")?
            .parse::<url::Url>()
            .map_err(|_| ConfigError::Invalid("DZZY_PUSH_PUBLIC_DELIVERY_URL"))?;
        if public_delivery_url.scheme() != "https"
            || public_delivery_url.host_str() != Some("push.dzzy.xyz")
        ...
        {
            return Err(ConfigError::Invalid("DZZY_PUSH_PUBLIC_DELIVERY_URL"));
        }
        let max_grant_lifetime_seconds = req(e, "DZZY_PUSH_MAX_GRANT_LIFETIME_SECONDS")?
            .parse::<i64>()
            .ok()
            .filter(|seconds| (1..=31_536_000).contains(seconds))
            .ok_or(ConfigError::Invalid("DZZY_PUSH_MAX_GRANT_LIFETIME_SECONDS"))?;
        let max_installation_lifetime_seconds = e
            .get("DZZY_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS")
            .map(String::as_str)
            .unwrap_or("7776000")
            .parse::<i64>()
            .ok()
            ...
            .ok_or(ConfigError::Invalid(
                "DZZY_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS",
            ))?;
        ...
        let endpoint_quota_window_seconds =
            bounded_positive("DZZY_PUSH_ENDPOINT_QUOTA_WINDOW_SECONDS", 10, 86_400)?;
        let endpoint_quota_max_deliveries =
            bounded_positive("DZZY_PUSH_ENDPOINT_QUOTA_MAX_DELIVERIES", 10, 10_000)?;
        let enabled_profiles = req(e, "DZZY_PUSH_ENABLED_PROFILES")?
            .split(',')
            .map(|profile| match profile {
                "dzzy-ios-production" => Ok(crate::model::AppProfile::DzzyIosProduction),
                "dzzy-ios-sandbox" => Ok(crate::model::AppProfile::DzzyIosSandbox),
                _ => Err(ConfigError::Invalid("DZZY_PUSH_ENABLED_PROFILES")),
            })
            .collect::<Result<HashSet<_>, _>>()?;
        if enabled_profiles.is_empty() {
            return Err(ConfigError::Invalid("DZZY_PUSH_ENABLED_PROFILES"));
        }
        Ok(Self {
            bind_addr: e
                .get("DZZY_PUSH_BIND_ADDR")
                .map(String::as_str)
                .unwrap_or("0.0.0.0:8080")
                .parse()
                .map_err(|_| ConfigError::Invalid("DZZY_PUSH_BIND_ADDR"))?,
            health_addr: e
                .get("DZZY_PUSH_HEALTH_ADDR")
                .map(String::as_str)
                .unwrap_or("0.0.0.0:8081")
                .parse()
                .map_err(|_| ConfigError::Invalid("DZZY_PUSH_HEALTH_ADDR"))?,
            public_delivery_url,
            ...
            app_attest_app_id: req(e, "DZZY_PUSH_APP_ATTEST_APP_ID")?.to_owned(),
            app_attest_root_cert_path: req(e, "DZZY_PUSH_APP_ATTEST_ROOT_CERT_PATH")?.into(),
            grant_keys,
            token_keys,
            apns_key_path: req(e, "DZZY_PUSH_APNS_KEY_PATH")?.into(),
            apns_key_id: req(e, "DZZY_PUSH_APNS_KEY_ID")?.to_owned(),
            apns_team_id: req(e, "DZZY_PUSH_APNS_TEAM_ID")?.to_owned(),
            apns_topic: req(e, "DZZY_PUSH_APNS_TOPIC")?.to_owned(),
        })
    }
}
...
mod tests {
    ...
    fn base() -> HashMap<String, String> {
        HashMap::from([
            ...
            (
                "DZZY_PUSH_PUBLIC_DELIVERY_URL".into(),
                "https://push.dzzy.xyz/v1/deliveries/apns".into(),
            ),
            (
                "DZZY_PUSH_MAX_GRANT_LIFETIME_SECONDS".into(),
                "2592000".into(),
            ),
            (
                "DZZY_PUSH_ENABLED_PROFILES".into(),
                "dzzy-ios-production".into(),
            ),
            (
                "DATABASE_URL".into(),
                "postgres://dzzy:test@localhost/dzzy".into(),
            ),
            ("DZZY_PUSH_APP_ATTEST_APP_ID".into(), "TEAM.app".into()),
            (
                "DZZY_PUSH_APP_ATTEST_ROOT_CERT_PATH".into(),
                "/apple-root.pem".into(),
            ),
            ("DZZY_PUSH_APNS_KEY_PATH".into(), "/key.p8".into()),
            ("DZZY_PUSH_APNS_KEY_ID".into(), "key".into()),
            ("DZZY_PUSH_APNS_TEAM_ID".into(), "team".into()),
            ("DZZY_PUSH_APNS_TOPIC".into(), "app".into()),
        ])
    }
    ...
    fn malformed_security_configuration_fails_startup() {
        for (key, value) in [
            ...
            ("DZZY_PUSH_APP_ATTEST_APP_ID", ""),
            ("DZZY_PUSH_ENABLED_PROFILES", "unknown-profile"),
            ("DZZY_PUSH_MAX_GRANT_LIFETIME_SECONDS", "0"),
            ("DZZY_PUSH_MAX_GRANT_LIFETIME_SECONDS", "31536001"),
            ("DZZY_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS", "0"),
        ] {
            let mut env = base();
    ...
    fn cross_keyring_id_or_material_reuse_fails_startup() {
        for token_keys in [
            ...
            let mut env = base();
            env.insert("DZZY_PUSH_TOKEN_KEYS".into(), token_keys);
    ...
    fn malformed_or_empty_keyrings_fail_startup() {
        for (variable, value) in [
            ("DZZY_PUSH_GRANT_KEYS", ""),
            ("DZZY_PUSH_GRANT_KEYS", "missing_separator"),
            ("DZZY_PUSH_GRANT_KEYS", "id:bad-base64"),
            ("DZZY_PUSH_TOKEN_KEYS", ""),
            ("DZZY_PUSH_TOKEN_KEYS", "missing_separator"),
            ("DZZY_PUSH_TOKEN_KEYS", "id:bad-base64"),
        ] {
            let mut env = base();
