...
//! - `OTEL_SERVICE_NAME` (default: `dzzy-relay`; read explicitly — not via SDK detector)
...
/// 3. Hard-coded fallback `dzzy-relay`.
///
...
/// `dzzy-relay` default.  We therefore read `OTEL_SERVICE_NAME` explicitly
...
pub fn service_resource() -> Resource {
    // Honor OTEL_SERVICE_NAME when set+non-empty; otherwise use dzzy-relay.
    ...
    let service_name = std::env::var("OTEL_SERVICE_NAME")
        .ok()
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "dzzy-relay".to_string());
...
mod tests {
    ...
    fn test_service_resource_default_when_env_unset() {
        ...
        assert_eq!(
            ...
            Some("dzzy-relay"),
            "expected dzzy-relay fallback when OTEL_SERVICE_NAME is unset"
        );
    }
    ...
    fn test_service_resource_empty_string_falls_back_to_default() {
        ...
        assert_eq!(
            service_name_from(&r).as_deref(),
            Some("dzzy-relay"),
            "expected dzzy-relay fallback when OTEL_SERVICE_NAME is empty"
        );
    }
