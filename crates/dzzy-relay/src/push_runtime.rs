...
async fn process_match_batch(state: &AppState, batch: dzzy_db::push::ClaimedMatchBatch) {
    ...
    let mut wakes: Vec<dzzy_db::push::WakeRequest> = Vec::new();
    for job in &batch.jobs {
        ...
        match match_job(job, &context) {
            ...
            Err(e) => {
                ...
                if job.attempt >= dzzy_db::push::MAX_MATCH_ATTEMPTS {
    ...
    match state.db.enqueue_push_wakes(community, &wakes).await {
        ...
        Err(e) => {
            ...
            for (event_id, attempt) in pending {
                if attempt >= dzzy_db::push::MAX_MATCH_ATTEMPTS {
                    completed.push(event_id);
                } else {
                    retry.push(event_id);
                }
...
fn match_job(
    job: &dzzy_db::push::BatchedMatch,
    context: &MatchContext,
) -> anyhow::Result<Vec<dzzy_db::push::WakeRequest>> {
    ...
    for lease in &context.leases {
        ...
        wakes.push(dzzy_db::push::WakeRequest {
...
fn push_filter_authorized_for_event(
    ...
    if dzzy_core::kind::event_kind_u32(event) != dzzy_core::kind::KIND_GIFT_WRAP {
        return true;
    }
        ...
        match state.db.usage_community_hosts().await {
            Ok(communities) => {
                for community in communities {
                    let community = dzzy_core::CommunityId::from_uuid(community.id);
                    let until = Utc::now() + TimeDelta::seconds(CLAIM_SECS);
...
async fn deliver_one(
    state: &AppState,
    http: &reqwest::Client,
    claimed: dzzy_db::push::ClaimedWake,
) {
    let outcome = match state
        ...
        .await
    {
        Ok(dzzy_db::push::RevalidateWakeOutcome::Deliver(wake)) => wake,
        Ok(dzzy_db::push::RevalidateWakeOutcome::Suppressed) => {
            let _ = state
                .db
...
async fn retry_or_fail(state: &AppState, wake: &dzzy_db::push::ClaimedWake, delay: i64) {
