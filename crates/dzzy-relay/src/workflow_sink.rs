...
use dzzy_core::kind::KIND_STREAM_MESSAGE;
use dzzy_core::tenant::CommunityId;
use dzzy_workflow::action_sink::{ActionSink, ActionSinkError};
use chrono::Utc;
...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let tenant = dzzy_core::tenant::TenantContext::resolved(community_id, host);
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    dzzy_db::DbError::ChannelNotFound(_) | dzzy_db::DbError::NotFound(_) => {
            ...
            //    - `dzzy:workflow` tag prevents recursive workflow triggering
            ...
            let thread_meta = Some(dzzy_db::event::ThreadMetadataParams {
...
mod integration_tests {
    ...
    //!   `cargo test -p dzzy-relay --lib workflow_sink -- --ignored`
    use super::*;
    use dzzy_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use dzzy_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
