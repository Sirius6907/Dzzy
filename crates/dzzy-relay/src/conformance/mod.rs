...
//! from `dzzy-conformance`. See `crates/dzzy-conformance/src/lib.rs` for
...
///   [`dzzy_db::Dzzy::communities_of_channels`]). On a hit, return the
...
fn project_row_community(
    ...
    channel_communities: &std::collections::HashMap<Uuid, dzzy_core::CommunityId>,
...
/// [`dzzy_db::Dzzy::communities_of_channels`] over the distinct
/// channel ids in `rows`.
