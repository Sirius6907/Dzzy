...
use dzzy_auth::AuthService;
use dzzy_core::CommunityId;
use dzzy_db::{Db, DbConfig};
use dzzy_pubsub::PubSubManager;
use dzzy_search::SearchService;

use dzzy_relay::config::Config;
use dzzy_relay::metrics as relay_metrics;
use dzzy_relay::router::{build_health_router, build_router};
use dzzy_relay::state::AppState;
use dzzy_relay::storage_sweep;
use dzzy_relay::telemetry;
use dzzy_workflow::WorkflowEngine;
use tokio_util::sync::CancellationToken;

fn dzzy_auto_migrate_enabled(value: Option<&str>) -> bool {
...
///
/// Fleet-wide totals (`dzzy_total_*`) always emit regardless of mode.
///
/// Set via `DZZY_USAGE_METRICS_PER_COMMUNITY`:
...
impl EmissionScope {
    fn from_env() -> Self {
        let raw = std::env::var("DZZY_USAGE_METRICS_PER_COMMUNITY")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase();
        match raw.as_str() {
            ...
            other => {
                warn!(
                    value = other,
                    "DZZY_USAGE_METRICS_PER_COMMUNITY: unknown value — defaulting to all"
                );
                EmissionScope::All
            }
        }
...
async fn main() -> anyhow::Result<()> {
    ...
    //
    // Build a single shared Resource (service.name=dzzy-relay by default, overridable
    ...
    let otel_layer = match &tracer_init {
        telemetry::TracerInit::Enabled(p) => {
            ...
            Some(tracing_opentelemetry::layer().with_tracer(p.tracer("dzzy-relay")))
        }
        _ => None,
    };
    ...
    tracing_subscriber::registry()
        ...
        .with(EnvFilter::from_default_env().add_directive("dzzy_relay=info".parse()?))
        .with(otel_layer)
        .init();
    ...
    metrics::gauge!("dzzy_audit_enabled").set(if config.audit_enabled { 1.0 } else { 0.0 });
    info!(
    ...
    let auto_migrate =
        dzzy_auto_migrate_enabled(std::env::var("DZZY_AUTO_MIGRATE").ok().as_deref());
    if auto_migrate {
        ...
        info!("Skipping database migrations because DZZY_AUTO_MIGRATE is not enabled");
    }
    ...
    // DZZY_AUTO_MIGRATE off and migration 0021 unapplied can never open the
    ...
    if config.require_relay_membership && config.relay_owner_pubkey.is_none() {
        error!(
            "DZZY_REQUIRE_RELAY_MEMBERSHIP=true but RELAY_OWNER_PUBKEY is not set or invalid. \
             Set RELAY_OWNER_PUBKEY to a valid 64-char hex pubkey."
        );
        return Err(anyhow::anyhow!(
            "RELAY_OWNER_PUBKEY required when DZZY_REQUIRE_RELAY_MEMBERSHIP=true"
        ));
    }
    ...
    if config.require_relay_membership && config.relay_private_key.is_none() {
        return Err(anyhow::anyhow!(
            "DZZY_RELAY_PRIVATE_KEY is required when DZZY_REQUIRE_RELAY_MEMBERSHIP=true. \
    ...
    let deployment_community = {
        let host = dzzy_relay::tenant::relay_url_authority(&config.relay_url);
        if host.is_empty() {
            if config.require_relay_membership {
                return Err(anyhow::anyhow!(
                    "Cannot derive a community host from DZZY_RELAY_URL ({:?}); a resolvable host is required when DZZY_REQUIRE_RELAY_MEMBERSHIP=true",
                    config.relay_url
                ));
            }
            error!(
            ...
            match db.ensure_configured_community(&host).await {
                ...
                Err(e) => {
                    if config.require_relay_membership {
                        ...
                        return Err(anyhow::anyhow!(
                            "Failed to ensure deployment community (required when DZZY_REQUIRE_RELAY_MEMBERSHIP=true): {e}"
                        ));
                    }
        ...
        match db.backfill_from_allowlist(community).await {
            ...
            Err(e) => {
                if config.require_relay_membership {
                    ...
                    return Err(anyhow::anyhow!(
                        "Failed to backfill pubkey_allowlist (required when DZZY_REQUIRE_RELAY_MEMBERSHIP=true): {e}"
                    ));
                } else {
        ...
        match db.bootstrap_owner(community, owner_pubkey).await {
            ...
            Err(e) => {
                if config.require_relay_membership {
                    ...
                    return Err(anyhow::anyhow!(
                        "Failed to bootstrap relay owner (required when DZZY_REQUIRE_RELAY_MEMBERSHIP=true): {e}"
                    ));
                } else {
                    error!(
    ...
    let audit = if config.audit_enabled {
        ...
        info!("Audit logging disabled by DZZY_AUDIT_ENABLED");
        None
    };
    ...
    let workflow_config = dzzy_workflow::WorkflowConfig::default();
    ...
    let relay_keypair = if let Some(hex) = &config.relay_private_key {
        nostr::Keys::parse(hex)
            .map_err(|e| anyhow::anyhow!("invalid DZZY_RELAY_PRIVATE_KEY: {e}"))?
        ...
        tracing::warn!(
            ...
            "Using hardcoded dev relay keypair (DZZY_REQUIRE_AUTH_TOKEN=false). \
             Set DZZY_RELAY_PRIVATE_KEY for production."
        );
        keys
    } else {
        panic!(
            "DZZY_RELAY_PRIVATE_KEY must be set when DZZY_REQUIRE_AUTH_TOKEN=true. \
    ...
    let media_storage = dzzy_media::MediaStorage::new(&config.media)
    ...
    // Inter-relay mesh (DZZY_MESH seam). `boot_mesh` returns None when the
    ...
    if let Some(handle) = dzzy_relay::mesh_boot::boot_mesh(
        &state.config,
    ...
    .await?
    {
        ...
        // DZZY_MESH_DEMO_ECHO) before peers can route traffic here.
    ...
    if std::env::var("DZZY_GIT_CONFORMANCE_PROBE")
        .map(|v| v != "false")
        .unwrap_or(true)
    {
        let race_width = std::env::var("DZZY_GIT_PROBE_WRITERS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(32);
        let race_rounds = std::env::var("DZZY_GIT_PROBE_ROUNDS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(3);
        let cfg = dzzy_relay::api::git::store::ProbeConfig {
            race_width,
            race_rounds,
        };
        tracing::info!(
    ...
    if config.require_relay_membership {
        match dzzy_relay::handlers::side_effects::reconcile_nip43_membership_snapshots(&state).await
        {
        ...
        let interval_secs = std::env::var("DZZY_NIP43_RECONCILE_INTERVAL_SECS")
            .ok()
        ...
        tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                match dzzy_relay::handlers::side_effects::reconcile_nip43_membership_snapshots(
                    &reconcile_state,
                )
                .await
                {
    ...
    // Only runs when DZZY_RECONCILE_CHANNELS=true (dev/CI environments).
    // Production relays create channels through the event pipeline and don't need this.
    if std::env::var("DZZY_RECONCILE_CHANNELS").is_ok() {
        let reconcile_state = Arc::clone(&state);
        tokio::spawn(async move {
            ...
            let tenant = match dzzy_relay::tenant::bind_deployment_community(
                &reconcile_state.db,
            ...
            for attempt in 0..24u32 {
                ...
                match dzzy_relay::handlers::side_effects::reconcile_channel_events(
                    &tenant,
                    &reconcile_state,
                )
                .await
    ...
    let action_sink = Arc::new(dzzy_relay::workflow_sink::RelayActionSink::new(&state));
    ...
    // together with the workflow engine in a future multi-pod coordination pass.
    {
        ...
        let reaper_interval_secs: u64 = std::env::var("DZZY_REAPER_INTERVAL_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(60);
        tokio::spawn(async move {
            ...
            loop {
                ...
                for channel in &expired {
                    ...
                    let tenant = dzzy_core::tenant::TenantContext::resolved(
                        channel.community_id,
                        channel.host.clone(),
                    );
                    let channel_id = channel.channel_id;
                    // Emit a system message so members see why the channel was archived.
                    if let Err(e) = dzzy_relay::handlers::side_effects::emit_system_message(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    ...
                    if let Err(e) = dzzy_relay::handlers::side_effects::emit_group_discovery_events(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    )
                    ...
                    dzzy_relay::handlers::side_effects::evict_all_channel_subscriptions(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    )
    ...
    if state.config.push_gateway_delivery_url.is_some() {
        tokio::spawn(dzzy_relay::push_runtime::run_matcher(Arc::clone(&state)));
        tokio::spawn(dzzy_relay::push_runtime::run_delivery_worker(Arc::clone(
            &state,
        )));
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for reminder in due {
                    ...
                    let reminder_tenant = dzzy_core::tenant::TenantContext::resolved(
    ...
    // (published by other relay instances) and fan out to local WS subscribers.
    {
        ...
        tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    Ok(channel_event) => {
                        dzzy_relay::handlers::event::fan_out_pubsub_event(
                            &state_for_sub,
                            channel_event,
                        )
                        .await;
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        metrics::counter!("dzzy_multinode_fanout_lag_total").increment(n);
        ...
        tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        metrics::counter!("dzzy_cache_invalidation_lag_total").increment(n);
    ...
    // so missed archive commands still converge without a global DB scan.
    {
        ...
        let interval_secs = std::env::var("DZZY_COMMUNITY_REVALIDATE_INTERVAL_SECS")
            .ok()
            .and_then(|value| value.parse::<u64>().ok())
            .unwrap_or(30)
            .clamp(1, 300);
            ...
            loop {
                match rx.recv().await {
                    Ok(scoped) => match scoped.command {
                        dzzy_pubsub::conn_control::ConnControl::DisconnectCommunity => {
                            state_for_conn_ctrl
                                .community_connections
                                .disconnect_community(scoped.community_id);
                        }
                        dzzy_pubsub::conn_control::ConnControl::DisconnectPubkey {
                            pubkey,
                            event_id,
                            reason,
                        } => {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        metrics::counter!("dzzy_conn_control_lag_total").increment(n);
    ...
    // Pool metrics: periodic background task polling DB + Redis pool stats.
    {
        ...
        let interval_secs = std::env::var("DZZY_POOL_METRICS_INTERVAL_SECS")
            .ok()
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                metrics::gauge!("dzzy_db_pool_size").set(db_stats.size as f64);
                metrics::gauge!("dzzy_db_pool_idle").set(db_stats.idle as f64);
                metrics::gauge!("dzzy_db_pool_active").set(active as f64);
                metrics::gauge!("dzzy_db_pool_max").set(db_stats.max as f64);

                if let Some(read_stats) = pool_state.db.read_pool_stats() {
                    let read_active = read_stats.size.saturating_sub(read_stats.idle);
                    metrics::gauge!("dzzy_db_read_pool_size").set(read_stats.size as f64);
                    metrics::gauge!("dzzy_db_read_pool_idle").set(read_stats.idle as f64);
                    metrics::gauge!("dzzy_db_read_pool_active").set(read_active as f64);
                    metrics::gauge!("dzzy_db_read_pool_max").set(read_stats.max as f64);
                    ...
                    match pool_state.db.fence().verified_through() {
                        Some(fence_ts) => {
                            ...
                            metrics::gauge!("dzzy_db_replica_fence_open").set(1.0);
                            metrics::gauge!("dzzy_db_replica_fence_lag_seconds").set(lag as f64);
                        }
                        None => {
                            metrics::gauge!("dzzy_db_replica_fence_open").set(0.0);
                        }
                    }
                }

                let rs = pool_state.redis_pool.status();
                metrics::gauge!("dzzy_redis_pool_available").set(rs.available as f64);
                metrics::gauge!("dzzy_redis_pool_size").set(rs.size as f64);
                metrics::gauge!("dzzy_redis_pool_max").set(rs.max_size as f64);
                metrics::gauge!("dzzy_redis_pool_waiting").set(rs.waiting as f64);
            }
        });
    }
    ...
    //   In-memory:  each pod exports its partition → dashboard uses sum()
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                metrics::gauge!("dzzy_usage_poller_is_leader").set(if leader.is_some() {
                    1.0
                } else {
                    0.0
                });
...
/// ```text
/// ┌─────────────────────────────────────────────────────────┐
/// │  Listener 1: TCP DZZY_BIND_ADDR:3000  (app router)   │
/// │  Listener 2: UDS DZZY_UDS_PATH        (app, optional)│
...
async fn serve(
    ...
    info!(addr = %config.bind_addr, "dzzy-relay TCP listening");
    ...
    if let Some(ref uds_path) = config.uds_path {
        ...
        match std::fs::symlink_metadata(uds_path) {
            ...
            Ok(_) => {
                return Err(anyhow::anyhow!(
                    "DZZY_UDS_PATH {uds_path} exists but is not a socket"
                ));
            }
            Err(_) => {}
        }
        ...
        info!(path = %uds_path, "dzzy-relay UDS listening");
    ...
    if config.uds_path.is_some() {
        tracing::warn!("DZZY_UDS_PATH set but UDS not supported on this platform");
    }
...
fn reminder_to_event(reminder: &dzzy_db::event::DueReminder) -> nostr::Event {
...
fn usage_metrics_interval_secs() -> u64 {
    std::env::var("DZZY_USAGE_METRICS_INTERVAL_SECS")
        .ok()
...
fn usage_metrics_idle_timeout_secs(interval_secs: u64) -> u64 {
    let configured = std::env::var("DZZY_USAGE_METRICS_IDLE_TIMEOUT_SECS")
        .ok()
        .and_then(|value| value.parse().ok());
    ...
    fn set(&self, value: f64) {
        match self {
            Self::WsConnections(community) => {
                metrics::gauge!("dzzy_community_ws_connections", "community" => community.clone())
                    .set(value);
            }
            Self::UsersOnline(community) => {
                metrics::gauge!("dzzy_community_users_online_pod", "community" => community.clone())
                    .set(value);
            }
            Self::Subscriptions(community) => {
                metrics::gauge!("dzzy_community_subscriptions", "community" => community.clone())
                    .set(value);
            }
        }
    }
...
fn refresh_legacy_active_gauge_recency() {
    metrics::gauge!("dzzy_ws_connections_active").increment(0.0);
    metrics::gauge!("dzzy_subscriptions_active").increment(0.0);
}
...
fn emit_in_memory_usage_metrics(
    ...
    metrics::gauge!("dzzy_total_ws_connections").set(total_connections as f64);
    metrics::gauge!("dzzy_total_users_online_pod").set(users_online.values().sum::<u64>() as f64);
    metrics::gauge!("dzzy_total_subscriptions").set(total_subscriptions as f64);
...
async fn run_usage_metrics_tick(
    state: &AppState,
    emission_scope: &EmissionScope,
    leader: &mut Option<dzzy_db::UsageMetricsLeader>,
...
async fn run_storage_sweep_tick(
    ...
    storage_sweep::maybe_spawn_sweep(
        ...
        config.timeout,
        async move {
            dzzy_media::fold_bucket_listing(max_objects, move |token| {
...
async fn emit_db_usage_metrics(
    ...
    // this tick. Fleet-wide totals (dzzy_total_*) always emit regardless.
    ...
    // dzzy_communities_total (no tag — fleet-wide count)
    metrics::gauge!("dzzy_communities_total").set(community_total as f64);

    // dzzy_community_users{community, type:human|agent}
    ...
    // rather than keeping the last nonzero value until process restart.
    {
        ...
        metrics::gauge!("dzzy_total_users", "type" => "human").set(total_human as f64);
        metrics::gauge!("dzzy_total_users", "type" => "agent").set(total_agent as f64);
        ...
        for (&id, community) in host_map {
            ...
            metrics::gauge!("dzzy_community_users", "community" => community.clone(), "type" => "human")
                .set(human as f64);
            metrics::gauge!("dzzy_community_users", "community" => community.clone(), "type" => "agent")
                .set(agent as f64);
        }
    }

    // dzzy_community_channels{community, type}
    ...
    // drops to zero emits 0 rather than retaining its last nonzero value.
    {
        ...
        for &ct in CHANNEL_TYPES {
            let total: i64 = host_map
                ...
                .sum();
            metrics::gauge!("dzzy_total_channels", "type" => ct).set(total as f64);
        }
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
    ...
    // dzzy_community_messages{community}
    // Emit 0 for communities with no messages so dashboards don't stale-read.
    {
        ...
        metrics::gauge!("dzzy_total_messages").set(total as f64);
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
                continue;
            ...
            metrics::gauge!("dzzy_community_messages", "community" => community.clone())
                .set(count as f64);
        }
    }

    // dzzy_community_relay_members{community, role}
    ...
    // CHECK constraint over {'owner', 'admin', 'member'}.
    {
        ...
        for &role in RELAY_ROLES {
            let total: i64 = host_map
                ...
                .sum();
            metrics::gauge!("dzzy_total_relay_members", "role" => role).set(total as f64);
        }
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
    ...
    // dzzy_community_workflows{community, status}
    ...
    // DB enum: {'active', 'disabled', 'archived'}.
    {
        ...
        for &status in WORKFLOW_STATUSES {
            let total: i64 = host_map
                ...
                .sum();
            metrics::gauge!("dzzy_total_workflows", "status" => status).set(total as f64);
        }
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
    ...
    // dzzy_community_git_repos{community}
    // Emit 0 for communities with no repos.
    {
        ...
        // Fleet total (always emitted).
        let total: i64 = rows.values().sum();
        metrics::gauge!("dzzy_total_git_repos").set(total as f64);
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
                continue;
            ...
            let count = rows.get(&id).copied().unwrap_or(0);
            metrics::gauge!("dzzy_community_git_repos", "community" => community.clone())
                .set(count as f64);
        }
    }
    ...
    for (data, label) in [
        ...
        metrics::gauge!("dzzy_total_active_users", "window" => label, "type" => "human")
            .set(total_human as f64);
        metrics::gauge!("dzzy_total_active_users", "window" => label, "type" => "agent")
            .set(total_agent as f64);
        metrics::gauge!("dzzy_total_active_users", "window" => label, "type" => "unknown")
    ...
    for (data, label) in [(active_channels_1d, "1d"), (active_channels_7d, "7d")] {
        ...
        // Fleet total (always emitted).
        let total: i64 = rows.values().sum();
        metrics::gauge!("dzzy_total_active_channels", "window" => label).set(total as f64);
        // Per-community series (gated by active_set).
        for (&id, community) in host_map {
            if !active_set.contains(&id) {
                continue;
            ...
            let count = rows.get(&id).copied().unwrap_or(0);
            metrics::gauge!(
                "dzzy_community_active_channels",
                "community" => community.clone(),
                "window" => label
            )
            .set(count as f64);
...
mod tests {
    ...
    #[test]
    fn dzzy_auto_migrate_is_opt_in() {
        assert!(!dzzy_auto_migrate_enabled(None));
        assert!(!dzzy_auto_migrate_enabled(Some("")));
        assert!(!dzzy_auto_migrate_enabled(Some("false")));
        assert!(!dzzy_auto_migrate_enabled(Some("0")));
        assert!(!dzzy_auto_migrate_enabled(Some("no")));

        assert!(dzzy_auto_migrate_enabled(Some("true")));
        assert!(dzzy_auto_migrate_enabled(Some("TRUE")));
        assert!(dzzy_auto_migrate_enabled(Some(" 1 ")));
        assert!(dzzy_auto_migrate_enabled(Some("yes")));
        assert!(dzzy_auto_migrate_enabled(Some("on")));
    }
    ...
    fn test_legacy_gauge_recency_refresh_preserves_lifecycle_deltas() {
        ...
        metrics::with_local_recorder(&recorder, || {
            let connections = metrics::gauge!("dzzy_ws_connections_active");
            let subscriptions = metrics::gauge!("dzzy_subscriptions_active");
        ...
        assert_eq!(values.get("dzzy_ws_connections_active"), Some(&0.0));
        assert_eq!(values.get("dzzy_subscriptions_active"), Some(&2.0));
    }
