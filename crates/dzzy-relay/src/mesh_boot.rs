//! Relay startup wiring for the inter-relay mesh (`DZZY_MESH` seam).
//!
//! [`boot_mesh`] is the ONLY place the relay constructs mesh machinery. It
//! returns `None` — and touches nothing — when `DZZY_MESH=off`, so mesh-off
//! deployments stay byte-identical to a relay built before this module
//! existed. When enabled, it:
//!
//! 1. binds the iroh endpoint on `DZZY_MESH_BIND_ADDR` (boot-unique keypair =
...
use dzzy_relay_mesh::endpoint::MeshEndpoint;
use dzzy_relay_mesh::gossip::GossipRecord;
use dzzy_relay_mesh::registry::{ReadyRecord, ReadyRegistry};
use dzzy_relay_mesh::{
...
///   either the `DZZY_MESH_DEMO_ECHO` consumer (testbed evidence runs: echo
...
/// `DZZY_MESH_DEMO_ECHO` consumer: echo every validated `Data` frame back to
...
const PROTO_VERSION: u16 = dzzy_relay_mesh::WIRE_VERSION as u16;
...
/// `DZZY_MESH_ADVERTISE_ADDR` (explicit, classic-LB shapes) →
...
fn advertise_addrs(endpoint: &MeshEndpoint) -> Vec<String> {
    if let Ok(addr) = std::env::var("DZZY_MESH_ADVERTISE_ADDR") {
...
/// Boot the mesh, or return `None` when `DZZY_MESH=off`.
///
...
/// sets `DZZY_MESH=on` wants the mesh or wants to know why not; silently
...
pub async fn boot_mesh(
    ...
    if !config.mesh.enabled {
        tracing::info!("mesh disabled (DZZY_MESH is not 'on') — single-instance behavior");
        return Ok(None);
    }
    ...
    dzzy_relay_mesh::runtime::spawn_registry_heartbeat(
        registry.clone(),
        ready_record,
...
mod tests {
    ...
    /// DZZY_MESH=off must be a hard no-op: no endpoint bind, no Redis write,
    ...
    /// Blocker fix (Wren review of 8b077fdb): absent `DZZY_MESH`, the mesh is
    /// OFF — an env-untouched image upgrade must not bind or write Redis.
    #[test]
    fn mesh_defaults_off_when_env_absent() {
        // `Config::from_env` in the test env has no DZZY_MESH set unless a
        // caller exported it; assert the fail-safe reading.
        if std::env::var("DZZY_MESH").is_ok() {
        ...
        assert!(!config.mesh.enabled, "DZZY_MESH absent must mean mesh off");
    }
    ...
    fn session_hello(sender: RuntimeId, profile: Profile) -> StreamHello {
        StreamHello {
            sender,
            role: dzzy_relay_mesh::StreamRole::Session {
                fenced: FencedHeader {
    ...
    async fn wired_datagram_consumer_shares_the_handle_fence() {
        ...
        impl dzzy_relay_mesh::RelayPeerTransport for NoopTransport {
