...
//!    participant-hash-idempotent DM model (`dzzy-db/src/dm.rs`).
...
use dzzy_core::kind::{event_kind_u32, KIND_STREAM_MESSAGE};
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            dzzy_core::CommunityId::from_uuid(Uuid::new_v4()),
            "example.org",
        )
    }
