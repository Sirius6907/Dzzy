...
//! [`dzzy_core::kind::is_moderation_command_kind`], list them in
...
//!
//! Lane ownership: L6 (Quinn) — plus `dzzy-cli` `moderation` command group.
...
use dzzy_db::moderation::NewAction;
...
fn ensure_actor_not_banned(
    restriction: &dzzy_db::moderation::RestrictionState,
) -> Result<(), String> {
...
async fn handle_resolve(
    ...
    let (target_pubkey, target_event_id) = match &report.target {
        dzzy_db::moderation::ReportTarget::Pubkey(p) => (Some(p.as_slice()), None),
        dzzy_db::moderation::ReportTarget::Event(e) => (None, Some(e.as_slice())),
        dzzy_db::moderation::ReportTarget::Blob(_) => (None, None),
    };
...
mod tests {
    ...
    fn banned_admin_cannot_reach_an_unban_command() {
        let banned = dzzy_db::moderation::RestrictionState {
            banned: true,
            muted_until: None,
        };
        assert_eq!(
        ...
        let timed_out = dzzy_db::moderation::RestrictionState {
            banned: false,
    ...
    fn resolve_audit_actions_are_allowed_by_db_check_vocabulary() {
        for action in ["dismiss", "escalate", "delete", "kick", "ban", "timeout"] {
            let audit_action = resolution_audit_action(action);
            assert!(
                dzzy_db::moderation::MODERATION_ACTION_CHECK_VOCAB.contains(&audit_action),
