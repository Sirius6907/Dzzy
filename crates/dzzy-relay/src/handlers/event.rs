...
use dzzy_core::event::StoredEvent;
use dzzy_core::kind::{
...
use dzzy_core::verification::verify_event;
use dzzy_core::CommunityId;
use dzzy_pubsub::EventTopic;
...
pub async fn filter_fanout_by_access(
    ...
    let matches = if dzzy_core::kind::is_persona_shared_kind(event_kind_u32(&stored_event.event)) {
        let author = stored_event.event.pubkey.to_bytes();
        matches
            .into_iter()
            .filter(|(conn_id, _)| {
...
pub(crate) async fn fan_out_event_to_local_subscribers(
    ...
    metrics::histogram!("dzzy_fanout_recipients").record(matches.len() as f64);
    if matches.is_empty() {
        return;
    }
...
pub async fn fan_out_pubsub_event(state: &Arc<AppState>, channel_event: dzzy_pubsub::ChannelEvent) {
    ...
    let channel_id = match channel_event.topic {
        dzzy_pubsub::EventTopic::Channel(id) => Some(id),
        dzzy_pubsub::EventTopic::Global => None,
    };
    ...
    let matches = state.sub_registry.fan_out_scoped(community_id, &stored);
    let matches = filter_fanout_by_access(state, community_id, &stored, matches, None).await;
    metrics::counter!("dzzy_multinode_fanout_total").increment(1);
    if matches.is_empty() {
        return;
    }
...
pub(crate) async fn dispatch_persistent_event(
    ...
    metrics::counter!("dzzy_post_commit_dispatch_scheduled_total").increment(1);
...
async fn dispatch_persistent_event_inner(
    ...
    // ingest seam (`crates/dzzy-relay/src/handlers/ingest.rs`'s
    ...
    let event_json = match serde_json::to_string(&stored_event.event) {
        ...
        Err(e) => {
            ...
            metrics::counter!("dzzy_post_commit_dispatch_errors_total", "stage" => "serialize")
                .increment(1);
            return 0;
        }
    };
    ...
    let owner_only_kind = kind_u32 == dzzy_core::kind::KIND_DM_VISIBILITY
        || kind_u32 == dzzy_core::kind::KIND_AGENT_TURN_METRIC;
    ...
    let is_relay_workflow_msg = stored_event.event.pubkey == state.relay_keypair.public_key()
        && stored_event
            ...
            .tags
            .iter()
            .any(|t| t.as_slice().first().map(|s| s.as_str()) == Some("dzzy:workflow"));

    if !dzzy_core::kind::is_workflow_execution_kind(kind_u32)
        && !dzzy_core::kind::is_command_kind(kind_u32)
...
async fn enqueue_event_created_audit(
    ...
    let audit_entry = dzzy_audit::NewAuditEntry {
        community_id: tenant.community(),
        action: dzzy_audit::AuditAction::EventCreated,
    ...
    if let Err(e) = audit_tx.send(audit_entry).await {
        ...
        metrics::counter!("dzzy_audit_send_errors_total").increment(1);
    }
}
...
pub async fn handle_event(event: Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
    ...
    metrics::counter!("dzzy_events_received_total", "kind" => kind_str).increment(1);
    ...
    metrics::counter!(
        "dzzy_community_events_received_total",
    ...
    if kind_u32 == dzzy_core::kind::KIND_AUTH {
        reject("invalid");
        conn.send(RelayMessage::ok(
            &event_id_hex,
            false,
    ...
    if kind_u32 == KIND_AGENT_OBSERVER_FRAME {
        if !scopes.is_empty() && !scopes.contains(&dzzy_auth::Scope::MessagesWrite) {
            reject("scope");
            conn.send(RelayMessage::ok(
                &event_id_hex,
                false,
    ...
    match super::ingest::ingest_event(&state, &conn.tenant, event, ingest_auth).await {
        Ok(result) => {
            if result.accepted {
                // dzzy_events_stored_total is emitted inside ingest_event()
            ...
            metrics::histogram!("dzzy_event_processing_seconds")
...
mod tests {
    ...
    async fn observer_frame_rate_limiter_is_scoped_by_community() {
        ...
        let community_a = dzzy_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xAAAA));
        let community_b = dzzy_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xBBBB));
    ...
    async fn observer_owner_cache_is_scoped_to_community() {
        ...
        let community_a = dzzy_core::CommunityId::from_uuid(Uuid::new_v4());
        let community_b = dzzy_core::CommunityId::from_uuid(Uuid::new_v4());
    ...
    mod pubsub_fanout {
        ...
        use dzzy_core::kind::{KIND_MEMBER_ADDED_NOTIFICATION, KIND_PRESENCE_UPDATE};
        use dzzy_pubsub::{ChannelEvent, EventTopic};
        ...
        fn register_global_sub(
            ...
            state.conn_manager.register(
                ...
                dzzy_core::tenant::CommunityId::from_uuid(Uuid::nil()),
        ...
        async fn global_presence_pubsub_event_fans_out_to_local_subscribers() {
            ...
            fan_out_pubsub_event(
                &state,
                ChannelEvent {
                    community_id: dzzy_core::tenant::CommunityId::from_uuid(Uuid::nil()),
        ...
        async fn local_echo_presence_pubsub_event_is_not_delivered_twice() {
            ...
            let community = dzzy_core::tenant::CommunityId::from_uuid(Uuid::nil());
        ...
        async fn local_echo_suppression_is_scoped_to_its_community() {
            ...
            let community_a = dzzy_core::tenant::CommunityId::from_uuid(Uuid::new_v4());
            let community_b = dzzy_core::tenant::CommunityId::from_uuid(Uuid::nil());
        ...
        async fn redis_presence_publish_reaches_second_relay_and_suppresses_origin_echo() {
            ...
            // a relay only PSUBSCRIBEs `dzzy:{community}:global` after it retains
            ...
            // Match dzzy-pubsub's own Redis round-trip test: give PSUBSCRIBE a
        ...
        async fn audit_records_caller_actor_not_relay_signer_for_relay_signed_event() {
            use dzzy_core::event::StoredEvent;
            use dzzy_core::tenant::{CommunityId, TenantContext};
            ...
            // Relay-signed event tagged dzzy:workflow so workflow triggering is
            ...
            let event = EventBuilder::new(Kind::from(KIND_PRESENCE_UPDATE as u16), "online")
                .tags([nostr::Tag::parse(["dzzy:workflow", "true"]).expect("workflow tag")])
        ...
        /// call that `dzzy_audit::service::tests::chains_are_independent_per_community`
    ...
    mod fanout_access {
        ...
        pub(super) async fn test_state_with_redis_url(redis_url: &str) -> Arc<AppState> {
            ...
            let media_storage =
                dzzy_media::MediaStorage::new(&config.media).expect("media storage");
        ...
        async fn open_channel_event_passes_through_unfiltered() {
            ...
            let community_id = dzzy_core::tenant::CommunityId::from_uuid(Uuid::nil());
            state
        ...
        async fn author_only_reminder_delivers_to_author_only() {
            ...
            let reminder = EventBuilder::new(
                Kind::Custom(dzzy_core::kind::KIND_EVENT_REMINDER as u16),
                "{}",
            )
    ...
    //
    //  * `ConnEntry` (`crates/dzzy-relay/src/state.rs:30-44`) records
    ...
    //  * `SubscriptionRegistry` (`crates/dzzy-relay/src/subscription.rs:57+`)
    ...
    mod redteam {
        ...
        fn register_conn(
            state: &AppState,
            community_id: dzzy_core::tenant::CommunityId,
            pubkey: Option<Vec<u8>>,
        ) -> Uuid {
            let conn_id = Uuid::new_v4();
            let (tx, _rx) = mpsc::channel(1);
        ...
        async fn channel_less_event_must_drop_recipient_in_different_community() {
            ...
            let community_a = dzzy_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xAAAA));
            let community_b = dzzy_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xBBBB));
