...
use dzzy_core::tenant::{normalize_host, TenantContext};
use url::{Host, Url};
...
/// present, the owner is (re)bootstrapped via [`dzzy_db::Db::bootstrap_owner`]
...
pub async fn provision_community(
    ...
    if request.create_only {
        ...
        {
            dzzy_db::CreateCommunityWithOwnerResult::Created(record) => record,
            dzzy_db::CreateCommunityWithOwnerResult::HostExists => {
                return Err("community already exists".to_string());
            }
            dzzy_db::CreateCommunityWithOwnerResult::LimitReached => {
                return Err(
...
mod tests {
    ...
    fn host_valid_bare_domain() {
        assert!(validate_host("acme.communities.dzzy.xyz").is_ok());
    }
