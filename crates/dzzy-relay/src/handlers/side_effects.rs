...
use dzzy_db::channel::{MemberRecord, MemberRole};
...
///
/// Dzzy accepts standard deletions for self-authored events, plus the owning
...
async fn actor_owns_any_owner_agent(
    ...
    members: &[dzzy_db::channel::MemberRecord],
    actor_bytes: &[u8],
) -> anyhow::Result<bool> {
...
pub async fn validate_admin_event(
    ...
    match kind {
        9000 => {
            ...
            if role_str.parse::<dzzy_db::channel::MemberRole>().is_err() {
            ...
            if channel.visibility == "private" {
                ...
                let role: dzzy_db::channel::MemberRole = role_str.parse().unwrap();
                if role.is_elevated() {
                    let actor_role: dzzy_db::channel::MemberRole = actor_member
                        .unwrap()
                        .role
                        .parse()
                        .unwrap_or(dzzy_db::channel::MemberRole::Member);
        ...
        9002 => {
            ...
            for t in event.tags.iter() {
                if t.kind().to_string() == "name" {
                    match t.content() {
                        Some(v)
                            if !dzzy_core::channel::canonical_channel_name(v)
                                .trim()
                                .is_empty() => {}
                        _ => {
...
async fn emit_addressable_discovery_event(
    ...
    let min_ts = {
        let existing = state
            .db
            .query_events(&dzzy_db::event::EventQuery {
                kinds: Some(vec![kind as i32]),
                channel_id: Some(channel_id),
                limit: Some(1),
                ..dzzy_db::event::EventQuery::for_community(tenant.community())
            })
            .await
            .unwrap_or_default();
        existing
...
pub async fn emit_group_discovery_events(
...

    {
        ...
        // Dzzy channels always require explicit membership
...
async fn handle_create_group(
    ...
    let channel_type: dzzy_db::channel::ChannelType = channel_type_str
        .parse()
    ...
    if visibility == dzzy_db::channel::ChannelVisibility::Open {
        state.invalidate_all_accessible_channels(tenant);
    }
...
async fn handle_join_request(
    ...
    state
        ...
        .add_member(
            ...
            channel_id,
            &actor_bytes,
            dzzy_db::channel::MemberRole::Member,
            None,
        )
        .await?;
    ...
    if let Err(e) = emit_membership_notification(
        ...
        &actor_bytes,
        &actor_bytes,
        dzzy_core::kind::KIND_MEMBER_ADDED_NOTIFICATION,
    )
    .await
    {
...
async fn handle_a_tag_deletion(
    ...
    match kind_num {
        ...
        dzzy_core::kind::KIND_WORKFLOW_DEF => {
...
async fn handle_git_repo_announcement(
    ...
    use dzzy_db::git_repo::ReserveOutcome;
...
/// carrying the announcer's pubkey in the `p` tag (dzzy extension).
...
pub async fn reconcile_nip43_membership_snapshots(state: &Arc<AppState>) -> anyhow::Result<usize> {
    ...
    for community in communities {
        let community_id = dzzy_core::CommunityId::from_uuid(community.id);
        ...
        match result {
            ...
            Ok(false) => {}
            Err(error) => {
                metrics::counter!("dzzy_nip43_membership_reconciliation_failures_total")
                    .increment(1);
    ...
    metrics::counter!("dzzy_nip43_membership_reconciliations_total").increment(reconciled as u64);
    Ok(reconciled)
}
...
pub async fn publish_nip43_membership_list(
    ...
    metrics::counter!("dzzy_nip43_membership_publications_total", "result" => "attempted")
        .increment(1);
    let result = publish_nip43_membership_list_inner(tenant, state).await;
    metrics::histogram!("dzzy_nip43_membership_publication_seconds")
        .record(started_at.elapsed().as_secs_f64());
    metrics::counter!(
        "dzzy_nip43_membership_publications_total",
