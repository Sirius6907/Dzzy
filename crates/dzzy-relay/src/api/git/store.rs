...
//! ## The 412 sharp edge
//!
//! `rust-s3 = "0.37"` is shared across the workspace with `dzzy-media`. The
...
impl GitStore {
    ...
    ///
    /// Credential selection mirrors [`dzzy_media::MediaStorage::new`]:
...
mod tests {
    ...
    fn partial_static_keys_are_rejected() {
        for (access, secret) in [("dzzy_dev", ""), ("", "dzzy_dev_secret")] {
...
mod probe {
    ...
    //!
    //! Pre-req: `docker compose up minio` and the `dzzy-git` bucket exists.
    ...
    async fn probe_412_surfacing() {
        if !probe_enabled() {
            eprintln!("skipping: set DZZY_GIT_S3_PROBE=1 to run against live MinIO");
            return;
        }
        let st = store();
