...
//! 4. Otherwise resolves the pusher's channel role via dzzy-channel binding
//! 5. Promotes Bot → Member (bots in a channel push as members)
//! 6. Calls `dzzy_core::git_perms::evaluate_push()`
...
use dzzy_core::channel::MemberRole;
use dzzy_core::git_perms::{evaluate_push, parse_protection_tags, Denial, RefUpdate, UpdateKind};
...
pub async fn hook_policy_check(
    ...
    let community = dzzy_core::CommunityId::from_uuid(community_uuid);
    ...
    let rules = match parse_protection_tags(&tags) {
        Ok(parsed) => {
            ...
            for unknown in &parsed.unknown_rules {
                warn!(repo = %req.repo_id, rule = %unknown, "unknown dzzy-protect rule (skipped)");
            }
            parsed.rules
        }
        Err(e) => {
    ...
    let channel_id = tags
        .iter()
        .find(|t| t.first().map(|s| s.as_str()) == Some("dzzy-channel"))
        .and_then(|t| t.get(1))
    ...
    let role = if is_repo_owner || is_managed_agent_owner {
        ...
        match channel_id {
            None => {
                warn!(repo = %req.repo_id, "hook callback: no dzzy-channel binding");
...
mod tests {
    ...
    fn bash_hmac_matches_rust_hmac() {
        ...
        let bash_script = format!(
            r#"
export LC_ALL=C
DZZY_REPO_ID="{repo_id}"
DZZY_REPO_OWNER="{repo_owner}"
DZZY_COMMUNITY_ID="{community_id}"
DZZY_PUSHER_PUBKEY="{pusher}"
DZZY_HOOK_SECRET="{secret}"
TIMESTAMP="{timestamp}"
...
# Compute HMAC-SHA256
printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$DZZY_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //'
"#,
            repo_id = repo_id,
...
DZZY_REPO_ID="{repo_id}"
REPO_ID_LEN=${{#DZZY_REPO_ID}}
HMAC_INPUT="${{REPO_ID_LEN}}:${{DZZY_REPO_ID}}|{owner}|{community_id}|{pusher}|"
sort "$HMAC_FILE" | while IFS=' ' read -r ref_name old_oid new_oid is_anc; do
    REF_LEN=${{#ref_name}}
    printf '%s%s%s:%s%s' "$old_oid" "$new_oid" "$REF_LEN" "$ref_name" "$is_anc"
done > "$HMAC_FILE.concat"
