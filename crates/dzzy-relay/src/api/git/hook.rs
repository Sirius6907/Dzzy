...
/// - `DZZY_HOOK_URL` — internal policy endpoint (http://127.0.0.1:{port}/internal/git/policy)
/// - `DZZY_HOOK_SECRET` — per-push HMAC secret
/// - `DZZY_REPO_ID` — repo identifier (d-tag)
/// - `DZZY_COMMUNITY_ID` — server-resolved community UUID for the git HTTP request
/// - `DZZY_PUSHER_PUBKEY` — authenticated pusher's hex pubkey
///
...
# Dzzy pre-receive hook — FAIL-CLOSED
...
: "${DZZY_REPO_ID:?error: DZZY_REPO_ID not set}"
: "${DZZY_REPO_OWNER:?error: DZZY_REPO_OWNER not set}"
: "${DZZY_COMMUNITY_ID:?error: DZZY_COMMUNITY_ID not set}"
: "${DZZY_PUSHER_PUBKEY:?error: DZZY_PUSHER_PUBKEY not set}"
: "${DZZY_HOOK_URL:?error: DZZY_HOOK_URL not set}"
: "${DZZY_HOOK_SECRET:?error: DZZY_HOOK_SECRET not set}"
...
REPO_ID_LEN=${#DZZY_REPO_ID}
HMAC_INPUT="${REPO_ID_LEN}:${DZZY_REPO_ID}|${DZZY_REPO_OWNER}|${DZZY_COMMUNITY_ID}|${DZZY_PUSHER_PUBKEY}|"
...
SIGNATURE=$(printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$DZZY_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //')
if [ -z "$SIGNATURE" ]; then
...
SAFE_REPO_ID=$(printf '%s' "$DZZY_REPO_ID" | sed 's/\\/\\\\/g; s/"/\\"/g')
BODY="{\"repo_id\":\"${SAFE_REPO_ID}\",\"repo_owner\":\"${DZZY_REPO_OWNER}\",\"community_id\":\"${DZZY_COMMUNITY_ID}\",\"pusher_pubkey\":\"${DZZY_PUSHER_PUBKEY}\",\"ref_updates\":[${REFS}],\"timestamp\":${TIMESTAMP},\"signature\":\"${SIGNATURE}\"}"
