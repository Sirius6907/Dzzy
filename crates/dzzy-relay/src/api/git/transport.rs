//! Smart HTTP git transport for Dzzy.
//!
...
impl axum::extract::FromRequestParts<Arc<AppState>> for GitAuth {
    ...
    async fn from_request_parts(
        ...
        let auth_header = parts
            ...
            .ok_or_else(|| {
                Response::builder()
                    ...
                    .header(
                        "WWW-Authenticate",
                        format!("Nostr realm=\"dzzy\", method=\"{method}\""),
                    )
        ...
        let pubkey =
            dzzy_auth::nip98::verify_nip98_event(&event_json, &expected_url, &event_method, None)
                .map_err(|e| {
...
fn acquire_git_permit(
    ...
    Arc::clone(&state.git_semaphore)
        ...
        .map_err(|_| {
            metrics::counter!(
                "dzzy_git_semaphore_rejections_total",
                "operation" => operation
            )
            .increment(1);
            Response::builder()
...
/// 0000
/// <pkt><head-oid> HEAD\0<caps> symref=HEAD:<head-ref> object-format=<fmt> agent=dzzy-git\n
...
pub async fn receive_pack(
    ...
    let hook_env = vec![
        ("DZZY_HOOK_URL", hook_url),
        (
            "DZZY_HOOK_SECRET",
            state.config.git_hook_hmac_secret.clone(),
        ),
        ("DZZY_REPO_ID", repo_name.to_string()),
        ("DZZY_REPO_OWNER", params.owner.clone()),
        (
            "DZZY_COMMUNITY_ID",
            auth.tenant.community().as_uuid().to_string(),
        ),
        ("DZZY_PUSHER_PUBKEY", pusher_hex.clone()),
    ...
    S: futures_util::Stream<Item = Result<bytes::Bytes, std::io::Error>>,
{
    ...
    fn poll_next(
        ...
        if self.deadline.as_mut().poll(cx).is_ready() {
            self.finished = true;
            metrics::counter!("dzzy_git_upload_pack_timeouts_total").increment(1);
...
impl<S> Drop for TimedByteStream<S> {
    fn drop(&mut self) {
        metrics::histogram!("dzzy_git_upload_pack_stream_seconds")
            .record(self.started_at.elapsed().as_secs_f64());
        metrics::histogram!("dzzy_git_upload_pack_stream_bytes").record(self.streamed_bytes as f64);
    }
}
...
mod track_c_tests {
    ...
    fn git_nip98_rejects_token_signed_for_wrong_community_host() {
        ...
        let err = dzzy_auth::nip98::verify_nip98_event(&event_json, &expected_for_b, "GET", None)
    ...
    fn git_nip98_accepts_token_signed_for_matching_community_host() {
        ...
        let pubkey =
            dzzy_auth::nip98::verify_nip98_event(&event_json, &expected_for_a, "GET", None)
