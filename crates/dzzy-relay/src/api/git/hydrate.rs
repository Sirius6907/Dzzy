...
pub async fn hydrate_for_read(
    ...
    let outcome = match &result {
        Ok(Some(repo)) => {
            metrics::histogram!("dzzy_git_hydrate_bytes").record(repo.hydrated_bytes() as f64);
            metrics::histogram!("dzzy_git_hydrate_packs").record(repo.hydrated_packs() as f64);
            "success"
        }
        Ok(None) => "missing",
    ...
    metrics::counter!("dzzy_git_hydrations_total", "outcome" => outcome).increment(1);
    metrics::histogram!("dzzy_git_hydrate_seconds", "outcome" => outcome)
...
mod tests {
    ...
    //
    // Run manually:
    //   DZZY_GIT_S3_PROBE=1 cargo test -p dzzy-relay --lib \
