...
//!   P4 agentic tool use     — agent + dzzy-dev-mcp writes a file on disk.
//!
...
//! `dzzy-agent` binary with the exact env vars the relay-mesh preset ships.
//!
...
async fn run() -> anyhow::Result<()> {
    ...
    // mode that broke relay-mesh agents when dzzy-agent's default 32768
    // budget met a 32k-context model). 150k output: passes dzzy-agent's own
    ...
    // P4: agentic tool use via dzzy-dev-mcp — write a real file inside the
    ...
    let prompt = format!(
        "Use your developer tools to create {marker_name} in the current working directory containing exactly the text DZZY_OK (no quotes, no newline commentary). Then confirm."
    );
    let mcp = vec![("dev".to_string(), repo_bin("dzzy-dev-mcp")?)];
    let (r, marker) =
        agent_chat_with_marker(&base, &served_id, None, &prompt, &mcp, &marker_name).await;
    let file_ok = std::fs::read_to_string(&marker)
        .map(|c| c.contains("DZZY_OK"))
        .unwrap_or(false);
    match r {
        Ok(text) => record(
            "P4 agentic tool use",
...
/// Spawn the real dzzy-agent with relay-mesh preset env and drive one ACP
...
async fn agent_chat_in_isolated_home(
    ...
    let agent = match repo_bin("dzzy-agent") {
        Ok(agent) => agent,
    ...
    command
        ...
        .env("DZZY_AGENT_PROVIDER", "openai")
        .env("DZZY_AGENT_MODEL", model)
        .env("OPENAI_COMPAT_BASE_URL", base)
        .env("OPENAI_COMPAT_MODEL", model)
        .env("OPENAI_COMPAT_API_KEY", "dzzy-mesh-local")
        .env("OPENAI_COMPAT_API", "chat")
        .env("DZZY_AGENT_MAX_OUTPUT_TOKENS", "4096")
        .env("DZZY_AGENT_THINKING_EFFORT", "none")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    ...
    if let Some(value) = max_output_tokens {
        command.env("DZZY_AGENT_MAX_OUTPUT_TOKENS", value);
    }
