...
//!
//! Proves the security claim of Dzzy's membership-derived mesh admission:
...
//!      same builder calls Dzzy desktop makes with a resolved member roster.
...
//! corrupt each other — exactly how Dzzy runs it in production anyway (one
//! desktop = one node).
//!
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let scratch = std::env::temp_dir().join(format!("dzzy-mesh-admission-{}", std::process::id()));
