...
mod tests {
    ...
    fn build_hints_section_combined() {
        ...
        let skill_dir = cwd.join(".agents/skills/dzzy-cli");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: dzzy-cli\ndescription: CLI reference for Dzzy managed agents\n---\nUse `dzzy` to manage agents.\n",
        )
        .unwrap();
        ...
        assert!(
            result.contains("dzzy-cli: CLI reference for Dzzy managed agents"),
            "missing skill bullet"
        );
        // Body must NOT be inlined — lazy loading only.
        assert!(
            !result.contains("Use `dzzy` to manage agents."),
