...
const PASSTHROUGH_ENV: &[&str] = &[
    ...
    "GIT_SSH_COMMAND",
    "GIT_CONFIG_GLOBAL",
    // Dzzy identity — dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile then
    // removes it from its own env (children never see it). DZZY_PRIVATE_KEY
    // and DZZY_RELAY_URL are kept for the dzzy CLI. DZZY_AUTH_TAG is a
...
mod content_tests {
    ...
    #[test]
    fn passthrough_includes_dzzy_owner_attestation() {
        assert!(PASSTHROUGH_ENV.contains(&"DZZY_AUTH_TAG"));
    }
