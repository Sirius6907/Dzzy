...
/// one. `expected_run_id` must match the run id dzzy-agent advertised via
...
/// session use `model_id` instead of the configured `DZZY_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) are unexpected — dzzy-agent
...
/// method goose uses for custom usage and status events.  Used by dzzy-agent
/// to emit the `usage_update` payload so dzzy-acp's `UsageTracker` can treat
/// dzzy-agent and goose symmetrically.
