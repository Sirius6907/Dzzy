...
[ACP](https://agentclientprotocol.com) is the Agent Client Protocol — JSON-RPC 2.0 over stdio between a client (Zed, JetBrains, dzzy-acp, …) and an agent. [MCP](https://modelcontextprotocol.io) is how the agent talks to its tools.

`dzzy-agent` is the agent.
...
← {"jsonrpc":"2.0","id":1,"result":{
    ...
    "agentInfo":{"name":"dzzy-agent","version":"0.1.0"}
  }}
...
|---|---|---|
| `DZZY_AGENT_PROVIDER` | — | Required. `anthropic`, `openai`, `databricks`, or `databricks_v2`. No implicit fallback — the agent errors at startup when this is unset. |
...
| `DZZY_AGENT_SYSTEM_PROMPT` | built-in | Inline system prompt. |
| `DZZY_AGENT_SYSTEM_PROMPT_FILE` | — | File path. Mutually exclusive with the above. |
| `DZZY_AGENT_MAX_ROUNDS` | `0` | Tool-loop iteration cap. 0 = unlimited. |
| `DZZY_AGENT_MAX_OUTPUT_TOKENS` | `32768` | Per LLM call. Headroom for large tool-call inputs (e.g. file writes via heredoc); Sonnet 4 / Opus 4 cap at 64K. |
| `DZZY_AGENT_MAX_CONTEXT_TOKENS` | `200000` | Provider context window used by the handoff gate. |
| `DZZY_AGENT_MAX_HANDOFFS` | `10` | Max context handoffs per session before falling back to truncation. |
| `DZZY_AGENT_LLM_TIMEOUT_SECS` | `240` | Max seconds with no response bytes before abandoning an LLM call (per-read inactivity, not wall-clock). |
| `DZZY_AGENT_TOOL_TIMEOUT_SECS` | `660` | Per-tool call timeout in seconds |
| `DZZY_AGENT_MAX_PARALLEL_TOOLS` | `8` | Max concurrent tool calls per turn (1 = sequential) |
| `DZZY_AGENT_MAX_SESSIONS` | unlimited | Max concurrent ACP sessions. Sessions are cheap; default has no cap. |
| `DZZY_AGENT_MAX_LINE_BYTES` | `4194304` | 4 MiB. Hard cap on inbound JSON-RPC frames. |
| `DZZY_AGENT_MAX_HISTORY_BYTES` | `1048576` | 1 MiB. Old turns are evicted past this. |
| `DZZY_AGENT_MAX_TOOL_RESULT_TEXT_BYTES` | `51200` | 50 KiB. Per-result cap on tool-output text; oversize is middle-elided (head + tail kept) with an inline marker. Images are exempt. |


## Providers

`dzzy-agent` speaks a few HTTP dialects. Pick with `DZZY_AGENT_PROVIDER`.

| Provider | `DZZY_AGENT_PROVIDER` | Endpoint (auto) | Tested with |
|---|---|---|---|
...
If `DZZY_AGENT_PROVIDER=anthropic` is selected without `ANTHROPIC_API_KEY`, or `DZZY_AGENT_PROVIDER=openai` is selected without `OPENAI_COMPAT_API_KEY`, the agent returns an error — there is no implicit fallback to another provider.
...
| Frame size | `DZZY_AGENT_MAX_LINE_BYTES` (default 4 MiB). Oversize → connection killed. |
...
| Session isolation | Unlimited concurrent sessions by default (configurable via `DZZY_AGENT_MAX_SESSIONS`). One prompt per session at a time. Each session gets its own MCP servers. |
...
| Limit | Default | Where |
|---|---|---|
| Inbound JSON-RPC frame | 4 MiB | `DZZY_AGENT_MAX_LINE_BYTES` |
| Single prompt | 1 MiB | `MAX_PROMPT_BYTES` |
| History window | 1 MiB | `DZZY_AGENT_MAX_HISTORY_BYTES` |
...
| Tool result text | 50 KiB | `DZZY_AGENT_MAX_TOOL_RESULT_TEXT_BYTES` |
...
| Loop rounds | 0 (unlimited) | `DZZY_AGENT_MAX_ROUNDS` |
| LLM read inactivity timeout | 240 s | `DZZY_AGENT_LLM_TIMEOUT_SECS` |
| Tool call timeout | 660 s | `DZZY_AGENT_TOOL_TIMEOUT_SECS` |
