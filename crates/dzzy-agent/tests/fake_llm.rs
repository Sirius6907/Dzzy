//! Integration test: fake LLM HTTP server + dzzy-agent subprocess.
//!
...
async fn init_session(h: &mut Harness) -> String {
    ...
    assert_eq!(r["result"]["agentInfo"]["name"], "dzzy-agent");
...
async fn rejects_oversized_line() {
    ...
    cmd.env("DZZY_AGENT_PROVIDER", "openai")
        ...
        .env("DZZY_AGENT_MAX_LINE_BYTES", "256")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .kill_on_drop(true);
...
async fn system_prompt_reaches_llm_system_role() {
    ...
    let default_prompt = "You are dzzy-agent";
    assert!(
...
async fn system_prompt_absent_no_canary() {
    ...
    assert!(
        system_content.contains("You are dzzy-agent"),
...
/// Wait for the `activeRunId` advert dzzy-agent emits at prompt start and
...
/// completion_tokens). dzzy-agent maps these to `accumulatedInputTokens` /
...
/// dzzy-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, dzzy-agent must
...
/// (so token counts are observed), dzzy-agent must still emit the usage
