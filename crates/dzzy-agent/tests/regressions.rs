...
impl Harness {
    async fn spawn_with_env(base_url: &str, extra: &[(&str, &str)]) -> Self {
        let bin = env!("CARGO_BIN_EXE_dzzy-agent");
        let mut cmd = tokio::process::Command::new(bin);
        cmd.env("DZZY_AGENT_PROVIDER", "openai")
            .env("OPENAI_COMPAT_API_KEY", "test")
            .env("OPENAI_COMPAT_MODEL", "fake-model")
            .env("OPENAI_COMPAT_BASE_URL", base_url)
            .env("DZZY_AGENT_LLM_TIMEOUT_SECS", "5")
            .env("DZZY_AGENT_TOOL_TIMEOUT_SECS", "5")
            .env("DZZY_AGENT_MAX_ROUNDS", "8")
            .env("DZZY_AGENT_MCP_INIT_TIMEOUT_SECS", "2");
        for (k, v) in extra {
            cmd.env(k, v);
        }
        cmd.stdin(Stdio::piped())
            ...
            .stderr(Stdio::piped())
            .kill_on_drop(true);
        let mut child = cmd.spawn().expect("spawn dzzy-agent");
...
async fn history_budget_evicts_old_turns() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("DZZY_AGENT_MAX_HISTORY_BYTES", &BUDGET.to_string()),
            ("DZZY_AGENT_MAX_HANDOFFS", "0"), // exercise truncation, not handoff
        ],
    )
    .await;
    let sid = init_session(&mut h, json!([])).await;
...
async fn hook_stop_blocks_premature_end() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ...
            ("DZZY_AGENT_STOP_MAX_REJECTIONS", "10"),
        ],
    )
    .await;
...
async fn hook_stop_budget_exhausted() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("DZZY_AGENT_STOP_MAX_REJECTIONS", "1"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn hook_stop_consecutive_end_turn_uses_rejection_budget() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("DZZY_AGENT_STOP_MAX_REJECTIONS", "2"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn hook_post_compact_injects_after_handoff() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ("MCP_HOOK_SERVERS", "fake"),
            ("DZZY_AGENT_MAX_HISTORY_BYTES", &(1024 * 1024).to_string()),
            // Allow at least one handoff.
            ("DZZY_AGENT_MAX_HANDOFFS", "3"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
async fn handoff_summary_prompt_includes_full_history_within_context_budget() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("DZZY_AGENT_MAX_CONTEXT_TOKENS", "10000"),
            ("DZZY_AGENT_MAX_OUTPUT_TOKENS", "1000"),
            ("DZZY_AGENT_MAX_HANDOFFS", "3"),
            (
                "DZZY_AGENT_MAX_HISTORY_BYTES",
...
async fn token_usage_over_budget_triggers_handoff() {
    ...
    let mut h = Harness::spawn_with_env(
        &llm.url,
        &[
            ("DZZY_AGENT_MAX_CONTEXT_TOKENS", "1000"),
            ("DZZY_AGENT_MAX_OUTPUT_TOKENS", "100"),
            ("DZZY_AGENT_MAX_HANDOFFS", "3"),
...
/// `_Stop` hook that takes longer than `DZZY_AGENT_HOOK_TIMEOUT_MS`
...
async fn hook_stop_timeout_failopen() {
    ...
    let mut h = Harness::spawn_with_env(
        ...
        &[
            ...
            ("DZZY_AGENT_HOOK_TIMEOUT_MS", "200"),
        ],
    )
    .await;
    let sid = init_session_with_fake_mcp(
...
/// sends `notifications/cancelled` to the MCP server. With dzzy-dev-mcp,
...
async fn cancel_kills_inflight_tool_via_mcp_notification() {
    // dzzy-dev-mcp is a separate crate; locate its binary relative to
    // the dzzy-agent test binary (they share the same target dir).
    let self_bin = std::path::PathBuf::from(env!("CARGO_BIN_EXE_dzzy-agent"));
    let dev_mcp_bin = self_bin.parent().unwrap().join("dzzy-dev-mcp");
    ...
    if !dev_mcp_is_executable {
        eprintln!(
            "SKIP: dzzy-dev-mcp not built at {}; run `cargo build -p dzzy-dev-mcp` first",
            dev_mcp_bin.display()
        );
        return;
    }
...
/// any MCP server (not just dzzy-dev-mcp) when a session is cancelled
