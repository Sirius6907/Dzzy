...
//! exercised manually via the `dzzy-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `dzzy-agent` binary with
...
use dzzy_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real dzzy-agent binary with `DATABRICKS_TOKEN` set (so the
...
impl AgentHarness {
    async fn spawn_provider(provider: &str, base_url: &str, model: &str) -> Self {
        ...
        cmd.env("DZZY_AGENT_PROVIDER", provider)
