...
// TODO(phase-4): Replace raw nostr::EventBuilder usage in cmd_set_presence with dzzy-sdk builder
...
pub async fn cmd_set_profile(
    ...
    let builder = dzzy_sdk::build_profile(
...
pub async fn cmd_get_presence(client: &DzzyClient, pubkeys_csv: &str) -> Result<(), CliError> {
...
pub async fn cmd_set_presence(client: &DzzyClient, status: &str) -> Result<(), CliError> {
    let builder = dzzy_sdk::build_presence_update(status).map_err(crate::validate::sdk_err)?;
    let event = client.sign_event(builder)?;
