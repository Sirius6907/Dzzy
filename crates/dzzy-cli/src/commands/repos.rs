...
use crate::client::{normalize_write_response, DzzyClient};
...
fn protection_pattern(tag: &Tag) -> Option<&str> {
    let values = tag.as_slice();
    (values.first().map(String::as_str) == Some("dzzy-protect"))
        .then(|| values.get(1).map(String::as_str))
        .flatten()
}
...
fn build_protection_tag(
    ...
    let mut values = vec!["dzzy-protect".to_string(), ref_pattern.to_string()];
...
fn build_updated_repo_announcement(
    ...
    dzzy_sdk::build_repo_announcement_with_tags(repo_id, &existing.content, tags)
...
fn protection_rules_json(event: &Event) -> Result<serde_json::Value, CliError> {
    ...
    let protections: Vec<serde_json::Value> = event
        ...
        .filter_map(|tag| {
            let values = tag.as_slice();
            (values.first().map(String::as_str) == Some("dzzy-protect")).then(|| {
                serde_json::json!({
...
async fn submit_repo_update(client: &DzzyClient, builder: EventBuilder) -> Result<(), CliError> {
...
pub async fn cmd_create_repo(
    ...
    let builder = dzzy_sdk::build_repo_announcement(
        repo_id,
        name,
        description,
        &clone_refs,
...
async fn current_repo(client: &DzzyClient, repo_id: &str) -> Result<Event, CliError> {
    validate_repo_id(repo_id)?;
...
async fn cmd_protect_list(client: &DzzyClient, repo_id: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::ReposCmd, client: &DzzyClient) -> Result<(), CliError> {
...
mod tests {
    ...
    fn protection_update_preserves_metadata_and_replaces_only_matching_pattern() {
        let existing = signed_repo(
            vec![
                tag(&["d", "demo"]),
                tag(&["name", "Demo"]),
                tag(&["dzzy-channel", "channel-id"]),
                tag(&["future-metadata", "preserve-me"]),
                tag(&["auth", &"a".repeat(64), "kind=30617", &"b".repeat(128)]),
                tag(&["dzzy-protect", "refs/heads/main", "push:member"]),
                tag(&["dzzy-protect", "refs/tags/*", "no-delete"]),
            ],
            "repository content",
            100,
        );
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| tag.as_slice() == ["dzzy-channel", "channel-id"]));
        assert!(updated
            .tags
            .iter()
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| { tag.as_slice() == ["dzzy-protect", "refs/tags/*", "no-delete"] }));
        assert_eq!(
            updated
                .tags
                .iter()
    ...
    fn protection_remove_preserves_other_patterns() {
        let existing = signed_repo(
            vec![
                tag(&["d", "demo"]),
                tag(&["dzzy-protect", "refs/heads/main", "no-delete"]),
                tag(&["dzzy-protect", "refs/heads/release", "push:owner"]),
            ],
            "",
            10,
        );
        ...
        assert!(updated
            .tags
            .iter()
            .any(|tag| { tag.as_slice() == ["dzzy-protect", "refs/heads/release", "push:owner"] }));
    }
    ...
    fn protection_update_rejects_malformed_existing_rules() {
        let existing = signed_repo(
            vec![
                tag(&["d", "demo"]),
                tag(&["dzzy-protect", "refs/heads/main"]),
            ],
            "",
            10,
        );
