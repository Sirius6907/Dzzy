...
const CUSTOM_EMOJI_SET_D_TAG: &str = dzzy_sdk::CUSTOM_EMOJI_SET_D_TAG;
...
/// own kind:30030 set (d=`dzzy:custom-emoji`).
async fn cmd_list(client: &DzzyClient) -> Result<(), CliError> {
    let filter = serde_json::json!({
        "kinds": [dzzy_sdk::kind::KIND_EMOJI_SET],
...
async fn fetch_own_emoji(client: &DzzyClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn publish_own_set(client: &DzzyClient, emojis: &[CustomEmoji]) -> Result<(), CliError> {
    let builder = dzzy_sdk::build_custom_emoji_set(emojis)
...
async fn cmd_set(client: &DzzyClient, shortcode: &str, url: &str) -> Result<(), CliError> {
    let normalized = dzzy_sdk::normalize_custom_emoji_shortcode(shortcode)
...
async fn cmd_rm(client: &DzzyClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = dzzy_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &DzzyClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
