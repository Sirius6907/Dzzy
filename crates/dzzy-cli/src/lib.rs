...
use client::DzzyClient;
use error::CliError;
use nostr::Keys;
use uuid::Uuid;

/// Run the Dzzy CLI from raw arguments (including `argv[0]`).
///
...
///
/// ```ignore
/// let code = dzzy_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from dzzy-acp/dzzy-dev-mcp)
    ...
    // dzzy-dev-mcp delegates to run_from_args, it has already installed ring; the
...
#[command(
    name = "dzzy",
    about = "Dzzy CLI — interact with a Dzzy relay",
    long_about = "\
Dzzy CLI — interact with a Dzzy relay

Configuration (flags override env vars):
  DZZY_RELAY_URL     Relay base URL        [default: http://localhost:3000]
  DZZY_PRIVATE_KEY   Nostr private key (hex or nsec)  [required]
  DZZY_AUTH_TAG      NIP-OA auth tag JSON  [optional]
...
struct Cli {
    /// Relay URL (http:// or https://). Overrides DZZY_RELAY_URL env var.
    #[arg(long, env = "DZZY_RELAY_URL", default_value = "http://localhost:3000")]
    relay: String,
...
#[derive(Subcommand)]
pub enum AgentsCmd {
    /// Open a prefilled create-agent form in the owner's Dzzy Desktop
    DraftCreate {
    ...
    /// Open a prefilled edit-agent form in the owner's Dzzy Desktop
    DraftUpdate {
...
running under DZZY_AUTH_TAG signs as itself, so it can only ever satisfy \
the self path (target == signer) — not the owner-of-agent path for another \
identity.\n\n\
Examples:\n  \
dzzy agents archive <PUBKEY> --reason retired\n  \
dzzy agents archive <PUBKEY> --reason bot-rebuilt --replaced-by <NEW_PUBKEY>"
    )]
    Archive {
...
dzzy agents unarchive <PUBKEY> --reason returned")]
    Unarchive {
        /// Target identity pubkey (hex)
        target_pubkey: String,
        /// Machine-readable reason code, max 64 UTF-8 bytes
...
pub enum MessagesCmd {
    ...
    #[command(
        after_help = "Examples:\n  dzzy messages send --channel <UUID> --content \"hello\"\n  dzzy messages send --channel <UUID> --content \"@alice check this\"\n  echo \"hello from stdin\" | dzzy messages send --channel <UUID> --content -"
    )]
    Send {
        /// Channel UUID (from 'dzzy channels list')
        #[arg(long)]
        channel: String,
    ...
    #[command(
        after_help = "Examples:\n  dzzy messages get --channel <UUID>\n  dzzy messages get --channel <UUID> --limit 50 --kinds 1,1984"
    )]
    Get {
        /// Channel UUID
        #[arg(long)]
    ...
    #[command(
        after_help = "Examples:\n  dzzy messages search --query checkout\n  dzzy messages search --author npub1... --since 1783497600\n  dzzy messages search --author Aaron --query checkout --limit 20"
    )]
    Search {
...
pub enum ChannelsCmd {
    ...
    #[command(
        after_help = "Examples:\n  dzzy channels list\n  dzzy channels list --visibility open"
    )]
    List {
    ...
    #[command(
        after_help = "Examples:\n  dzzy channels search --query composer\n  dzzy channels search --query dzzy-chat-composer --exact\n  dzzy channels search --query design --include-archived"
    )]
    Search {
    ...
    #[command(
        after_help = "Examples:\n  dzzy channels create --name general --type stream --visibility open\n  dzzy channels create --name design --type forum --visibility open --description \"Design discussions\"\n  dzzy channels create --name standup --type stream --visibility open --ttl 3600  # ephemeral, archived after 1h idle\n  dzzy channels create --name project-x --template \"Dzzy Team\"  # type/visibility/canvas/roster from the template; explicit flags override"
    )]
    Create {
        /// Channel name
        #[arg(long)]
...
pub enum WorkflowsCmd {
    ...
    #[command(
        after_help = "Examples:\n  dzzy workflows trigger --workflow <UUID>\n  dzzy workflows trigger --workflow <UUID> --inputs '{\"key\":\"value\"}'"
    )]
    Trigger {
        /// Workflow UUID
        #[arg(long)]
...
pub enum NotesCmd {
    ...
    #[command(
        after_help = "Examples:\n  echo '# Hello' | dzzy notes set --name hello --title 'Hello' --content -\n  dzzy notes set --name hello --tag onboarding --content - < draft.md"
    )]
    Set {
...
pub enum PatchesCmd {
    ...
    #[command(
        after_help = "Examples:\n  git format-patch -1 HEAD --stdout | dzzy patches send --repo-owner <hex> --repo-id myrepo --patch-file - --root\n  dzzy patches send --repo-owner <hex> --repo-id myrepo --patch-file 0001-fix.patch --reply-to <prev-patch-id>"
    )]
    Send {
...
pub enum PrCmd {
    ...
    #[command(
        after_help = "Examples:\n  dzzy pr open --repo-owner <hex> --repo-id myrepo --subject 'Fix bug' --body-file - --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo --branch-name fix-bug\n  dzzy pr update --repo-owner <hex> --repo-id myrepo --pr <event> --pr-author <hex> --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo"
    )]
    Open {
        /// Repo owner pubkey (64-char hex)
        #[arg(long)]
...
/// Subcommands for `dzzy mem`.
#[derive(Subcommand)]
pub enum MemCmd {
    /// List non-tombstoned memory entries
    Ls {
        /// Owner pubkey (hex). Overrides DZZY_AUTH_TAG.
        #[arg(long)]
        owner: Option<String>,
    ...
    Patch {
        ...
        /// against. Hashes the exact UTF-8 bytes returned by `dzzy mem get`,
        /// not normalized lines. Run `dzzy mem hash <slug>` to capture this
        /// before editing.
        #[arg(long)]
...
/// `DZZY_RELAY_URL` — moderation commands are community-global and carry no
...
pub enum ModerationCmd {
    ...
    #[command(
        after_help = "Examples:\n  dzzy moderation reports\n  dzzy moderation reports --status open --limit 20"
    )]
    Reports {
    ...
    #[command(
        after_help = "Examples:\n  dzzy moderation resolve --report <REPORT_EVENT_ID> --status dismissed --action dismiss\n  dzzy moderation resolve --report <REPORT_EVENT_ID> --status resolved --action ban --reason \"rule 3\""
    )]
    Resolve {
    ...
    #[command(
        after_help = "Examples:\n  dzzy moderation ban --pubkey <HEX>\n  dzzy moderation ban --pubkey <HEX> --expires-in 604800 --reason \"repeated spam\""
    )]
    Ban {
    ...
    #[command(
        after_help = "Examples:\n  dzzy moderation timeout --pubkey <HEX> --expires-in 3600\n  dzzy moderation timeout --pubkey <HEX> --expires-at 1783500000 --reason \"cool off\""
    )]
    Timeout {
        /// Target member pubkey (hex)
        #[arg(long)]
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let private_key_str = cli.private_key.ok_or_else(|| {
        CliError::Auth("DZZY_PRIVATE_KEY is required (use --private-key or set env var)".into())
    })?;
    let keys = Keys::parse(&private_key_str)
        .map_err(|e| CliError::Key(format!("invalid DZZY_PRIVATE_KEY: {e}")))?;
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref json) if !json.is_empty() => {
            let tag = dzzy_sdk::nip_oa::parse_auth_tag(json)
                .map_err(|e| CliError::Auth(format!("DZZY_AUTH_TAG is malformed: {e}")))?;
            dzzy_sdk::nip_oa::verify_auth_tag(json, &keys.public_key()).map_err(|e| {
                CliError::Auth(format!(
                    "DZZY_AUTH_TAG verification failed for pubkey {}: {e}",
    ...
    let client = DzzyClient::new(relay_url, keys, auth_tag, auth_tag_json)?;
