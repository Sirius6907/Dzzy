...
SIDECARS=(dzzy-acp dzzy-agent dzzy-dev-mcp git-credential-nostr dzzy)
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo "Run 'cargo build --release -p dzzy-acp -p dzzy-agent -p dzzy-dev-mcp -p git-credential-nostr -p dzzy-cli' first." >&2
    exit 1
fi
