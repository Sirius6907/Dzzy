#!/usr/bin/env bash
# =============================================================================
# start-relay-for-tests.sh — Start the Dzzy relay and its backing services
# =============================================================================
...
wait_healthy "Postgres" "dzzy-postgres"
wait_healthy "Redis" "dzzy-redis"
wait_healthy "MinIO" "dzzy-minio"
...
export PGSCHEMA_PLAN_DB=dzzy
export PGSCHEMA_PLAN_USER=dzzy
export PGSCHEMA_PLAN_PASSWORD=dzzy_dev

./bin/pgschema apply --file schema/schema.sql --auto-approve
docker exec -i -e PGPASSWORD="${PGPASSWORD}" dzzy-postgres \
...
# psql is not on PATH in the hermit env; postgres runs as the dzzy-postgres
...
if command -v psql >/dev/null 2>&1; then
  ...
  seed_psql() { docker exec -e PGPASSWORD="${PGPASSWORD}" dzzy-postgres psql -U "${PGUSER}" -d "${PGDATABASE}" -qtA "$@"; }
fi
seed_psql -c "
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  for bin in dzzy-relay git-credential-nostr; do
  ...
  cargo build --profile "${CARGO_PROFILE}" -p dzzy-relay -p git-credential-nostr
  ok "Relay built"
fi
...
nohup env \
  ...
  RELAY_URL=ws://localhost:3000 \
  DZZY_BIND_ADDR=0.0.0.0:3000 \
  DZZY_REQUIRE_AUTH_TOKEN=false \
  DZZY_RECONCILE_CHANNELS=true \
  DZZY_GIT_PROBE_WRITERS=8 \
  "./target/${CARGO_PROFILE}/dzzy-relay" > /tmp/dzzy-relay.log 2>&1 &
echo $! > /tmp/dzzy-relay.pid

# ── Poll readiness ───────────────────────────────────────────────────────────

log "Waiting for relay readiness..."
for attempt in $(seq 1 60); do
  if ! kill -0 "$(cat /tmp/dzzy-relay.pid)" 2>/dev/null; then
    err "Relay process died"
    cat /tmp/dzzy-relay.log
    exit 1
  fi
