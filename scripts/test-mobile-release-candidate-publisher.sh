...
case "${1:-}:${2:-}" in
  api:repos/block/dzzy/rulesets/14378754)
    case "$*" in
    ...
    ;;
  api:repos/block/dzzy/git/ref/heads/main) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:repos/block/dzzy/commits/*) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:--paginate)
    [[ "$3" == "repos/block/dzzy/git/matching-refs/tags/mobile-v1.2.3-rc." ]]
    printf '%s' "${GH_EXISTING_REFS:-}"
    ;;
  api:--method)
    endpoint="$4"
    case "$endpoint" in
      repos/block/dzzy/git/tags)
        record "$*"
        printf '%s\n' "$GH_TAG_OBJECT_SHA"
        ;;
      repos/block/dzzy/git/refs)
        record "$*"
        ;;
      *) exit 2 ;;
    esac
    ;;
  api:repos/block/dzzy/git/ref/tags/mobile-v1.2.3-rc.*)
    if [[ "$*" == *'.object.type'* ]]; then
...
export GITHUB_REPOSITORY=block/dzzy
...
grep -Fq -- '-f message=Dzzy Mobile 1.2.3 release candidate 1' "$GH_CALLS"
...
if GITHUB_REPOSITORY=attacker/dzzy "$publisher" 1.2.3 1 "$GH_TARGET_SHA" >/dev/null 2>&1; then
