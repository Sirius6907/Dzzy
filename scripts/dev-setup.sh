...
load_env() {
  ...
  # Smooth the local rename path for developers with a pre-Dzzy .env copied
  ...
  if [[ "${DATABASE_URL:-}" == "postgres://sprout:sprout_dev@localhost:5432/sprout" ]]; then
    warn "Migrating legacy default DATABASE_URL from sprout to dzzy for this setup run"
    DATABASE_URL="postgres://dzzy:dzzy_dev@localhost:5432/dzzy"
  fi
  if [[ "${PGUSER:-}" == "sprout" ]]; then PGUSER="dzzy"; fi
  if [[ "${PGPASSWORD:-}" == "sprout_dev" ]]; then PGPASSWORD="dzzy_dev"; fi
  if [[ "${PGDATABASE:-}" == "sprout" ]]; then PGDATABASE="dzzy"; fi

  export DATABASE_URL="${DATABASE_URL:-postgres://dzzy:dzzy_dev@localhost:5432/dzzy}"
...
cleanup_legacy_sprout_containers() {
  ...
  warn "Stopping/removing legacy sprout-* dev containers so dzzy-* containers can bind the standard ports"
...
fail_if_local_redis_blocks_compose() {
  ...
  if docker ps --format '{{.Names}}' | grep -qx 'dzzy-redis'; then
    return
  fi
  local redis_pids
...
"${REPO_ROOT}/bin/cargo" run -p dzzy-admin -- migrate
...
echo ""
echo -e "${GREEN}=======================================================${NC}"
echo -e "${GREEN}  Dzzy dev environment is ready!${NC}"
echo -e "${GREEN}=======================================================${NC}"
echo ""
