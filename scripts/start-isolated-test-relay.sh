...
# default `dzzy-*` dev stack. Backing services run under the dedicated
# `dzzy-harness` Compose project (docker-compose.harness.yml); the relay runs
# in the foreground on override ports.
#
#   Topology (reuse this exact tuple for desktop parity runs):
#     compose project : dzzy-harness
#     postgres        : localhost:5471  (db=dzzy, user=dzzy, pass=dzzy_dev)
#     redis           : localhost:6471
#     minio           : localhost:9471 (console 9472)
#     relay main      : localhost:3030   ← DZZY_E2E_RELAY_URL=http://localhost:3030
...
#   docker compose -p dzzy-harness -f docker-compose.harness.yml down -v
# =============================================================================
set -euo pipefail
...
# ── Backing services (scoped to dzzy-harness only) ───────────────────────────
...
wait_pg() {
  for _ in $(seq 1 60); do
    if docker compose -p "${PROJECT}" -f "${COMPOSE_FILE}" exec -T postgres \
         pg_isready -U dzzy >/dev/null 2>&1; then
...
# ── Schema + partitions ──────────────────────────────────────────────────────
export PGPASSWORD=dzzy_dev
psql_h() { docker compose -p "${PROJECT}" -f "${COMPOSE_FILE}" exec -T postgres \
  psql -U dzzy -d dzzy -v ON_ERROR_STOP=1 "$@"; }

log "Resetting isolated database and applying schema..."
# This database belongs only to the dzzy-harness Compose project. Reset it on
...
export PGSCHEMA_PLAN_DB=dzzy PGSCHEMA_PLAN_USER=dzzy PGSCHEMA_PLAN_PASSWORD=dzzy_dev
export PGHOST=localhost PGPORT=${PG_PORT} PGUSER=dzzy PGDATABASE=dzzy
...
# DZZY_DB_HOST/PORT rather than the shared `dzzy-postgres` container.)
log "Seeding community (host=${COMMUNITY_HOST}), channels, and members..."
DZZY_COMMUNITY_HOST="${COMMUNITY_HOST}" \
  DZZY_DB_HOST=localhost DZZY_DB_PORT=${PG_PORT} DZZY_DB_USER=dzzy \
  DZZY_DB_PASS=dzzy_dev DZZY_DB_NAME=dzzy \
  DZZY_DB_DOCKER_CONTAINER="${PROJECT}-postgres-1" \
...
cargo build --profile "${CARGO_BUILD_PROFILE}" -p dzzy-relay
ok "Relay built"
...
tmux new-session -d -s "${TMUX_SESSION}" "cd '${REPO_ROOT}' && env \
  DATABASE_URL=postgres://dzzy:dzzy_dev@localhost:${PG_PORT}/dzzy \
  REDIS_URL=redis://localhost:${REDIS_PORT} \
  RELAY_URL=ws://localhost:${RELAY_MAIN} \
  DZZY_BIND_ADDR=0.0.0.0:${RELAY_MAIN} \
  DZZY_HEALTH_PORT=${RELAY_HEALTH} \
  DZZY_METRICS_PORT=${RELAY_METRICS} \
  DZZY_S3_ENDPOINT=http://localhost:${MINIO_PORT} \
  DZZY_S3_ACCESS_KEY=dzzy_dev \
  DZZY_S3_SECRET_KEY=dzzy_dev_secret \
  DZZY_S3_BUCKET=dzzy-media \
  DZZY_REQUIRE_AUTH_TOKEN=false \
  DZZY_RECONCILE_CHANNELS=true \
  './target/${CARGO_TARGET_PROFILE}/dzzy-relay' > '${RELAY_LOG}' 2>&1"

# Wait for the main port to accept connections.
for _ in $(seq 1 30); do
  if curl -s -o /dev/null "http://localhost:${RELAY_MAIN}/"; then
    ok "Relay live — DZZY_E2E_RELAY_URL=http://localhost:${RELAY_MAIN}"
