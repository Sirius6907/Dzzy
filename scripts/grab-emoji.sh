#!/usr/bin/env bash
# grab-emoji.sh — Register custom Slack emoji in Dzzy
#
# Looks up each emoji name in your Slack workspace and registers it in Dzzy
# via `dzzy emoji set`, making it available as :name: in the Dzzy emoji picker.
#
# Usage:
#   SLACK_TOKEN=xoxp-... ./scripts/grab-emoji.sh [--name <dzzy-name>] <emoji-name> [emoji-name ...]
#
# Options:
#   --name <dzzy-name>  Override the shortcode used in Dzzy (only valid with a single emoji)
#
# Env:
#   SLACK_TOKEN  — Slack user token (xoxp-...) with emoji:read scope
#
# Output:
#   name → registered as :name: in Dzzy   on success
...
if [[ $# -eq 0 ]]; then
  echo "Usage: SLACK_TOKEN=xoxp-... $0 [--name <dzzy-name>] <emoji-name> [emoji-name ...]" >&2
  exit 1
fi

if [[ -n "$DZZY_NAME" && $# -ne 1 ]]; then
...
if ! command -v dzzy &>/dev/null; then
  echo "ERROR: 'dzzy' not found in PATH. Install the Dzzy CLI and retry." >&2
  exit 1
fi
...
for emoji_name in "$@"; do
  ...
  dzzy_shortcode="${DZZY_NAME:-$emoji_name}"
  ...
  # Register in Dzzy
  set_output=$(dzzy emoji set --shortcode "$dzzy_shortcode" --url "$emoji_url" 2>&1) || {
    echo "${emoji_name} → ERROR: dzzy emoji set failed — ${set_output}"
    continue
  }

  echo "${emoji_name} → registered as :${dzzy_shortcode}: in Dzzy"
done
