...
require_canonical_repository() {
  ...
  origin_url="$(git config --get remote.origin.url 2>/dev/null)" || \
    fail_release_ruleset "origin is required and must point to block/dzzy" || return 1
  case "$origin_url" in
    git@github.com:block/dzzy.git|ssh://git@github.com/block/dzzy.git|https://github.com/block/dzzy.git|https://github.com/block/dzzy)
      ;;
    *)
      fail_release_ruleset "origin must point to canonical block/dzzy, not '$origin_url'" || return 1
      ;;
  esac
}
...
require_release_tag_ruleset() {
  ...
  ruleset_endpoint="repos/block/dzzy/rulesets/$RELEASE_TAG_RULESET_ID"
