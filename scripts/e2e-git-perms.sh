...
#   - Relay built: cargo build --release --bin dzzy-relay
...
check_deps() {
    ...
    if [[ ! -x "${REPO_ROOT}/target/release/dzzy-relay" ]]; then
        missing+=("dzzy-relay (cargo build --release --bin dzzy-relay)")
    fi
...
export RUST_LOG="dzzy_relay=warn"
...
./target/release/dzzy-relay > /tmp/dzzy-relay-e2e.log 2>&1 &
RELAY_PID=$!
...
for i in $(seq 1 "$RELAY_STARTUP_TIMEOUT"); do
    ...
    if [[ $i -eq "$RELAY_STARTUP_TIMEOUT" ]]; then
        fail "Relay did not start within ${RELAY_STARTUP_TIMEOUT}s. Check /tmp/dzzy-relay-e2e.log"
    fi
    sleep 1
done
...
CREATE_REPO=$(send_event "$OWNER_PRIVKEY" "$KIND_CREATE_REPO" "" \
    "[\"d\", \"$REPO_NAME\"], [\"dzzy-channel\", \"$CHANNEL_ID\"]")
...
git -C "$BOT1_DIR" -c user.name="Bot1" -c user.email="bot1@dzzy.test" \
...
if git_push "$BOT1_PRIVKEY" "$BOT1_DIR" -u origin main; then
    ...
    tail -20 /tmp/dzzy-relay-e2e.log
...
git -C "$BOT2_DIR" -c user.name="Bot2" -c user.email="bot2@dzzy.test" \
...
git -C "$UNSIGNED_DIR" -c user.name="Bot1" -c user.email="bot1@dzzy.test" \
...
(
    ...
    git -C "$SYMLINK_DIR" -c user.name="Bot1" -c user.email="bot1@dzzy.test" \
...
(
    ...
    git -C "$MISSING_DIR" -c user.name="Bot1" -c user.email="bot1@dzzy.test" \
