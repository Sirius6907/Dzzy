...
# can finish, so agent workers (goose, dzzy-agent, ...) it spawned in their
...
# `DZZY_MANAGED_AGENT` env var from the shell: on macOS `pkill -f` matches only
...
#   <instance-id> is the desktop bundle identifier, e.g. `xyz.block.dzzy.app.dev`
#   (main checkout) or `xyz.block.dzzy.app.dev.my-branch` (a worktree).
