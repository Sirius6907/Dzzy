...
export PGUSER="${PGUSER:-dzzy}"
export PGPASSWORD="${PGPASSWORD:-dzzy_dev}"
export PGDATABASE="${PGDATABASE:-dzzy}"
...
elif docker exec dzzy-postgres psql --version >/dev/null 2>&1; then
  run_psql() {
    docker exec -i -e PGPASSWORD="${PGPASSWORD}" dzzy-postgres \
      psql -U "${PGUSER}" -d "${PGDATABASE}" "$@"
  }
else
  echo "error: neither psql nor dzzy-postgres docker psql is available" >&2
  exit 1
fi
...
upload_fixture() {
  ...
  docker exec -i dzzy-minio mc pipe --quiet --attr "Content-Type=${mime}" \
    "local/${DZZY_S3_BUCKET:-dzzy-media}/${hash}.${extension}" < "${path}"
  printf '%s' "${sidecar}" | docker exec -i dzzy-minio mc pipe --quiet \
    --attr "Content-Type=application/json" \
    "local/${DZZY_S3_BUCKET:-dzzy-media}/_meta/${community_id}/${hash}.json"
}

fixture_dir="$(mktemp -d "${TMPDIR:-/tmp}/dzzy-admin-feedback.XXXXXX")"
...
printf '%s\n' "dzzy feedback diagnostics" "area: composer" \
  "event: resumed_from_sleep" "result: composer_unresponsive" > "${composer_diagnostics}"
printf '%s\n' "dzzy feedback diagnostics" "area: workspace-switching" \
...
if ! docker exec dzzy-minio mc alias set local http://localhost:9000 \
  "${DZZY_S3_ACCESS_KEY:-dzzy_dev}" "${DZZY_S3_SECRET_KEY:-dzzy_dev_secret}" >/dev/null; then
