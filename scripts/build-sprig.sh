#!/usr/bin/env bash
# Build Sprig — one deploy-anywhere multicall binary for the Dzzy ACP
# harness, agent, and developer MCP. The archive exposes these command names:
#
#   sprig            implementation binary
#   dzzy-acp       link to sprig (ACP harness)
#   dzzy-agent     link to sprig (ACP-compliant agent)
#   dzzy-dev-mcp   link to sprig (developer MCP server; also dispatches
#                    rg/tree/dzzy/git-credential-nostr/git-sign-nostr)
#
# Usage:
...
BUNDLE_BIN="sprig"
COMMANDS=(dzzy-acp dzzy-agent dzzy-dev-mcp)
...
Sprig is the all-in-one Dzzy agent binary for deploy-anywhere environments.
...
- `dzzy-acp` — ACP harness that bridges Dzzy channel events to an
  ACP-compliant agent over stdio.
- `dzzy-agent` — ACP-compliant agent (spawns MCP servers, calls LLMs).
- `dzzy-dev-mcp` — Developer MCP server (shell, str_replace, todo) and
  multicall entrypoint for `rg`, `tree`, `dzzy`, `git-credential-nostr`,
  `git-sign-nostr`.
...
```bash
# Agent provider
export DZZY_AGENT_PROVIDER=anthropic            # or openai
export ANTHROPIC_API_KEY=sk-...
export ANTHROPIC_MODEL=claude-sonnet-4-20250514

# Nostr identity (shared by dzzy-acp, git auth, signing, and dzzy CLI)
export NOSTR_PRIVATE_KEY=nsec1...
export DZZY_PRIVATE_KEY="$NOSTR_PRIVATE_KEY"
export DZZY_RELAY_URL=https://your-relay.example.com
```
README
