...
if [[ "$instance_id" != "xyz.block.dzzy.app.dev" && "$instance_id" != xyz.block.dzzy.app.dev.* ]]; then
    echo "reset-desktop-standalone-state: refusing non-dev bundle identifier: $instance_id" >&2
    exit 1
fi
if [[ "$keyring_service" != "dzzy-desktop-dev" && "$keyring_service" != dzzy-desktop-dev.* ]]; then
...
case "${DZZY_TEST_PLATFORM:-$(uname -s)}" in
    Darwin)
