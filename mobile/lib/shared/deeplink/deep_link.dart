/// Parsing for `dzzy://` deep links.
///
/// Mirrors the desktop handler in `desktop/src-tauri/src/deep_link.rs`:
/// `dzzy://message?channel=<uuid>&id=<hex>[&thread=<hex>]` references a
...
/// `dzzy://join?relay=<ws(s)://relay>&code=<code>` form is only an installed-app
/// handoff from the web landing page.
class InviteDeepLink extends DzzyDeepLink {
...
/// A parsed `dzzy://message` deep link.
class MessageDeepLink extends DzzyDeepLink {
...
/// Parse a `dzzy://message?…` URI into a [MessageDeepLink].
///
/// Returns `null` for non-`dzzy` schemes, non-`message` hosts (e.g.
/// `dzzy://connect` which is desktop-only), or links missing a non-empty
/// `channel` or `id` param.
MessageDeepLink? parseMessageDeepLink(Uri uri) {
  if (uri.scheme != 'dzzy' || uri.host != 'message') return null;
...
/// Parse canonical HTTPS invite links and `dzzy://join` app handoffs.
///
/// Accepted forms:
/// - `https://<relay>/invite/<code>` -> `wss://<relay>` + code
/// - `http://<relay>/invite/<code>` -> `ws://<relay>` + code
/// - `dzzy://join?relay=<ws(s)://relay>&code=<code>` -> relay + code
///
...
/// Parse any supported Dzzy deep link.
DzzyDeepLink? parseDzzyDeepLink(Uri uri) =>
