...
final _dzzyToolNames = <String>{..._dzzyReadTools, ..._dzzyWriteTools};

final _dzzyToolNamesByLength = _dzzyToolNames.toList()
  ..sort((a, b) => b.length.compareTo(a.length));

final _dzzyToolTitleAliases = <(RegExp, String)>[
...
String? _findDzzyToolName(String value, bool includeShortNames) {
  final alias = _findDzzyToolAlias(value);
  if (alias != null) return alias;

  final normalized = _normalizeToolNameText(value);
  for (final name in _dzzyToolNamesByLength) {
...
String? _findDzzyToolAlias(String value) {
  final normalizedPhrase = value
      .trim()
      .toLowerCase()
      .replaceAll(RegExp(r'[_-]+'), ' ')
      .replaceAll(RegExp(r'\s+'), ' ');
  for (final (pattern, name) in _dzzyToolTitleAliases) {
...
_parsePromptText(String text) {
  ...
  for (final section in sections) {
    if (section.title.toLowerCase().startsWith('dzzy event')) {
...
({String title, String toolName, String? dzzyToolName}) _extractToolIdentity(
  Map<String, dynamic> update,
) {
  final candidates = _collectToolNameCandidates(update);
  String? knownName;
  for (final c in candidates) {
    knownName = _findDzzyToolName(c, true);
    if (knownName != null) break;
  }
  knownName ??= _findDzzyToolName(_safeJsonEncode(update), false);
...
List<TranscriptItem> buildTranscript(List<ObserverFrame> events) {
  ...
  void upsertTool(
    ...
    final existing = itemsById[id];
    final canonicalDzzyToolName =
        dzzyToolName ?? _findDzzyToolName(toolName, true);
    if (existing is ToolItem) {
      if (!_isGenericToolTitle(title)) {
        existing.title = title;
      }
      if (canonicalDzzyToolName != null) {
        existing.dzzyToolName = canonicalDzzyToolName;
        existing.toolName = canonicalDzzyToolName;
      } else if (existing.dzzyToolName == null &&
    ...
    final item = ToolItem(
      id: id,
      title: title,
      toolName: canonicalDzzyToolName ?? toolName,
      dzzyToolName: canonicalDzzyToolName,
      status: status,
      args: args,
      result: result,
      isError: isError,
