...
val uploadKeystorePath = providers.environmentVariable("DZZY_ANDROID_UPLOAD_KEYSTORE_PATH").orNull
val uploadKeystorePassword = providers.environmentVariable("DZZY_ANDROID_UPLOAD_KEYSTORE_PASSWORD").orNull
val uploadKeyAlias = providers.environmentVariable("DZZY_ANDROID_UPLOAD_KEY_ALIAS").orNull
val uploadKeyPassword = providers.environmentVariable("DZZY_ANDROID_UPLOAD_KEY_PASSWORD").orNull
val uploadSigningValues =
    mapOf(
        "DZZY_ANDROID_UPLOAD_KEYSTORE_PATH" to uploadKeystorePath,
        "DZZY_ANDROID_UPLOAD_KEYSTORE_PASSWORD" to uploadKeystorePassword,
        "DZZY_ANDROID_UPLOAD_KEY_ALIAS" to uploadKeyAlias,
        "DZZY_ANDROID_UPLOAD_KEY_PASSWORD" to uploadKeyPassword,
    )
...
val releaseSigningMode =
    providers.environmentVariable("DZZY_ANDROID_RELEASE_SIGNING").orNull ?: "upload-keystore"
val externalReleaseSigning = releaseSigningMode == "external"
if (releaseSigningMode !in setOf("upload-keystore", "external")) {
    throw GradleException(
        "DZZY_ANDROID_RELEASE_SIGNING must be \"upload-keystore\" or \"external\", got: " +
            releaseSigningMode,
    )
}
if (externalReleaseSigning && uploadSigningValues.values.any { !it.isNullOrBlank() }) {
    throw GradleException(
        "DZZY_ANDROID_RELEASE_SIGNING=external must not be combined with " +
            "DZZY_ANDROID_UPLOAD_* credentials; unset one of them.",
    )
}

android {
    namespace = "xyz.sirius6907.dzzy.mobile"
    ...
    defaultConfig {
        applicationId = "xyz.sirius6907.dzzy.mobile"
...
gradle.taskGraph.whenReady {
    ...
    if (buildsRelease && externalReleaseSigning) {
        ...
        // guard above already rejected any DZZY_ANDROID_UPLOAD_* values.
        return@whenReady
    }
    if (buildsRelease && !hasUploadSigning) {
        throw GradleException(
            "Release builds require Android upload signing credentials. Missing: " +
                ...
                ". For central APK Signer pipelines set DZZY_ANDROID_RELEASE_SIGNING=external.",
        )
    }
    if (buildsRelease) {
        ...
        if (!configuredKeystore.isAbsolute) {
            throw GradleException(
                "DZZY_ANDROID_UPLOAD_KEYSTORE_PATH must be absolute: $configuredKeystore",
            )
        }
        ...
        if (keystore.canonicalFile.toPath().startsWith(repositoryRoot.toPath())) {
            throw GradleException(
                "DZZY_ANDROID_UPLOAD_KEYSTORE_PATH must be outside the repository: $keystore",
            )
        }
        if (!keystore.isFile || !keystore.canRead()) {
            throw GradleException(
                "DZZY_ANDROID_UPLOAD_KEYSTORE_PATH is not a readable file: $keystore",
            )
        }
    }
}
