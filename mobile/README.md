...
Flutter mobile client for Dzzy.
...
- `DZZY_ANDROID_UPLOAD_KEYSTORE_PATH`: path to a CI-vended keystore file
- `DZZY_ANDROID_UPLOAD_KEYSTORE_PASSWORD`
- `DZZY_ANDROID_UPLOAD_KEY_ALIAS`
- `DZZY_ANDROID_UPLOAD_KEY_PASSWORD`
...
a local upload keystore must set `DZZY_ANDROID_RELEASE_SIGNING=external`. That
mode produces an unsigned release bundle and refuses to run if any
`DZZY_ANDROID_UPLOAD_*` value is also set.
