...
import 'package:dzzy/features/pairing/pairing_provider.dart';
import 'package:dzzy/features/pairing/pairing_socket.dart';
import 'package:dzzy/shared/auth/auth.dart';

/// Tests for [PairingNotifier]'s legacy `dzzy://` payload parsing and
...
///   - Initial state.
///   - Parsing every documented payload format (raw base64, `dzzy://`
///     prefix, whitespace).
...
void main() {
  group('PairingNotifier', () {
    ...
    test(
      ...
      () async {
        ...
        const code =
            ...
            '&relay=wss%3A%2F%2Fpairing.dzzy.xyz&v=1';
    ...
    test('accepts dzzy scheme prefix', () async {
      container = createContainer();

      final code = 'dzzy://${_encodePairingCode()}';
      await container.read(pairingProvider.notifier).pair(code);
