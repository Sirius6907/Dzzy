...
import 'package:dzzy/shared/community/community.dart';
import 'package:dzzy/shared/community/community_storage.dart';
  ...
  group('CommunityStorage', () {
    ...
    group('migration', () {
      test('migrates saved workspaces to communities', () async {
        ...
        fakeSecure['dzzy_workspaces'] = jsonEncode([legacy.toJson()]);
        fakeSecure['dzzy_active_workspace_id'] = legacy.id;

        final loaded = await storage.loadAll();

        expect(loaded.single.id, legacy.id);
        expect(await storage.loadActiveId(), legacy.id);
        expect(fakeSecure['dzzy_communities'], isNotNull);
        expect(fakeSecure['dzzy_workspaces'], isNull);
        expect(fakeSecure['dzzy_active_workspace_id'], isNull);
      });

      test('migrates legacy keys to community on first load', () async {
        fakeSecure['dzzy_relay_url'] = 'https://legacy.example.com';
        fakeSecure['dzzy_token'] = 'legacy_token';
        fakeSecure['dzzy_pubkey'] = 'legacy_pub';
        fakeSecure['dzzy_nsec'] = 'legacy_nsec';
        ...
        expect(fakeSecure['dzzy_relay_url'], isNull);
        expect(fakeSecure['dzzy_token'], isNull);
        expect(fakeSecure['dzzy_pubkey'], isNull);
        expect(fakeSecure['dzzy_nsec'], isNull);
      ...
      test('migration generates name from localhost URL', () async {
        fakeSecure['dzzy_relay_url'] = 'http://localhost:3000';
        fakeSecure['dzzy_token'] = 'tok';
