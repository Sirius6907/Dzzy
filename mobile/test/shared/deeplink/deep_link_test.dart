import 'package:dzzy/shared/deeplink/deep_link.dart';
  ...
  group('parseMessageDeepLink', () {
    test('parses channel and id', () {
      final link = parseMessageDeepLink(
        Uri.parse('dzzy://message?channel=d14cd131&id=abc123'),
      );
      expect(
        link,
    ...
    test('parses optional thread param', () {
      final link = parseMessageDeepLink(
        Uri.parse('dzzy://message?channel=d14cd131&id=abc123&thread=root99'),
      );
    ...
    test('treats empty thread as absent', () {
      final link = parseMessageDeepLink(
        Uri.parse('dzzy://message?channel=d14cd131&id=abc123&thread='),
      );
    ...
    test('rejects missing channel', () {
      expect(parseMessageDeepLink(Uri.parse('dzzy://message?id=abc')), isNull);
    });

    test('rejects empty channel', () {
      expect(
        parseMessageDeepLink(Uri.parse('dzzy://message?channel=&id=abc')),
        isNull,
      );
    });
    ...
    test('rejects missing id', () {
      expect(
        parseMessageDeepLink(Uri.parse('dzzy://message?channel=d14cd131')),
        isNull,
      );
    });

    test('rejects non-dzzy scheme', () {
      expect(
    ...
    test('rejects non-message host (connect is desktop-only)', () {
      expect(
        parseMessageDeepLink(Uri.parse('dzzy://connect?relay=wss://x')),
        isNull,
      );
    });
  });
...
void _inviteTests() {
  group('parseInviteDeepLink', () {
    ...
    test('parses dzzy join handoff link', () {
      final link = parseInviteDeepLink(
        Uri.parse(
    ...
    test('preserves policy receipt in dzzy join handoff', () {
      final link = parseInviteDeepLink(
        Uri.parse(
          'dzzy://join?relay=wss%3A%2F%2Frelay.example.com&code=abc123&policy_receipt=receipt.value',
        ),
      );
      expect(
        link,
    ...
    test('rejects dzzy join without websocket relay or code', () {
      expect(
        parseInviteDeepLink(
          Uri.parse('dzzy://join?relay=https://relay.example.com&code=abc'),
        ),
        isNull,
      );
      expect(
        parseInviteDeepLink(
          Uri.parse('dzzy://join?relay=wss://relay.example.com'),
        ),
        isNull,
      );
      expect(
        parseInviteDeepLink(Uri.parse('dzzy://connect?relay=wss://x')),
        isNull,
      );
    });

    test('rejects dzzy join with dangerous relay schemes', () {
      ...
      for (final hostile in [
        ...
        expect(
          parseInviteDeepLink(Uri.parse('dzzy://join?relay=$encoded&code=abc')),
          isNull,
