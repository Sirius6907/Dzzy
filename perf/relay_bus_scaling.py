#!/usr/bin/env python3
"""Reproducible Dzzy relay Redis bus scaling harness.
...
DZZY_PREFIX = "dzzy"
OLD_GLOBAL_CHANNEL = f"{DZZY_PREFIX}:global"
...
def scoped_global_channel(community: str) -> str:
    # Mirrors crates/dzzy-pubsub/src/topic.rs EventTopic::Global:
    # format!("dzzy:{}:global", self.community_id)
    return f"{DZZY_PREFIX}:{community}:global"
...
def print_rows(args: argparse.Namespace, rows: list[Measurement]) -> None:
    print("Dzzy relay Redis bus scaling harness")
    print("====================================")
    print(
        "scenario: "
