# Dzzy relay bus scaling harness
...
- **new/community-scoped bus:** each pod retains only the server-resolved community topics for which it has local subscribers (`dzzy:{community_id}:global` or `dzzy:{community_id}:channel:{channel_id}`).
...
The scoped Redis channel format corresponds to `dzzy_pubsub::EventTopicKey::redis_channel()` in `crates/dzzy-pubsub/src/topic.rs`:

- global: `dzzy:{community_id}:global`
- channel: `dzzy:{community_id}:channel:{channel_id}`
