...
This guide is for AI agents contributing to the Dzzy codebase. It covers
...
Dzzy spans five repos. This one (`Sirius6907/dzzy`) is the OSS source for the relay, desktop, mobile, and CLI. The others handle internal builds and deployment:

| Repo | Purpose |
|------|---------|
| [Sirius6907/dzzy](https://github.com/Sirius6907/dzzy) | OSS source — relay, desktop app, mobile app, CLI, agent harness |
...
crates/
  # Relay + core
  dzzy-relay          # WebSocket relay server — main entry point; also hosts git + huddle audio
  dzzy-core           # Core types, event verification, filter matching, kind registry
  dzzy-db             # Postgres event store and data access layer
  dzzy-auth           # Authentication and authorization
  dzzy-pubsub         # Redis pub/sub fan-out, presence, typing indicators
  dzzy-search         # Postgres FTS full-text search
  dzzy-audit          # Hash-chain audit log
  dzzy-media          # Blossom/S3 media storage
  # Agent surface
  dzzy-acp            # ACP harness bridging Dzzy events to AI agents
  dzzy-agent          # Minimal ACP-compliant agent (non-streaming, tool-calls-as-output)
  dzzy-dev-mcp        # Developer MCP server — shell + file-edit tools
  dzzy-persona        # Agent persona packs
  dzzy-workflow       # YAML-as-code workflow engine (evalexpr conditions)
  # Clients + interop
  dzzy-pair-relay     # Ephemeral sidecar relay for NIP-AB device pairing
  dzzy-pairing-cli    # CLI for NIP-AB device pairing interop testing
  ...
  dzzy-sdk            # Typed Nostr event builders
  dzzy-admin          # Operator CLI for relay administration
  dzzy-ws-client      # Shared NIP-42 WebSocket client (connect, auth, publish)
  dzzy-test-client    # Integration test client and E2E test suite
...
Run `just test` for integration tests if you touched `dzzy-relay`,
`dzzy-db`, or `dzzy-auth` — these require a running Postgres and Redis.
...
- `POST /query` — Nostr REQ filters over HTTP. NIP-50 `search` filters
  are routed to `dzzy-search` (Postgres FTS) automatically.
...
`dzzy-core/src/kind.rs`. New features get new kind integers — add them here
...
**Agent-facing operations go in `dzzy-cli`**: New agent-facing features belong in `dzzy-cli` — add a subcommand there first, then wire the REST/WebSocket call in `client.rs`. `dzzy-dev-mcp` (shell + file tools for `dzzy-agent`) is separate.

**Workflow conditions**: `dzzy-workflow` uses
...
`dzzy` is the agent-first CLI. Auth env vars
(`DZZY_RELAY_URL`, `DZZY_PRIVATE_KEY`, `DZZY_AUTH_TAG`) are auto-injected
by the ACP harness into managed agent subprocesses. In development, set
`DZZY_PRIVATE_KEY` and `DZZY_RELAY_URL` in your environment manually.

### Building the CLI

```bash
cargo build --release -p dzzy-cli
```

Binary location: `./target/release/dzzy`. Add `./target/release` to `PATH`
or invoke with the full path.

### Deep Links

`dzzy://message?channel=<uuid>&id=<hex>` links reference a specific message
thread. To read the linked thread:

```bash
dzzy messages thread --channel <uuid> --event <hex> --format compact
```
...
`dzzy --format compact channels list`, NOT `dzzy channels list --format compact`.

See `crates/dzzy-cli/TESTING.md` for the full live-testing runbook.
...
E2E tests live in `crates/dzzy-test-client/tests/`:
...
> **Do NOT use `dzzy upload`, the relay media endpoint, or any third-party
...
fields are optional and passed through to `__DZZY_E2E_EMIT_MOCK_MESSAGE__`:
...
gh pr view <pr> --repo Sirius6907/dzzy --json comments \
  --jq '.comments[] | select(.body | test("pr-<pr>--")) | {id, url}'
gh api -X DELETE repos/Sirius6907/dzzy/issues/comments/<stale-comment-id>
```
...
`__DZZY_E2E_EMIT_MOCK_MESSAGE__` — messages are silently dropped without a
...
description. See [PR #803](https://github.com/Sirius6907/dzzy/pull/803).

---

## Common Gotchas

1. **Kind `39000` for channel metadata, not `41`** — kind 41 is NIP-01 (unused). All kinds defined in `dzzy-core/src/kind.rs`.
