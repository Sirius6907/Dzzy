...
Dzzy has three independent release lanes. Desktop and relay use release PRs.
...
| Relay | `just release-relay` | `ghcr.io/block/dzzy` container image |
...
exact candidate tag. The mobile handoff to the private `dzzy-releases` pipeline
...
1. **`just release-relay`** runs locally on `main`, creates or updates a
   `relay-release/<version>` PR, bumps `crates/dzzy-relay/Cargo.toml`,
...
1. **Publish a candidate.** From a clean checkout whose `origin` is the
   canonical `block/dzzy` repository, run
   ...
   `mobile-vX.Y.Z-rc.N` tag there through the dedicated `dzzy-release-bot`
...
2. **Build the exact tag.** Enter the candidate tag as `mobile_ref` in the
   private Dzzy mobile Buildkite pipeline. OSS CI deliberately cannot trigger
...
| Relay | `crates/dzzy-relay/Cargo.toml` |
...
```sh
gh workflow run signed-macos-canary.yml --repo block/dzzy --ref main
```
...
create or move tags, and cannot update `dzzy-desktop-latest` or `latest.json`.

Download the artifact from the completed run:

```sh
gh run download <run-id> --repo block/dzzy --name <artifact-name>
```
...
[Release Mobile pipeline](https://buildkite.com/runway/dzzy-mobile-releases) with
...
[dzzy-releases README](https://github.com/squareup/dzzy-releases#cutting-a-release)
...
2. **`dzzy-desktop-latest`**: the rolling auto-updater release.
...
- **Write access** to the `block/dzzy` GitHub repository
- An `origin` remote whose configured URL is the canonical `block/dzzy`
  repository
- `gh` CLI version 2.87.0 or newer, authenticated with permission to dispatch
  the candidate workflow
- Release tag ruleset [`14378754`](https://github.com/block/dzzy/rules/14378754)
  active for `mobile-v*`, with creation, update, deletion, and non-fast-forward
  protections and `dzzy-release-bot` as its sole always-bypass actor
- The `dzzy-release-bot` App credentials configured for GitHub Actions
- The following **GitHub Actions secrets** must also be configured for the
  desktop release lane:

  | Secret | Purpose |
  |--------|---------|
  | `DZZY_UPDATER_PUBLIC_KEY` | Tauri updater public key (minisign) |
...
must be installed on `block/dzzy`, have Contents write and Metadata read, and
...
Confirm `dzzy-release-bot` remains the sole always-bypass actor for the active
...
Verify that the `dzzy-desktop-latest` release exists and contains a
