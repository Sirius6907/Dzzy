...
helm lint deploy/charts/dzzy-push-gateway >/dev/null
helm template push deploy/charts/dzzy-push-gateway >"$out"
# Production values must attach push.dzzy.xyz to an explicit Gateway.
production_args=(
  -f deploy/charts/dzzy-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'appAttestAppId=REALTEAM.xyz.dzzy'
...
helm lint deploy/charts/dzzy-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/dzzy-push-gateway "${production_args[@]}" >"$production_out"
...
runtime={'app.kubernetes.io/name':'dzzy-push-gateway','app.kubernetes.io/instance':'push','app.kubernetes.io/component':'runtime'}
...
assert jenv['DZZY_PUSH_RUNTIME_DATABASE_ROLE']['value']=='dzzy_push_gateway_runtime'
...
required={'DATABASE_URL','DZZY_PUSH_APNS_KEY_ID','DZZY_PUSH_APNS_TEAM_ID','DZZY_PUSH_APNS_TOPIC','DZZY_PUSH_GRANT_KEYS','DZZY_PUSH_TOKEN_KEYS','DZZY_PUSH_MAX_GRANT_LIFETIME_SECONDS'}
assert required <= env
...
np=next(x for x in nps if x['metadata']['name']=='push-dzzy-push-gateway')
migration_np=next(x for x in nps if x['metadata']['name']=='push-dzzy-push-gateway-migration')
...
assert 'push.dzzy.xyz' in route['spec']['hostnames']
PY

# Enabling a route without a Gateway attachment must fail schema validation.
if helm template push deploy/charts/dzzy-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/dzzy-push-gateway -f deploy/charts/dzzy-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
helm template push deploy/charts/dzzy-push-gateway \
...
np=next(x for x in xs if x and x.get('kind')=='NetworkPolicy' and x['metadata']['name']=='push-dzzy-push-gateway')
...
if helm template push deploy/charts/dzzy-push-gateway \
  --set podMonitor.enabled=true \
