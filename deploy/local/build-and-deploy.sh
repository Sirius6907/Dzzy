#!/usr/bin/env bash
# Local docker-desktop k8s testbed for the Dzzy relay mesh.
#
...
NS="${NS:-dzzy-mesh}"
RELEASE="${RELEASE:-dzzy}"
IMAGE_REPO="${IMAGE_REPO:-dzzy-relay}"
IMAGE_TAG="${IMAGE_TAG:-mesh-local}"
CHART="${REPO_ROOT}/deploy/charts/dzzy"
...
if [ "$helm_rc" != 0 ]; then
  ...
  kubectl -n "$NS" describe pods -l app.kubernetes.io/name=dzzy | tee "$EVID/describe-onfail.txt"
  kubectl -n "$NS" logs -l app.kubernetes.io/name=dzzy --tail=100 --all-containers | tee "$EVID/logs-onfail.txt"
  die "helm install returned rc=$helm_rc"
fi

# ── 4. verify 3/3 Ready ───────────────────────────────────────────────────────
# Find the relay Deployment: everything under this release named "dzzy" except
...
# chart name, so the name isn't always "<release>-dzzy".)
DEPLOY=""
...
# The bundled MinIO + init pods share app.kubernetes.io/name=dzzy, so select by
...
RELAY_PODS=$(kubectl -n "$NS" get pods \
  -l "app.kubernetes.io/name=dzzy,app.kubernetes.io/instance=$RELEASE" \
