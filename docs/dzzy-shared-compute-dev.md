# Dzzy shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`Dzzy Desktop → dzzy-acp → dzzy-agent → MeshLLM SDK → local/remote compute`

It does not use a substitute agent harness.

## Before starting

Run from the `block/dzzy` repository root on the mesh-enabled branch.
...
`~/.dzzy-dev`, and local Docker volumes; it preserves the installed Dzzy app's
data, production keyring, and `~/.dzzy`. The first dev page load also clears
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to Dzzy**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local Dzzy**.
...
Stop only stale Dzzy/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the Dzzy window to open and for the terminal to
...
Dzzy may download the model on first use. The model picker ranks models for the
...
resolves to the bundled `dzzy-agent`. No API key is required.
...
**dzzy-agent** as the runtime, **Dzzy shared compute** as the LLM provider,
...
- no current Dzzy membership snapshot is available (admission fails closed).

## Security boundary

Dzzy publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current Dzzy membership controls which node identities are
...
`DZZY_MESH_IROH_RELAYS` defaults to Iroh's production relay set. Set it to `0`
...
MeshLLM—not the Dzzy relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the Dzzy-side checks above.
