...
tags: [nostr, nip, push-notifications, dzzy, draft]
status: draft
created: 2026-07-02
---
...
`silent` remains a matching preference only. The public Dzzy APNs profile sends the one fixed reconnect alert and does not expose relay-selected notification classes to the transport boundary.
...
```json
{"v":1,"challenge_id":"<uuid>","challenge":"<challenge>","key_id":"<standard-base64>","attestation":"<standard-base64 CBOR>","app_profile":"dzzy-ios-production","endpoint":"<lowercase APNs-token hex>","endpoint_epoch":1,"expires_at":<unix-seconds>}
```

`expires_at` MUST satisfy `now < expires_at <= now + configured_max_installation_lifetime`; the selected profile MUST be enabled. The exact transcript is domain `dzzy.push.enroll.v1` followed by this ordered object:

```json
{"v":1,"audience":"https://push.dzzy.xyz/v1/installations","challenge_id":"<uuid>","challenge":"<challenge>","key_id":"<standard-base64>","app_profile":"<registered-profile>","endpoint":"<lowercase-hex>","endpoint_epoch":1,"expires_at":<unix-seconds>}
```
...
`not_before <= now + 300`, `not_before < expires_at`, and `expires_at <= min(now + configured_max_grant_lifetime, installation.expires_at)`. The endpoint epoch MUST equal the current installation epoch. For each `(installation_handle, relay_pubkey)`, generation MUST strictly increase. Transcript domain `dzzy.push.delegate.v1`; ordered object:

```json
{"v":1,"audience":"https://push.dzzy.xyz/v1/delegations","challenge_id":"<uuid>","challenge":"<challenge>","installation_handle":"<uuid>","endpoint_epoch":<integer>,"generation":<integer>,"relay_pubkey":"<hex>","not_before":<integer>,"expires_at":<integer>}
```
...
`new_endpoint_epoch` MUST equal `endpoint_epoch + 1` without overflow. Transcript domain `dzzy.push.rotate-endpoint.v1`; ordered object:

```json
{"v":1,"audience":"https://push.dzzy.xyz/v1/installations/endpoint","challenge_id":"<uuid>","challenge":"<challenge>","installation_handle":"<uuid>","endpoint_epoch":<integer>,"new_endpoint_epoch":<integer>,"endpoint":"<lowercase-hex>"}
```
...
Transcript domain `dzzy.push.revoke-delegation.v1`; ordered object:

```json
{"v":1,"audience":"https://push.dzzy.xyz/v1/delegations/revoke","challenge_id":"<uuid>","challenge":"<challenge>","installation_handle":"<uuid>","relay_pubkey":"<hex>","generation":<integer>}
```
...
`new_endpoint_epoch` MUST equal `endpoint_epoch + 1` without overflow. Transcript domain `dzzy.push.revoke-installation.v1`; ordered object:

```json
{"v":1,"audience":"https://push.dzzy.xyz/v1/installations/revoke","challenge_id":"<uuid>","challenge":"<challenge>","installation_handle":"<uuid>","endpoint_epoch":<integer>,"new_endpoint_epoch":<integer>}
```
...
`POST /v1/deliveries/apns` has the exact externally configured URL `https://push.dzzy.xyz/v1/deliveries/apns`. Request:
...
- Public APNs gateway profile: base URL `https://push.dzzy.xyz`; app profiles `dzzy-ios-production`, `dzzy-ios-sandbox`; wire version `1`
