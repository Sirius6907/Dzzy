# Contributing to Dzzy

Welcome, and thank you for your interest in contributing! Dzzy is an
...
unacceptable behavior to **conduct@dzzy-relay.org**.
...
```bash
# 1. Clone the repo
git clone https://github.com/block/dzzy.git
cd dzzy
...
just reset   # Wipe all dev state and recreate it; installed Dzzy is preserved
```

Development desktop state uses separate bundle identifiers
(`xyz.block.dzzy.app.dev` and per-worktree variants), a separate keyring service
(`dzzy-desktop-dev`), and `~/.dzzy-dev`. `just reset` removes those dev-only
locations and the local Docker volumes. It does not touch the installed app's
`xyz.block.dzzy.app` data, `dzzy-desktop` keyring service, or `~/.dzzy` nest.
...
End-to-end tests live in `crates/dzzy-test-client/tests/`:
...
of `dzzy-core` types, which are shared everywhere).
...
Dzzy is developed across multiple repositories. This repo (`block/dzzy`)
...
**External contributors:** Fork `block/dzzy`, open a PR, and CI runs
...
1. **Define the kind constant** in `dzzy-core/src/kind.rs`:
...
2. **Define the payload type** in the appropriate module in `dzzy-core/src/`
...
3. **Register the kind's required scope** in
   `crates/dzzy-relay/src/handlers/ingest.rs` inside
...
4. **Handle post-storage side effects** by adding a match arm in
   `crates/dzzy-relay/src/handlers/side_effects.rs` inside
   `handle_side_effects()`:
   ...
   `crates/dzzy-relay/src/api/` and register it in
   `crates/dzzy-relay/src/router.rs`.

5. **Persist to the database** — if the event needs to be queryable, add a
   handler in `dzzy-db/src/` (e.g., `dzzy-db/src/my_feature.rs`) with
...
8. **Write tests** — add a unit test for payload serialization in
   `dzzy-core` and an integration test in `dzzy-test-client` that sends
...
1. **Define the handler** in the appropriate module under
   `crates/dzzy-relay/src/api/`. Resolve the request tenant before any auth or
   data lookup, use NIP-98 when the endpoint accepts user credentials, and keep
   community scoping explicit.

2. **Register the route** in `crates/dzzy-relay/src/router.rs` using the
   narrowest path possible. Do not add new `/api/*` compatibility routes unless
   the product decision explicitly calls for one.

3. **Add database queries** in `dzzy-db/src/` only when the endpoint cannot be
   expressed through the existing event query paths.

4. **Handle errors** using the `api_error()`, `internal_error()`, and
   `not_found()` helpers in `dzzy-relay/src/api/mod.rs`. Return
   `(StatusCode, Json<Value>)` tuples.

5. **Write tests** with the `dzzy-test-client` harness in
   `crates/dzzy-test-client/tests/`, covering auth, community scoping, and the
...
Dzzy is licensed under the **Apache License, Version 2.0**. See
...
*Thank you for contributing to Dzzy. Every bug report, documentation fix,
