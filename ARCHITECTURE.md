...
A Dzzy **community** is the tenant-visible workspace selected by the request host.
...
Dzzy is a Rust monorepo, licensed Apache 2.0 under Block, Inc.
...
│                           CLIENTS                                    │
│                                                                      │
│  Human (Nostr app, web, mobile)    Agent (CLI tools via dzzy-cli)    │
│           │                                    │                     │
│           └──────────── WebSocket ─────────────┘                    │
└─────────────────────────────────────────────────────────────────────┘
                               │
...
└──────────┬──────────────┬──────────────────────────────────────────┘
     ...
     ┌──────────────┐
     │  Postgres    │  ← dzzy-search (FTS over the search_tsv
...
```
dzzy-core    (zero I/O — types, verification, filter matching, kind registry)
    │
    ├── dzzy-db          (Postgres: events, channels, tokens, workflows, audit)
    ├── dzzy-auth        (NIP-42, NIP-98, API tokens, scopes, rate limiting)
    ├── dzzy-pubsub      (Redis pub/sub, presence, typing indicators)
    ├── dzzy-search      (Postgres FTS: query, delete)
    ├── dzzy-audit       (hash-chain tamper-evident log)
    └── dzzy-workflow    (YAML-as-code automation engine)
         │
         └── dzzy-relay       (ties everything together — the server)

dzzy-acp            (agent harness — bridges relay @mentions → AI agents via ACP/JSON-RPC)
dzzy-sdk            (typed Nostr event builders — used by dzzy-acp and dzzy-cli)
dzzy-media          (Blossom/S3 media storage)
dzzy-cli            (agent-first CLI)
dzzy-admin          (operator CLI: relay membership + key generation)
dzzy-test-client    (integration test harness + manual CLI)
```

**Key architectural principle:** The relay is the single source of truth. `dzzy-relay` orchestrates all subsystems by calling them directly — it imports `dzzy-db`, `dzzy-auth`, `dzzy-pubsub`, `dzzy-search`, `dzzy-audit`, and `dzzy-workflow`. However, those subsystems are isolated from each other: `dzzy-workflow` never calls `dzzy-pubsub`, `dzzy-search` never calls `dzzy-db`, etc. Cross-subsystem coordination happens only through the relay. In multi-community mode, the relay also owns propagation of `TenantContext`; service crates should receive community-scoped inputs rather than independently deriving tenancy from client-controlled event tags.

---

## 2. The Protocol

Dzzy uses Nostr NIP-01 on the wire. Every action is a JSON event with six fields:
...
| 40000–49999 | Dzzy custom kinds |

### Dzzy Custom Kinds (selected)
...
`dzzy-core` defines all 81 kinds as `pub const KIND_*: u32` and exports `ALL_KINDS: &[u32]`. Kinds are `u32` (NIP-01 specifies unsigned integer; `u32` covers the full range). Dzzy uses both standard Nostr kinds (e.g., kind 7 for reactions) and custom ranges (40000+).

Note: `KIND_AUTH` (22242) is `pub const KIND_AUTH: u32` in `dzzy-core/src/kind.rs` and imported by `dzzy-relay/src/handlers/event.rs`. `KIND_CANVAS` (40100) is likewise `pub const KIND_CANVAS: u32` in `dzzy-core/src/kind.rs`.
...
Workflow loop prevention: workflow execution kinds (46001–46012), relay-signed messages with `dzzy:workflow` tag, and `KIND_GIFT_WRAP` are excluded from triggering workflows. All other stored events (including kind 9 stream messages) trigger workflow evaluation.
...
### dzzy-core — Shared Types and Verification
...
### dzzy-auth — Authentication and Authorization
...
- Dev-only key derivation: `SHA-256("dzzy-test-key:{username}")` — gated behind `#[cfg(any(test, feature = "dev"))]`. The `dev` feature must not be enabled in production relay deployments.
...
### dzzy-pubsub — Redis Pub/Sub, Presence, Typing

Manages Redis pub/sub fan-out, presence tracking, and typing indicators. In multi-community mode all tenant-visible keys are prefixed or otherwise partitioned by community (`dzzy:{community}:...`) so channel fan-out, presence, typing, and cache invalidation cannot cross hosts.

**Architecture:**

```
Publisher  → pool connection   → PUBLISH dzzy:channel:{uuid}
Subscriber → dedicated PubSub  → PSUBSCRIBE dzzy:channel:*
...
**Presence:** `SET dzzy:presence:{pubkey_hex} {status} EX 90` — 90-second TTL (3× the 30-second heartbeat interval). Single missed heartbeat does not cause presence flap.

**Typing indicators:**
```
ZADD dzzy:typing:{channel_id} {now_unix} {pubkey_hex}
ZREMRANGEBYSCORE dzzy:typing:{channel_id} -inf {now - 5.0}
EXPIRE dzzy:typing:{channel_id} 60
```
...
### dzzy-search — Postgres FTS Integration
...
- Permission filtering is **caller's responsibility** — `dzzy-search` returns
...
### dzzy-audit — Hash-Chain Audit Log
...
Real-time voice lives inside `dzzy-relay` (`src/audio/`), not a separate crate. A WebSocket endpoint (`wss://.../huddle/{channel_id}/audio`) authenticates each participant with a NIP-42 challenge, checks channel membership, admits them to an in-memory room, and forwards opaque Opus frames between peers. No external SFU.
...
### dzzy-acp — Agent Communication Protocol Harness

Standalone binary that bridges Dzzy relay events to AI agents via the [Agent Communication Protocol](https://agentclientprotocol.com/) (ACP).

**Architecture:**

```
Dzzy Relay ──WS──→ dzzy-acp ──stdio (ACP/JSON-RPC)──→ Agent (goose/codex/claude)
```

`dzzy-acp` spawns AI agent subprocesses (1–32, default 1), connects to the relay via WebSocket with NIP-42 auth, discovers channels via REST API, and queues `@mention` events per channel. At most one prompt is in-flight per channel. Queued events are batched into a single prompt sent via `session/prompt` over ACP.
...
- Depends on `dzzy-core` (kind constants) and `dzzy-sdk` (relay/REST utilities).

**Does NOT:** persist state.

---

### dzzy-admin — Operator CLI
...
The `dzzy-admin` binary is shipped in the relay Docker image (`/usr/local/bin/dzzy-admin`) and is the recommended way to manage relay membership in production. Use `./run.sh add-member`, `./run.sh remove-member`, and `./run.sh list-members` in Docker Compose deployments.

---

### dzzy-test-client — Integration Test Harness

**`DzzyTestClient`** wraps a WebSocket connection with a `VecDeque<RelayMessage>` buffer for message interleaving. Methods: `connect`, `connect_unauthenticated`, `authenticate`, `send_event`, `send_text_message`, `subscribe`, `close_subscription`, `recv_event`, `collect_until_eose`, `disconnect`.
...
`src/main.rs` is a manual testing CLI (`dzzy-test-cli`) with `--send`, `--subscribe`, `--channel`, `--url`, `--kind` flags.
...
| Concern | Mechanism |
|---------|-----------|
| Schnorr signatures | `verify_event()` in `dzzy-core` — every event verified before storage |
...
`is_private_ip()` in `dzzy-core` covers:
...
Applied in: `dzzy-workflow` (CallWebhook action), `dzzy-core` (shared utility).
...
|---------|------|-----|---------|
| `dzzy:channel:{uuid}` | Pub/Sub channel | — | Event fan-out (single-community form; shared multi-community Redis must use `dzzy:{community}:channel:{uuid}` or equivalent) |
| `dzzy:presence:{pubkey_hex}` | String | 90s | Online/away status (single-community form; shared multi-community Redis must scope by community) |
| `dzzy:typing:{channel_uuid}` | Sorted Set | 60s | Active typers (5s window; shared multi-community Redis must scope by community) |
...
| 2 | **No rate limiting implementation** | `RateLimiter` trait exists in `dzzy-auth`. Only implementation is `AlwaysAllowRateLimiter` (test stub, gated behind `#[cfg(any(test, feature = "test-utils"))]`). `RateLimitConfig` defines 4 tiers (human, agent-standard, agent-elevated, agent-platform) but none are enforced. |
