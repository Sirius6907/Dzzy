...
const WELCOME_CHANNEL_ENSURED_STORAGE_KEY_PREFIX =
  "dzzy-welcome-channel-ensured.v2:";
const ONBOARDING_COMPLETION_STORAGE_KEY_PREFIX = "dzzy-onboarding-complete.v1:";
const DEFAULT_MOCK_PUBKEY = "deadbeef".repeat(8);
// The relay HTTP/WS URLs follow DZZY_E2E_RELAY_URL (same env var seed.ts reads),
...
const DEFAULT_RELAY_HTTP_URL =
  process.env.DZZY_E2E_RELAY_URL ?? "http://localhost:3000";
...
See [[mem/preferences/ui-density]] and [[mem/projects/dzzy-memory-viewer]] for details.
...
A retired launch checklist used to live at [[mem/archive/deleted-launch-checklist]], but that memory was deleted after the plan changed.`,
      ...
      outgoingRefs: [
        ...
        "mem/projects/dzzy-memory-viewer",
    ...
    memories: [
      ...
      {
        slug: "mem/projects/dzzy-memory-viewer",
        body: "Building the IXI-7 read-only memory viewer in the profile panel.\n\nChild memory: [[mem/projects/dzzy-memory-viewer/notes]]",
        eventId: "mock-project",
        createdAt: 1_700_000_300,
        outgoingRefs: ["mem/projects/dzzy-memory-viewer/notes"],
      },
      {
        slug: "mem/projects/dzzy-memory-viewer/notes",
...
async function seedDefaultCommunity(
  ...
  await page.addInitScript(
    ({ fallback, identityOverrideKey, relayUrl }) => {
      ...
      window.localStorage.setItem("dzzy-active-community-id", communityId);
    },
    {
      fallback: fallbackPubkey,
      identityOverrideKey: "dzzy:e2e-identity-override.v1",
      relayUrl: relayWsUrl ?? DEFAULT_RELAY_WS_URL,
    },
  );
}
  ...
  await page.addInitScript(
    ({
      ...
      const testWindow = window as Window & {
        __DZZY_E2E__?: Record<string, unknown>;
        __DZZY_E2E_APP_BADGE_COUNT__?: number;
        __DZZY_E2E_APP_BADGE_STATE__?: string;
      ...
      const currentConfig = testWindow.__DZZY_E2E__ ?? {};

      testWindow.__DZZY_E2E__ = {
        ...currentConfig,
      ...
      testWindow.__DZZY_E2E_APP_BADGE_COUNT__ = 0;
      testWindow.__DZZY_E2E_APP_BADGE_STATE__ = "none";
      testWindow.__DZZY_E2E_CLICK_NOTIFICATION__ = (index: number) => {
      ...
      testWindow.__DZZY_E2E_NOTIFICATIONS__ = notificationLog;
    },
    {
      identity,
      mock: options.mock,
...
export async function installRelayBridge(
  ...
  await installBridge(page, {
    mode: "relay",
    user,
    // Thread DZZY_E2E_RELAY_URL into BOTH transports. The app defaults these to
