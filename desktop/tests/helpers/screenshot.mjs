#!/usr/bin/env node
//
// Standalone Playwright screenshot helper for the Dzzy desktop app.
//
...
// `DZZY_HEADED=1` runs a real browser window instead of headless.
//
...
// CI is unaffected.
const browser = await chromium.launch({ headless: !process.env.DZZY_HEADED });
...
await page.addInitScript(() => {
  ...
  window.localStorage.setItem("dzzy-communities", JSON.stringify([community]));
...
await page.addInitScript(
  ({ updateReady }) => {
    ...
    window.__DZZY_E2E_APP_BADGE_COUNT__ = 0;
  },
  ...
  if (args.messages) {
    ...
    for (const ch of targetChannels) {
      await page.waitForFunction(
        (name) =>
          window.__DZZY_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({
            channelName: name,
          }) ?? false,
        ch,
        { timeout: 10000 },
    ...
    for (const msg of messages) {
      await page.evaluate(
        (m) => {
          window.__DZZY_E2E_EMIT_MOCK_MESSAGE__?.(m);
        },
