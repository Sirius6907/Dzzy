...
test("Share compute has a clear empty state and starts and stops sharing", async ({
  ...
  await expect(card).toContainText(
    "Dzzy downloads remote models when sharing starts",
  );
  ...
  await expect
    .poll(() =>
      page.evaluate(() => (window as E2eWindow).__DZZY_E2E_COMMANDS__ ?? []),
    )
...
test("consuming a peer's compute does NOT light the Share toggle", async ({
  ...
  await page.waitForFunction(
    () => typeof (window as E2eWindow).__DZZY_E2E_SET_MESH__ === "function",
  );
  await page.evaluate(() => {
    (window as E2eWindow).__DZZY_E2E_SET_MESH__?.({
      nodeState: "running",
      nodeMode: "client",
    });
  });
  ...
  const stopIssued = await page.evaluate(() =>
    ((window as E2eWindow).__DZZY_E2E_COMMANDS__ ?? []).includes(
      "mesh_stop_node",
    ),
  );
