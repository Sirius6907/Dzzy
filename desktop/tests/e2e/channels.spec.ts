...
async function hasOutgoingEventWithContent(
  ...
  return page.evaluate((expectedContent) => {
    return (
      ...
      ).__DZZY_E2E_COMMAND_LOG__?.some((entry) => {
      ...
      return page.evaluate(
        ({ currentChannelName, kind }) => {
          return (
            ...
            ).__DZZY_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({
...
test("start a new direct message from the sidebar", async ({ page }) => {
  ...
  await expect(page.locator(".dzzy-poof-burst")).toHaveCount(1);
  await page.mouse.up();
...
test("closes direct message results while opening", async ({ page }) => {
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __DZZY_E2E__?: { mock?: { openDmDelayMs?: number } };
    };
    testWindow.__DZZY_E2E__ ??= {};
    testWindow.__DZZY_E2E__.mock ??= {};
    testWindow.__DZZY_E2E__.mock.openDmDelayMs = 1_000;
  });
...
test("does not reopen a sent direct message after leaving during cache reseed", async ({
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __DZZY_E2E__?: { mock?: { channelsReadDelayMs?: number } };
    };
    testWindow.__DZZY_E2E__ ??= {};
    testWindow.__DZZY_E2E__.mock ??= {};
    testWindow.__DZZY_E2E__.mock.channelsReadDelayMs = 1_000;
  });
...
test("shows and clears activity indicators for active channel agents", async ({
  ...
  await page.evaluate((pubkey) => {
    window.__DZZY_E2E_EMIT_MOCK_TYPING__?.({
      channelName: "agents",
      pubkey,
    });
...
test("profile renders live activity for a viewer-owned relay agent", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__DZZY_E2E_SEED_ACTIVE_TURNS__ ===
      "function",
  );
  await page.evaluate(
    ({ agentPubkey, channelId }) => {
      const seedActiveTurns = (window as MockFeedWindow)
        .__DZZY_E2E_SEED_ACTIVE_TURNS__;
      if (!seedActiveTurns) {
...
async function seedHomeInboxMention(
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__DZZY_E2E_PUSH_MOCK_FEED_ITEM__ ===
      "function",
  );
  ...
  await page.evaluate(
    ({
      ...
      const pushFeedItem = (window as MockFeedWindow)
        .__DZZY_E2E_PUSH_MOCK_FEED_ITEM__;
      if (!pushFeedItem) {
...
test("stopping a managed bot in one community leaves its other communities running", async ({
  ...
  // (`seedDefaultCommunity` follows DZZY_E2E_RELAY_URL); the agent also has a
