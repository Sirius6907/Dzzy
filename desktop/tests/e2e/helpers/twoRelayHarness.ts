...
export class TwoRelayHarness {
  ...
  static async create(relays: readonly RelaySpec[]) {
    return new TwoRelayHarness(
      await mkdtemp(join(tmpdir(), "dzzy-ae-e2e-")),
      relays,
    );
  }

  async startRelays(binary = process.env.DZZY_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("DZZY_E2E_RELAY_BIN is required for the live gate");
    await Promise.all(
  ...
  async restartRelay(name: string, binary = process.env.DZZY_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("DZZY_E2E_RELAY_BIN is required for the live gate");
  ...
  async startAcp(
    ...
    const binary = process.env.DZZY_E2E_ACP_BIN;
    if (!binary)
      throw new Error("DZZY_E2E_ACP_BIN is required for the live gate");
    return this.spawnOwned(name, binary, [], {
      DZZY_RELAY_URL: relayWsUrl,
      DZZY_PRIVATE_KEY: privateKey,
      DZZY_AUTH_TAG: "",
      DZZY_ACP_LAZY_POOL: "true",
      DZZY_ACP_AGENT_COMMAND: process.execPath,
      DZZY_ACP_AGENT_ARGS: resolve("tests/e2e/fixtures/fake-acp-agent.mjs"),
      DZZY_E2E_CLI_BIN: process.env.DZZY_E2E_CLI_BIN,
      ...extraEnv,
    });
  }
  ...
  private async startRelay(binary: string, relay: RelaySpec) {
    const child = this.spawnOwned(relay.name, binary, [], {
      ...
      DZZY_BIND_ADDR: `127.0.0.1:${relay.ports.main}`,
      DZZY_HEALTH_PORT: String(relay.ports.health),
      DZZY_METRICS_PORT: String(relay.ports.metrics),
