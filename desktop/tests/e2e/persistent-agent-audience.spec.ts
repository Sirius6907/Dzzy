...
async function seedAudience(page: Page, pubkeys: string[], theme = "dzzy") {
  await page.addInitScript(
    ({ audience, scope, selectedTheme }) => {
      window.localStorage.setItem("dzzy:keep-addressed-agents-active", "1");
      window.localStorage.setItem(
        "dzzy:persistent-agent-audiences:v2",
      ...
      page.evaluate(
        ({ owner, channelId, rootId }) => {
          const stored = JSON.parse(
            localStorage.getItem("dzzy:persistent-agent-audiences:v2") ?? "{}",
          );
...
for (const theme of ["dzzy", "dzzy-dark"]) {
