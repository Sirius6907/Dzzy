...
/**
 * SCROLL-BACK latency profile for one channel (#dzzy-bugs) against a LIVE
 ...
 *   DZZY_E2E_RELAY_URL=http://127.0.0.1:13000 \
 *   DZZY_COMMUNITY_HOST=sprout-oss.stage.blox.sqprod.co \
 *   DZZY_PERF_NSEC=nsec1... \
 *   npx playwright test --config=playwright.perf.config.ts scrollback-dzzybugs.perf.ts
 */

const RELAY_HTTP =
  process.env.DZZY_E2E_RELAY_URL ?? "https://sprout-oss.stage.blox.sqprod.co";
const NSEC = process.env.DZZY_PERF_NSEC ?? "";
const COMMUNITY_HOST = process.env.DZZY_COMMUNITY_HOST ?? "";
const TARGET_CHANNEL = process.env.DZZY_PERF_CHANNEL ?? "dzzy-bugs";
const PAGES = Number(process.env.DZZY_PERF_PAGES ?? 10);

const IDENTITY_OVERRIDE_KEY = "dzzy:e2e-identity-override.v1";
const ONBOARDING_PREFIX = "dzzy-onboarding-complete.v1:";
const WELCOME_PREFIX = "dzzy-welcome-channel-ensured.v2:";
...
function deriveIdentity(nsec: string) {
  ...
  if (decoded.type !== "nsec") throw new Error("DZZY_PERF_NSEC is not an nsec");
...
test("MEASURE: scroll-back pagination latency in target channel", async ({
  ...
  if (!NSEC) throw new Error("Set DZZY_PERF_NSEC to a real member nsec");
  ...
  await page.addInitScript(
    ({ ident, onboardingPrefix, welcomePrefix, relayUrl, overrideKey }) => {
      ...
      const w = window as unknown as { __DZZY_E2E__?: Record<string, unknown> };
      w.__DZZY_E2E__ = { ...(w.__DZZY_E2E__ ?? {}), identity: ident };
    },
    {
      ident: identity,
