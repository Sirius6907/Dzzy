...
test.describe("config bridge screenshots", () => {
  ...
  test("01 — folded config panel", async ({ page }) => {
    ...
    await expect(panel.getByText("Set in Dzzy").first()).toBeVisible();
  ...
  test("06 — dzzy-agent empty MCP servers", async ({ page }) => {
    await installMockBridge(page, {
      managedAgents: [
        {
          pubkey: DZZY_AGENT_PUBKEY,
          name: "Dzzy Agent",
          status: "running" as const,
          channelNames: ["agents"],
        },
    ...
    const panel = await openAgentProfileFromChannel(page, "Dzzy Agent");
