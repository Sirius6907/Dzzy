...
// The fresh-profile default is the Dzzy theme, which pins the neutral accent
// regardless of the stored accent color. Accent-driven review foreground
// assertions must run on a non-Dzzy theme for the seeded accent to apply.
...
async function installVideoReviewHarness(
  ...
  await page.addInitScript(
    ({ accentColor, themeName }) => {
      if (themeName) {
        window.localStorage.setItem("dzzy-theme", themeName);
      }
      window.localStorage.setItem("dzzy-accent-color", accentColor);
