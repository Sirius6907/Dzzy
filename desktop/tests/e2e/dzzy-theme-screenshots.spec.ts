...
const SHOTS = "test-results/dzzy-theme";
const THEME_STORAGE_KEY = "dzzy-theme";
  ...
  await page.addInitScript(
    ({ channelId, pubkey }) => {
      window.localStorage.setItem(
        `dzzy-channel-sections.v1:${pubkey}`,
        JSON.stringify({
          version: 1,
          sections: [
            {
...
async function expectDzzySidebarPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
  ...
  const sidebarScroller = page.locator(".dzzy-sidebar-scrollbar");
  ...
  await expect(page.locator("[data-dzzy-sidebar-secondary]").first()).toHaveCSS(
    "color",
    mutedColor,
  );
...
async function expectDzzyContentShadow(page: Page, mode: "light" | "dark") {
  const effects = await page.evaluate(() => {
    const shell = document.querySelector(".dzzy-huddle-shell");
    const content = document.querySelector("[data-dzzy-content-surface]");
    const shadowViewport = document.querySelector(
      "[data-dzzy-shadow-viewport]",
    );
    return {
...
async function expectDzzyGradientPaint(
  page: Page,
  mode: "light" | "dark",
): Promise<string> {
  const paint = await page.evaluate(() => {
    const root = document.documentElement;
    const appSurface = document.querySelector(".dzzy-huddle-app-surface");
    const lightLayer = document.querySelector('[data-dzzy-gradient="light"]');
    const darkLayer = document.querySelector('[data-dzzy-gradient="dark"]');
    ...
    return {
      ...
      theme: root.getAttribute("data-dzzy-theme"),
  ...
  expect(paint.theme).toBe(mode === "light" ? "dzzy" : "dzzy-dark");
...
async function expectDzzySettingsPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
    mode === "light" ? "rgba(0, 0, 0, 0.4)" : "rgba(255, 255, 255, 0.4)";
  ...
  await expectDzzyGradientPaint(page, mode);
...
async function expectAppliedDzzyTheme(
  page: Page,
  themeName: "dzzy" | "dzzy-dark",
  storedTheme: "dzzy" | "dzzy-dark" = themeName,
) {
  const isDark = themeName === "dzzy-dark";
  await expect
    .poll(() =>
      page.evaluate((storageKey) => {
        ...
        return {
          ...
          isDark: root.classList.contains("dark"),
          dzzyTheme: root.getAttribute("data-dzzy-theme"),
          gradientTop: styles.getPropertyValue("--dzzy-gradient-top").trim(),
          gradientBottom: styles
            .getPropertyValue("--dzzy-gradient-bottom")
            .trim(),
        };
      }, THEME_STORAGE_KEY),
    )
...
test("dzzy light sidebar gradient", async ({ page }) => {
  await seedTheme(page, "dzzy");
  await installMockBridge(page);
  await openChannel(page);
  await expectDzzyGradientPaint(page, "light");
  await expectDzzySidebarPalette(page, "light");
  await expectDzzyContentShadow(page, "light");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/01-dzzy-light-sidebar.png` });
});

test("dzzy dark sidebar gradient", async ({ page }) => {
  await seedTheme(page, "dzzy-dark");
  await installMockBridge(page);
  await openChannel(page);
  await expectDzzyGradientPaint(page, "dark");
  await expectDzzySidebarPalette(page, "dark");
  await expectDzzyContentShadow(page, "dark");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await expect(page.locator("[data-dzzy-content-surface]")).toHaveCSS(
    "background-color",
    "rgb(26, 26, 26)",
  );
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/02-dzzy-dark-sidebar.png` });
});
...
test("appearance picker — system tab (Dzzy follows OS)", async ({ page }) => {
  await seedTheme(page, "dzzy");
  await installMockBridge(page);
...
test("appearance picker — light tab (Dzzy)", async ({ page }) => {
  await seedTheme(page, "dzzy");
  await installMockBridge(page);
...
test("appearance picker — dark tab (Dzzy Dark)", async ({ page }) => {
  await seedTheme(page, "dzzy-dark");
  await installMockBridge(page);
...
test("settings nav uses Dzzy active pill + hover (light)", async ({ page }) => {
  await seedTheme(page, "dzzy");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
  ...
  // Appearance is the active section here; its nav row should carry the Dzzy
  ...
  await expectDzzySettingsPalette(page, "light");
...
test("settings nav uses Dzzy active pill + hover (dark)", async ({ page }) => {
  await seedTheme(page, "dzzy-dark");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
  ...
  await expect(sidebar).toBeVisible({ timeout: 10_000 });
  await page.getByTestId("settings-nav-appearance").click();
  await expectDzzySettingsPalette(page, "dark");
...
test("appearance hides accent picker under Dzzy", async ({ page }) => {
  await seedTheme(page, "dzzy");
  await installMockBridge(page);
  const panel = await openAppearance(page, "light");
  // The accent picker is hidden while a Dzzy theme is active. Its neutral
...
test("accent picker reveals/hides when toggling Dzzy", async ({ page }) => {
  // Start on a non-Dzzy theme so the accent picker is present, then select the
  // Dzzy tile — the picker should animate out and unmount. Reselecting a
  // non-Dzzy tile brings it back. Asserts the presence toggle (the motion
  ...
  // Switch to Dzzy — picker should leave (allow the exit animation to settle).
  await page.getByTestId("theme-option-dzzy").click();
  await expect(page.getByTestId("accent-color-neutral")).toHaveCount(0);

  // Back to a non-Dzzy theme — picker returns.
  await page.getByTestId("theme-option-github-light").click();
  await expect(page.getByTestId("accent-color-neutral")).toBeVisible();
});

test("Dzzy light and dark modes apply live without a reload", async ({
  page,
}) => {
  await seedTheme(page, "dzzy");
  await installMockBridge(page);
  await openAppearance(page, "light");
  await expectAppliedDzzyTheme(page, "dzzy");
  const lightGradient = await expectDzzyGradientPaint(page, "light");

  await page.getByTestId("appearance-mode-dark").click();
  await expectAppliedDzzyTheme(page, "dzzy-dark");
  const darkGradient = await expectDzzyGradientPaint(page, "dark");
...
test("Dzzy follows native system theme changes without a reload", async ({
  page,
}) => {
  await seedTheme(page, "dzzy");
  await page.addInitScript(() => {
  ...
  await expectAppliedDzzyTheme(page, "dzzy-dark", "dzzy");
  await expectDzzyGradientPaint(page, "dark");

  await emitNativeThemeChange(page, "light");
  await expectAppliedDzzyTheme(page, "dzzy", "dzzy");
  await expectDzzyGradientPaint(page, "light");
});
