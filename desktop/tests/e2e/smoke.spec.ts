...
test("Dzzy shared compute explains automatic model selection", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => {
  ...
  await expect(
    page.getByText(
      "Dzzy will choose an available shared model when the agent starts.",
    ),
  ).toBeVisible();
  await expect(page.locator("#persona-custom-model")).toHaveCount(0);
});

test("create agent persists Dzzy shared compute with auto model", async ({
  page,
}) => {
...
test("create agent supports parallelism and system prompt overrides", async ({
  ...
  // The dzzy-agent runtime auto-selects once the ACP runtime catalog loads;
