...
async function setMockWebsocketSendsStalled(
  ...
  await page.evaluate((shouldStall) => {
    ...
    ).__DZZY_E2E_SET_STALL_WEBSOCKET_SENDS__;
    if (!setter) {
...
async function disconnectMockWebsockets(page: import("@playwright/test").Page) {
  const disconnected = await page.evaluate(() => {
    ...
    ).__DZZY_E2E_DISCONNECT_MOCK_WEBSOCKETS__;
    if (!disconnect) {
...
async function restartMockWebsockets(page: import("@playwright/test").Page) {
  const restarted = await page.evaluate(() => {
    ...
    ).__DZZY_E2E_RESTART_MOCK_WEBSOCKETS__;
    if (!restart)
...
async function driveConnectionDegraded(
  ...
  await page.evaluate((s) => {
    const setter = (
      window as Window & {
        __DZZY_E2E_SET_RELAY_CONNECTION_STATE__?: (state: string) => void;
      }
    ...
    .poll(
      () =>
        page.evaluate(() => {
          ...
          ).__DZZY_E2E_GET_RELAY_CONNECTION_STATE__;
...
test("service restart close resets accumulated backoff", async ({ page }) => {
  ...
  await expect
    .poll(
      () =>
        page.evaluate(() => window.__DZZY_E2E_GET_RELAY_CONNECTION_STATE__?.()),
      { timeout: 2_500 },
    )
    .toBe("connected");
