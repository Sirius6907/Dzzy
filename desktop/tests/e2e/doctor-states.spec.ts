...
/** dzzy-agent is always available and has no auth step. */
const DZZY_AGENT_AVAILABLE = {
  id: "dzzy-agent",
  label: "Dzzy Agent",
  avatar_url: "",
  availability: "available",
...
const CODEX_NOT_INSTALLED = {
  ...
  mcp_command: null,
  install_hint:
    "Dzzy requires the Codex CLI; the desktop app alone is not enough.",
...
test.describe("Doctor panel state screenshots", () => {
  ...
  test("00-runtime-card-layout", async ({ page }) => {
    ...
    for (const runtimeId of ["goose", "claude", "dzzy-agent"]) {
      await expect(
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              stdout: "",
              stderr:
                "The installer finished, but Dzzy still could not use codex (observed: NotInstalled).",
              exit_code: null,
              hint: "Dzzy requires the vendor CLI executable, not only its desktop app. If the CLI was installed while Dzzy was open, restart Dzzy and check again.",
            },
          ],
        },
      ],
  ...
  test("11-outdated-adapter-warning", async ({ page }) => {
    ...
    await expect(dialog).toContainText(
      "Older Dzzy releases using the legacy adapter may lose community access",
    );
