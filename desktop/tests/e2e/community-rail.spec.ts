...
async function seedCommunities(
  ...
  await page.addInitScript(
    ({ list, active }) => {
      window.localStorage.setItem("dzzy-communities", JSON.stringify(list));
      window.localStorage.setItem("dzzy-active-community-id", active);
    },
...
test.describe("community rail", () => {
  ...
  test("keeps profile community actions available to members without invite access", async ({
    ...
    await expect
      .poll(() =>
        page.evaluate(() => {
          return (window.__DZZY_E2E_COMMAND_LOG__ ?? []).findLast(
  ...
  test("enters a remembered channel before live validation completes", async ({
    ...
    const rememberedChannelId = await page.evaluate((communityId) => {
      const source = window.localStorage.getItem(
        "dzzy-channels.v1:ws://localhost:3000",
      );
      ...
      window.localStorage.setItem(
        "dzzy-channels.v1:ws://localhost:3001",
        source,
      );
      window.localStorage.setItem(
        "dzzy-community-destinations",
        JSON.stringify({
          [communityId]: {
            kind: "channel",
  ...
  test("clears a remembered channel that is unavailable after switching", async ({
    ...
    await expect
      .poll(() =>
        page.evaluate(() =>
          window.localStorage.getItem("dzzy-channels.v1:ws://localhost:3000"),
        ),
      )
      .not.toBeNull();
    await page.evaluate(() => {
  ...
  test("drag-to-reorder updates the stored community order and survives reload", async ({
    ...
    await page.addInitScript(
      ({ list, active }) => {
        if (!window.localStorage.getItem("dzzy-communities")) {
          window.localStorage.setItem("dzzy-communities", JSON.stringify(list));
        }
        if (!window.localStorage.getItem("dzzy-active-community-id")) {
          window.localStorage.setItem("dzzy-active-community-id", active);
        }
      },
