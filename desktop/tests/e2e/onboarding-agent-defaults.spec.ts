...
function runtime(
  id: "dzzy-agent" | "claude" | "codex" | "goose",
  availability: string,
...
test("setup shows only Claude Code and Codex as detected harnesses", async ({
  ...
  await installMockBridge(
    ...
    {
      acpRuntimesCatalog: [
        runtime("dzzy-agent", "available", { status: "not_applicable" }),
  ...
  await expect(page.getByTestId("onboarding-runtime-dzzy-agent")).toHaveCount(
    0,
  );
...
test("defaults renders only fields supported by the selected harness", async ({
  ...
  await installMockBridge(
    page,
    {
      ...
      globalAgentConfig: {
        env_vars: { DZZY_AGENT_THINKING_EFFORT: "high" },
        provider: null,
        model: "stale-model",
...
test("defaults requires a choice when multiple visible harnesses are ready", async ({
  ...
  await expect(
    page.getByTestId("global-agent-default-harness-option-dzzy-agent"),
  ).toHaveCount(0);
  await page.getByTestId("global-agent-default-harness-option-codex").click();
