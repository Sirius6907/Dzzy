...
//    Both DZZY_UPDATER_PUBLIC_KEY and DZZY_UPDATER_ENDPOINT are required -
...
const updaterPubkey = process.env.DZZY_UPDATER_PUBLIC_KEY;
const updaterEndpoint = process.env.DZZY_UPDATER_ENDPOINT;

const missing = [];
if (!updaterPubkey) missing.push("DZZY_UPDATER_PUBLIC_KEY");
if (!updaterEndpoint) missing.push("DZZY_UPDATER_ENDPOINT");
if (missing.length > 0) {
  console.error(
