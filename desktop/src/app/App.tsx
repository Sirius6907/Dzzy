...
import { createDzzyQueryClient } from "@/shared/api/queryClient";
...
import { DzzySpider } from "@/shared/ui/dzzy-logo/DzzySpider";
import { FuzzyLogo } from "@/shared/ui/dzzy-logo/FuzzyLogo";
...
// can take longer than that — without a hold, the spider is unmounted before it is
...
// actionability); a spec can opt back in via __DZZY_E2E__.bootSplashHoldMs.
function bootSplashHoldMs(): number {
  const e2e = (
    window as Window & {
      __DZZY_E2E__?: { bootSplashHoldMs?: number };
    }
  ).__DZZY_E2E__;
  if (e2e) {
...
// Animated Dzzy mark for the loading gates. The static DzzyMark renders in
...
function DzzyLoader({
  ...
  return (
    <div className={cn("relative", tintClassName, className)}>
      <DzzyMark className="block h-auto w-full" />
      <FuzzyLogo
        ariaLabel={ariaLabel}
...
// centered Dzzy spider flying over it — the same static mark as before, now with
// its wings flapping (ported from the Dzzy website's wing-flap). Replaces the
// old "Setting up your community" text, which stays as an sr-only caption.
function AppLoadingGate() {
  return (
    <div
      className="dzzy-setup-loading-shell flex min-h-dvh flex-col items-center justify-center overflow-hidden px-6 py-10"
    ...
    >
      ...
      <DzzySpider className="relative z-10 h-auto w-28" />
    </div>
  );
}
...
function CommunityQueryProvider({ children }: { children: ReactNode }) {
  const [queryClient] = useState(createDzzyQueryClient);

  useEffect(() => setAvatarProfileSyncQueryClient(queryClient), [queryClient]);

  useEffect(() => {
    const e2eWindow = window as Window & {
      __DZZY_E2E__?: unknown;
      __DZZY_E2E_QUERY_CLIENT__?: typeof queryClient;
    };
    if (!e2eWindow.__DZZY_E2E__) {
      return;
    }

    e2eWindow.__DZZY_E2E_QUERY_CLIENT__ = queryClient;
    return () => {
      if (e2eWindow.__DZZY_E2E_QUERY_CLIENT__ === queryClient) {
        delete e2eWindow.__DZZY_E2E_QUERY_CLIENT__;
      }
    };
  }, [queryClient]);
...
function CommunityApp({
  ...
  // overlay just keeps the spider on screen long enough to be seen, then fades.
