...
const DESKTOP_NOTIFICATION_ACTION_EVENT = "dzzy:desktop-notification-action";
...
type TestWindow = Window & {
  ...
  __DZZY_E2E_APP_BADGE_STATE__?: AppBadgeState["kind"];
};
    ...
    try {
      pluginListener = await onAction((notification) => {
        const target = parseNotificationTarget(
          notification.extra?.dzzyNotificationTarget,
        );
        if (!target) {
          return;
        }
...
export async function setDesktopAppBadge(state: AppBadgeState): Promise<void> {
  if (typeof window !== "undefined") {
    ...
    testWindow.__DZZY_E2E_APP_BADGE_COUNT__ =
      state.kind === "count" ? state.count : 0;
    testWindow.__DZZY_E2E_APP_BADGE_STATE__ = state.kind;
  }
