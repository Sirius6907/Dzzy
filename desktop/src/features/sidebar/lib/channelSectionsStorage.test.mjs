...
test("storageKey: returns expected format with pubkey", () => {
  assert.equal(storageKey("abc123"), "dzzy-channel-sections.v1:abc123");
});
...
test("storageKey: with relayUrl includes normalized+encoded relay in key", () => {
  ...
  assert.equal(
    key,
    `dzzy-channel-sections.v1:pk1:${encodeURIComponent(normalizeRelayUrl(relay))}`,
  );
});

test("storageKey: without relayUrl returns legacy pubkey-only key", () => {
  assert.equal(storageKey("pk1"), "dzzy-channel-sections.v1:pk1");
  assert.equal(storageKey("pk1", undefined), "dzzy-channel-sections.v1:pk1");
});
