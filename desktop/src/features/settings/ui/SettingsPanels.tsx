  ...
  return (
    ...
    >
      <SystemPreferencePreviewFrame
        ...
        darkGradient={darkName ? DZZY_GRADIENT_STOPS[darkName] : undefined}
        darkVars={darkVars}
        lightGradient={DZZY_GRADIENT_STOPS[lightName]}
        lightVars={lightVars}
      />
      <span
        className={cn(
  ...
  return (
    ...
    >
      <ThemePreviewFrame
        ...
        sidebarGradient={DZZY_GRADIENT_STOPS[name]}
        vars={vars}
      />
      <span
        className={cn(
...
// it enters from above (slides *down* into place when a non-Dzzy theme reveals
// it) and exits upward (slides up behind the grid when Dzzy hides it). No
...
function ThemeSettingsCard() {
  ...
  // Dzzy themes pin a neutral accent (GitHub black in light, white in dark),
  // so the accent picker is hidden while a Dzzy theme is active. `themeName` is
  // the effective theme, so this also covers System mode resolving to Dzzy.
  const accentPickerHidden = isDzzyTheme(themeName);
  ...
  return (
    ...
    >
      <SettingsSectionHeader
        title="Appearance"
        description="Choose a theme for Dzzy."
      />
      ...
      <div className="relative mb-6">
        ...
        {/* Bottom fade — hidden while the accent picker is visible so its
            near-white gradient (Dzzy light) can't mask the swatches below it
      ...
      {/* Accent color picker — hidden for Dzzy themes (pinned neutral accent).
