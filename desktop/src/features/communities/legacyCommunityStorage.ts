...
const DZZY_COMMUNITIES_KEY = "dzzy-communities";
const DZZY_ACTIVE_COMMUNITY_KEY = "dzzy-active-community-id";
const DZZY_ONBOARDING_COMPLETION_STORAGE_KEY_PREFIX =
  "dzzy-onboarding-complete.v1:";
...
export function applyLegacyCommunityStorage(
  ...
  const currentCommunitiesRaw = storage.getItem(DZZY_COMMUNITIES_KEY);
  ...
  if (shouldWriteCommunities && legacyStorage.workspaces) {
    storage.setItem(DZZY_COMMUNITIES_KEY, legacyStorage.workspaces);
  }

  const currentActiveCommunityId = storage.getItem(DZZY_ACTIVE_COMMUNITY_KEY);
  if (
    legacyStorage.activeWorkspaceId &&
    (!currentActiveCommunityId || shouldWriteCommunities)
  ) {
    storage.setItem(DZZY_ACTIVE_COMMUNITY_KEY, legacyStorage.activeWorkspaceId);
  }

  for (const completion of legacyStorage.onboardingCompletions) {
    const key = `${DZZY_ONBOARDING_COMPLETION_STORAGE_KEY_PREFIX}${completion.pubkey}`;
...
/**
 * Seed Dzzy localStorage from legacy Sprout WebKit localStorage before the app
 ...
 * Dzzy does not already have community state, except for the known broken
 * Sprout→Dzzy first-run handoff that created a single localhost community.
 */
export async function migrateLegacyCommunityStorageBeforeRender(): Promise<void> {
  ...
  const currentCommunitiesRaw =
    window.localStorage.getItem(DZZY_COMMUNITIES_KEY);
  const hasCurrentActiveCommunity = window.localStorage.getItem(
    DZZY_ACTIVE_COMMUNITY_KEY,
  );
  if (
    currentCommunitiesRaw &&
