...
test("applyLegacyCommunityStorage seeds missing communities and active community", () => {
  ...
  assert.equal(storage.getItem("dzzy-communities"), legacyCommunities);
  assert.equal(storage.getItem("dzzy-active-community-id"), "legacy-community");
});

test("applyLegacyCommunityStorage preserves existing non-local Dzzy communities", () => {
  const storage = createMemoryStorage({
    "dzzy-communities": currentCommunities,
    "dzzy-active-community-id": "current-community",
  });
  ...
  assert.equal(storage.getItem("dzzy-communities"), currentCommunities);
  assert.equal(
    storage.getItem("dzzy-active-community-id"),
    "current-community",
  );
});
...
test("applyLegacyCommunityStorage replaces broken localhost first-run community", () => {
  const storage = createMemoryStorage({
    "dzzy-communities": localhostCommunities,
    "dzzy-active-community-id": "local-community",
  });
...
test("applyLegacyCommunityStorage treats trailing-slash localhost as broken", () => {
  const storage = createMemoryStorage({
    "dzzy-communities": JSON.stringify([
      {
        id: "local-community",
        name: "Local Dev",
        relayUrl: "ws://localhost:3000/",
...
test("applyLegacyCommunityStorage migrates onboarding completion keys", () => {
  ...
  assert.equal(storage.getItem("dzzy-onboarding-complete.v1:abc123"), "true");
});
