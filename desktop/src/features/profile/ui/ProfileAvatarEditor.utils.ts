...
const EMOJI_MART_SHADOW_CSS = `
  ...
  #root {
    --padding: var(--dzzy-emoji-picker-padding, 16px);
    --sidebar-width: 0px;
    display: flex;
  ...
  #root::after {
    background: linear-gradient(
      to bottom,
      rgba(var(--dzzy-emoji-picker-rgb-background), 0),
      rgba(var(--dzzy-emoji-picker-rgb-background), 0.98)
    );
    bottom: calc(var(--dzzy-emoji-picker-nav-button-size, 40px) + 24px);
    content: "";
    height: var(--dzzy-emoji-picker-fade-height, 0px);
    left: 0;
    opacity: var(--dzzy-emoji-picker-fade-opacity, 0);
    pointer-events: none;
    position: absolute;
    right: 0;
    z-index: 3;
  ...
  .scroll {
    ...
    padding-top: var(--dzzy-emoji-picker-scroll-padding-top, 28px);
    width: 100%;
  }
  ...
  .category button[data-dzzy-selected="true"] .background {
  ...
  #nav {
    ...
    flex: 0 0 auto;
    justify-content: space-between;
    padding: 8px var(--dzzy-emoji-picker-nav-padding-x, 24px) 16px;
  }
  ...
  #nav button {
    ...
    display: flex;
    flex: 0 0 var(--dzzy-emoji-picker-nav-button-size, 40px);
    height: var(--dzzy-emoji-picker-nav-button-size, 40px);
    justify-content: center;
    transition:
      background-color var(--duration) var(--easing),
      color var(--duration) var(--easing),
      transform var(--duration) var(--easing);
    width: var(--dzzy-emoji-picker-nav-button-size, 40px);
  }
  ...
  #nav svg,
  #nav img {
    height: var(--dzzy-emoji-picker-category-icon-size, 24px);
    width: var(--dzzy-emoji-picker-category-icon-size, 24px);
  }
`;
...
export function useEmojiMartStyles(
  ...
  React.useEffect(() => {
    ...
    const installEmojiMartStyles = () => {
      ...
      if (!shadowRoot.querySelector("#dzzy-emoji-mart-style")) {
        const style = document.createElement("style");
        style.id = "dzzy-emoji-mart-style";
...
export function useEmojiMartThemeVars() {
  ...
  React.useEffect(() => {
    const updateThemeVars = () => {
      ...
      setThemeVars({
        "--dzzy-emoji-picker-rgb-background": muted ?? "54, 58, 79",
        "--dzzy-emoji-picker-rgb-color": foreground ?? "245, 247, 255",
        "--dzzy-emoji-picker-rgb-input": background ?? "47, 51, 68",
