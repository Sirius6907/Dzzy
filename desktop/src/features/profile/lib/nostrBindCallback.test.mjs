...
test("buildNostrBindCallbackUrl returns a UTF-8 base64url payload in the fragment", () => {
  const response = JSON.stringify({ content: "Dzzy ⚡", sig: "+/=" });
  const result = buildNostrBindCallbackUrl(
    "https://example.com/dzzy?source=bind",
    response,
  );
  const url = new URL(result);

  assert.equal(url.origin, "https://example.com");
  assert.equal(url.pathname, "/dzzy");
  assert.equal(url.search, "?source=bind");
  assert.match(url.hash, /^#dzzy_bind=v1\.[A-Za-z0-9_-]+$/);
  assert.equal(decodePayload(result), response);
  assert.equal(url.searchParams.has("dzzy_bind"), false);
});

test("buildNostrBindCallbackUrl replaces an existing fragment", () => {
  const result = buildNostrBindCallbackUrl(
    "https://example.com/dzzy#stale-fragment",
    "signed",
  );

  assert.equal(new URL(result).hash, "#dzzy_bind=v1.c2lnbmVk");
});

test("buildNostrBindCallbackUrl rejects callback URLs beyond the opener ceiling", () => {
  assert.throws(
    () =>
      buildNostrBindCallbackUrl("https://example.com/dzzy", "x".repeat(4_096)),
    /too large/,
  );
});
