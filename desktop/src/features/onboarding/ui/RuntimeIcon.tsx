...
function isDzzyRuntime(runtime: AcpRuntimeCatalogEntry): boolean {
  return runtime.id.trim().toLowerCase() === "dzzy-agent";
}

export function getRuntimeDisplayLabel(
  runtime: AcpRuntimeCatalogEntry,
): string {
  return isDzzyRuntime(runtime) ? "Dzzy" : runtime.label;
}
...
export function RuntimeIcon({
  ...
  if (isDzzyRuntime(runtime)) {
    return <DzzyMark className="h-7 w-10 text-foreground" />;
  }
