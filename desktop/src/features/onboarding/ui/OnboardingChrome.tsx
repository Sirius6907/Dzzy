 ...
 * light-blue label (`--dzzy-onboarding-cta-label`, available on any
 * `.dzzy-onboarding-neutral-theme` subtree).
 */
export const ONBOARDING_PRIMARY_CTA_CLASS = `${ONBOARDING_CTA_SHAPE} text-[var(--dzzy-onboarding-cta-label)]`;

/**
 * Primary-CTA styling for the landing screen only: the shared pill with the
 * chartreuse label (`--dzzy-welcome-chartreuse`). The blue label is reserved
 * for the CTAs inside the steps.
 */
export const ONBOARDING_LANDING_CTA_CLASS = `${ONBOARDING_CTA_SHAPE} text-[var(--dzzy-welcome-chartreuse)]`;

/**
 * Icon-control styling for onboarding surfaces that sit on the textured card:
 * olive backup ink (`--dzzy-onboarding-backup-ink`) with a plain
...
export const ONBOARDING_INK_ICON_CLASS =
  "text-[color:var(--dzzy-onboarding-backup-ink)] hover:bg-transparent hover:text-foreground";

/**
 * Shared onboarding chrome shown on every page after the landing screen: a
 * static Dzzy mark pinned to the top-left, and a centered pagination track that
  ...
  return (
    ...
    >
      <span className="block w-11" data-testid="onboarding-logo">
        <DzzyMark className="h-auto w-full" />
      </span>
      <div
