 ...
 * Provider path: the preferred Dzzy Agent or Goose runtime has provider and
...
export function resolveAgentReadiness(
  ...
  if (scope === "any") {
    for (const runtime of runtimes) {
      if (runtime.id === "dzzy-agent") continue;
      if (
  ...
  const preferredRuntime =
    scope === "preferred"
      ...
      : runtimes.find((runtime) => runtime.id === "dzzy-agent");
  ...
  if (preferredRuntime.id !== "dzzy-agent" && preferredRuntime.id !== "goose") {
    return { ready: false };
  }
  ...
  if (provider.length > 0 && model.length > 0) {
    ...
    if (allKeysPresent) {
      return { ready: true, reason: "dzzy-agent" };
    }
  }
