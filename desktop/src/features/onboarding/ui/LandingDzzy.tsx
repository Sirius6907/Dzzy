...
// Autonomous wander: each spider drifts on its own smooth loop.
const WANDER_X = 26;
const WANDER_Y = 20;

export function LandingDzzy() {
  const fieldRef = React.useRef<HTMLDivElement>(null);
  const spiderRefs = React.useRef<(HTMLSpanElement | null)[]>([]);
  const pointer = React.useRef<{ x: number; y: number } | null>(null);
  const offsets = React.useRef(SPIDERS.map(() => ({ x: 0, y: 0 })));
  ...
  React.useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      beeRefs.current.forEach((el, i) => {
        ...
        // Per-spider wander: two incommensurate sine waves, phase-shifted by index.
        const phase = i * 1.7;
        const wx =
        ...
        let ry = 0;
        if (p) {
          const cx = rect.left + (rect.width * parseFloat(spider.left)) / 100;
          const cy = rect.top + (rect.height * parseFloat(spider.top)) / 100;
          const ox = cx - p.x;
          const oy = cy - p.y;
        ...
        el.style.transform = `translate(${cur.x}px, ${cur.y}px) rotate(${spider.rotate}deg)`;
      });
