/**
 * Remark plugin that detects bare `dzzy://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `dzzy://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `dzzy://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](dzzy://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /(?:dzzy|dzzy):\/\/message\?[^\s<>"')\]]+/g;
