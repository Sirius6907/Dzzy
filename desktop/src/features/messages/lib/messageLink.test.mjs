...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects dzzy:// with wrong host", () => {
  const r = parseMessageLink(`dzzy://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`dzzy://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`dzzy://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy dzzy://message links", () => {
  const r = parseMessageLink(`dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("isMessageLink matches dzzy://message and legacy dzzy://message", () => {
  assert.equal(
    isMessageLink(`dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("dzzy://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("dzzy://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`;
