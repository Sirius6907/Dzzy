...
test("dzzy://message deep-link routes in-app, not the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink(`dzzy://message?channel=${CHANNEL}&id=${MESSAGE}`, handlers);
...
test("non-message dzzy:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("dzzy://channel?foo=bar", handlers);
  assert.deepEqual(external, ["dzzy://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed dzzy://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with dzzy://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("dzzy://message?nope=1", handlers);
  assert.deepEqual(external, ["dzzy://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
