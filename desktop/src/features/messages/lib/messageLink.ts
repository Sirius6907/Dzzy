/**
 * `dzzy://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `dzzy://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "dzzy:";
...
/**
 * Build a `dzzy://message` URL for a given channel + message.
 *
...
/**
 * Parse a `dzzy://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("dzzy://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return href.startsWith("dzzy://message?") || href === "dzzy://message";
}
 ...
 * CommonMark autolinks (`<dzzy://message?...>`) and explicitly labeled links
