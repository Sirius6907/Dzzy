...
export function registerCustomEmojiMarkdownIt(
  ...
  const RULE_NAME = "dzzy_custom_emoji";
  const TOKEN_TYPE = "dzzy_custom_emoji";
  ...
  const rule = (state: any, silent: boolean): boolean => {
    ...
    // word char. This keeps prose and URLs intact — `not:dzzy:` and
    // `http://x:y:dzzy:` must NOT turn the inner `:dzzy:` into an image;
