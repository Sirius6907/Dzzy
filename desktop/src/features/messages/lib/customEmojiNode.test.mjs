...
test("rule fires for a boundary :shortcode: (start of string)", () => {
  const rule = captureRule(["dzzy"]);
  const { matched, advanced } = runRule(rule, ":dzzy:", 0);
  assert.equal(matched, true);
  assert.equal(advanced, ":dzzy:".length);
});

test("rule fires for a :shortcode: preceded by whitespace", () => {
  const rule = captureRule(["dzzy"]);
  // pos points at the `:` after the space.
  const { matched } = runRule(rule, "hi :dzzy:", 3);
  assert.equal(matched, true);
});

test("rule does NOT fire when the colon is glued to a word char (not:dzzy:)", () => {
  const rule = captureRule(["dzzy"]);
  // pos points at the `:` immediately after `not`.
  const { matched } = runRule(rule, "not:dzzy:", 3);
  assert.equal(matched, false);
});

test("rule does NOT fire inside a URL-like sequence (http://x:y:dzzy:)", () => {
  const rule = captureRule(["dzzy"]);
  const src = "http://x:y:dzzy:";
  // pos points at the `:` immediately after `y` (a word char).
  const { matched } = runRule(rule, src, src.indexOf(":dzzy:"));
  assert.equal(matched, false);
});
...
test("rule fires after punctuation boundary (e.g. parenthesis)", () => {
  ...
  const { matched } = runRule(rule, "(:dzzy:)", 1);
  assert.equal(matched, true);
});
