...
function activityTitle(item) {
  return formatToolTitle(item.dzzyToolName ?? item.toolName, item.title);
}
...
test("buildTranscript renders Prompt context + user message for a multi-block session/prompt frame", () => {
  ...
  assert.deepEqual(
    ...
    ["Agent Memory — core", "Context", "Dzzy event: @mention"],
...
test("buildTranscript keeps read_file activity categorized by the actual tool when output names Dzzy tools", () => {
  const [item] = toolItems([
    ...
    acpToolUpdate(11, {
      ...
      content: {
        type: "text",
        text: 'const DZZY_READ_TOOLS = new Set(["get_feed", "get_event"]);\nconst DZZY_WRITE_TOOLS = new Set(["delete_message"]);',
      },
    }),
  ]);

  assert.equal(item.toolName, "read_file");
  assert.equal(item.dzzyToolName, null);
...
test("buildTranscript keeps shell activity categorized by the actual tool when grep output names Dzzy tools", () => {
  const [item] = toolItems([
    acpToolUpdate(20, {
      sessionUpdate: "tool_call",
...
test("buildTranscript categorizes explicit Dzzy tool calls for the activity bar", () => {
  const [item] = toolItems([
    acpToolUpdate(30, {
      sessionUpdate: "tool_call",
  ...
  assert.equal(item.dzzyToolName, "get_feed");
  ...
  const transcript = buildTranscript([
    {
      ...
      payload: {
        ...
        params: {
          toolCallId: "tool-1",
          title: "Confirm force-with-lease push to block/dzzy.",
          options: [
...
test("buildTranscript five-section system prompt card is standalone with all sections; CheckCheck context contains only Dzzy/thread context", () => {
  ...
  //       the session/prompt:context sections (Dzzy event + Thread context),
      ...
      payload: {
        ...
        params: {
          systemPrompt: [
            ...
            "Fetch current content with: dzzy canvas get --channel 44444444-4444-4444-4444-444444444444",
          ].join("\n"),
        },
      },
    },
  ...
  // the session/prompt:context item — Dzzy/thread context only, no system-prompt sections.
  ...
  // Must have Dzzy event and Thread context sections, NOT Base/System/Team Instructions/Core Memory/Channel Canvas.
  assert.ok(
    contextSectionTitles.some((t) => t.toLowerCase().includes("dzzy")),
    "prompt context must contain a Dzzy event section",
  );
  assert.ok(
