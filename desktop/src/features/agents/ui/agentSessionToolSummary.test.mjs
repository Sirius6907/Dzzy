...
test("buildCompactToolSummary formats Dzzy send_message preview", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "send_message",
      dzzyToolName: "send_message",
      title: "Send Message",
...
test("buildCompactToolSummary treats dzzy messages send commands as messages", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "dzzy-dev-mcp__shell",
      args: {
        command:
          'dzzy --format compact messages send --channel channel-1 --content "@Ned are you working"',
      },
    }),
  );
...
test("buildCompactToolSummary returns null preview for piped stdin sends", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      ...
      args: {
        command:
          'echo "hello from stdin" | ./target/release/dzzy messages send --channel channel-1 --content -',
      },
    }),
  );
...
test("buildCompactToolSummary formats view_image thumbnail source", () => {
  ...
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "dzzy-dev-mcp__view_image",
      args: { source },
    }),
  );
...
test("buildCompactToolSummary promotes non-send dzzy CLI commands to relay ops", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: {
        command: "dzzy channels get --channel channel-1",
      },
    }),
  );
  ...
  assert.equal(summary.presentation, "inline");
  assert.equal(summary.shellContent, "dzzy channels get --channel channel-1");
});

test("buildCompactToolSummary exposes shellContent for shell-sourced dzzy CLI reads", () => {
  const command =
    "sleep 45; dzzy messages thread --channel channel-uuid --event abc | tail -n 20";
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: { command },
...
test("buildCompactToolSummary derives structured actions for native Dzzy MCP tools", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "get_channel",
      dzzyToolName: "get_channel",
      args: {
