...
import { DzzyAgentModelTuningFields } from "./dzzyAgentModelTuningFields";
import { isDzzyAgentRuntime } from "./dzzyAgentConfig";
...
export function EditAgentAdvancedFields({
  ...
  /** Active LLM model — forwarded to DzzyAgentModelTuningFields for effort filtering. */
  model?: string;
  /**
   * The actual/prospective runtime id used to decide whether to show the
   * dzzy-agent model-tuning fields. Uses `prospectiveRuntimeId` from
   * EditAgentDialog — the resolved runtime, not the "inherit"/"custom" sentinel.
   */
  modelTuningRuntimeId: string;
  parallelism: string;
  /** Active LLM provider id — forwarded to DzzyAgentModelTuningFields for effort filtering. */
  provider?: string;
  ...
  return (
    <div className="space-y-5 pt-2">
      ...
      {/* Tier-1 dzzy-agent model-tuning knobs — only shown for dzzy-agent. */}
      {isDzzyAgentRuntime(modelTuningRuntimeId) ? (
        <DzzyAgentModelTuningFields
          envVars={envVars}
