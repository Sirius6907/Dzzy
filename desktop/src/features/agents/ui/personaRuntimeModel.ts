 ...
 * persona runtime string (e.g. `dzzy-agent`). A catalog lookup then returns
 ...
 *
 * - dzzy-agent / goose → "capable" (`isProviderCapable`, id-based).
 ...
 * persona (e.g. dzzy-agent/Anthropic). Persisting the local (empty) provider +
 ...
 *
 * MODEL follows the same transition rule. dzzy-agent/goose readiness requires a
