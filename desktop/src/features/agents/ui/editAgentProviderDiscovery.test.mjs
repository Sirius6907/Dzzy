...
test("editAgent_providerFieldVisible_forDzzyAgent", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("dzzy-agent"),
    true,
    "dzzy-agent runtime must expose the provider picker",
  );
});
...
test("editAgent_providerOptions_includesDatabricksV2AndV1OnOSS", () => {
  ...
  const options = getPersonaProviderOptions("", "dzzy-agent");
...
test("editAgent_providerOptions_includesCurrentIfCustom", () => {
  const options = getPersonaProviderOptions("my-custom-llm", "dzzy-agent");
  const ids = options.map((o) => o.id);
  assert.ok(
...
//
// Switching to dzzy-agent runtime (which supports LLM provider selection)
// must make the provider field visible, enabling live discovery.

test("editAgent_runtimeSwitch_toDzzyAgentEnablesProvider", () => {
  // Simulate: user switches from "claude" to "dzzy-agent"
  const previousRuntime = "claude";
  const nextRuntime = "dzzy-agent";
...
test("editAgent_modelClearedOnRuntimeChange", () => {
  const previousRuntime = "dzzy-agent";
...
test("editAgent_catalogArrival_rederivesRuntimeIdWhenNotTouched", () => {
  ...
  const agentCommand = "/usr/local/bin/dzzy-agent";
  const catalog = [
    { id: "dzzy-agent", command: agentCommand, defaultArgs: [] },
...
test("editAgent_noOpSavePreservesProvider_whenCatalogLate", () => {
  ...
  // If the catalog-arrival effect correctly sets selectedRuntimeId = "dzzy-agent",
  // then llmProviderFieldVisible = true and the provider is preserved on save.
  const selectedRuntimeId = "dzzy-agent"; // correctly derived after catalog arrival
  const savedProvider = "databricks_v2";
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible once catalog derives dzzy-agent runtime",
  );
  assert.equal(
    providerUpdate,
    undefined,
...
test("editAgent_runtimeDropdown_pinsHarnessWhenConcreteCatalogRuntimeSelected", () => {
  ...
  const _nextRuntimeId = "dzzy-agent";
  const catalogRuntime = {
    id: "dzzy-agent",
    command: "/usr/local/bin/dzzy-agent",
    defaultArgs: [],
  };
...
test("editAgent_runtimeDropdown_keepsInheritWhenCatalogEntryHasNoCommand", () => {
  ...
  const NO_RUNTIME_DROPDOWN_VALUE = "__none__";
  const nextValue = "dzzy-agent"; // catalog entry, but adapter missing
  const nextRuntimeId =
    nextValue === NO_RUNTIME_DROPDOWN_VALUE ? "" : nextValue;
  const resolvedRuntimeId = nextRuntimeId || "custom";
  const nextRuntime = { id: "dzzy-agent", command: null, defaultArgs: [] };
...
test("editAgent_missingRequiredEnvKey_blocksSaveViaValidity", () => {
  ...
  const base = {
    ...
    selectedRuntimeId: "dzzy-agent",
    inheritHarness: false,
    agentCommand: "dzzy-agent",
...
test("editAgent_inheritedAgentRuntimeSwitch_producesConsistentCommandProviderPair", () => {
  ...
  //
  // After fix: selecting dzzy-agent sets inheritHarness=false, so agentCommandUpdate
  // resolves to the dzzy-agent command, and provider persists consistently.

  // Initial state: inherited Claude agent
  const inheritHarness = false; // after fix: pinned by runtime switch
  const selectedRuntimeCommand = "/usr/local/bin/dzzy-agent";
  ...
  const selectedRuntimeId = "dzzy-agent";
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
...
test("editAgent_inheritCheckboxRoundTrip_doesNotPersistProviderOnInheritedRuntime", () => {
  ...
  // → user picks dzzy-agent (inheritHarness→false, selectedRuntimeId='dzzy-agent')
  // → user picks databricks_v2
  // → user RE-CHECKS inherit (inheritHarness→true, selectedRuntimeId STAYS 'dzzy-agent')
  ...
  const runtimes = [
    { id: "claude", command: "/usr/local/bin/claude", defaultArgs: [] },
    { id: "dzzy-agent", command: "/usr/local/bin/dzzy-agent", defaultArgs: [] },
  ];
  const selectedRuntimeId = "dzzy-agent"; // dropdown state (stale after re-check)
  ...
  const chosenProvider = "databricks_v2"; // chosen while dropdown was dzzy-agent

  // llmProviderFieldVisible is driven by the live dropdown (dzzy-agent → true).
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field is visible (dropdown shows dzzy-agent) — this is the UX state",
  );
...
test("editAgent_inheritCheckboxRoundTrip_clearsStaleSavedProviderWhenRevertingToInherit", () => {
  // Variant: agent previously had a provider saved (e.g. was pinned to dzzy-agent
  ...
  const selectedRuntimeId = "dzzy-agent"; // dropdown state
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
// persona's runtime is dzzy-agent/Goose legitimately carries a provider
...
test("editAgent_inheritedDzzyAgentProvider_preservedOnNameOnlySave", () => {
  // Inherited dzzy-agent persona with databricks_v2 snapshot.
  // User makes a name-only edit (never touches runtime or provider).
  // The catalog-arrival effect correctly derived selectedRuntimeId="dzzy-agent".

  const inheritHarness = true; // agentCommandOverride == null → inheriting
  const agentCommand = "/usr/local/bin/dzzy-agent"; // inherited dzzy-agent command
  const runtimes = [
    { id: "dzzy-agent", command: "/usr/local/bin/dzzy-agent", defaultArgs: [] },
    { id: "claude", command: "/usr/local/bin/claude", defaultArgs: [] },
  ];
  const selectedRuntimeId = "dzzy-agent"; // correctly derived by catalog-arrival effect
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
  const savedProvider = "databricks_v2"; // valid provider snapshot
  const currentProvider = "databricks_v2"; // unchanged by user

  // llmProviderFieldVisible (UX) is true since dropdown shows dzzy-agent.
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible for inherited dzzy-agent",
  );

  // The effective runtime for submit: inherited → match agentCommand in catalog → dzzy-agent.
  const llmProviderCanPersistAtSubmit = computeCanPersistAtSubmit({
    inheritHarness,
    agentCommand,
    runtimes,
  ...
  assert.equal(
    llmProviderCanPersistAtSubmit,
    true,
    "inherited dzzy-agent runtime IS provider-capable — provider must be persistable",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited dzzy-agent must omit provider (not send null to clear it)",
  );
});

test("editAgent_inheritedDzzyAgentProvider_clearsWhenUserSwitchesToInheritedClaude", () => {
  // An agent inheriting dzzy-agent with databricks_v2, but the persona was
...
test("editAgent_findingE_emptyCatalog_providerOmittedNotCleared", () => {
  ...
  // Inherited dzzy-agent agent with saved databricks_v2.
  // A name-only save must omit provider (not send null).

  const inheritHarness = true;
  const agentCommand = "dzzy-agent"; // inherited command (short form)
...
test("editAgent_findingE_commandNullCatalogEntry_providerPreservedByIdMatch", () => {
  // Form 2: catalog loaded, but dzzy-agent's entry has command:null (adapter
  ...
  const inheritHarness = true;
  const agentCommand = "dzzy-agent"; // inherited command (short form = runtime id)
  const runtimes = [
    { id: "dzzy-agent", command: null, defaultArgs: [] }, // adapter missing
  ...
  assert.equal(
    capability,
    "capable",
    "command:null dzzy-agent entry must resolve to 'capable' via id-based fallback",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited dzzy-agent (command:null) must omit provider, not clear it",
  );
});
...
test("editAgent_findingE_lockedRuntimeStillClears", () => {
  ...
  const runtimes = [
    { id: "dzzy-agent", command: "dzzy-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  const selectedRuntimeId = "dzzy-agent"; // stale dropdown state (irrelevant)
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
test("editAgent_findingE_capableDzzyAgentLoadedCatalog_preservedOnNoOpSave", () => {
  // Confirm loaded-catalog inherited dzzy-agent still preserves provider.
  // This is Finding D's good path — must not regress with the tri-state change.

  const inheritHarness = true;
  const agentCommand = "dzzy-agent";
  const runtimes = [
    { id: "dzzy-agent", command: "dzzy-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  ...
  assert.equal(capability, "capable", "loaded dzzy-agent must be 'capable'");
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "no-op save on inherited dzzy-agent (loaded catalog) must omit provider",
  );
});
...
//
// Problem: dzzy-agent stores agentCommand="dzzy-agent" (short name) while the
// catalog entry has command="/Applications/Dzzy.app/.../dzzy-agent" (resolved path).
...
test("editAgent_bugA_inheritedShortName_resolvesViaIdFallback", () => {
  // The core regression: agentCommand is the short name "dzzy-agent" but the
  // catalog's command is the resolved binary path. Command match fails; id
  // fallback must rescue it.

  const agentCommand = "dzzy-agent"; // short name stored by effective_agent_command
  const runtimes = [
    {
      id: "dzzy-agent",
      command: "/Applications/Dzzy.app/Contents/MacOS/dzzy-agent", // resolved path
  ...
  assert.equal(
    selectedRuntimeId,
    "dzzy-agent",
    "short-name agentCommand must resolve to dzzy-agent id via id-fallback when command path differs",
  );
});
...
test("editAgent_bugA_inheritedShortName_commandMatchStillWinsWhenPresent", () => {
  ...
  const agentCommand = "/usr/local/bin/dzzy-agent"; // full path (explicit pin)
  const runtimes = [
    {
      id: "dzzy-agent",
      command: "/usr/local/bin/dzzy-agent",
...
test("editAgent_bugA_inheritedShortName_discoveryGatePasses", () => {
  // Once selectedRuntimeId is correctly resolved to "dzzy-agent" via id-fallback,
  ...
  const agentCommand = "dzzy-agent"; // short name
  const runtimes = [
    {
      id: "dzzy-agent",
      command: "/Applications/Dzzy.app/Contents/MacOS/dzzy-agent",
      availability: "available",
      defaultArgs: [],
    },
  ];
  ...
  assert.equal(
    selectedRuntimeId,
    "dzzy-agent",
    "id-fallback must resolve selectedRuntimeId to dzzy-agent",
  );
  assert.ok(
  ...
  assert.equal(
    ...
    "/Applications/Dzzy.app/Contents/MacOS/dzzy-agent",
    "discoveryAgentCommand must be the resolved path from the catalog entry",
  );
  assert.equal(
    canDiscoverModelOptions,
    true,
    "discovery gate must pass for inherited dzzy-agent with databricks_v2 provider once id-fallback resolves the runtime",
  );
});
...
test("requiredCredentialEnvKeys: dzzy-agent + anthropic → ANTHROPIC_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("dzzy-agent", "anthropic");
  assert.deepEqual(keys, ["ANTHROPIC_API_KEY"]);
});

test("requiredCredentialEnvKeys: dzzy-agent + openai → OPENAI_COMPAT_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("dzzy-agent", "openai");
  assert.deepEqual(keys, ["OPENAI_COMPAT_API_KEY"]);
});

test("requiredCredentialEnvKeys: dzzy-agent + databricks → DATABRICKS_HOST only (no token)", () => {
  const keys = requiredCredentialEnvKeys("dzzy-agent", "databricks");
...
test("requiredCredentialEnvKeys: dzzy-agent + databricks_v2 → DATABRICKS_HOST only", () => {
  const keys = requiredCredentialEnvKeys("dzzy-agent", "databricks_v2");
  assert.deepEqual(keys, ["DATABRICKS_HOST"]);
});
...
test("requiredCredentialEnvKeys: dzzy-agent + no provider → empty (provider not yet selected)", () => {
  const keys = requiredCredentialEnvKeys("dzzy-agent", "");
...
test("blockSave_dzzyAgentAnthropicMissingKey_blocked", () => {
  // Will's exact case: dzzy-agent / anthropic / opus / no ANTHROPIC_API_KEY
  const requiredKeys = requiredCredentialEnvKeys("dzzy-agent", "anthropic");
...
test("blockSave_dzzyAgentAnthropicKeyProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("dzzy-agent", "anthropic");
...
test("blockSave_dzzyAgentAnthropicEmptyStringKey_blocked", () => {
...
test("blockSave_dzzyAgentDatabricksMissingHost_blocked", () => {
  const requiredKeys = requiredCredentialEnvKeys("dzzy-agent", "databricks");
  const envVars = {};
  assert.equal(
    hasRequiredEnvKeyMissing(requiredKeys, envVars),
    true,
...
test("blockSave_dzzyAgentDatabricksHostProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("dzzy-agent", "databricks");
...
//   FALSE-ALLOW: claude pin → inherit dzzy-agent persona → missing ANTHROPIC_API_KEY
//     → must be BLOCKED (prospective runtime is dzzy-agent/anthropic, key absent)
//   FALSE-BLOCK: dzzy-agent pin → inherit claude persona
//     → must NOT be blocked (claude has no dialog-fixable credential requirement)

test("blockSave_inheritTransition_claudePin_toDzzyAgentPersona_missingKey_blocked", () => {
  ...
  // from persona" where the persona uses dzzy-agent/anthropic.
  // prospectiveRuntimeId resolves to "dzzy-agent"; providerForRequiredKeys must
  // use the PROSPECTIVE runtime's provider-field visibility (dzzy-agent supports
  // provider selection) rather than the current locked runtime's suppression.
  const prospectiveRuntimeId = "dzzy-agent"; // resolved from persona's agentCommand
  ...
  // providerForRequiredKeys must be "anthropic" (dzzy-agent supports selection)
  ...
  assert.equal(
    missing,
    true,
    "inheriting dzzy-agent/anthropic persona with no key must BLOCK save (false-allow prevented)",
  );
});

test("blockSave_inheritTransition_dzzyAgentPin_toClaudePersona_notBlocked", () => {
  // Scenario: agent is pinned to dzzy-agent/anthropic. The user checks
