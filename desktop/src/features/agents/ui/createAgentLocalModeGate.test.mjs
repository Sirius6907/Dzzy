...
test("localMode_dzzyAgent_supportsProviderSelection", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("dzzy-agent"),
    true,
    "dzzy-agent must support LLM provider selection",
  );
});
...
test("localMode_dzzyAgent_emptyProvider_notSatisfied", () => {
  // Scenario: user selects dzzy-agent but leaves provider empty.
  // Rust readiness requires DZZY_AGENT_PROVIDER — empty = NotReady.
...
test("localMode_dzzyAgent_emptyModel_notSatisfied", () => {
  // Scenario: dzzy-agent + anthropic + API key present, but model left empty.
  // Rust readiness requires DZZY_AGENT_MODEL — empty = NotReady.
...
// ── Gate: dzzy-agent / anthropic with missing key → markers shown ─────────

test("localMode_dzzyAgent_anthropic_missingKey_notSatisfied", () => {
  // Scenario: user selects dzzy-agent/anthropic + fills model, but hasn't
...
test("localMode_dzzyAgent_anthropic_allRequired_present_allowed", () => {
...
test("localMode_requiredEnvKeys_surfaces_anthropicKey", () => {
  ...
  const allKeys = requiredCredentialEnvKeys("dzzy-agent", "anthropic");
  assert.ok(
    allKeys.includes("ANTHROPIC_API_KEY"),
    "requiredCredentialEnvKeys must include ANTHROPIC_API_KEY for dzzy-agent/anthropic",
  );
});
...
test("localMode_requiredEnvKeys_gate_and_envVarsEditor_share_same_key_set", () => {
  ...
  const fullKeys = requiredCredentialEnvKeys("dzzy-agent", "anthropic");
...
test("localMode_goose_providerSatisfiedByFileConfig_noNormalizedFieldRequired", () => {
  // Scenario: goose, no provider in Dzzy env but file config has provider + model.
...
test("providerDefaultLabel_noGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("dzzy-agent", undefined);
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_emptyGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("dzzy-agent", "");
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_globalSet_returnsInheritLabel", () => {
  ...
  const label = getDefaultLlmProviderLabel("dzzy-agent", "anthropic");
  assert.equal(
    label,
...
test("providerDefaultLabel_globalSetWithWhitespace_trimsAndReturnsInherit", () => {
  ...
  const label = getDefaultLlmProviderLabel("dzzy-agent", "  openai  ");
  assert.equal(
    label,
...
test("providerDefaultLabel_sharedCompute_neverLeaksInternalId", () => {
  assert.equal(
    getDefaultLlmProviderLabel("dzzy-agent", "relay-mesh"),
    "Use agent defaults (Dzzy shared compute)",
  );
});
...
test("bakedDefaults_emptyGlobal_usesBuildValuesForCreateAndEditLabels", () => {
  const bakedEnv = [
    { key: "DZZY_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    {
      key: "DZZY_AGENT_MODEL",
      value: "goose-claude-opus-4-8",
      masked: false,
    },
    { key: "DZZY_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];
  ...
  const model = resolveInheritedDefault(null, bakedEnv, "DZZY_AGENT_MODEL");
  const effort = resolveInheritedDefault(
    null,
    bakedEnv,
    "DZZY_AGENT_THINKING_EFFORT",
  );
  const providerOptions = getPersonaProviderOptions("", "dzzy-agent");
...
test("bakedDefaults_explicitGlobalsOverrideBuildValues", () => {
  const bakedEnv = [
    { key: "DZZY_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    { key: "DZZY_AGENT_MODEL", value: "build-model", masked: false },
    { key: "DZZY_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];

  assert.deepEqual(
    resolveInheritedDefault("anthropic", bakedEnv, "DZZY_AGENT_PROVIDER"),
    { source: "global", value: "anthropic" },
  );
  assert.deepEqual(
    resolveInheritedDefault("global-model", bakedEnv, "DZZY_AGENT_MODEL"),
    { source: "global", value: "global-model" },
  );
  assert.deepEqual(
    resolveInheritedDefault("low", bakedEnv, "DZZY_AGENT_THINKING_EFFORT"),
...
test("f3b_buildTemplateModelDropdownOptions_anthropicGlobalModelSet_containsInheritOption", () => {
  ...
  const staticOptions = getPersonaModelOptions("dzzy-agent", "anthropic");
...
test("f3b_buildTemplateModelDropdownOptions_blankProviderGlobalModelSet_noDoubleSeed", () => {
  ...
  const staticOptions = getPersonaModelOptions("dzzy-agent", "");
...
// include ANTHROPIC_API_KEY for explicit dzzy-agent/anthropic so that the
...
test("providerConfig_explicitAnthropic_requiredKeysIncludesApiKey", () => {
  ...
  const required = requiredCredentialEnvKeys("dzzy-agent", "anthropic");
  assert.ok(
    required.includes("ANTHROPIC_API_KEY"),
    "dzzy-agent + explicit anthropic must list ANTHROPIC_API_KEY as required",
  );
});
...
test("providerConfig_inheritThenExplicitAnthropic_sameRequiredKeys", () => {
  ...
  const inheritKeys = requiredCredentialEnvKeys("dzzy-agent", "");
  const explicitKeys = requiredCredentialEnvKeys("dzzy-agent", "anthropic");
  assert.ok(
...
test("providerConfig_databricks_requiredKeyIsHost_noSecretClear", () => {
  ...
  const databricksKeys = requiredCredentialEnvKeys("dzzy-agent", "databricks");
  assert.ok(
    databricksKeys.includes("DATABRICKS_HOST"),
    "dzzy-agent + databricks must list DATABRICKS_HOST as required",
  );
