...
function upsertTool(
  ...
  const existing = d.itemsById.get(id);
  const canonicalDzzyToolName =
    dzzyToolName ?? findDzzyToolName(toolName, true);
  if (existing?.type === "tool") {
    const updatedTitle = !isGenericToolTitle(title) ? title : existing.title;
    let updatedToolName = existing.toolName;
    let updatedDzzyToolName = existing.dzzyToolName;
    if (canonicalDzzyToolName) {
      updatedDzzyToolName = canonicalDzzyToolName;
      updatedToolName = canonicalDzzyToolName;
    } else if (!existing.dzzyToolName && !isGenericToolTitle(toolName)) {
    ...
    const descriptor = classifyTool({
      ...
      dzzyToolName: updatedDzzyToolName,
      args: updatedArgs,
  ...
  const resolvedToolName = canonicalDzzyToolName ?? toolName;
  const descriptor = classifyTool({
    title,
