...
} from "@/features/agents/ui/dzzyAgentConfig";
import {
  EffortSelectField,
  useEffortAutoClear,
} from "@/features/agents/ui/dzzyAgentModelTuningFields";
...
export function AgentConfigFields({
  ...
  const bakedProvider = React.useMemo(
    () => bakedEnv.find((e) => e.key === "DZZY_AGENT_PROVIDER")?.value ?? null,
    [bakedEnv],
  );
  ...
  const bakedEffort = React.useMemo(
    () =>
      bakedEnv.find((e) => e.key === DZZY_AGENT_THINKING_EFFORT)?.value ?? null,
    [bakedEnv],
  );
                  ...
                  <EnvVarsEditor
                    ...
                    value={Object.fromEntries(
                      Object.entries(config.env_vars).filter(
                        ([k]) => k !== DZZY_AGENT_THINKING_EFFORT,
                      ),
                    )}
                  />
                </motion.div>
