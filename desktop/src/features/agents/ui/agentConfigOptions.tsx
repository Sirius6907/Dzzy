...
import { DZZY_AGENT_THINKING_EFFORT } from "./dzzyAgentConfig";
 ...
 *   secretEnvVar.
 *
 * Mirrors the Rust `readiness::dzzy_agent_requirements` /
...
export const PERSONA_LLM_PROVIDER_OPTIONS: readonly PersonaModelOption[] = [
  ...
  { id: "relay-mesh", label: "Dzzy shared compute" },
...
> = {
  ...
  "dzzy-agent": [DEFAULT_MODEL_OPTION],
 ...
 *
 * dzzy-agent and goose use provider-specific credentials; claude and codex
...
export function requiredCredentialEnvKeys(
  ...
  if (normalizedRuntime !== "dzzy-agent" && normalizedRuntime !== "goose") {
    return [];
  }
...
export function runtimeSupportsLlmProviderSelection(runtimeId: string) {
  return runtimeId === "dzzy-agent" || runtimeId === "goose";
}
...
export function resetConfigForHarnessChange(
  ...
  delete nextEnvVars[DZZY_AGENT_THINKING_EFFORT];
 ...
 *
 * Two classes of required field for provider-selection runtimes (dzzy-agent,
...
export function computeLocalModeGate({
  ...
  /** Env keys that are not set in Dzzy but are satisfied in the runtime's
