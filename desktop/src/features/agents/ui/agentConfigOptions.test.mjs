...
test("getPersonaProviderOptions returns databricks v1 and v2 when hideProviderIds is empty", () => {
  const options = getPersonaProviderOptions("", "dzzy-agent", "", new Set());
...
test("getPersonaProviderOptions with no hideProviderIds omits the tail for a known provider", () => {
  const options = getPersonaProviderOptions("anthropic", "dzzy-agent");
  const tail = options.at(-1);
...
test("getDefaultPersonaRuntime ignores an unavailable global preference", () => {
  ...
  assert.equal(getDefaultPersonaRuntime(runtimes, "claude")?.id, "dzzy-agent");
});

test("getDefaultPersonaRuntime returns dzzy-agent over goose when both are available", () => {
  const runtimes = [
    makeRuntime("goose"),
    makeRuntime("dzzy-agent"),
    makeRuntime("claude"),
  ...
  assert.equal(result?.id, "dzzy-agent");
});

test("getDefaultPersonaRuntime falls back to goose when dzzy-agent is unavailable", () => {
  const runtimes = [
    makeRuntime("dzzy-agent", "not_installed"),
    makeRuntime("goose"),
  ];
  const result = getDefaultPersonaRuntime(runtimes);
...
test("getDefaultPersonaRuntime returns first available when neither dzzy-agent nor goose is available", () => {
  const runtimes = [
    makeRuntime("dzzy-agent", "adapter_missing"),
...
test("runtimeSupportsLlmProviderSelection is true for dzzy-agent and goose", () => {
  assert.equal(runtimeSupportsLlmProviderSelection("dzzy-agent"), true);
...
test("resetConfigForHarnessChange clears harness-specific values", () => {
  const config = {
    env_vars: { DZZY_AGENT_THINKING_EFFORT: "high", KEEP_ME: "yes" },
    model: "claude-opus",
    preferred_runtime: "dzzy-agent",
    provider: "anthropic",
  };
...
test("getPersonaModelOptions for dzzy-agent with anthropic filters out zero-value default", () => {
  // anthropic requires explicit model — zero-value option is filtered out
  const options = getPersonaModelOptions("dzzy-agent", "anthropic");
...
test("getPersonaModelOptions for dzzy-agent with no provider returns default model", () => {
  const options = getPersonaModelOptions("dzzy-agent", "");
