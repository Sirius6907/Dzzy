...
import {
  ...
  DZZY_AGENT_MAX_OUTPUT_TOKENS,
  DZZY_AGENT_MAX_ROUNDS,
  DZZY_AGENT_THINKING_EFFORT,
  DZZY_AGENT_THINKING_EFFORT_VALUES,
  getProviderEffortConfig,
  isDzzyAgentRuntime,
} from "./dzzyAgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("DZZY_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...DZZY_AGENT_THINKING_EFFORT_VALUES],
...
test("DZZY_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(DZZY_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, DZZY_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected DZZY_AGENT_* names", () => {
  assert.equal(DZZY_AGENT_THINKING_EFFORT, "DZZY_AGENT_THINKING_EFFORT");
  assert.equal(DZZY_AGENT_MAX_OUTPUT_TOKENS, "DZZY_AGENT_MAX_OUTPUT_TOKENS");
  assert.equal(DZZY_AGENT_MAX_CONTEXT_TOKENS, "DZZY_AGENT_MAX_CONTEXT_TOKENS");
  assert.equal(DZZY_AGENT_MAX_ROUNDS, "DZZY_AGENT_MAX_ROUNDS");
});
...
test("isDzzyAgentRuntime returns true only for dzzy-agent id", () => {
  assert.equal(isDzzyAgentRuntime("dzzy-agent"), true);
});

test("isDzzyAgentRuntime returns false for other runtimes", () => {
  assert.equal(isDzzyAgentRuntime("goose"), false);
  assert.equal(isDzzyAgentRuntime("custom"), false);
  assert.equal(isDzzyAgentRuntime(""), false);
  assert.equal(isDzzyAgentRuntime("dzzy-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, DZZY_AGENT_THINKING_EFFORT, "high");
  assert.equal(result[DZZY_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [DZZY_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, DZZY_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, DZZY_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact DZZY_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  ...
  assert.equal(result[DZZY_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [DZZY_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, DZZY_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, DZZY_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact DZZY_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    DZZY_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[DZZY_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [DZZY_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(initial, DZZY_AGENT_MAX_CONTEXT_TOKENS, "");
  assert.equal(Object.hasOwn(result, DZZY_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact DZZY_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, DZZY_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[DZZY_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [DZZY_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, DZZY_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, DZZY_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [DZZY_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, DZZY_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[DZZY_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[DZZY_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      DZZY_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(DZZY_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[DZZY_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isDzzyAgent` derivation in CreateAgentRuntimeFields.
...
test("isDzzyAgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves dzzy-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is dzzy-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "dzzy-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "dzzy-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isDzzyAgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isDzzyAgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'dzzy-agent' MUST trigger model-tuning fields",
  );
});

test("isDzzyAgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned dzzy-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "dzzy-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "dzzy-agent";

  assert.equal(isDzzyAgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isDzzyAgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isDzzyAgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-dzzy-agent", () => {
  // E.g. user switches from dzzy-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isDzzyAgentRuntime(prospectiveRuntimeId), false);
});

test("isDzzyAgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
...
// useEffect in DzzyAgentModelTuningFields and AgentDefaultsSettingsCard
