...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "Dzzy event", there is no event content to surface, so
  // userText is empty and the title falls through to "Dzzy event".
  ...
  assert.equal(result.userTitle, "Dzzy event");
...
test("parsePromptText yields a null pubkey when From has no hex", () => {
  const text = ["[Dzzy event: note]", "From: Someone", "Content: hi"].join(
    "\n",
  );
...
test("parsePromptText defaults the title to 'Dzzy event' when no kind is present", () => {
  const text = ["[Dzzy event]", "Content: x"].join("\n");
  const result = parsePromptText(text);
  assert.equal(result.userTitle, "Dzzy event");
});
...
test("extractToolIdentity ignores Dzzy tool names that only appear in file contents", () => {
...
test("extractToolIdentity still recognizes explicit Dzzy tool fields", () => {
  const identity = extractToolIdentity({
...
test("parseSystemPromptSections pins the realistic Workspace+Base+System+Core harness shape", () => {
  // The real Dzzy harness emits [Workspace] content before [Base]. The parser
  ...
  const framed = [
    "[Workspace]",
    "You are operating inside the Dzzy platform.",
    "",
    "[Base]",
...
test("parseSystemPromptSections pins the full Base+System+Core+Canvas harness shape", () => {
  ...
  const framed = [
    ...
    "Fetch current content with: dzzy canvas get --channel 94a444a4-c0a3-5966-ab05-530c6ddc2301",
  ].join("\n");
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
...
test("parseSystemPromptSections extracts Team Instructions with Core Memory and Channel Canvas (full 5-section shape)", () => {
  ...
  const framed = [
    "[Base]",
    "You are a helpful AI assistant running in Dzzy.",
    "",
    "[System]",
  ...
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
    { title: "Base", body: "You are a helpful AI assistant running in Dzzy." },
    {
      title: "System",
