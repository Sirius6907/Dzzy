...
const dzzyAgentRuntime = {
  ...gooseRuntime,
  id: "dzzy-agent",
  label: "Dzzy Agent",
  command: "dzzy-agent-cmd",
  mcpCommand: null,
};
...
test("mapping carries the runtime and definition fields", async () => {
  ...
  assert.equal(input.acpCommand, "dzzy-acp");
...
test("Dzzy shared compute definition carries native provider and auto model", async () => {
  const input = await buildInstanceInputForDefinition(
    persona({
      runtime: "dzzy-agent",
      provider: "relay-mesh",
    ...
    { ...gooseRuntime, id: "dzzy-agent", command: "dzzy-agent" },
  );
  assert.equal(input.agentCommand, "dzzy-agent");
...
// ── item-13 regression: dzzy-agent-first default runtime ─────────────────────
//
// Before this fix, resolveStartRuntimeForDefinition used runtimes[0] (catalog
// order: goose, claude, codex, dzzy-agent), so an installed goose would beat
// the bundled dzzy-agent sidecar as the default for runtime-less personas.
// The fix applies the preference order: dzzy-agent → goose → first available.

test("item-13: goose+dzzy-agent both available — persona with no runtime resolves dzzy-agent", () => {
  const { runtime, warnings } = resolveStartRuntimeForDefinition(
    persona({ runtime: undefined }),
    [gooseRuntime, claudeRuntime, dzzyAgentRuntime],
  );
  assert.equal(
    runtime.id,
    "dzzy-agent",
    "dzzy-agent must win over catalog-first goose for runtime-less personas",
  );
  assert.deepEqual(warnings, []);
});
