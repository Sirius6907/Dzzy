...
test("Dzzy Agent exposes provider, model, and Dzzy-owned effort", () => {
  const model = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("dzzy-agent", {
      modelEnvVar: "DZZY_AGENT_MODEL",
      providerEnvVar: "DZZY_AGENT_PROVIDER",
      thinkingEnvVar: "DZZY_AGENT_THINKING_EFFORT",
    }),
    scope: "global",
  });
  ...
  assert.equal(field(model, "effort").optionSource, "dzzyAgentCatalog");
...
test("catalog mismatch cleanup is named and restricted to onboarding", () => {
  const selectedRuntime = runtime("dzzy-agent", {
    modelEnvVar: "DZZY_AGENT_MODEL",
    providerEnvVar: "DZZY_AGENT_PROVIDER",
    thinkingEnvVar: "DZZY_AGENT_THINKING_EFFORT",
  });
