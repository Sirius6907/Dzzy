 ...
 *   dzzy-agent — classifies LLM failures into `AgentError` variants with
 *                  JSON-RPC codes (`-32001` auth, `-32002` model-not-found,
 *                  `-32000` generic), defined in `crates/dzzy-agent/src/types.rs`.
 *   dzzy-acp   — preserves the code structurally in
...
export function friendlyAgentLastError(
  ...
  // Match either the unwrapped dzzy-agent prefix or the dzzy-acp v0 wrap.
  if (
