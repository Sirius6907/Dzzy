...
type E2eConfig = {
  ...
  mock?: {
    ...
    // Pending community deep links (dzzy://join / dzzy://connect / dzzy://add-community) seeded into
 ...
 * every member's own set (d=`dzzy:custom-emoji`). We serve TWO member-authored
 * sets from distinct pubkeys so the e2e exercises the union/collapse path, not
 * a single relay-owned set. `:dzzy:` is the stable shortcode exercised by
  ...
  return [
    createMockEvent(
      ...
      [
        ...
        ["emoji", "dzzy", "https://example.com/e2e/dzzy.png"],
    ...
    createMockEvent(
      ...
      [
        ...
        // member B claims :dzzy: with a DIFFERENT url — unionCustomEmoji must
        // collapse it to one deterministic winner, never expose two URLs.
        ["emoji", "dzzy", "https://example.com/e2e/dzzy-b.png"],
      ],
      "b".repeat(64),
    ),
  ];
...
declare global {
  interface Window {
    __DZZY_E2E__?: E2eConfig;
    __DZZY_E2E_COMMANDS__?: string[];
    __DZZY_E2E_COMMAND_PAYLOADS__?: Array<{
      command: string;
      payload: unknown;
    }>;
    __DZZY_E2E_COMMAND_LOG__?: Array<{
      command: string;
      payload: unknown;
    }>;
    __DZZY_E2E_WEBVIEW_ZOOM__?: number;
    __DZZY_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?: (input: {
      channelName: string;
      kind?: number;
    }) => boolean;
    __DZZY_E2E_HAS_MOCK_OWNER_KIND_SUBSCRIPTION__?: (input: {
      ownerPubkey: string;
      kind: number;
    }) => boolean;
    __DZZY_E2E_EMIT_MOCK_MESSAGE__?: (input: {
      channelName: string;
      content: string;
    ...
    __DZZY_E2E_PREPEND_MOCK_HISTORY__?: (input: {
      channelName: string;
      count: number;
      startIndex?: number;
      lineCount?: number;
    ...
    __DZZY_E2E_EMIT_MOCK_TYPING__?: (input: {
      channelName: string;
      pubkey?: string;
    }) => RelayEvent;
    __DZZY_E2E_INVOKE_MOCK_COMMAND__?: (
      command: string,
      payload?: Record<string, unknown>,
    ) => Promise<unknown>;
    __DZZY_E2E_PUSH_MOCK_FEED_ITEM__?: (item: RawFeedItem) => RawFeedItem;
    /** Replace an existing feed item by id (or push if not found) and fire the updated event. */
    __DZZY_E2E_REPLACE_MOCK_FEED_ITEM__?: (
      oldId: string,
      item: RawFeedItem,
    ) => RawFeedItem;
    __DZZY_E2E_SIGNED_EVENTS__?: Array<{
      content: string;
      createdAt?: number;
      kind: number;
      tags: string[][];
    ...
    __DZZY_E2E_REJECT_PROJECT_EVENT_KINDS__?: number[];
    /** Structured merge error returned by the mock native merge command. */
    __DZZY_E2E_PROJECT_MERGE_ERROR__?: {
      code: string;
      message: string;
      recovery: {
    ...
    __DZZY_E2E_PROJECT_OWNER_OVERRIDE__?: string;
    /** Project history kinds rejected with CLOSED for aggregate-query tests. */
    __DZZY_E2E_REJECT_PROJECT_QUERY_KINDS__?: number[];
    /** Captured aggregate project-history filters for request-count assertions. */
    __DZZY_E2E_PROJECT_QUERY_FILTERS__?: MockFilter[];
    __DZZY_E2E_PROJECT_REPO_SYNC_STATUS__?: {
    ...
    __DZZY_E2E_SET_RELAY_CONNECTION_STATE__?: (state: ConnectionState) => void;
    __DZZY_E2E_GET_RELAY_CONNECTION_STATE__?: () => ConnectionState;
    __DZZY_E2E_SET_STALL_WEBSOCKET_SENDS__?: (stall: boolean) => void;
    __DZZY_E2E_DISCONNECT_MOCK_WEBSOCKETS__?: () => number;
    __DZZY_E2E_RESTART_MOCK_WEBSOCKETS__?: () => number;
    __DZZY_E2E_SET_MESH__?: (mesh: {
      admitted?: boolean;
    ...
    __DZZY_E2E_SEED_ACTIVE_TURNS__?: (input: {
      agentPubkey: string;
      channelId: string;
      turnId: string;
      kind?: "turn_started" | "turn_completed";
    }) => void;
    __DZZY_E2E_SEED_OBSERVER_EVENTS__?: (input: {
      agentPubkey: string;
      events: Array<{
        seq: number;
        timestamp: string;
    ...
    __DZZY_E2E_EMIT_MOCK_READ_STATE__?: (input: {
      clientId: string;
      contexts: Record<string, number>;
      createdAt: number;
      slotId: string;
    }) => unknown;
    __DZZY_E2E_SEED_MOCK_REMINDERS__?: (reminders: RelayEvent[]) => void;
    __DZZY_E2E_QUERY_CLIENT__?: {
      invalidateQueries: (filters: { queryKey: readonly unknown[] }) => unknown;
    };
    __DZZY_E2E_MD_PARSE_COUNT__?: () => number;
    /**
     ...
     */
    __DZZY_E2E_ACTIVATE_TIMEOUT__?: (expiresAtMs: number) => void;
    /**
     ...
     * __DZZY_E2E_INVOKE_MOCK_COMMAND__. Call after the mutation to make the
     * updated channel state visible to subscribers.
     */
    __DZZY_E2E_INVALIDATE_CHANNELS__?: () => Promise<void>;
    /**
     ...
     * follow up with __DZZY_E2E_INVALIDATE_CHANNELS__ to flush the cache.
     *
     * Only the listed fields are writeable; omitted fields are left unchanged.
     */
    __DZZY_E2E_MUTATE_CHANNEL__?: (opts: {
      channelId: string;
     ...
     * are held in a queue and not resolved until `__DZZY_E2E_RELEASE_GET_EVENT__()`
     ...
     */
    __DZZY_E2E_DEFER_GET_EVENT__?: string | null;
     ...
     *  requests released. */
    __DZZY_E2E_RELEASE_GET_EVENT__?: () => number;
    /** Count of `get_event` invocations for the current defer-target ID since
     *  the last time `__DZZY_E2E_DEFER_GET_EVENT__` was set. */
    __DZZY_E2E_GET_EVENT_CALL_COUNT__?: number;
  }
}
...
// in e2e (instead of the `dzzy-media://` fallback). The reaction guard
...
// reacting with it exercises the proxy rewrite (unlike the `:dzzy:` fixture,
...
const E2E_IDENTITY_OVERRIDE_STORAGE_KEY = "dzzy:e2e-identity-override.v1";
...
// When `window.__DZZY_E2E_DEFER_GET_EVENT__` is set to a target event ID,
// `handleGetEvent` holds calls for that ID in this queue.  All other event IDs
// continue to resolve immediately.
// `window.__DZZY_E2E_RELEASE_GET_EVENT__()` flushes the queue and returns the
...
function buildMockConfigSurface(pubkey: string): {
  ...
  // panel witnesses distinct provenance labels in one frame: "Set in Dzzy",
  ...
  const dzzyAgentSurface = {
    ...gooseSurface,
    runtimeId: "dzzy-agent",
    runtimeLabel: "Dzzy Agent",
    advanced: [],
    extensions: [],
    sources: {
  ...
  switch (pubkey) {
    ...
    case PUBKEY_DZZY_AGENT:
      return dzzyAgentSurface;
    default:
      return gooseSurface;
  }
}
...
function buildSeededManagedAgent(seed: MockManagedAgentSeed): MockManagedAgent {
  ...
  return {
    ...
    log_lines: [
      `dzzy-acp starting: relay=${DEFAULT_RELAY_WS_URL} agent_pubkey=${seed.pubkey} parallelism=1`,
...
//
// This entire module (e2eBridge.ts) is loaded only when `window.__DZZY_E2E__`
...
/**
 * Mirror the relay's channel-window row set (dzzy-db `thread.rs`, NIP-CW
...
const MOCK_PROJECT_SEEDS = [
  {
    ...
    name: "dzzy",
    description:
      "Relay, desktop, and mobile clients for the Dzzy community platform.",
...
const MOCK_PROJECT_BRANCHES_KEY = "dzzy-e2e-project-branches";
...
function buildMockProjectEvents(): RelayEvent[] {
  ...
  for (const [projectIndex, seed] of MOCK_PROJECT_SEEDS.entries()) {
    const owner =
      projectIndex === 0
        ? (window.__DZZY_E2E_PROJECT_OWNER_OVERRIDE__ ?? seed.owner)
        : seed.owner;
...
async function handleDiscoverAcpRuntimes(
  ...
  const defaultCatalog: RawAcpRuntimeCatalogEntry[] = [
    ...
    },
    {
      ...
      availability: "available",
      command: "dzzy-agent",
      binary_path: "/usr/local/bin/dzzy-agent",
      default_args: [],
      mcp_command: "dzzy-dev-mcp",
      install_hint: "Ships with the Dzzy desktop app.",
      install_instructions_url: "https://github.com/block/dzzy",
      can_auto_install: false,
...
async function handleDiscoverManagedAgentPrereqs(
  ...
  return {
    acp: {
      command:
        configuredPrereqs?.acp?.command ?? args.input?.acpCommand ?? "dzzy-acp",
      resolved_path:
        configuredPrereqs?.acp?.resolvedPath ??
        "/Users/wesb/dev/dzzy/target/debug/dzzy-acp",
...
async function handleCreateManagedAgent(
  ...
  const pubkey = crypto
    ...
    .padEnd(64, "0")
    .slice(0, 64);
  const agentCommand = args.input.agentCommand ?? "dzzy-agent";
  const agentArgs =
  ...
  const managedAgent: MockManagedAgent = {
    ...
    acp_command: args.input.acpCommand ?? "dzzy-acp",
    ...
    log_lines: [
      `dzzy-acp starting: relay=${args.input.relayUrl ?? DEFAULT_RELAY_WS_URL} agent_pubkey=${pubkey} parallelism=${mintParallelism}`,
...
async function handleStartManagedAgent(
  ...
  if (isRelayMeshManagedAgent(agent)) {
    ...
    if (!hasLiveTarget) {
      throw new Error(
        "Dzzy shared compute cannot start because no live member is serving this model.",
      );
    }
  }
...
async function handleGetEvent(
  ...
  // Defer/release seam: when __DZZY_E2E_DEFER_GET_EVENT__ is set to this
  // event's ID, hold this call in the queue until __DZZY_E2E_RELEASE_GET_EVENT__()
  ...
  if (
    window.__DZZY_E2E_DEFER_GET_EVENT__ &&
    window.__DZZY_E2E_DEFER_GET_EVENT__ === args.eventId
  ) {
    // Increment the count only for calls that are actually deferred.
    window.__DZZY_E2E_GET_EVENT_CALL_COUNT__ =
      (window.__DZZY_E2E_GET_EVENT_CALL_COUNT__ ?? 0) + 1;
...
function sendToMockSocket(args: {
  ...
  if (type === "REQ") {
    ...
    if (
      ...
      window.__DZZY_E2E_PROJECT_QUERY_FILTERS__ ??= [];
      window.__DZZY_E2E_PROJECT_QUERY_FILTERS__.push(filter);
      const rejectedKinds =
        window.__DZZY_E2E_REJECT_PROJECT_QUERY_KINDS__ ?? [];
  ...
  if (type === "EVENT") {
    ...
    if (isMockProjectScopedEvent(event)) {
      ...
      const rejectionIndex =
        window.__DZZY_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(event.kind) ??
        -1;
      if (rejectionIndex >= 0) {
        window.__DZZY_E2E_REJECT_PROJECT_EVENT_KINDS__?.splice(
          rejectionIndex,
          1,
        );
        sendWsText(socket.handler, [
...
export function maybeInstallE2eTauriMocks() {
  ...
  mockWindows("main");
  window.__DZZY_E2E_COMMANDS__ = [];
  window.__DZZY_E2E_COMMAND_PAYLOADS__ = [];
  window.__DZZY_E2E_COMMAND_LOG__ = [];
  window.__DZZY_E2E_SIGNED_EVENTS__ = [];
  window.__DZZY_E2E_WEBVIEW_ZOOM__ = 1;
  window.__DZZY_E2E_EMIT_MOCK_MESSAGE__ = ({
    channelName,
    content,
    parentEventId,
    pubkey,
  ...
  window.__DZZY_E2E_PREPEND_MOCK_HISTORY__ = prependMockHistory;
  window.__DZZY_E2E_EMIT_MOCK_TYPING__ = ({ channelName, pubkey }) => {
    const channel = mockChannels.find(
      (candidate) => candidate.name === channelName,
    );
    if (!channel) {
  ...
  window.__DZZY_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__ = ({ channelName, kind }) => {
    const channel = mockChannels.find(
      (candidate) => candidate.name === channelName,
    );
    if (!channel) {
  ...
  window.__DZZY_E2E_HAS_MOCK_OWNER_KIND_SUBSCRIPTION__ = ({
    ownerPubkey,
    kind,
  }) => hasMockOwnerKindSubscription(ownerPubkey, kind);
  window.__DZZY_E2E_PUSH_MOCK_FEED_ITEM__ = (item) => {
    const category = item.category === "mention" ? "mentions" : item.category;
    mockFeedOverrides[category].unshift(item);
    window.dispatchEvent(new CustomEvent("dzzy:e2e-home-feed-updated"));
    return item;
  };
  window.__DZZY_E2E_REPLACE_MOCK_FEED_ITEM__ = (oldId, item) => {
    const category = item.category === "mention" ? "mentions" : item.category;
  ...
  window.__DZZY_E2E_MD_PARSE_COUNT__ = getMarkdownParseCount;
  window.__DZZY_E2E_ACTIVATE_TIMEOUT__ = (expiresAtMs: number) => {
  ...
  window.__DZZY_E2E_INVALIDATE_CHANNELS__ = async () => {
    await window.__DZZY_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["channels"],
    });
  };
  window.__DZZY_E2E_MUTATE_CHANNEL__ = ({
    channelId,
    channelType,
    removeMemberPubkey,
  }) => {
  ...
  window.__DZZY_E2E_GET_EVENT_CALL_COUNT__ = 0;
  window.__DZZY_E2E_DEFER_GET_EVENT__ = null;
  deferredGetEventQueue = [];
  window.__DZZY_E2E_RELEASE_GET_EVENT__ = () => {
  ...
  window.__DZZY_E2E_EMIT_MOCK_READ_STATE__ = ({
    clientId,
    contexts,
    createdAt,
    slotId,
  ...
  window.__DZZY_E2E_SET_RELAY_CONNECTION_STATE__ = (state) => {
  ...
  window.__DZZY_E2E_GET_RELAY_CONNECTION_STATE__ = () =>
    relayClient.getConnectionState();

  window.__DZZY_E2E_SEED_MOCK_REMINDERS__ = (reminders) => {
  ...
  window.__DZZY_E2E_SET_MESH__ = (mesh) => {
  ...
  window.__DZZY_E2E_SEED_ACTIVE_TURNS__ = ({
    agentPubkey,
    channelId,
    turnId,
    kind = "turn_started",
  ...
  window.__DZZY_E2E_SEED_OBSERVER_EVENTS__ = ({ agentPubkey, events }) => {
  ...
  const handleMockCommand = async (command: string, payload: unknown) => {
    ...
    window.__DZZY_E2E_COMMANDS__?.push(command);
    const loggedPayload = (() => {
      try {
    ...
    window.__DZZY_E2E_COMMAND_PAYLOADS__?.push({
      command,
      payload: loggedPayload,
    });
    window.__DZZY_E2E_COMMAND_LOG__?.push({ command, payload });
    ...
    switch (command) {
      ...
      case "check_builderlab_community_name":
        return {
          available: true,
          normalized_host: `${(payload as { name?: string })?.name ?? "community"}.communities.dzzy.xyz`,
        };
      case "create_builderlab_community": {
        ...
        return {
          community: activeConfig?.mock?.builderlabCreatedCommunity ?? {
            id: `hosted-${name}`,
            name,
            normalized_host: `${name}.communities.dzzy.xyz`,
          },
        };
      }
      ...
      case "sign_out":
        ...
        // assert invocation via __DZZY_E2E_COMMANDS__ and the pending UI.
        return;
      case "get_nsec": {
      ...
      case "get_project_repo_snapshot":
        return {
          ...
          files: [
            ...
            {
              path: "crates/dzzy-relay/src/api/git/transport.rs",
              kind: "blob",
              size: 33120,
              preview_content:
                "// Smart HTTP git transport\n// Handles upload-pack and receive-pack for Dzzy git repos.\n",
            },
          ],
        };
      ...
      case "get_project_repo_sync_status":
        return (
          window.__DZZY_E2E_PROJECT_REPO_SYNC_STATUS__ ?? {
            local_path: null,
            local_branch: null,
            local_branches: [],
            local_head: null,
      ...
      case "push_project_local_repository": {
        ...
        const status = window.__DZZY_E2E_PROJECT_REPO_SYNC_STATUS__;
      ...
      case "clone_project_repository": {
        const path = "/tmp/dzzy/REPOS/mock-project";
      ...
      case "sign_project_pull_request_status": {
        ...
        window.__DZZY_E2E_SIGNED_EVENTS__?.push(event);
        getMockProjectEventStore().push(event);
        return null;
      }
      case "sign_project_pull_request_review_request": {
        ...
        window.__DZZY_E2E_SIGNED_EVENTS__?.push({
          content: event.content,
          kind: event.kind,
          tags: event.tags,
        });
      ...
      case "merge_project_pull_request": {
        ...
        if (window.__DZZY_E2E_PROJECT_MERGE_ERROR__) {
          throw window.__DZZY_E2E_PROJECT_MERGE_ERROR__;
        }
        ...
        const rejectionIndex =
          window.__DZZY_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(
            statusEvent.kind,
          ) ?? -1;
      ...
      case "open_project_merge_recovery_terminal": {
        ...
        return {
          path: "/tmp/dzzy/REPOS/dzzy",
          cloned: false,
          recoveryRef: `refs/dzzy/merge-recovery/${input.expectedCommit}`,
          targetRef: `refs/dzzy/merge-recovery-target/${"f".repeat(40)}`,
        };
      }
      ...
      case "export_agent_snapshot":
        ...
        // Specs assert invocation via __DZZY_E2E_COMMANDS__.
        return true;
      ...
      case "discover_agent_models": {
        ...
        if (provider === "relay-mesh") {
          ...
          if (mockMeshState.models.length === 0) {
            throw new Error(
              "no Dzzy shared compute serving members are available",
            );
          }
        }
        const models =
      ...
      case "download_file":
        ...
        // command was invoked via `__DZZY_E2E_COMMANDS__`, not the dialog.
        return true;
      ...
      case "plugin:webview|set_webview_zoom":
        window.__DZZY_E2E_WEBVIEW_ZOOM__ = (payload as { value: number }).value;
        return;
      ...
      case "list_save_subscriptions": {
        ...
        if (!win.__DZZY_E2E_IPC_COUNTERS__) {
          win.__DZZY_E2E_IPC_COUNTERS__ = {};
        }
        const ipcCounters = win.__DZZY_E2E_IPC_COUNTERS__ as Record<
          string,
          number
        >;
  ...
  window.__DZZY_E2E_INVOKE_MOCK_COMMAND__ = (command, payload) =>
