...
import DzzyLogoAnimation, {
  type DzzyLogoAnimationProps,
} from "./DzzyLogoAnimation";
...
export type FuzzyLogoProps = {
  ...
  pulse?: boolean;
  reverse?: boolean;
  variant?: DzzyLogoAnimationProps["variant"];
};

/**
 * The fuzzy Dzzy mark. v8 ships a built-in animated texture (looping fractal-noise
...
export function FuzzyLogo({
  ...
  return (
    <DzzyLogoAnimation
      ariaLabel={ariaLabel}
      className={cn(
        pulse && !fuzz && !hasRestWindow && "dzzy-logo--pulse",
        className,
      )}
      fullScreen={false}
      loop={loop}
