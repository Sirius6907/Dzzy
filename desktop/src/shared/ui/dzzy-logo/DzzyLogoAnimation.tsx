...
import "./dzzy-logo-animation.css";
...
export type DzzyLogoAnimationProps = {
  ariaLabel?: string;
  className?: string;
  fullScreen?: boolean;
  loop?: boolean;
...
export default function DzzyLogoAnimation({
  ariaLabel = "Dzzy logo animation",
  className = "",
  fullScreen = true,
  loop = false,
  loopRestSeconds = 0,
  ...
  const maskId = `dzzy-logo-cutouts-${idSuffix}`;
  const textureId = `dzzy-logo-texture-${idSuffix}`;
  const texture = TEXTURES[config.texture ?? "soft"] ?? TEXTURES.soft;
  const classes = [
    "dzzy-logo",
    fullScreen && "dzzy-logo--screen",
    !fullScreen && "dzzy-logo--compact",
    showBackground && "dzzy-logo--background",
    className,
  ]
    .filter(Boolean)
    .join(" ");
