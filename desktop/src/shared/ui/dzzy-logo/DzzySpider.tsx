...
/**
 * The Dzzy spider mark with flapping wings. Geometry is identical to the static
 * {@link DzzyMark} (v8 final keyframe) — the same silhouette, rendered in
 * `currentColor` so it tints per-theme — with the wing-flap keyframes (ported
 * from the Dzzy website) beating the wings on an infinite loop.
 *
 ...
 * keep flapping regardless. The `spider-wing-layer` masks reproduce the slot
 ...
 */
export function DzzySpider({ className }: { className?: string }) {
  const maskId = `flapping-spider-cutouts-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;
  ...
  const wingLayer =
    "spider-wing-layer absolute top-[20.3236%] h-[59.3528%] w-[39.3562%]";
  const wingSvg = "spider-wing block h-full w-full";
  ...
  return (
    <div
      ...
      className={[
        ...
        .join(" ")}
    >
      <div className={`${wingLayer} spider-wing-layer-left left-0`}>
        <svg
          aria-hidden="true"
          className={`${wingSvg} spider-wing-left`}
      ...
      <div className={`${wingLayer} spider-wing-layer-right right-0`}>
        <svg
          aria-hidden="true"
          className={`${wingSvg} spider-wing-right`}
          viewBox="0 0 183.4 183.4"
          fill="currentColor"
        >
          <circle cx="91.7" cy="91.7" r="91.7" />
