...
function ImageBlock({ alt, dim, resolvedSrc, src, thumbSrc }: ImageBlockProps) {
  ...
  React.useLayoutEffect(() => {
    ...
    const spoiler = trigger.closest(".dzzy-spoiler[data-spoiler]");
    if (!spoiler) return;
 ...
 *
 * Dzzy renders inside a native webview whose default context menu has no
...
function createMarkdownComponents(
  ...
  function MarkdownAnchor({
    ...
    // Intercept `dzzy://message?channel=…&id=…` links so a click navigates
  ...
  return {
    ...
    "message-link": function MarkdownMessageLink({
      ...
      if (!parsed.ok) {
        // Malformed `dzzy://message?…` — render the raw URL as plain text
