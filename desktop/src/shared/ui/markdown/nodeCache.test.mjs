...
test("customEmoji is keyed by value, not identity", () => {
  clearMarkdownNodeCache();
  const emoji = [{ shortcode: "dzzy", url: "https://relay/dzzy.png" }];
  ...
  const second = renderCachedMarkdown({
    ...BASE,
    content: "hi :dzzy:",
    customEmoji: [{ shortcode: "dzzy", url: "https://relay/dzzy.png" }],
  });
  assert.equal(first, second);
  ...
  const third = renderCachedMarkdown({
    ...BASE,
    content: "hi :dzzy:",
    customEmoji: [{ shortcode: "dzzy", url: "https://relay/other.png" }],
  });
