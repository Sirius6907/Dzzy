...
export function imageReserveStyle(args: {
  ...
  if (hiddenSpoilerMediaSize) {
    ...
    return {
      "--dzzy-spoiler-media-aspect-ratio": ratio,
      "--dzzy-spoiler-media-width": `${hiddenSpoilerMediaSize.width}px`,
      aspectRatio: ratio,
      height: "auto",
...
export function isInsideHiddenSpoiler(element: Element): boolean {
  return (
    element.closest('.dzzy-spoiler[data-spoiler][data-revealed="false"]') !==
    null
  );
}

/**
 * `urlTransform` for `<ReactMarkdown>` that preserves `dzzy://message?…`
