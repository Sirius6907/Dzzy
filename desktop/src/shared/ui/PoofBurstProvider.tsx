...
export const POOF_TRIGGER_CLASS = "dzzy-poof-trigger";
export const POOF_ORIGIN_CLASS = "dzzy-poof-origin";
export const POOF_POINTER_ORIGIN_CLASS = "dzzy-poof-pointer-origin";
...
export function PoofBurstProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      <div aria-hidden="true" className="dzzy-poof-layer">
        {bursts.map((burst) => (
          <div
            className="dzzy-poof-burst"
            key={burst.id}
            style={
              {
                "--dzzy-poof-size": `${burst.size}px`,
                "--dzzy-poof-x": `${burst.x}px`,
                "--dzzy-poof-y": `${burst.y}px`,
              } as PoofStyle
            }
          >
            {POOF_FRAMES.map((frame, index) => (
              <img
                alt=""
                className={`dzzy-poof-frame dzzy-poof-frame-${index + 1}`}
                decoding="async"
                draggable={false}
