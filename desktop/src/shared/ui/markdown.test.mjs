...
// which would break copy → paste → click for `dzzy://message?…` links
// end-to-end. We pass a custom `urlTransform` that delegates to the
// default for `dzzy://message` and legacy `dzzy://message` hrefs.
//
...
test("messageLinkUrlTransform: preserves dzzy://message href", () => {
  const html = renderMarkdown(
    "Click [here](dzzy://message?channel=abc&id=xyz)",
  );
  // HTML-encoded `&` in attributes is fine — the browser decodes back to `&`.
  assert.match(html, /href="dzzy:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves dzzy://message autolink href", () => {
  const html = renderMarkdown("<dzzy://message?channel=abc&id=xyz>");
  assert.match(html, /href="dzzy:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves dzzy://message href with thread", () => {
  const html = renderMarkdown(
    "[link](dzzy://message?channel=c1&id=m1&thread=t1)",
  );
  assert.match(html, /href="dzzy:\/\/message\?[^"]*thread=t1"/);
});
...
test("messageLinkUrlTransform: preserves legacy dzzy://message href", () => {
  const html = renderMarkdown(
    "Click [here](dzzy://message?channel=abc&id=xyz)",
  );
  assert.match(html, /href="dzzy:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
...
test("messageLinkUrlTransform: leaves non-message dzzy:// schemes to default", () => {
  // `dzzy://connect?relay=…` is handled by a different code path (Tauri
  ...
  const html = renderMarkdown(
    "[connect](dzzy://connect?relay=wss://relay.example)",
  );
  assert.match(html, /href=""/);
});
...
// `remark-gfm`'s autolinker only covers http(s)://, so bare `dzzy://message`
...
test("remarkMessageLinks: bare dzzy://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("dzzy://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
  assert.equal(para.children[0].value, "dzzy://message?channel=c&id=m");
  assert.equal(para.children[0].data.hName, "message-link");
});

test("remarkMessageLinks: legacy bare dzzy://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("dzzy://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
...
test("remarkMessageLinks: mid-sentence URL splits surrounding text", () => {
  const tree = runPlugin(
    paragraph(text("see dzzy://message?channel=c&id=m here")),
  );
...
test("remarkMessageLinks: two URLs in one text node both replaced", () => {
  const tree = runPlugin(
    paragraph(
      text(
        "first dzzy://message?channel=a&id=1 then dzzy://message?channel=b&id=2 done",
      ),
    ),
  );
  const kids = tree.children[0].children;
  ...
  assert.equal(links[0].value, "dzzy://message?channel=a&id=1");
  assert.equal(links[1].value, "dzzy://message?channel=b&id=2");
});

test("remarkMessageLinks: trailing sentence punctuation stays outside URL", () => {
  for (const punctuation of [".", ",", ";", ":", "!", "?"]) {
    const tree = runPlugin(
      paragraph(text(`see dzzy://message?channel=c&id=m${punctuation}`)),
    );
    const kids = tree.children[0].children;
    ...
    assert.equal(kids[1].value, "dzzy://message?channel=c&id=m", punctuation);
...
test("remarkMessageLinks: URL inside parens keeps closing paren outside", () => {
  const tree = runPlugin(
    paragraph(text("see (dzzy://message?channel=c&id=m) for details")),
  );
  const kids = tree.children[0].children;
...
test("remarkMessageLinks: non-message dzzy:// URLs are not matched", () => {
  const original = "dzzy://connect?relay=wss://x.example";
...
test("remarkMessageLinks: text inside inlineCode is left alone", () => {
    ...
    children: [
      {
        type: "paragraph",
        children: [
          { type: "inlineCode", value: "dzzy://message?channel=c&id=m" },
        ],
      },
    ],
  };
  ...
  assert.equal(kids[0].value, "dzzy://message?channel=c&id=m");
});
...
function nudgeBody(agentPubkey) {
  return [
    ...
    "",
    "Open Edit Agent in the Dzzy app to set these.",
    "",
    "```dzzy:config-nudge",
    JSON.stringify({
      agent_name: "Fizz",
