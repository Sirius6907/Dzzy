...
export const THEME_STORAGE_KEY = "dzzy-theme";
const CACHE_KEY = "dzzy-theme-cache";
export const ACCENT_STORAGE_KEY = "dzzy-accent-color";
export const NEUTRAL_ACCENT = "neutral";
const FOLLOW_SYSTEM_KEY = "dzzy-follow-system";
...
const DZZY_VIBRANCY_MATERIAL = "sidebar";
...
function applyAccentColor(value: string) {
  ...
  if (value === NEUTRAL_ACCENT) {
    ...
    root.style.setProperty("--dzzy-selected-accent", foreground);
    root.style.setProperty(
      "--dzzy-video-review-accent",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
    root.style.setProperty(
      "--dzzy-video-review-accent-foreground",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
  ...
  root.style.setProperty("--dzzy-selected-accent", accentHsl);
  root.style.setProperty("--dzzy-video-review-accent", accentHsl);
  root.style.setProperty(
    "--dzzy-video-review-accent-foreground",
...
/**
 * The Dzzy themes ship with a fixed neutral accent (the GitHub black/white
 * foreground) rather than a user-selectable accent color. When a Dzzy theme is
 ...
 */
export function isDzzyTheme(themeName: string): boolean {
  return themeName === "dzzy" || themeName === "dzzy-dark";
}

/**
 * Resolve the accent to actually apply for a theme: Dzzy themes are pinned to
...
function resolveEffectiveAccent(
  ...
  return isDzzyTheme(themeName) ? NEUTRAL_ACCENT : accentColor;
}

/**
 * Toggle the opaque Dzzy sidebar-gradient marker. This is always safe to apply
 * synchronously: `data-dzzy-sidebar` paints solid gradient colors, so it never
 * makes the window see-through. The *translucent* treatment (transparent
 * root/body) is handled separately via {@link setDzzyTranslucent} because it
 * must be sequenced against the native vibrancy layer — see
 * {@link applyDzzyVibrancy}.
 */
function applyDzzySidebar(themeName: string) {
  const root = document.documentElement;
  if (isDzzyTheme(themeName)) {
    root.setAttribute("data-dzzy-sidebar", "");
    // Keep the concrete Dzzy variant on the root as well as the generic
    ...
    root.setAttribute("data-dzzy-theme", themeName);
  } else {
    root.removeAttribute("data-dzzy-sidebar");
    root.removeAttribute("data-dzzy-theme");
    // Leaving Dzzy: drop translucency synchronously here too. Going *opaque*
    // never shows desktop/prior content through, so there's no ordering risk
    // on the way out — only on the way in.
    setDzzyTranslucent(false);
  }
}
 ...
 * transparent root/body themselves are driven by the `data-dzzy-translucent`
 ...
 * underneath" flicker). {@link applyDzzyVibrancy} owns that sequencing.
 */
function setDzzyTranslucent(enabled: boolean) {
  const root = document.documentElement;
  if (enabled) {
    root.setAttribute("data-dzzy-translucent", "");
  } else {
    root.removeAttribute("data-dzzy-translucent");
  }
}

/**
 * Monotonic token identifying the most recent vibrancy request. Because
 * {@link applyDzzyVibrancy} awaits the native `set_window_vibrancy` IPC, a rapid
 * Dzzy → non-Dzzy toggle can fire two overlapping calls whose awaits resolve out
 ...
 * translucency — otherwise the earlier Dzzy call could re-add
 * `data-dzzy-translucent` after the later non-Dzzy call already cleared it,
 * leaving the window transparent under a non-Dzzy theme.
 */
let dzzyVibrancyRequest = 0;

/**
 * Whether the native vibrancy layer is confirmed installed for a Dzzy theme.
 ...
 */
let dzzyVibrancyReady = false;

/** The native layer does not need rebuilding when Dzzy only changes mode. */
let dzzyVibrancyEnabled = false;

/**
 * Enable the CSS translucency treatment, but only once BOTH prerequisites for
 * the current request are in place:
 *
 *  1. the native vibrancy layer is installed ({@link dzzyVibrancyReady}), and
 *  2. the Dzzy sidebar marker + gradient vars are applied (`data-dzzy-sidebar`,
 *     set synchronously by {@link applyDzzySidebar} inside {@link applyTheme}).
 *
 * Translucency clears the body/sidebar surfaces so the vibrancy layer shows
 * through; enabling it before the Dzzy gradient vars are installed would flash a
 * transparent/unstyled sidebar. `applyTheme` (theme vars) and
 * `applyDzzyVibrancy` (native layer) are independent async effects that can win
 ...
 */
function maybeEnableDzzyTranslucent(themeName: string, requestToken: number) {
  if (requestToken !== dzzyVibrancyRequest) return;
  if (!isDzzyTheme(themeName) || !isMacPlatform()) return;
  if (!dzzyVibrancyReady) return;
  if (!document.documentElement.hasAttribute("data-dzzy-sidebar")) return;
  setDzzyTranslucent(true);
}
 ...
 * behind it:
 *
 * - Entering Dzzy (macOS): install the vibrancy layer first (await the IPC),
 ...
 * - Leaving Dzzy: translucency was already removed synchronously in
 *   `applyDzzySidebar` (safe — opaque never shows through), so here we just
 *   clear the native layer.
 *
 * On non-macOS `set_window_vibrancy` is a no-op and translucency stays off, so
 * these platforms fall back to the opaque Dzzy gradient.
 *
 * Overlapping calls are guarded by {@link dzzyVibrancyRequest} so a stale async
 * continuation can't re-enable translucency after a newer theme superseded it.
 */
async function applyDzzyVibrancy(themeName: string) {
  const dzzy = isDzzyTheme(themeName);
  const requestToken = ++dzzyVibrancyRequest;

  // Dzzy Light and Dzzy Dark use the same native material. Rebuilding the
  ...
  if (dzzy && dzzyVibrancyEnabled && dzzyVibrancyReady) {
    maybeEnableDzzyTranslucent(themeName, requestToken);
    return;
  }
  ...
  try {
    await invokeTauri<void>("set_window_vibrancy", {
      enabled: dzzy,
      material: DZZY_VIBRANCY_MATERIAL,
    });
    ...
    if (requestToken !== dzzyVibrancyRequest) return;
    dzzyVibrancyEnabled = dzzy;
    // Native layer is installed. Record readiness and try to enable translucency
    // — but only if `applyDzzySidebar` has already installed the Dzzy gradient
    // vars. If that effect hasn't landed yet (the IPC won the race), it will
    // call maybeEnableDzzyTranslucent itself once the marker is applied.
    if (dzzy && isMacPlatform()) {
      dzzyVibrancyReady = true;
      maybeEnableDzzyTranslucent(themeName, requestToken);
    }
  } catch (error) {
...
function applyCachedVars(): string | null {
  try {
    ...
    applyDzzySidebar(themeName);

    const accent =
      window.localStorage.getItem(ACCENT_STORAGE_KEY) ?? DEFAULT_ACCENT;
    // Pin Dzzy themes to the neutral accent here too, matching applyTheme.
    // Otherwise a cached Dzzy theme + non-neutral stored accent flashes the
...
async function applyTheme(
  ...
  root.classList.remove("light", "dark");
  root.classList.add(isDark ? "dark" : "light");
  applyDzzySidebar(name);
  // The Dzzy gradient vars are now installed. If the vibrancy layer already
  // resolved for the current request (the IPC won the race against this theme
  // load), enable translucency now — otherwise applyDzzyVibrancy does it. This
  // is the second half of the two-effect handshake; the token guards against a
  // superseding theme switch.
  maybeEnableDzzyTranslucent(name, dzzyVibrancyRequest);
  ...
  // new theme for a frame — the flicker seen when switching to Dzzy. Dzzy
...
export function ThemeProvider({
  ...
  const [followSystem, setFollowSystemState] = useState<boolean>(() => {
    ...
    // Fresh profiles (no saved theme) default to System mode so the Dzzy
  ...
  useEffect(() => {
    if (!isValidThemeName(effectiveTheme)) return;
    void applyDzzyVibrancy(effectiveTheme);
  }, [effectiveTheme]);
  ...
  // changes. applyTheme already applies the (Dzzy-neutral-aware) accent in the
