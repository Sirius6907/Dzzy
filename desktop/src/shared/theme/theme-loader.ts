...
/**
 * Dzzy theme name. Dzzy is a first-party light theme that reuses GitHub
 ...
 * applied by {@link ThemeProvider} toggling a `data-dzzy-sidebar` attribute
 * on the document root; the CSS lives in `shared/styles/globals/theme.css`.
 */
export const DZZY_THEME_NAME = "dzzy";

/**
 * Dzzy Dark theme name. The dark-mode counterpart to {@link DZZY_THEME_NAME}:
 ...
 * {@link ThemeProvider} toggles the shared `data-dzzy-sidebar` attribute for
 * this theme too; the `.dark` root class selects the dark gradient values.
 *
 * Dzzy and Dzzy Dark are paired in {@link THEME_PAIRS}, so the picker shows a
 * combined "Dzzy" tile under System mode (follow-OS) plus a single "Dzzy" tile
 * under Light and a "Dzzy Dark" tile under Dark.
 */
export const DZZY_DARK_THEME_NAME = "dzzy-dark";

/** The Shiki bundle Dzzy borrows its base palette from. */
export const DZZY_BASE_THEME: SyntaxThemeName = "github-light";

/** The Shiki bundle Dzzy Dark borrows its base palette from. */
export const DZZY_DARK_BASE_THEME: SyntaxThemeName = "github-dark";

/**
 * Resolve a theme name to the real Shiki bundled theme it maps to.
 *
 * Most themes map to themselves, but the Dzzy aliases (`dzzy` / `dzzy-dark`)
 ...
 * first; passing a raw Dzzy alias makes Shiki throw and code blocks fall
 * back to unhighlighted plain text.
 */
export function resolveShikiThemeName(name: string): SyntaxThemeName {
  if (name === DZZY_THEME_NAME) return DZZY_BASE_THEME;
  if (name === DZZY_DARK_THEME_NAME) return DZZY_DARK_BASE_THEME;
  return name as SyntaxThemeName;
}

// Available themes. "dzzy" is a Dzzy-branded theme that reuses the
...
> = {
  // Dzzy reuses the github-light palette; its gradient is applied separately.
  dzzy: () => import("shiki/themes/github-light.mjs"),
  // Dzzy Dark reuses the github-dark palette; dark gradient applied separately.
  "dzzy-dark": () => import("shiki/themes/github-dark.mjs"),
...
export const THEME_PAIRS: ReadonlyMap<SyntaxThemeName, SyntaxThemeName> =
  new Map([
    // Light → Dark
    // Dzzy is the first-party pair; keep it first so it leads every category.
    ["dzzy", "dzzy-dark"],
