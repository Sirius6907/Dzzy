...
const CHANNELS = ["dzzy-bugs", "dzzy dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#dzzy", CHANNELS), {
    query: "dzzy",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #dzzy", CHANNELS), {
    query: "dzzy",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#dzzy", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#dzzy de", CHANNELS), {
    query: "dzzy de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#dzzy de` — `#` preceded by `x`, no boundary → no query even though
  // "dzzy de" is a prefix of a known channel.
  assert.equal(at("#", "x#dzzy de", CHANNELS), null);
});
