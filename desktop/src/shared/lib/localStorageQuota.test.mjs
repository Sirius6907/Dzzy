...
test("evicts pure caches and retries on quota failure", () => {
  ...
  install(ls);
  ls.store.set("dzzy-channel-messages.v1:relay:chan", "big");
  ls.store.set("dzzy-channels.v1:relay", "big");

  assert.equal(setLocalStorageItemWithRecovery("k", "v"), true);
  assert.equal(ls.getItem("k"), "v");
  assert.equal(ls.getItem("dzzy-channel-messages.v1:relay:chan"), null);
  assert.equal(ls.getItem("dzzy-channels.v1:relay"), null);
});

test("returns false when eviction frees nothing", () => {
  const ls = makeQuotaLocalStorage({ maxEntries: 2 });
  install(ls);
  ls.store.set("dzzy-workspaces", "keep");
  ls.store.set("dzzy-active-workspace-id", "keep");

  assert.equal(setLocalStorageItemWithRecovery("k", "v"), false);
  assert.equal(ls.getItem("k"), null);
  assert.equal(ls.getItem("dzzy-workspaces"), "keep");
});
