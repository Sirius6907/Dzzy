...
test("detects known custom emoji-only shortcode messages", () => {
  assert.equal(isEmojiOnlyMessage(":dzzy:", CUSTOM_EMOJI), true);
  assert.equal(isEmojiOnlyMessage(":dzzy: :party_parrot:", CUSTOM_EMOJI), true);
  assert.equal(isEmojiOnlyMessage(":Dzzy:", CUSTOM_EMOJI), true);
});

test("allows mixed unicode and custom emoji", () => {
  assert.equal(isEmojiOnlyMessage("😀 :dzzy: ❤️", CUSTOM_EMOJI), true);
});
