/**
 * Utilities for extracting and parsing the `dzzy:config-nudge` sentinel that
 * `dzzy-acp`'s setup-listener appends to its kind:9 nudge body.
 *
...
/**
 * The structured payload embedded in the `dzzy:config-nudge` sentinel block.
...
const FENCE_OPEN = "```dzzy:config-nudge";
const FENCE_CLOSE = "```";
...
/**
 * Strip the `dzzy:config-nudge` sentinel block (and any preceding blank line)
