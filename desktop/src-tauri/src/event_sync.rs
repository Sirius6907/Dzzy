...
pub fn spawn_event_sync(app: tauri::AppHandle, owner_keys: nostr::Keys) {
    tauri::async_runtime::spawn(async move {
        ...
        .await
        {
            eprintln!("dzzy-desktop: event-sync: spawn_blocking failed: {e}");
        }
    });
}
...
pub fn migrate_personas_to_events(app: &tauri::AppHandle, keys: &nostr::Keys) {
    ...
    match migrate_personas_in_dir(&base_dir, keys) {
        ...
        Ok(migrated) => {
            eprintln!(
                "dzzy-desktop: persona-event-migration: {migrated} personas migrated to retention"
            );
        }
        Err(e) => {
            eprintln!("dzzy-desktop: persona-event-migration: {e}");
        }
    }
}
...
fn migrate_personas_in_dir(base_dir: &Path, keys: &nostr::Keys) -> Result<u32, String> {
    ...
    use dzzy_core_pkg::kind::KIND_PERSONA;
    use nostr::JsonUtil;
...
pub fn migrate_teams_to_events(app: &tauri::AppHandle, keys: &nostr::Keys) {
    ...
    match migrate_teams_in_dir(&base_dir, keys) {
        Ok(0) => {}
        Ok(migrated) => {
            eprintln!("dzzy-desktop: team-event-migration: {migrated} teams migrated to retention");
        }
        Err(e) => {
            eprintln!("dzzy-desktop: team-event-migration: {e}");
        }
    }
}
...
fn migrate_teams_in_dir(base_dir: &Path, keys: &nostr::Keys) -> Result<u32, String> {
    ...
    use dzzy_core_pkg::kind::KIND_TEAM;
    use nostr::JsonUtil;
