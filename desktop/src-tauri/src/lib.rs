...
fn reveal_initial_window<R: tauri::Runtime>(window: &tauri::Window<R>) {
    if let Err(error) = window.show() {
        eprintln!("dzzy-desktop: failed to reveal main window: {error}");
        return;
    }
    if let Err(error) = window.set_focus() {
        eprintln!("dzzy-desktop: failed to focus main window: {error}");
    }
}
...
fn set_initial_window_backing<R: tauri::Runtime>(window: &tauri::Window<R>) {
    ...
    if let Err(error) = window.set_background_color(Some(tauri::window::Color(17, 21, 24, 255))) {
        eprintln!("dzzy-desktop: failed to set initial window backing: {error}");
    }
}
...
async fn clear_initial_window_backing<R: tauri::Runtime>(window: &tauri::Window<R>) {
    ...
    if let Err(error) = window.set_background_color(None) {
        eprintln!("dzzy-desktop: failed to clear initial window backing: {error}");
    }
}
...
async fn wait_for_stable_initial_window_geometry<R: tauri::Runtime>(window: &tauri::Window<R>) {
    ...
    eprintln!("dzzy-desktop: initial window geometry did not settle before reveal timeout");
}
...
pub fn run() {
        ...
        .build()
    {
        Ok(runtime) => {
            ...
            eprintln!(
                "dzzy-mesh: installed tokio runtime with {} MiB worker stacks",
        ...
        Err(error) => {
            ...
            eprintln!("dzzy-mesh: failed to build big-stack tokio runtime, using default: {error}");
        }
    }
                ...
                .on_webview_ready(|webview| {
                    ...
                    #[cfg(target_os = "macos")]
                    {
                        ...
                        tauri::async_runtime::spawn(async move {
                            ...
                            {
                                eprintln!(
                                    "dzzy-desktop: initial render did not commit before reveal timeout"
                                );
                            }
    ...
    #[cfg(not(dzzy_updater_enabled))]
    let builder = builder;

    let app = builder
        .register_asynchronous_uri_scheme_protocol("dzzy-media", |ctx, request, responder| {
            let app = ctx.app_handle().clone();
            tauri::async_runtime::spawn(async move {
                let response = media_proxy::handle_dzzy_media(&app, &request).await;
        ...
        .setup(move |app| {
            ...
            if let Err(e) = resolve_persisted_identity(&app_handle, &state) {
                eprintln!("dzzy-desktop: fatal: identity resolution failed: {e}");
                std::process::exit(1);
            }
            ...
            let owner_keys = match state.keys.lock() {
                Ok(k) => k.clone(),
                Err(e) => {
                    eprintln!("dzzy-desktop: fatal: owner keys lock poisoned: {e}");
                    std::process::exit(1);
                }
            };
            ...
            if let Err(e) = backfill_persona_snapshots(&app_handle) {
                eprintln!("dzzy-desktop: persona-snapshot backfill failed: {e}");
            }
            ...
            // Create the Dzzy nest (~/.dzzy or ~/.dzzy-dev for dev builds) before
            ...
            if let Err(error) = ensure_nest() {
                eprintln!("dzzy-desktop: failed to create nest: {error}");
            }
            ...
            // from the shared ~/.dzzy nest into the new dedicated ~/.dzzy-dev
            // nest so no work is lost when the nest is first namespaced.
            // Runs only when nest_dir() resolved to ~/.dzzy-dev (dev instance).
            // Suppressed after a reset so re-importing ~/.dzzy into ~/.dzzy-dev
            // doesn't re-populate what was just wiped.
            let is_dev_nest = managed_agents::nest_dir()
                .and_then(|p| p.file_name().map(|n| n.to_os_string()))
                .is_some_and(|n| n == ".dzzy-dev");
            ...
            if let Ok(exe) = std::env::current_exe() {
                if let Some(parent) = exe.parent() {
                    if let Err(error) = managed_agents::ensure_cli_symlink(parent, is_dev_nest) {
                        eprintln!("dzzy-desktop: failed to create CLI symlink: {error}");
                    }
                }
            }
            ...
            if !recovery_mode {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    let Ok(db_path) = managed_agents::managed_agents_base_dir(&flush_handle)
                        ...
                        eprintln!("dzzy-desktop: event-flush: cannot resolve retention db path");
                        return;
                    };
                    loop {
                        ...
                        if let Err(e) =
                            managed_agents::persona_events::flush_pending_events(&db_path, &state)
                                .await
                        {
                            eprintln!("dzzy-desktop: event-flush: {e}");
                        }
    ...
    app.run(move |app_handle, event| match event {
        ...
        RunEvent::Exit => {
            ...
            // End the process only after Dzzy and Mesh shutdown above, while
