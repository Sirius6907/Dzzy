...
//!
//! A small set of *reserved* keys — Dzzy's identity and secrets — are
//! rejected at save time and stripped at runtime so a typo or malicious
//! value can't swap the agent's nsec. Behavior knobs (GOOSE_MODE, DZZY_ACP_MODEL, DZZY_ACP_SYSTEM_PROMPT, …) remain
...
/// Env var keys that Dzzy sets itself and users must not override from
...
/// implications. Behavior knobs (GOOSE_MODE, DZZY_ACP_MODEL, DZZY_ACP_SYSTEM_PROMPT, …) remain freely
...
pub(crate) const RESERVED_ENV_KEYS: &[&str] = &[
    ...
    "DZZY_ACP_RESPOND_TO",
    "DZZY_ACP_RESPOND_TO_ALLOWLIST",
    "DZZY_ACP_AGENT_OWNER",
...
/// `=`. That means a key like `DZZY_AUTH_TAG=x` with value `forged`
/// lands as `DZZY_AUTH_TAG=x=forged` in the child env and
/// `getenv("DZZY_AUTH_TAG")` returns `"x=forged"` — a full reserved-
...
pub fn validate_user_env_keys(env_vars: &BTreeMap<String, String>) -> Result<(), String> {
    ...
    if !reserved.is_empty() {
        return Err(format!(
            "the following env vars are reserved by Dzzy and cannot be overridden: {}",
            reserved.join(", ")
        ));
    }
...
pub(crate) fn merged_user_env(
    ...
    merged.retain(|k, v| {
        if is_reserved_env_key(k) {
            eprintln!(
                "dzzy-desktop: ignoring reserved env var `{k}` from persona/agent overrides"
            );
            return false;
        }
        if !is_well_formed_env_key(k) {
            ...
            eprintln!(
                "dzzy-desktop: ignoring malformed env var key `{}` from persona/agent overrides",
        ...
        if v.contains('\0') {
            ...
            eprintln!(
                "dzzy-desktop: ignoring env var `{k}` with NUL byte in value"
            );
            return false;
        }
        if v.len() > MAX_ENV_VALUE_BYTES {
            eprintln!(
                "dzzy-desktop: ignoring env var `{k}` with oversize value ({} bytes > {MAX_ENV_VALUE_BYTES})",
                v.len()
            );
            return false;
        }
