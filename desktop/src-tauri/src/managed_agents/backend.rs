...
/// Enumerate PATH for dzzy-backend-* executables. Returns (id, path) pairs.
...
/// `resolve_command(format!("dzzy-backend-{id}"))` to prevent a compromised
...
pub fn resolve_provider_binary(provider_id: &str) -> Result<PathBuf, String> {
    ...
    match found {
        ...
        None => Err(format!(
            "provider 'dzzy-backend-{provider_id}' not found on PATH"
        )),
    }
}
