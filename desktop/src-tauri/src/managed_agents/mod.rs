...
pub(crate) use agent_env::{
    baked_build_env, build_dzzy_agent_provider_defaults, discovery_env_with_baked_floor,
};
mod backend;
...
/// Returns the Dzzy nest directory (`~/.dzzy`) if it exists as a real
...
/// invoked, so that `~/.dzzy` exists and gets cached.
///
...
pub fn default_agent_workdir() -> Option<std::path::PathBuf> {
    ...
    WORKDIR
        .get_or_init(|| {
            // Prefer ~/.dzzy if it exists (created by ensure_nest()).
