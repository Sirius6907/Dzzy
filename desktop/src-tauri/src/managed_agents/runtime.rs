...
use super::agent_env::build_dzzy_agent_provider_defaults;
...
/// Binary name fragments for all known agent/harness processes that Dzzy
...
pub(crate) const KNOWN_AGENT_BINARIES: &[&str] = &[
    ...
    "codex_acp",
    "goose",
    // dzzy-dev-mcp's multicall personalities (rg, tree, dzzy,
...
/// carry `DZZY_MANAGED_AGENT` in its environment (checked by the caller via
/// `process_has_dzzy_marker()`). This avoids sweeping unrelated node processes.
...
/// Callers must additionally verify `DZZY_MANAGED_AGENT` ownership.
...
pub(crate) fn process_belongs_to_us(pid: u32) -> bool {
    ...
    // the caller's `process_has_dzzy_marker()` check decides true ownership.
...
/// The value stamped into the `DZZY_MANAGED_AGENT` env var of every agent we
/// spawn, identifying *which* desktop instance owns it. We use the app's bundle
/// identifier (`xyz.block.dzzy.app` for release, `xyz.block.dzzy.app.dev`
...
/// build's agents, and vice versa). This is what lets two Dzzys coexist on
/// one machine without one's cleanup nuking the other's agents.
pub(crate) fn current_instance_id(app: &AppHandle) -> String {
    app.config().identifier.clone()
}

/// Build the full `DZZY_MANAGED_AGENT=<instance-id>` env entry we match
/// against when scanning processes. Kept here so the spawn stamp and the sweep
/// matcher can never drift apart.
fn dzzy_marker_entry(instance_id: &str) -> Vec<u8> {
    format!("DZZY_MANAGED_AGENT={instance_id}").into_bytes()
}

/// Check if a running process is one of *our* managed agents: it must carry
/// `DZZY_MANAGED_AGENT=<instance_id>` in its environment, where `instance_id`
/// is this desktop instance's id. A process stamped with a *different* instance
/// id belongs to another live Dzzy app and must never be reaped here.
#[cfg(target_os = "macos")]
pub(crate) fn process_has_dzzy_marker(pid: u32, instance_id: &str) -> bool {
    let marker = dzzy_marker_entry(instance_id);
...
#[cfg(not(unix))]
pub(crate) fn process_has_dzzy_marker(_pid: u32, _instance_id: &str) -> bool {
    false
}
...
/// and signal the groups. An orphaned grandchild (e.g. `goose` or `dzzy-dev-mcp`)
...
fn resolve_pgids_and_kill(candidate_pids: &[i32]) {
    ...
    if pgids.is_empty() && candidate_groups > 0 {
        eprintln!(
            "dzzy-desktop: orphan sweep: skipped all {candidate_groups} candidate group(s) (live foreign group leader or candidate already exited); nothing signalled"
        );
    }
...
pub(crate) fn valid_agent_runtime_receipt(
    ...
    canonical == receipt.key
        ...
        && process_has_dzzy_marker(receipt.pid, &receipt.desktop_instance_id)
}
...
/// agent binary stamped with *this* instance's `DZZY_MANAGED_AGENT` marker
...
/// while leaving another live Dzzy instance's agents untouched.
#[cfg(target_os = "macos")]
pub(crate) fn sweep_system_agent_processes(instance_id: &str, skip_pids: &[u32]) {
    ...
    for &pid in &pids {
        ...
        if !process_has_dzzy_marker(upid, instance_id) {
            continue;
        }
    ...
    if !orphans.is_empty() {
        eprintln!(
            "dzzy-desktop: system sweep found {} orphaned agent process(es), cleaning up",
            orphans.len()
        );
...
pub(crate) fn sweep_system_agent_processes(instance_id: &str, skip_pids: &[u32]) {
    ...
    for entry in entries.flatten() {
        ...
        if !process_belongs_to_us(upid) || !process_has_dzzy_marker(upid, instance_id) {
            continue;
        }
        // Live descendants of a tracked harness are exempt — see sweep::is_live_descendant_*.
...
pub(crate) fn sweep_system_agent_processes_with_grace(
    ...
    if !confirmed.is_empty() {
        eprintln!(
            "dzzy-desktop: periodic sweep confirmed {} orphaned agent process(es), cleaning up",
            confirmed.len()
        );
...
/// Binary names for the Dzzy desktop/Tauri process. Used by dead-instance
/// detection to confirm the owning desktop is still alive.
const DESKTOP_BINARY_NAMES: &[&str] = &["Dzzy", "dzzy-desktop", "dzzy_desktop"];

/// Check if a process name matches a known Dzzy desktop binary.
...
/// JSON as `"identifier":"xyz.block.dzzy.app.dev"` and in environment entries
...
/// `xyz.block.dzzy.app` from matching inside `xyz.block.dzzy.app.dev`.
...
/// Extract the `DZZY_MANAGED_AGENT` value from a process's environment.
/// Returns `None` if the process doesn't have the marker or can't be read.
#[cfg(target_os = "macos")]
fn extract_dzzy_marker_value(pid: u32) -> Option<String> {
    let prefix = b"DZZY_MANAGED_AGENT=";
...
#[cfg(not(unix))]
fn extract_dzzy_marker_value(_pid: u32) -> Option<String> {
    None
}

/// Check if a Dzzy desktop process is still alive for the given instance ID.
/// Scans all user-owned processes named "Dzzy" or "dzzy-desktop" and checks
...
/// Reap agent processes belonging to dead Dzzy desktop instances.
///
/// Scans all user processes for `DZZY_MANAGED_AGENT=*`, groups them by
/// instance ID, and for each foreign instance (≠ `our_instance_id`) checks
/// whether a Dzzy desktop binary is still alive for that instance. If not,
...
pub(crate) fn reap_dead_instance_agents(our_instance_id: &str, skip_pids: &[u32]) {
    ...
    for &pid in &pids {
        ...
        let Some(agent_instance_id) = extract_dzzy_marker_value(upid) else {
            continue;
        };
    ...
    for (instance_id, agent_pids) in &foreign_agents {
        ...
        eprintln!(
            "dzzy-desktop: reaping {} orphaned agent(s) from dead instance '{instance_id}'",
            agent_pids.len()
        );
...
pub fn kill_stale_tracked_processes(
    ...
    for record in records.iter_mut() {
        ...
        if !runtimes.keys().any(|key| key.pubkey == record.pubkey) {
            if process_belongs_to_us(pid) && process_has_dzzy_marker(pid, instance_id) {
...
pub(crate) fn build_respond_to_env(
    ...
    if record.respond_to == super::types::RespondTo::Allowlist {
        set.push(("DZZY_ACP_RESPOND_TO_ALLOWLIST", normalized.join(",")));
    } else {
        remove.push("DZZY_ACP_RESPOND_TO_ALLOWLIST");
    }
    ...
    // DZZY_ACP_AGENT_OWNER keeps those records functional. Modern records
    // (`auth_tag = Some(...)`) use `DZZY_AUTH_TAG` as before.
    if record.auth_tag.is_none() {
        if let Some(owner) = owner_hex {
            set.push(("DZZY_ACP_AGENT_OWNER", owner.to_string()));
        } else {
            remove.push("DZZY_ACP_AGENT_OWNER");
        }
    } else {
        remove.push("DZZY_ACP_AGENT_OWNER");
    }
...
pub fn spawn_agent_child(
    ...
    let resolved_mcp_command: Option<std::path::PathBuf> = if effective_mcp_command.is_empty() {
        ...
        match resolve_command(effective_mcp_command) {
            ...
            None => {
                eprintln!(
                    "dzzy-desktop: mcp_command {effective_mcp_command:?} not found, skipping"
                );
                None
            }
        }
    ...
    //   - bundled sidecars (dzzy, dzzy-acp, etc.) via exe parent (Contents/MacOS/)
    ...
    command.env("DZZY_PRIVATE_KEY", &record.private_key_nsec);
    command.env("DZZY_RELAY_URL", &effective_relay_url);
    command.env("DZZY_ACP_LAZY_POOL", if lazy { "true" } else { "false" });
    command.env("DZZY_ACP_AGENT_COMMAND", &resolved_agent_command);
    command.env("DZZY_ACP_AGENT_ARGS", agent_args.join(","));
    match &resolved_mcp_command {
        Some(mcp_cmd) => {
            command.env("DZZY_ACP_MCP_COMMAND", mcp_cmd);
        }
        None => {
            command.env("DZZY_ACP_MCP_COMMAND", "");
        }
    }
    // Enable MCP hook tools (_Stop, _PostCompact) for agents that need them.
    // Uses "*" because build_mcp_servers() hard-codes the server name to "dzzy-mcp".
    ...
    // into DZZY_ACP_SETUP_PAYLOAD.  dzzy-acp detects this env var on startup
    // and enters the minimal setup-listener mode instead of the agent pool.
    //
    // SECURITY: DZZY_ACP_SETUP_PAYLOAD is in RESERVED_ENV_KEYS so user env
    ...
    // source and dzzy-acp only transports the payload.
    //
    // The JSON format mirrors `setup_mode::SetupPayload` in dzzy-acp:
    ...
    let spawned_setup_mode;
    {
        ...
        let setup_payload_json =
            if let AgentReadiness::NotReady { requirements } = agent_readiness(&effective) {
                ...
                match serde_json::to_string(&payload) {
                    ...
                    Err(e) => {
                        eprintln!(
                            "dzzy-desktop: failed to serialize setup payload for {}: {e}",
                            record.name
                        );
                        None
                    }
        ...
        //   1. DZZY_ACP_SETUP_PAYLOAD is in RESERVED_ENV_KEYS, so
        ...
        command.env_remove("DZZY_ACP_SETUP_PAYLOAD");

        // Set the payload only when desktop computed NotReady.
        if let Some(json) = setup_payload_json {
            command.env("DZZY_ACP_SETUP_PAYLOAD", json);
            eprintln!(
                "dzzy-desktop: agent {} not ready — spawning in setup-listener mode",
                record.name
            );
        }
    }
    // Only emit DZZY_ACP_IDLE_TIMEOUT when the user has explicitly set an
    // override. When unset, the dzzy-acp harness applies its own default
    // (see `DEFAULT_IDLE_TIMEOUT_SECS` in crates/dzzy-acp/src/config.rs),
    // which is the single source of truth. The previously-emitted
    // `DZZY_ACP_TURN_TIMEOUT` is deprecated upstream and was pinning every
    // agent to the desktop's stale default (320s), bypassing harness bumps.
    if let Some(idle) = record.idle_timeout_seconds {
        command.env("DZZY_ACP_IDLE_TIMEOUT", idle.to_string());
    }

    if let Some(max_dur) = record.max_turn_duration_seconds {
        command.env("DZZY_ACP_MAX_TURN_DURATION", max_dur.to_string());
    }
    command.env("DZZY_ACP_AGENTS", record.parallelism.to_string());
    command.env("DZZY_ACP_MULTIPLE_EVENT_HANDLING", "steer");
    command.env("DZZY_ACP_DEDUP", "queue");
    ...
    if let Some(instructions) = &team_instructions {
        command.env("DZZY_ACP_TEAM_INSTRUCTIONS", instructions);
    } else {
        command.env_remove("DZZY_ACP_TEAM_INSTRUCTIONS");
    }
    ...
    if let Some(prompt) = &effective_prompt {
        command.env("DZZY_ACP_SYSTEM_PROMPT", prompt);
    } else {
        command.env_remove("DZZY_ACP_SYSTEM_PROMPT");
    }
    if let Some(model) = effective_model {
        command.env("DZZY_ACP_MODEL", model);
    } else {
        command.env_remove("DZZY_ACP_MODEL");
    }
    // Baked-in provider defaults for internal builds (dzzy-releases sets
    // DZZY_BUILD_DZZY_AGENT_* at compile time; OSS builds bake nothing).
    // Written FIRST so that record/persona metadata env vars below override them.
    build_dzzy_agent_provider_defaults(&mut command);
    if let Some(meta) = runtime_meta {
    ...
    command.env_remove("DZZY_ACP_PRIVATE_KEY");
    command.env_remove("DZZY_ACP_API_TOKEN");
    command.env_remove("DZZY_API_TOKEN");

    if let Some(ref auth_tag) = record.auth_tag {
        command.env("DZZY_AUTH_TAG", auth_tag);
    } else {
        command.env_remove("DZZY_AUTH_TAG");
    }
    ...
    command.env("DZZY_ACP_RELAY_OBSERVER", "true");

    // ── Git credential helper for Dzzy relay ──────────────────────────
    //
    // Agents need to clone/push repos hosted on the Dzzy relay's git
    ...
    //
    // NOSTR_PRIVATE_KEY mirrors DZZY_PRIVATE_KEY — keep in sync.
    if let Some(cred_helper) = resolve_command("git-credential-nostr") {
        ...
        eprintln!(
            "dzzy-desktop: git-credential-nostr not found — agent {} will not have automatic Dzzy git auth",
            record.name,
        );
    }
    ...
    // filtering. Precedence: baked floor < Dzzy-set env above < GLOBAL <
    // PERSONA < per-agent.
    //
    // These writes go LAST so user-provided values win over every Dzzy-set env
    // above — EXCEPT reserved keys (DZZY_PRIVATE_KEY, NOSTR_PRIVATE_KEY,
    // DZZY_AUTH_TAG, DZZY_API_TOKEN, DZZY_ACP_PRIVATE_KEY, DZZY_ACP_API_TOKEN),
    // which `merged_user_env` strips. Those carry Dzzy's identity and must
    ...
    // Dzzy shared compute is stored as a native provider; derive the OpenAI-compatible
    ...
    command
        .env("DZZY_MANAGED_AGENT", current_instance_id(app))
        .env("DZZY_MANAGED_AGENT_START_NONCE", &start_nonce);
    ...
    // terminal pops for dzzy-acp.exe and lingers (the app itself sets
...
fn child_rust_log_filter() -> String {
    match std::env::var("RUST_LOG") {
        Ok(existing) if existing.contains("dzzy_acp") => existing,
        Ok(existing) if !existing.trim().is_empty() => format!("{existing},dzzy_acp=info"),
        _ => "dzzy_acp=info".to_string(),
    }
}
...
/// before running the reader, so DzzyExplicit-tagged fields can be re-tagged to
