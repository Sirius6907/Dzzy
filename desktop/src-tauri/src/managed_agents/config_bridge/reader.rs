...
pub(crate) fn read_config_surface(
    ...
    let (file_config, file_was_read) = runtime_meta
        ...
        .and_then(|id| match id {
            ...
            "dzzy-agent" => super::dzzy_agent::read_config_file().map(|c| (c, true)),
            _ => None,
        })
        .unwrap_or_else(|| (RuntimeFileConfig::default(), false));

    // Tier 2a: record-level values (Dzzy-explicit).
    ...
    let normalized = NormalizedConfig {
        ...
        system_prompt: build_system_prompt_field(
            &record
                .system_prompt
                .clone()
                .or_else(|| record.env_vars.get("DZZY_ACP_SYSTEM_PROMPT").cloned()),
    ...
    for (k, v) in &record.env_vars {
        ...
        advanced.push(ConfigField {
            ...
            label: k.clone(),
            value: Some(v.clone()),
            origin: ConfigOrigin::DzzyExplicit,
            schema_type: ConfigFieldType::String,
...
fn build_model_field(
    ...
    // Precedence: Dzzy-explicit > ACP current > config file
    let (value, origin) = if let Some(ref m) = record_model {
        (Some(m.clone()), ConfigOrigin::DzzyExplicit)
    ...
    // Dzzy-explicit model shadowing a config-file model. The live-session
...
/// `(record_model, DzzyExplicit)` for a genuine-explicit agent that live-
...
fn build_provider_field(
    ...
    let tiers: &[(Option<&str>, ConfigOrigin)] = &[
        (record_provider.as_deref(), ConfigOrigin::DzzyExplicit),
...
fn build_thinking_field(
    ...
    let tiers: &[(Option<&str>, ConfigOrigin)] = &[
        (record_effort.as_deref(), ConfigOrigin::DzzyExplicit),
...
/// env_vars map the field is DzzyExplicit + RespawnWithEnvVar; otherwise if the
...
/// Record/env prompt wins (DzzyExplicit, respawnable); a config-file prompt it
...
fn build_system_prompt_field(
    ...
    if let Some(v) = record_prompt {
        return Some(NormalizedField {
            ...
            origin: ConfigOrigin::DzzyExplicit,
            write_via: ConfigWriteMechanism::RespawnWithEnvVar {
                env_key: "DZZY_ACP_SYSTEM_PROMPT".to_string(),
            },
