...
pub enum ConfigOrigin {
    /// Explicitly set in Dzzy UI / ManagedAgentRecord (highest precedence).
    DzzyExplicit,
    ...
    /// re-tag injected fields from `DzzyExplicit` to `PersonaDefault`.
    PersonaDefault,
    ...
    /// the linked persona specifies a value. Re-tagged from `DzzyExplicit` by the
