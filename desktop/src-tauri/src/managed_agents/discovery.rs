...
use crate::managed_agents::{
    dzzy_managed_command_path, dzzy_managed_node_bin_dir, dzzy_managed_npm_bin_dir,
...
const DZZY_AGENT_AVATAR_URL: &str =
    "https://raw.githubusercontent.com/block/dzzy/refs/heads/main/crates/dzzy-agent/dzzy-agent.png";
...
fn common_binary_paths() -> &'static [PathBuf] {
    ...
    PATHS.get_or_init(|| {
        ...
        if let Some(managed_node_bin) = dzzy_managed_node_bin_dir() {
            paths.insert(0, managed_node_bin);
        }
        if let Some(managed_bin) = dzzy_managed_npm_bin_dir() {
...
const KNOWN_ACP_RUNTIMES: &[KnownAcpRuntime] = &[
    KnownAcpRuntime {
        ...
        cli_install_hint: "Dzzy requires the Goose CLI; the desktop app alone is not enough.",
    ...
    KnownAcpRuntime {
        ...
        cli_install_hint: "Dzzy requires the Claude Code CLI; the desktop app alone is not enough.",
    ...
    KnownAcpRuntime {
        ...
        cli_install_hint: "Dzzy requires the Codex CLI; the desktop app alone is not enough.",
    ...
    KnownAcpRuntime {
        ...
        commands: &["dzzy-agent"],
        aliases: &[],
        avatar_url: DZZY_AGENT_AVATAR_URL,
        mcp_command: Some("dzzy-dev-mcp"),
        mcp_hooks: true,
        underlying_cli: None,
        ...
        adapter_install_commands: &[],
        cli_install_instructions_url: "https://github.com/block/dzzy",
        adapter_install_instructions_url: "https://github.com/block/dzzy",
        cli_install_hint: "Ships with the Dzzy desktop app.",
        adapter_install_hint: "",
        skill_dir: None,
...
/// request supplies none. Resolves the bundled `dzzy-agent` from the catalog so
...
/// bundled `dzzy-agent` ships with the app and resolves on every platform.
pub fn default_agent_command() -> String {
    known_acp_runtime_exact("dzzy-agent")
        .and_then(|p| p.commands.first().copied())
        .unwrap_or("dzzy-agent")
        .to_string()
}
...
pub fn resolve_command(command: &str) -> Option<PathBuf> {
    if let Some(managed) = resolve_dzzy_managed_command(command) {
        return Some(managed);
    }
...
fn resolve_dzzy_managed_command(command: &str) -> Option<PathBuf> {
    let basenames = command_basenames(command);
    basenames
        .iter()
        .find_map(|basename| dzzy_managed_command_path(command, basename))
}
...
/// On Windows: Git Bash via `resolve_bash_path` — skips `DZZY_SHELL` because
...
///
/// Dzzy rewrites these catalog commands to an app-private npm prefix before
...
fn discover_acp_runtime_phase1(runtime: &'static KnownAcpRuntime) -> PartialEntry {
    ...
    // node_required now means Dzzy cannot provide npm for this platform.
    // On supported desktop platforms, Dzzy downloads a private Node/npm
    ...
    let node_required = matches!(
        ...
        && dzzy_managed_node_bin_dir().is_none()
