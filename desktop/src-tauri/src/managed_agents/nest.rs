//! Dzzy Nest — persistent agent workspace at `~/.dzzy`.
//!
//! Creates a shared knowledge directory on first launch so every
//! Dzzy-spawned agent starts with orientation (AGENTS.md) and a
...
/// Default SKILL.md content for the dzzy-cli skill.
/// Written to ~/.dzzy/.agents/skills/dzzy-cli/SKILL.md on first init.
const DZZY_CLI_SKILL_MD: &str = include_str!("nest_skill.md");
...
/// when the Tauri app-data directory name starts with `"xyz.block.dzzy.app.dev"`.
...
/// Returns `true` iff `path` ends with the dev-nest directory name (`.dzzy-dev`).
///
...
/// Returns `true` when the running binary is using the dev nest (`~/.dzzy-dev`).
///
...
/// directory starts with `"xyz.block.dzzy.app.dev"`.
///
/// Returns `false` when:
/// - The nest is the production nest (`~/.dzzy`, signed DMG).
...
/// Creates the Dzzy nest at `~/.dzzy` if it doesn't already exist.
///
...
/// Creates a Dzzy nest at the given `root` path.
///
/// - Creates the root directory and all subdirectories.
/// - Writes `AGENTS.md` only if it doesn't already exist.
/// - Writes `.agents/skills/dzzy-cli/SKILL.md` only if it doesn't already exist.
/// - Creates harness-specific symlinks pointing to the canonical
///   `.agents/skills/dzzy-cli` directory for each known provider.
...
pub fn ensure_nest_at(root: &Path) -> Result<(), String> {
    ...
    // Write dzzy-cli skill to the harness-agnostic .agents path.
        ...
        .open(&skill_md)
    {
        Ok(mut file) => {
            use std::io::Write;
            file.write_all(DZZY_CLI_SKILL_MD.as_bytes())
    ...
    // Migration of the old .claude/skills/dzzy-cli real dir is handled in
...
///
/// Dev builds (`is_dev = true`) use `"dzzy-dev"` so that a running DMG and a
/// concurrent dev build each own a separate link and never clobber each other —
/// the same isolation that separates `~/.dzzy` (prod) from `~/.dzzy-dev` (dev).
...
/// Ensures `~/.local/bin/dzzy` (prod) or `~/.local/bin/dzzy-dev` (dev) is a
...
/// `~/.dzzy` and `~/.dzzy-dev` nests (see [`NEST_DIR_DEV`]).
///
/// On every boot: replaces any existing symlink unconditionally (the `dzzy` /
/// `dzzy-dev` name is our namespace), creates a new one if absent, and leaves
...
pub fn ensure_cli_symlink(exe_parent: &Path, is_dev: bool) -> Result<(), String> {
    let dzzy_bin = exe_parent.join("dzzy");
    if !dzzy_bin.exists() {
...
///
/// Preserves everything from the `<!-- BEGIN DZZY MANAGED` marker onward
...
fn refresh_skill_md_if_stale(root: &Path) -> Result<(), String> {
    let agents_skill_dir = root.join(".agents/skills/dzzy-cli");
    let version_path = agents_skill_dir.join(".skill-version");
    if read_version_file(&version_path) >= NEST_SKILL_VERSION {
        return Ok(());
    }

    // Migration: if .claude/skills/dzzy-cli exists as a real directory
    ...
    let skill_content = if old_is_real_dir {
        ...
        fs::read_to_string(old_skill_dir.join("SKILL.md"))
            .unwrap_or_else(|_| DZZY_CLI_SKILL_MD.to_string())
    } else {
        DZZY_CLI_SKILL_MD.to_string()
    };
    ...
    // Create/replace the .claude/skills/dzzy-cli symlink.
    #[cfg(unix)]
    {
        ...
        let symlink_path = root.join(".claude/skills/dzzy-cli");
        ...
        create_symlink(
            std::path::Path::new("../../.agents/skills/dzzy-cli"),
            &symlink_path,
        )
...
pub fn render_dynamic_section(
    ...
    let active_agents = if agents.is_empty() {
        "## Active Agents\n\n*(No agents deployed yet. Add agents in the Dzzy desktop app.)*"
            .to_string()
    } else {
        let mut table =
...
pub fn try_regenerate_nest(app: &AppHandle) {
    if let Err(error) = regenerate_nest_context(app) {
        eprintln!("dzzy-desktop: nest context regeneration failed: {error}");
    }
}
