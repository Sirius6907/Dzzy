...
fn nest_dir_is_under_home() {
    if let Some(dir) = nest_dir() {
        // Accepts both .dzzy (prod) and .dzzy-dev (dev) depending on
        // whether init_nest_dir was called before this test ran.
        let name = dir.file_name().and_then(|n| n.to_str()).unwrap_or("");
        assert!(
            name == NEST_DIR_PROD || name == NEST_DIR_DEV,
            "nest_dir must end with .dzzy or .dzzy-dev, got {dir:?}"
        );
    }
}

#[test]
fn init_nest_dir_prod_sets_dzzy() {
    ...
    if let Some(d) = dir {
        ...
        assert!(
            name == NEST_DIR_PROD || name == NEST_DIR_DEV,
            "nest_dir suffix must be .dzzy or .dzzy-dev, got {d:?}"
        );
    }
}
...
fn ensure_nest_skill_dir_has_700_permissions() {
    ...
    // Symlinks (e.g. .goose/skills/dzzy-cli) are skipped by the chmod loop.
    for dir in [
        ".agents",
        ".agents/skills",
        ".agents/skills/dzzy-cli",
        ".goose",
        ".goose/skills",
        ".claude",
        ".claude/skills",
...
fn ensure_nest_migrates_old_skill_dir() {
    ...
    let _ = fs::remove_file(root.join(".claude/skills/dzzy-cli"));
    let _ = fs::remove_dir_all(root.join(".agents/skills/dzzy-cli"));
    let old_skill_dir = root.join(".claude/skills/dzzy-cli");
    ...
    let _ = fs::remove_file(root.join(".agents/skills/dzzy-cli/.skill-version"));
    ...
    let new_skill = root.join(".agents/skills/dzzy-cli/SKILL.md");
    ...
    let old_path = root.join(".claude/skills/dzzy-cli");
    assert!(
        old_path
            .symlink_metadata()
            .unwrap()
...
fn ensure_skill_symlinks_are_idempotent() {
    ...
    for dir in [".goose/skills", ".claude/skills", ".codex/skills"] {
        ...
        assert_eq!(
            ...
            "symlink at {dir}/dzzy-cli should use relative target"
        );
    }
}
...
fn ensure_skill_symlinks_skips_existing_path_during_initial_pass() {
    ...
    // migrates pre-existing real directories at .claude/skills/dzzy-cli to
    ...
    let real_dir = root.join(".claude/skills/dzzy-cli");
    ...
    assert!(
        real_dir
            ...
            .file_type()
            .is_symlink(),
        ".claude/skills/dzzy-cli should be migrated to a symlink"
    );
    // The canonical path now holds the migrated content.
    let canonical = root.join(".agents/skills/dzzy-cli/SKILL.md");
    assert_eq!(
...
fn ensure_skill_symlinks_skip_dangling_symlink() {
    ...
    let dangling = codex_skills.join("dzzy-cli");
...
#[test]
fn cli_link_name_prod_is_dzzy() {
    assert_eq!(cli_link_name(false), "dzzy");
}

#[test]
fn cli_link_name_dev_is_dzzy_dev() {
    assert_eq!(cli_link_name(true), "dzzy-dev");
}
...
fn ensure_cli_symlink_creates_symlink_prod() {
    ...
    fs::write(exe_parent.join("dzzy"), "binary").unwrap();

    let local_bin = tmp.path().join("local_bin");
    fs::create_dir_all(&local_bin).unwrap();

    // Prod link name is "dzzy"; simulate the symlink creation path.
    let link = local_bin.join(cli_link_name(false));
    std::os::unix::fs::symlink(exe_parent.join("dzzy"), &link).unwrap();
    assert!(link.symlink_metadata().unwrap().file_type().is_symlink());
    assert_eq!(fs::read_link(&link).unwrap(), exe_parent.join("dzzy"));
}
...
fn ensure_cli_symlink_creates_symlink_dev() {
    ...
    // Dev link must be "dzzy-dev", never "dzzy".
    assert_eq!(cli_link_name(true), "dzzy-dev");
    ...
    assert!(!local_bin.join("dzzy").exists());
}
...
fn ensure_cli_symlink_does_not_clobber_regular_file_dev() {
    ...
    fs::create_dir_all(&local_bin).unwrap();
    let link = local_bin.join(cli_link_name(true));
    fs::write(&link, "user-installed dzzy-dev binary").unwrap();

    // Regular files at the dev path are also preserved.
    assert!(link.symlink_metadata().unwrap().file_type().is_file());
    assert_eq!(
        fs::read_to_string(&link).unwrap(),
        "user-installed dzzy-dev binary"
    );
}
...
fn test_upsert_managed_section_with_markers() {
    ...
    fs::write(
            &file,
            "# Header\n\nsome content\n\n<!-- BEGIN DZZY MANAGED — regenerated automatically, do not edit below -->\nold section\n<!-- END DZZY MANAGED -->\n\nafter\n",
        )
        .unwrap();
    ...
    assert!(result.contains("<!-- BEGIN DZZY MANAGED"));
    assert!(result.contains("<!-- END DZZY MANAGED -->"));
...
fn test_upsert_managed_section_without_markers() {
    ...
    let begin_pos = result.find("<!-- BEGIN DZZY MANAGED").unwrap();
...
fn test_upsert_marker_in_code_block() {
    ...
    fs::write(
        &file,
        "# Header\n\n    <!-- BEGIN DZZY MANAGED — some indented marker -->\n\nReal content here\n",
    )
    .unwrap();
    ...
    assert!(
        result.contains("    <!-- BEGIN DZZY MANAGED — some indented marker -->"),
    ...
    let begin_pos = result
        .find("<!-- BEGIN DZZY MANAGED — regenerated")
...
fn refresh_skill_md_writes_version_file() {
    ...
    let root = tmp.path().join(".dzzy");
    ensure_nest_at(&root).unwrap();
    let version = fs::read_to_string(root.join(".agents/skills/dzzy-cli/.skill-version")).unwrap();
...
fn refresh_skill_overwrites_on_version_bump() {
    ...
    let skill_md = root.join(".agents/skills/dzzy-cli/SKILL.md");
...
fn test_path_is_dev_nest_dev_path_returns_true() {
    let path = std::path::Path::new("/Users/someone/.dzzy-dev");
    assert!(
        path_is_dev_nest(path),
        ".dzzy-dev path must be identified as dev nest"
    );
}

#[test]
fn test_path_is_dev_nest_prod_path_returns_false() {
    let path = std::path::Path::new("/Users/someone/.dzzy");
    assert!(
        !path_is_dev_nest(path),
        ".dzzy path must not be identified as dev nest"
    );
}

#[test]
fn test_path_is_dev_nest_unrelated_path_returns_false() {
    let path = std::path::Path::new("/Users/someone/.dzzy-staging");
    assert!(
        !path_is_dev_nest(path),
