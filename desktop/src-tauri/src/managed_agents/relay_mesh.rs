...
pub const RELAY_MESH_API_KEY_PLACEHOLDER: &str = "dzzy-mesh-local";
pub const RELAY_MESH_PROVIDER_ID: &str = "relay-mesh";
pub const RELAY_MESH_AUTO_MODEL_ID: &str = "auto";

/// Translate the native Dzzy shared compute provider into the OpenAI-compatible
/// transport understood by dzzy-agent. These are derived runtime details, not
...
pub fn apply_relay_mesh_env(
    ...
    let model = model
        ...
        .to_string();
    env.insert("DZZY_AGENT_PROVIDER".to_string(), "openai".to_string());
    env.insert("DZZY_AGENT_MODEL".to_string(), model.clone());
    env.insert(
    ...
    env.insert("DZZY_AGENT_THINKING_EFFORT".to_string(), "none".to_string());
}
...
/// local mesh client endpoint created by Dzzy's relay-mesh preset.
///
...
fn relay_mesh_model_id_from_env(record: &ManagedAgentRecord) -> Option<String> {
    ...
    let provider = record.env_vars.get("DZZY_AGENT_PROVIDER")?.trim();
    if provider != "openai" {
        return None;
    }
...
mod tests {
    ...
    fn relay_mesh_model_id_detects_mesh_preset_env() {
        ...
        rec.env_vars = BTreeMap::from([
            ("DZZY_AGENT_PROVIDER".to_string(), "openai".to_string()),
            (
                "OPENAI_COMPAT_BASE_URL".to_string(),
    ...
    fn native_provider_uses_context_safe_non_reasoning_budget() {
        ...
        assert_eq!(
            env.get("DZZY_AGENT_MAX_OUTPUT_TOKENS").map(String::as_str),
            Some("4096")
        );
        assert_eq!(
            env.get("DZZY_AGENT_THINKING_EFFORT").map(String::as_str),
            Some("none")
        );
    }
