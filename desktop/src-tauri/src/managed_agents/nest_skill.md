...
name: dzzy-cli
description: >
  Dzzy CLI for relay operations: owner-reviewed agent drafts, messaging,
...
`DZZY_PRIVATE_KEY` is set by the harness at runtime or by the developer's environment. If missing, tell the user to set it (hex or nsec format). Never read or echo the value.

`DZZY_RELAY_URL` defaults to `http://localhost:3000`. In development, the user may need to set this to a staging or production relay URL.

`DZZY_AUTH_TAG` is required for `dzzy agents draft-create` and `dzzy agents draft-update` because those commands send owner-reviewed Desktop drafts. If missing, explain that this managed agent cannot open owner-reviewed agent drafts from chat.
...
```bash
dzzy --format compact channels list          # [{channel_id, name}]
dzzy --format compact messages get --channel <UUID>  # [{id, content, created_at}]
dzzy --format compact users get              # [{pubkey, display_name}]
dzzy --format compact feed get               # [{id, content, created_at}]
```
...
dzzy messages send --channel <UUID> --content "@Alice @Bob review please"
```
...
7. **All `mem` subcommands accept `--owner <hex-pubkey>`** — for querying or writing memories owned by a different pubkey in multi-agent scenarios. Defaults to the owner from `DZZY_AUTH_TAG`.
...
```bash
HASH=$(dzzy mem hash <slug>)                                    # 1. get current SHA-256
# ... build unified diff ...
dzzy mem patch <slug> --base-hash "$HASH" --patch-file diff.patch  # 2. apply with check
```
...
1. `dzzy messages get --channel <UUID> --limit 50` — note the maximum `created_at` from results
2. Sleep 10-30 seconds
3. `dzzy messages get --channel <UUID> --since <max_created_at> --limit 50`
