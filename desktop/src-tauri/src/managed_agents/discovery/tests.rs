...
use super::{
    ...
    refresh_login_shell_path, DZZY_AGENT_AVATAR_URL, CLAUDE_CODE_AVATAR_URL, CODEX_AVATAR_URL,
    GOOSE_AVATAR_URL,
};
...
#[test]
fn default_agent_command_resolves_bundled_dzzy_agent() {
    // The create-path default must be the bundled dzzy-agent, never the
    // bare `goose` that isn't on PATH on a stock Windows install.
    assert_eq!(default_agent_command(), "dzzy-agent");
    // And dzzy-agent takes no `acp` arg — confirm no arg leakage from the default.
    assert_eq!(
...
#[test]
fn resolves_dzzy_agent_avatar() {
    assert_eq!(
        managed_agent_avatar_url("dzzy-agent"),
        Some(DZZY_AGENT_AVATAR_URL.to_string())
    );
    assert_eq!(
        managed_agent_avatar_url("/usr/local/bin/dzzy-agent"),
        Some(DZZY_AGENT_AVATAR_URL.to_string())
    );
}
...
fn login_shell_lookup_treats_command_as_data() {
    let marker =
        std::env::temp_dir().join(format!("dzzy-discovery-marker-{}", uuid::Uuid::new_v4()));
...
fn explicit_path_resolution_ignores_non_executable_files() {
    ...
    let dir = std::env::temp_dir().join(format!("dzzy-discovery-path-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("dzzy-acp");
...
fn create_time_override_none_when_persona_runtime_not_installed() {
    ...
    // falls back to the default (`dzzy-agent`) and sends THAT command with
    ...
    // persona is re-edited. Baking `Some("dzzy-agent")` here is the exact
    // bug this resolver chain exists to kill.
    let personas = vec![persona_with_runtime("p1", Some("claude"))];
    assert_eq!(
        create_time_agent_command_override(Some("p1"), &personas, Some("dzzy-agent"), false),
        None
    );
}
...
fn probe_codex_acp_major_version_parses_1x_output() {
    ...
    let dir = std::env::temp_dir().join(format!("dzzy-probe-1x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
...
fn probe_codex_acp_major_version_returns_none_for_nonzero_exit() {
    ...
    let dir = std::env::temp_dir().join(format!("dzzy-probe-0x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
...
fn codex_adapter_availability_available_for_1x_binary() {
    ...
    let dir = std::env::temp_dir().join(format!("dzzy-avail-1x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(
        &bin,
...
fn codex_adapter_availability_outdated_for_0x_binary() {
    ...
    let dir = std::env::temp_dir().join(format!("dzzy-avail-0x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(&bin, "#!/bin/sh\nexit 1\n").expect("write script");
    std::fs::set_permissions(&bin, std::fs::Permissions::from_mode(0o755)).expect("chmod script");
...
fn probe_codex_acp_major_version_returns_none_for_hung_direct_child() {
    ...
    let dir = std::env::temp_dir().join(format!("dzzy-probe-hung-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(
        &bin,
...
fn probe_codex_acp_major_version_returns_version_when_descendant_holds_pipe_open() {
    ...
    let dir = std::env::temp_dir().join(format!("dzzy-probe-descendant-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(
        &bin,
