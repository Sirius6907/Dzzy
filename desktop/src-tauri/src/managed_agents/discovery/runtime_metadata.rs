...
pub(crate) struct KnownAcpRuntime {
    ...
    /// `Some(dir)` → Dzzy creates a symlink at `<nest>/<dir>/dzzy-cli`
    /// pointing to the canonical `.agents/skills/dzzy-cli`. `None` → this
    ...
    /// authenticated. `None` for runtimes that have no login step (goose, dzzy-agent).
