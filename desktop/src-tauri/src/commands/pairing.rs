...
use dzzy_core_pkg::kind::KIND_PAIRING;
use dzzy_core_pkg::pairing::qr::encode_qr;
use dzzy_core_pkg::pairing::session::PairingSession;
use dzzy_core_pkg::pairing::types::{AbortReason, PayloadType};
...
/// Parse a relay EVENT message into a nostr 0.36 Event (dzzy-core compatible).
...
mod pairing_relay_tests {
    ...
    fn configured_pairing_relay_takes_precedence_over_legacy_path() {
        let document = serde_json::json!({
            "pairing_relay_url": "wss://pairing.dzzy.xyz",
            "supported_nips": [43]
        });

        assert_eq!(
            pairing_relay_from_nip11(&document),
            PairingRelay::Configured("wss://pairing.dzzy.xyz".to_string())
        );
    }
    ...
    fn invalid_pairing_relay_url_falls_back_to_legacy_path() {
        let document = serde_json::json!({
            "pairing_relay_url": "https://pairing.dzzy.xyz",
            "supported_nips": [43]
        });
    ...
    fn configured_pairing_relay_resolves_to_configured_url() {
        let resolved = resolve_pairing_relay_url(
            "wss://flint.communities.dzzy.xyz",
            PairingRelay::Configured("wss://pairing.dzzy.xyz".to_string()),
        )
        .expect("resolve configured pairing relay");

        assert_eq!(resolved, "wss://pairing.dzzy.xyz");
    }

    #[test]
    fn legacy_pairing_relay_appends_pair_path() {
        let resolved = resolve_pairing_relay_url(
            "wss://flint.communities.dzzy.xyz/community",
            PairingRelay::LegacyPath,
        )
        .expect("resolve legacy pairing relay");

        assert_eq!(resolved, "wss://flint.communities.dzzy.xyz/community/pair");
    }
