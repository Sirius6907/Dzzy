...
/// `read_config_surface` tag those injected fields `DzzyExplicit`, then re-tag
...
/// (c) the reader tagged it `DzzyExplicit`. A value the user set explicitly in
/// Dzzy keeps `had_* == true` and is never re-tagged.
fn resolve_config_surface(
    ...
    let had_prompt =
        record.system_prompt.is_some() || record.env_vars.contains_key("DZZY_ACP_SYSTEM_PROMPT");
    ...
    //     record's own model is the baseline (DzzyExplicit). Gated behind
    ...
    let baseline = if had_model {
        if model_overridden {
            record
                .model
                .clone()
                .map(|m| (m, ConfigOrigin::DzzyExplicit))
        } else {
            None
        }
    } else {
    ...
    if !had_prompt {
        if let Some(p) = persona_prompt {
            record
                .env_vars
                .insert("DZZY_ACP_SYSTEM_PROMPT".to_string(), p);
        }
    }
    if !had_model {
    ...
    // Re-tag persona-sourced fields from DzzyExplicit to PersonaDefault.
    if !had_prompt {
    ...
    // Re-tag global-sourced fields from DzzyExplicit to GlobalDefault.
    if inject_global_model {
    ...
    // Re-tag persona-snapshotted model from DzzyExplicit to PersonaDefault.
    ...
    // is actually linked — non-persona agents with an explicit model keep DzzyExplicit.
...
/// Re-tag a field's origin from `DzzyExplicit` to `PersonaDefault`, leaving any
...
/// the binary at compile time via `DZZY_BUILD_AGENT_ENV`. The backend readiness
...
/// - `DZZY_AGENT_PROVIDER`, `DZZY_AGENT_MODEL` — agent runtime selection
/// - `DZZY_AGENT_THINKING_EFFORT` — non-secret enum (none/minimal/low/medium/high/xhigh/max)
...
///
/// Provider and model arrive as `DZZY_AGENT_PROVIDER` / `DZZY_AGENT_MODEL`
...
/// Re-tag a field's origin from `DzzyExplicit` to `GlobalDefault`, leaving any
/// other origin untouched. No-op when the field is absent.
...
mod tests {
    ...
    /// A model the user set explicitly in Dzzy must never be re-tagged to
    /// `PersonaDefault`, even when the linked persona also has a model.
    #[test]
    fn explicit_record_model_outranks_persona_and_keeps_dzzy_explicit_origin() {
    ...
    /// `DzzyExplicit` (its true source — NOT `PersonaDefault`). FAILS against the
    /// shipped no-persona early-return, which left X as primary and Y struck.
    #[test]
    fn genuine_explicit_live_switch_renders_y_over_x_dzzy_explicit_secondary() {
        let mut record = agent_record();
        record.persona_id = None;
        record.model = Some("model-x".to_string());
        let personas: Vec<AgentDefinition> = vec![];
        ...
        assert_eq!(model.overridden_origin, Some(ConfigOrigin::DzzyExplicit));
    }
    ...
    fn global_default_live_switch_renders_global_model_as_secondary_global_default() {
        ...
        assert_eq!(
            ...
            "override baseline origin must be GlobalDefault, not PersonaDefault or DzzyExplicit"
        );
    }
    ...
    fn baked_env_non_secret_key_shows_real_value() {
        let entries = baked_env_from_map(&[("DZZY_AGENT_PROVIDER", "databricks_v2")]);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].key, "DZZY_AGENT_PROVIDER");
    ...
    fn baked_env_empty_value_filtered_out() {
        let entries = baked_env_from_map(&[("DZZY_AGENT_PROVIDER", "")]);
    ...
    fn baked_env_mixed_keys_correct_masking() {
        let entries = baked_env_from_map(&[
            ...
            ("DZZY_AGENT_MODEL", "goose-claude-opus-4-8"),
        ...
        let provider = entries
            .iter()
            .find(|e| e.key == "DZZY_AGENT_PROVIDER")
            .unwrap();
    ...
    fn baked_env_thinking_effort_is_unmasked() {
        // DZZY_AGENT_THINKING_EFFORT is a non-secret enum — must not be masked.
        let entries = baked_env_from_map(&[("DZZY_AGENT_THINKING_EFFORT", "medium")]);
        assert_eq!(entries.len(), 1);
        let effort = entries
            .iter()
            .find(|e| e.key == "DZZY_AGENT_THINKING_EFFORT")
            .unwrap();
    ...
    fn baked_env_allowlist_is_case_insensitive() {
        ...
        assert!(super::is_safe_to_reveal("dzzy_agent_provider"));
        assert!(super::is_safe_to_reveal("DZZY_AGENT_PROVIDER"));
        assert!(super::is_safe_to_reveal("dzzy_agent_model"));
        assert!(super::is_safe_to_reveal("DZZY_AGENT_MODEL"));
        assert!(super::is_safe_to_reveal("dzzy_agent_thinking_effort"));
        assert!(super::is_safe_to_reveal("DZZY_AGENT_THINKING_EFFORT"));
