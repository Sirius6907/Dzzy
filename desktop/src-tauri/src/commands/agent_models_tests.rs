...
fn saved_agent_model_discovery_uses_record_snapshot() {
    let record: crate::managed_agents::ManagedAgentRecord = serde_json::from_str(
        r#"{
            ...
            "env_vars": {
                ...
                "DZZY_PRIVATE_KEY": "must-not-leak"
            },
    ...
    assert!(!config.env.contains_key("DZZY_PRIVATE_KEY"));
}
...
// ---------------------------------------------------------------------------
//
// Parse/filter/pagination tests live in crates/dzzy-agent/src/catalog.rs
...
fn update_request_turn_timeout_parses_for_wire_compat() {
    ...
    // (DZZY_ACP_TURN_TIMEOUT is deprecated and ignored by the harness). That
