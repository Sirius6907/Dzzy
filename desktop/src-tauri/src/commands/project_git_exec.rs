...
pub(crate) fn validate_clone_url(clone_url: &str) -> Result<(), String> {
    ...
    // Dzzy git remotes are served at `…/git/<owner-pubkey>/<repo-id>` — a
    ...
    let segments = parsed
        ...
        .unwrap_or_default();
    let is_dzzy_repo_path = segments
        .iter()
        ...
        .unwrap_or(false);
    if !is_dzzy_repo_path {
        return Err("clone URL must point at a Dzzy git repository".into());
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn validate_clone_url_requires_dzzy_repo_shape() {
