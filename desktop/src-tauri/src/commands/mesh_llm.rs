...
const RELAY_MESH_RUNTIME_NO_TARGET: &str =
    "Dzzy shared compute requires a live serving member; start serving the selected model on a member, then try again";
...
pub(crate) async fn resolve_trusted_owner_ids_or_self_only(state: &AppState) -> Vec<String> {
    match resolve_trusted_owner_ids(state).await {
        Ok(owners) => owners,
        Err(error) => {
            eprintln!("dzzy-mesh: roster query failed; allowing only this node: {error}");
            Vec::new()
        }
    }
}
...
fn mesh_readiness_failure_message(
    ...
    match failure {
        MeshReadinessFailure::CatalogNeverSynced => format!(
            "Dzzy shared compute connected to the serving member but could not sync \
        ...
        MeshReadinessFailure::RoutingNeverCompleted => format!(
            "Dzzy shared compute found \"{model_id}\" on a serving member but inference \
...
pub(crate) async fn ensure_client_node_for_model(
    ...
    let mut runtime = state.mesh_llm_runtime.lock().await;
    if runtime.is_some() {
        return Err("mesh node changed while starting Dzzy shared compute client".to_string());
    }
...
/// encrypted iroh relay addresses, so no Dzzy relay connection coordination is
...
pub(crate) async fn ensure_relay_mesh_for_record(
    ...
    let target = match resolve_mesh_bootstrap_target(&state, &model_id).await {
        ...
        Ok(None) => {
            return Err(
                "Dzzy shared compute cannot start because no live member is serving this model. Start serving it on a member, then try again."
                    .to_string(),
            );
        }
        Err(error) => {
            return Err(format!(
                "could not refresh Dzzy shared compute serving members: {error}"
            ));
        }
    };
...
mod tests {
    ...
    /// Dzzy shared compute as a client"). That forbade exactly what a user should be
    ...
    ///   cargo test -p dzzy-desktop --features mesh-llm \
