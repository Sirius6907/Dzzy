...
/// Query available models from an agent via `dzzy-acp models --json`.
///
...
pub async fn discover_agent_models(
    ...
    // Dzzy shared compute discovery must not depend on the local OpenAI ingress: that
        ...
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        ...
        .await
        .map_err(|error| format!("Dzzy shared compute model discovery failed: {error}"))?;
        ...
        if availability.models.is_empty() {
            return Err(availability.reason.unwrap_or_else(|| {
                "No live Dzzy shared compute models are available".to_string()
            }));
        }
    ...
    if input.provider.as_deref().map(str::trim)
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        return Err("Dzzy shared compute is not available in this build".to_string());
    }
...
// ---------------------------------------------------------------------------
//
// Delegates to dzzy_agent_pkg::catalog::discover_databricks_models, which
...
fn databricks_agent_provider(provider: &str) -> dzzy_agent_pkg::config::Provider {
    if provider.trim().eq_ignore_ascii_case("databricks_v2")
        || provider.trim().eq_ignore_ascii_case("databricks-v2")
    {
        dzzy_agent_pkg::config::Provider::DatabricksV2
    } else {
        dzzy_agent_pkg::config::Provider::Databricks
    }
}
...
async fn discover_databricks_models(
    ...
    let cfg = dzzy_agent_pkg::config::Config::for_discovery(agent_provider, api_key, host);
    ...
    let entries = match dzzy_agent_pkg::discover_databricks_models(&cfg).await {
        Ok(e) => e,
        Err(dzzy_agent_pkg::AgentError::LlmAuth(_)) => {
...
pub async fn update_managed_agent(
    ...
    let (summary, sync_params, rollback) = {
        ...
        // DZZY_ACP_TURN_TIMEOUT is deprecated and ignored by the harness.
...
/// Normalize raw `dzzy-acp models --json` output into a typed DTO for the frontend.
///
