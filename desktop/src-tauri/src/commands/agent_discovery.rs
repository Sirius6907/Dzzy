...
fn install_acp_runtime_blocking(runtime_id: &str) -> Result<InstallRuntimeResult, String> {
    ...
    // Dzzy-private prefix before execution.
...
async fn restart_single_agent_after_install(
    ...
    let runtime_keys = match stop_result {
        ...
        Ok(Err(e)) => {
            eprintln!("dzzy-desktop: install_acp_runtime: skipping restart of {pubkey}: {e}");
            return InstallRestartOutcome::Skipped;
        }
        Err(e) => {
            eprintln!(
                "dzzy-desktop: install_acp_runtime: spawn_blocking failed for stop of {pubkey}: {e}"
            );
            return InstallRestartOutcome::Skipped;
        }
    };
        ...
        .await
    {
        Ok(_) => {
            eprintln!(
                "dzzy-desktop: install_acp_runtime: restarted setup-mode agent {pubkey} after install"
            );
            InstallRestartOutcome::Restarted
        }
        Err(e) => {
            eprintln!(
                "dzzy-desktop: install_acp_runtime: failed to start {pubkey} after install: {e}"
            );
            if let Err(save_err) = persist_last_error_on_install(app, pubkey, &e) {
                eprintln!(
                    "dzzy-desktop: install_acp_runtime: failed to persist last_error for {pubkey}: {save_err}"
                );
            }
...
/// Dzzy-managed npm locations set, and the user's PATH set. This is the
...
///
/// On Windows, resolves Git Bash via `resolve_bash_path` (skips `DZZY_SHELL`
...
fn install_shell_command(command: &str) -> Result<std::process::Command, String> {
    ...
    let managed: Vec<std::path::PathBuf> = [
        crate::managed_agents::dzzy_managed_node_bin_dir(),
        crate::managed_agents::dzzy_managed_npm_bin_dir(),
    ]
    .into_iter()
    .flatten()
    .collect();
...
/// Windows: Git Bash via `resolve_bash_path` — skips `DZZY_SHELL` because install
/// commands use bash-only `-l -c` syntax. A `DZZY_SHELL=pwsh` user gets a green
...
/// Strips hermit-managed vars and establishes the Dzzy-managed npm prefix so adapters
...
fn apply_npm_env(cmd: &mut std::process::Command) {
    ...
    if let Some(prefix) = crate::managed_agents::dzzy_managed_npm_prefix() {
...
fn install_powershell_command(command: &str) -> std::process::Command {
    ...
    // Compose PATH: managed Dzzy dirs first, then inherited process PATH.
...
mod tests {
    ...
    fn test_install_shell_command_includes_process_path_on_windows() {
        ...
        // The sentinel must appear LAST — managed Dzzy dirs must have precedence.
        assert!(
    ...
    /// dzzy-agent has no install commands on any platform.
    #[test]
    fn test_dzzy_agent_has_no_install_commands() {
        let dzzy = crate::managed_agents::known_acp_runtime_exact("dzzy-agent").unwrap();
        assert!(
            dzzy.cli_install_commands_for_os().is_empty(),
            "dzzy-agent ships with the app — must never have install commands"
        );
    }
...
/// Returns the Windows-only Git Bash prerequisite used by dzzy-agent's shell MCP.
