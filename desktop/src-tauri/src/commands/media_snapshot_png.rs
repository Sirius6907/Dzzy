//! Preservation of Dzzy snapshot tEXt chunks through the upload sanitizer.
//!
...
//! (`dzzy_agent_snapshot` / `dzzy_team_snapshot`); the sanitizer's re-encode
...
//! relay allowlists exactly these keywords in `dzzy-media::validation` — the
//! two lists must stay in sync.

/// tEXt keywords that carry Dzzy snapshot manifests (`.agent.png` /
/// `.team.png`). These chunks are the product payload of agent/team sharing —
/// they must survive the metadata strip.
const PNG_SNAPSHOT_KEYWORDS: [&[u8]; 2] = [b"dzzy_agent_snapshot", b"dzzy_team_snapshot"];

/// Extract the raw bytes of the first Dzzy snapshot tEXt chunk (length + type
...
mod tests {
    ...
    fn test_sanitizer_preserves_agent_snapshot_text_chunk() {
        ...
        for keyword in ["dzzy_agent_snapshot", "dzzy_team_snapshot"] {
    ...
    fn test_agent_snapshot_survives_full_share_pipeline() {
        ...
        let snapshot = AgentSnapshot {
            format: "dzzy-agent-snapshot".to_string(),
            version: 1,
        ...
        // Relay ingest path.
        let relay_config = dzzy_media_pkg::MediaConfig {
        ...
        assert_eq!(
            dzzy_media_pkg::validation::validate_content(&sanitized, &relay_config)
