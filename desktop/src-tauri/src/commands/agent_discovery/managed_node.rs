...
fn managed_node_unsupported_step() -> InstallStepResult {
    InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "Dzzy does not provide a managed Node.js runtime for {}-{} yet",
            std::env::consts::OS,
            std::env::consts::ARCH
        ),
        exit_code: None,
        hint: Some(
            "Install Node.js from https://nodejs.org, restart Dzzy, then click Install again."
                .to_string(),
        ),
    }
}
...
fn managed_node_install_hint() -> String {
    "Dzzy could not install its private Node.js runtime. Check your network and app-data directory permissions, then click Install again.".to_string()
}
...
fn managed_node_runtime_ready() -> bool {
    let Some(node) = crate::managed_agents::dzzy_managed_node_bin_path() else {
        return false;
    };
    if !node.is_file() {
        return false;
...
pub(super) fn managed_node_runtime_supported() -> bool {
    MANAGED_NODE_ARTIFACT.is_some() && crate::managed_agents::dzzy_managed_node_bin_dir().is_some()
}
...
pub(super) fn ensure_managed_node_runtime_blocking() -> Result<(), Box<InstallStepResult>> {
    ...
    let Some(root) = crate::managed_agents::dzzy_managed_node_root() else {
        return Err(Box::new(managed_node_failed_step(
            "failed to resolve Dzzy app-data directory for private Node.js runtime".to_string(),
        )));
    };
...
/// Guidance text shown when the Dzzy-private npm prefix is not available.
fn managed_npm_prefix_hint() -> String {
    "Dzzy could not create its private Node tools directory. Check app-data directory permissions, restart Dzzy, then click Install again.".to_string()
}
...
pub(super) fn managed_npm_command(command: &str) -> Result<Option<String>, Box<InstallStepResult>> {
    ...
    let Some(prefix) = crate::managed_agents::dzzy_managed_npm_prefix() else {
        return Err(Box::new(InstallStepResult {
            step: "adapter".to_string(),
            command: command.to_string(),
            success: false,
            stdout: String::new(),
            stderr: "failed to resolve Dzzy app-data directory for private npm prefix".to_string(),
            exit_code: None,
    ...
    if let Err(error) = std::fs::create_dir_all(&prefix) {
        return Err(Box::new(InstallStepResult {
            ...
            stdout: String::new(),
            stderr: format!(
                "failed to create Dzzy private npm prefix '{}': {error}",
                prefix.display()
            ),
            exit_code: None,
            hint: Some(managed_npm_prefix_hint()),
...
pub(super) fn npm_eacces_hint(stderr: &str, _command: &str) -> Option<String> {
    if stderr.contains("EACCES: permission denied") || stderr.contains("npm error EACCES") {
        Some(
            "npm could not write to Dzzy's private Node tools directory. Check app-data directory permissions, restart Dzzy, then click Install again."
                .to_string(),
        )
    } else {
        None
...
mod tests {
    ...
    #[test]
    fn test_npm_eacces_hint_guidance_mentions_dzzy_private_dir() {
        let hint = npm_eacces_hint("EACCES: permission denied", "npm install -g foo").unwrap();
        assert!(
            hint.contains("Dzzy's private Node tools directory"),
            "hint: {hint}"
        );
    }
    ...
    fn test_rewrite_npm_install_uses_private_prefix() {
        assert_eq!(
            ...
            "npm install --global --prefix '/tmp/Dzzy Node' @agentclientprotocol/codex-acp"
        );
    }
    ...
    fn test_rewrite_npm_i_uses_private_prefix() {
        assert_eq!(
            rewrite_npm_global_install("npm i -g some-package", "'/tmp/dzzy'"),
            "npm i --global --prefix '/tmp/dzzy' some-package"
        );
    }
    ...
    fn test_rewrite_npm_uninstall_uses_private_prefix() {
        assert_eq!(
            rewrite_npm_global_install("npm uninstall -g @zed-industries/codex-acp", "'/tmp/dzzy'"),
            "npm uninstall --global --prefix '/tmp/dzzy' @zed-industries/codex-acp"
        );
    }
    ...
    fn test_rewrite_ignores_non_global_command() {
        assert_eq!(
            rewrite_npm_global_install("npm install foo", "'/tmp/dzzy'"),
            "npm install foo"
        );
    }
    ...
    fn test_shell_quote_escapes_single_quotes() {
        assert_eq!(
            shell_quote(std::path::Path::new("/tmp/Dzzy's Node")),
            "'/tmp/Dzzy'\\''s Node'"
        );
    }
