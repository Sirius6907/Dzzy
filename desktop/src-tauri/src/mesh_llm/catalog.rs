...
/// Dzzy-curated tier picks. These are the models we know survive the agent
...
const CURATED_LARGE_DESCRIPTION: &str =
    "Gemma 4 26B MoE (4B active) — Dzzy default for 64GB+ machines";
...
/// The Dzzy-curated recommendation for a machine's rated memory capacity.
fn dzzy_recommended_model(rated_gb: Option<u64>) -> &'static str {
    match rated_gb {
...
pub struct MeshCatalogEntry {
    ...
    /// Dzzy-curated pick — known to survive the agent harness. Curated
...
fn build_catalog(
    ...
    // The compiled MODEL_CATALOG does not know the Dzzy large pick; it
    ...
    let recommended = Some(dzzy_recommended_model(rated_capacity_gb(vram_bytes)).to_string());
...
mod tests {
    ...
    #[test]
    fn recommendation_follows_dzzy_curated_tiers() {
