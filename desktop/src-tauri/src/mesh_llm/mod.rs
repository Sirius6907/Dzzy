...
pub use coordinator::{start_coordinator, MeshCoordinator, KIND_DZZY_MESH_MEMBER_STATUS};
...
const MESH_STATUS_KIND: u64 = KIND_DZZY_MESH_MEMBER_STATUS as u64;
const MESH_API_PORT_ENV: &str = "DZZY_MESH_API_PORT";
const MESH_CONSOLE_PORT_ENV: &str = "DZZY_MESH_CONSOLE_PORT";
...
const MESH_IROH_RELAYS_ENV: &str = "DZZY_MESH_IROH_RELAYS";
    ...
    pub async fn start(mut request: StartMeshNodeRequest) -> anyhow::Result<Self> {
        ...
        let handle = match request.mode {
            MeshNodeMode::Serve => {
                ...
                // the Dzzy relay is the source of the roster; possession of a
...
pub fn models_from_status_payload(payload: Option<&serde_json::Value>) -> Vec<MeshModelOption> {
    ...
    if let Some(payload) = payload {
        ...
        // `runtime.models`. Dzzy-authored status reports use `models`. Do not
