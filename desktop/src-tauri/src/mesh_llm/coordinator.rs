...
//!
//! Dzzy publishes a client-signed, replaceable discovery note containing the
...
//! The Dzzy relay is only a generic Nostr store for membership and discovery;
...
/// NIP-51 bookmark-set kind with a reserved d-tag so existing Dzzy relays accept
/// and store it through their generic user-state path. The relay needs no mesh
/// handler or kind-registry change.
pub const KIND_DZZY_MESH_MEMBER_STATUS: u16 = dzzy_core_pkg::kind::KIND_BOOKMARK_SET as u16;
const STATUS_D_TAG_PREFIX: &str = "dzzy-mesh-member-status";
    ...
    let roster_watcher = tokio::spawn(async move {
        ...
        loop {
            ...
            if let Err(error) = reconcile_roster(&state, &mut pending_shrink).await {
                eprintln!("dzzy-mesh: roster reconcile failed: {error}");
            }
        }
    });
...
fn roster_reconcile_action(
    ...
    let fresh = match query {
        Err(error) => {
            eprintln!(
                "dzzy-mesh: roster reconcile query failed; keeping current allowlist: {error}"
            );
...
async fn reconcile_roster(
    ...
    let fresh = match roster_reconcile_action(current_owners, pending_shrink.as_deref(), query) {
        ...
        RosterReconcileAction::AwaitConfirm(reduced) => {
            eprintln!("dzzy-mesh: roster shrink observed; awaiting confirmation before restart");
    ...
    eprintln!("dzzy-mesh: membership roster changed; restarting mesh node with fresh allowlist");
    if let Err(error) = running.stop().await {
        eprintln!("dzzy-mesh: stopping mesh node for roster restart failed: {error}");
    }
...
pub(crate) async fn publish_current_status_once(app: &AppHandle, reason: &str) {
    ...
    {
        Ok(Ok(())) => {}
        Ok(Err(error)) => eprintln!("dzzy-mesh: status report after {reason} failed: {error}"),
        Err(_) => eprintln!("dzzy-mesh: status report after {reason} timed out"),
    }
}
...
pub(crate) async fn publish_stopped_status_once(app: &AppHandle, reason: &str) {
    ...
    .await
    {
        Ok(Ok(())) => {}
        Ok(Err(error)) => {
            eprintln!("dzzy-mesh: stopped status report after {reason} failed: {error}");
        }
        Err(_) => eprintln!("dzzy-mesh: stopped status report after {reason} timed out"),
    }
}
...
pub(crate) fn build_status_report_event(
    ...
    let k = Tag::parse(["k", "dzzy-mesh-status"]).map_err(|error| error.to_string())?;
    Ok(nostr::EventBuilder::new(
        nostr::Kind::Custom(KIND_DZZY_MESH_MEMBER_STATUS),
        payload.to_string(),
    )
    .tags([d, k]))
}
