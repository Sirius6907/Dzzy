...
//!
//! Mental model:
//!   app launch → start_stt_download (background) → ~/.dzzy/models/parakeet-tdt-ctc-110m-en/
//!   app launch → start_tts_download (background) → ~/.dzzy/models/pocket-tts/
//!   STT pipeline → is_stt_ready() → stt_model_dir() → run inference
//!   TTS pipeline → is_tts_ready() → tts_model_dir() → run synthesis
//!
//! Models are downloaded once and cached. A version manifest (`.dzzy-model-manifest`)
...
//! `~/.dzzy/models/moonshine-tiny/` is removed best-effort once the new STT
...
const MANIFEST_FILENAME: &str = ".dzzy-model-manifest";
...
(https://github.com/k2-fsa/sherpa-onnx); Dzzy ships this conversion
unmodified.
...
Dzzy ships all ONNX/model artifacts and the reference voice WAV unmodified,
...
#[derive(Clone)]
struct ModelSlot {
    dir_name: &'static str,                  // subdir under ~/.dzzy/models/
        ...
        Fut: std::future::Future<Output = Result<(), String>> + Send,
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(e) = download_fn(http_client).await {
                eprintln!("dzzy-desktop: {name} download failed: {e}");
...
impl ModelManager {
    /// Create a new `ModelManager` rooted at `~/.dzzy/models/`.
    ///
    /// Returns `None` if the home directory cannot be resolved.
    pub fn new() -> Option<Self> {
        let models_dir = dirs::home_dir()?.join(".dzzy").join("models");
        Some(Self {
            models_dir,
    ...
    async fn download_stt_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        eprintln!("dzzy-desktop: downloading STT model from {STT_DOWNLOAD_URL}");
        ...
        .await?;
        eprintln!("dzzy-desktop: downloaded {bytes} bytes, wrote to disk");
        ...
        eprintln!("dzzy-desktop: extracting STT archive…");
        ...
        eprintln!(
            "dzzy-desktop: STT model ready at {}",
    ...
    ///
    /// Downloads files into `~/.dzzy/models/pocket-tts/`:
    ///   - five ONNX sessions (Pocket TTS + Mimi codec)
    ///   - `vocab.json` / `token_scores.json` for sherpa-onnx text conditioning
    ///   - upstream `LICENSE` plus Dzzy's `MODEL_LICENSE.txt` attribution sidecar
    ...
    async fn download_tts_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        for (i, (url, filename)) in downloads.iter().enumerate() {
            eprintln!("dzzy-desktop: downloading Pocket TTS {filename} from {url}");
            ...
            eprintln!("dzzy-desktop: downloaded {bytes} bytes ({filename}), wrote to disk");
        ...
        eprintln!(
            "dzzy-desktop: Pocket TTS model ready at {}",
...
///
/// Removes `~/.dzzy/models/moonshine-tiny/` if present (~70 MB on disk).
...
async fn cleanup_legacy_moonshine_dir(models_dir: &Path) {
    ...
    match tokio::fs::remove_dir_all(&legacy).await {
        Ok(()) => eprintln!(
            "dzzy-desktop: removed legacy STT model dir {}",
            legacy.display()
        ),
        Err(e) => eprintln!(
            "dzzy-desktop: could not remove legacy STT model dir {}: {e} \
