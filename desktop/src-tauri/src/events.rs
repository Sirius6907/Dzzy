...
//!
//! Mirrors the dzzy-sdk builder patterns but uses nostr 0.37 API
...
use dzzy_core_pkg::kind::{KIND_IA_ARCHIVE_REQUEST, KIND_IA_UNARCHIVE_REQUEST};
use nostr::{EventBuilder, EventId, Kind, Tag};
use uuid::Uuid;

// ── Constants ────────────────────────────────────────────────────────────────

/// Maximum content size — matches dzzy-sdk (64 KiB).
const MAX_CONTENT_BYTES: usize = 64 * 1024;

/// Maximum mention count — matches dzzy-sdk.
const MAX_MENTIONS: usize = 50;

/// Maximum emoji length in characters — matches dzzy-sdk.
...
pub fn build_create_channel(
    ...
    let name = dzzy_sdk_pkg::canonical_channel_name(name);
...
pub fn build_update_channel(
    ...
    let name = name.map(dzzy_sdk_pkg::canonical_channel_name);
