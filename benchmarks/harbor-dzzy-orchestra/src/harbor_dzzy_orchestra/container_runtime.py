"""Run the production Dzzy agent stack inside the Harbor task container.

Each provisioned identity is a full ``dzzy-acp`` → ``dzzy-agent`` →
``dzzy-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``dzzy`` CLI on
PATH) that the desktop app gives a Dzzy agent. The harness stays outside:
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Dzzy agent process cannot be launched or exits early."""
...
class DzzyContainerRuntime:
    """Launch one production Dzzy agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        dzzy_acp_binary: str = "dzzy-acp",
        dzzy_agent_binary: str = "dzzy-agent",
        dzzy_dev_mcp_binary: str = "dzzy-dev-mcp",
        dzzy_cli_binary: str = "dzzy",
        relay_gateway: str = "",
        ...
        self.dzzy_acp_binary = dzzy_acp_binary
        self.dzzy_agent_binary = dzzy_agent_binary
        self.dzzy_dev_mcp_binary = dzzy_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.dzzy_cli_binary = dzzy_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        if not workers:
            raise RuntimeLaunchError("Dzzy orchestration requires at least one worker")
        trial_dir = self.logs_dir / "dzzy"
        ...
        try:
            ...
            # The task arrives exactly as it would in production Dzzy: a
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/dzzy-acp": self.dzzy_acp_binary,
            f"{REMOTE_BIN}/dzzy-agent": self.dzzy_agent_binary,
            f"{REMOTE_BIN}/dzzy-dev-mcp": self.dzzy_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/dzzy-acp')} </dev/null "
    ...
    def _agent_env(
        ...
        *,
        ...
        return {
            **endpoint.env,
            "DZZY_RELAY_URL": trial.relay_ws_url,
            "DZZY_PRIVATE_KEY": credential.nostr_secret_key,
            # Desktop parity: the GUI also sets NOSTR_PRIVATE_KEY on dzzy-acp
            # so dzzy-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "DZZY_AUTH_TAG": credential.nostr_auth_tag,
            "DZZY_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/dzzy-agent",
            "DZZY_ACP_AGENT_ARGS": "",
            "DZZY_ACP_MCP_COMMAND": f"{REMOTE_BIN}/dzzy-dev-mcp",
            "DZZY_ACP_CHANNELS": trial.channel_id,
            "DZZY_ACP_SUBSCRIBE": "mentions",
            "DZZY_ACP_RESPOND_TO": "anyone",
            "DZZY_ACP_NO_MEMORY": "true",
            "DZZY_ACP_SYSTEM_PROMPT_FILE": remote_prompt,
            "DZZY_AGENT_PROVIDER": endpoint.provider,
            "DZZY_AGENT_MODEL": credential.llm_endpoint,
            "DZZY_AGENT_MAX_OUTPUT_TOKENS": str(
                agent_class.generation.max_output_tokens
            ),
            "DZZY_AGENT_MAX_CONTEXT_TOKENS": str(
                agent_class.generation.context_window_tokens
            ),
            "DZZY_AGENT_MAX_ROUNDS": str(
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._dzzy_json(
                trial.user, trial,
    ...
    async def _dzzy_json(
        self, credential: AgentCredential, trial: TrialHandle, *args: str
    ) -> Any:
        process = await asyncio.create_subprocess_exec(
            self.dzzy_cli_binary,
            *args,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env={
                **os.environ,
                "DZZY_RELAY_URL": self._user_relay_url(trial),
                "DZZY_PRIVATE_KEY": credential.nostr_secret_key,
                "DZZY_AUTH_TAG": credential.nostr_auth_tag,
            },
        )
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"dzzy {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("dzzy returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return DzzyContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Dzzy workspace's team context: each agent
    ...
    def _reject_identity_overrides(endpoint: EndpointLaunchConfig) -> None:
        forbidden = {
            ...
            "DZZY_AUTH_TAG",
            "DZZY_ACP_CHANNELS",
            "DZZY_ACP_MCP_COMMAND",
            "DZZY_ACP_AGENT_COMMAND",
        }
