...
importable:
    ...
    uv run --project benchmarks/harbor-dzzy-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux dzzy-acp/dzzy-agent/dzzy-dev-mcp>
"""
...
AGENT_IMPORT = "harbor_dzzy_orchestra:DzzyOrchestraAgent"
PROVISIONER_FACTORY = "harbor_dzzy_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="JSON config for the Dzzy relay/Postgres provisioner",
    )
    parser.add_argument(
        "--dzzy-bin-dir", type=Path, default=None,
        help="Directory with the host dzzy CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir", type=Path, required=True,
        help="Directory with Linux builds of dzzy-acp/dzzy-agent/dzzy-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"dzzy binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --dzzy-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "dzzy_acp_binary": agent_binaries["dzzy-acp"],
        "dzzy_agent_binary": agent_binaries["dzzy-agent"],
        "dzzy_dev_mcp_binary": agent_binaries["dzzy-dev-mcp"],
        "dzzy_cli_binary": binaries["dzzy"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/block/dzzy",
        "agent_display_name": f"Dzzy Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.dzzy_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.dzzy_bin_dir)
