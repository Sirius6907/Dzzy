#!/usr/bin/env python3
"""One-command benchmark: bring up the Dzzy stack in Docker and run it.
...
- A dedicated ``dzzy-benchmark`` compose project reusing the production
...
- ``--gui`` adds that user to the relay membership list and opens the Dzzy
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/harbor-dzzy-orchestra/testbed \
        benchmarks/harbor-dzzy-orchestra/scripts/benchmark.py [--gui] [...]
"""
...
COMPOSE_PROJECT = "dzzy-benchmark"
COMPOSE_FILES = (
...
GUI_BUNDLE_IDENTIFIER = "xyz.block.dzzy.app.benchmark"
...
AGENT_BINARIES = ("dzzy-acp", "dzzy-agent", "dzzy-dev-mcp")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Open the Dzzy desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
...
def load_state() -> dict[str, str]:
    ...
    from harbor_dzzy_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_dzzy_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        "DZZY_IMAGE": os.environ.get("DZZY_IMAGE", "ghcr.io/block/dzzy:main"),
        "DZZY_DOMAIN": "localhost",
        "RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
        "DZZY_MEDIA_BASE_URL": f"http://localhost:{RELAY_HTTP_PORT}/media",
        "DZZY_MEDIA_SERVER_DOMAIN": "localhost",
        "DZZY_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "DZZY_REQUIRE_AUTH_TOKEN": "true",
        "DZZY_REQUIRE_RELAY_MEMBERSHIP": "true",
        "DZZY_ALLOW_NIP_OA_AUTH": "true",
        "DZZY_AUTO_MIGRATE": "true",
        "DZZY_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "dzzy_relay=info,dzzy_db=info,dzzy_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "DZZY_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "DZZY_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
        "POSTGRES_DB": "dzzy",
        "POSTGRES_USER": "dzzy",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "TYPESENSE_API_KEY": state["typesense_api_key"],
        "DZZY_S3_ACCESS_KEY": state["s3_access_key"],
        "DZZY_S3_SECRET_KEY": state["s3_secret_key"],
        "DZZY_S3_BUCKET": "dzzy-media",
        "DZZY_HTTP_PORT": str(RELAY_HTTP_PORT),
        "DZZY_PG_HOST_PORT": str(PG_HOST_PORT),
        "DZZY_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return (
        f"postgresql://dzzy:{state['postgres_password']}"
        f"@127.0.0.1:{PG_HOST_PORT}/dzzy"
    )
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host dzzy CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host dzzy CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "dzzy-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    dzzy-acp → dzzy-agent → dzzy-dev-mcp stack, so the binaries must be
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the Dzzy desktop app logged in as the benchmark user.

    The relay runs closed (membership required), so the user pubkey is first
    added to the relay membership list via dzzy-admin inside the container —
    ...
    subprocess.run(
        compose_command(
            "exec", "-T", "relay",
            "dzzy-admin", "add-member", "--pubkey", state["user_pubkey"],
        ),
        check=True,
    )
    ...
    for name in ("dzzy-acp", "dzzy-agent", "dzzy-dev-mcp", "git-credential-nostr", "dzzy"):
        stub = sidecar_dir / f"{name}-{triple}"
        if not stub.exists():
            stub.touch()
    real_cli = sidecar_dir / f"dzzy-{triple}"
    real_cli.write_bytes(binaries["dzzy"].read_bytes())
    real_cli.chmod(0o755)

    print(
        f"Opening Dzzy GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    # workspace's relay URL overrides DZZY_RELAY_URL by design. Reusing the
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "Dzzy Benchmark"}
    )
    return subprocess.Popen(
        ...
        env={
            **os.environ,
            "DZZY_RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
            "DZZY_PRIVATE_KEY": state["user_secret_key"],
        },
    )
