# Harbor Dzzy Orchestra

A stock-Harbor custom agent that runs a manifest-defined team through the real
Dzzy stack. Harbor sees one `DzzyOrchestraAgent`; behind that adapter, one
...
`dzzy-acp` → `dzzy-agent` → `dzzy-dev-mcp` process tree the desktop app
launches: the production MCP toolset (shell, file tools, todo) with the
`dzzy` CLI on the shell's PATH. No Harbor fork or patch is required.
...
```bash
uv run --project benchmarks/harbor-dzzy-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent harbor_dzzy_orchestra:DzzyOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=harbor_dzzy_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/harbor-dzzy-orchestra --agent-kwarg dzzy_acp_binary=<LINUX_BIN>/dzzy-acp --agent-kwarg dzzy_agent_binary=<LINUX_BIN>/dzzy-agent --agent-kwarg dzzy_dev_mcp_binary=<LINUX_BIN>/dzzy-dev-mcp --agent-kwarg dzzy_cli_binary=target/debug/dzzy --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```

`dzzy_acp_binary`/`dzzy_agent_binary`/`dzzy_dev_mcp_binary` must be **Linux**
...
any Linux base image works). `dzzy_cli_binary` is the **host** CLI the harness
...
Each trial gets fresh keys and a private Dzzy channel. The provisioner archives
rather than deletes that channel, leaving the relay/Postgres event timeline
and the per-agent acp/agent logs (downloaded into the trial's `dzzy/`
artifacts) available for analysis.

## Leaderboard runs

`just benchmark` is the one-command path: it stands up a dedicated Docker
stack (`dzzy-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
relay membership list and opens the Dzzy desktop app logged in as them, so
...
uv run --project benchmarks/harbor-dzzy-orchestra/testbed \
    benchmarks/harbor-dzzy-orchestra/scripts/run_leaderboard.py \
    --dataset terminal-bench/terminal-bench-2-1 \
    --attempts 5 \
    --manifest benchmarks/harbor-dzzy-orchestra/manifests/<TEAM>.yaml \
    --endpoint-config benchmarks/harbor-dzzy-orchestra/testbed/endpoints/<ENDPOINTS>.json \
...
```bash
cd benchmarks/harbor-dzzy-orchestra
