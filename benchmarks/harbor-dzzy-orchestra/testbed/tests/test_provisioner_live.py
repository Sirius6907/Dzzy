...
Gated by DZZY_TESTBED_LIVE=1 with stack coordinates in the environment:
  DZZY_TESTBED_RELAY_HTTP   (default http://localhost:3000)
  DZZY_TESTBED_RELAY_WS     (default ws://host.docker.internal:3000)
  DZZY_TESTBED_OWNER_KEY    relay owner secret key (hex)
  DZZY_TESTBED_PG_DSN       benchmark Postgres DSN
"""
...
pytestmark = pytest.mark.skipif(
    os.environ.get("DZZY_TESTBED_LIVE") != "1",
    reason="live testbed suite; set DZZY_TESTBED_LIVE=1 with a running stack",
)


@pytest.fixture()
def provisioner() -> DzzyTrialProvisioner:
    owner_key = os.environ.get("DZZY_TESTBED_OWNER_KEY")
    dsn = os.environ.get("DZZY_TESTBED_PG_DSN")
    if not owner_key or not dsn:
        pytest.fail("DZZY_TESTBED_OWNER_KEY and DZZY_TESTBED_PG_DSN are required")
    return DzzyTrialProvisioner(
        TestbedConfig(
            relay_http_url=os.environ.get(
                "DZZY_TESTBED_RELAY_HTTP", "http://localhost:3000"
            ),
            relay_ws_url=os.environ.get(
                "DZZY_TESTBED_RELAY_WS", "ws://host.docker.internal:3000"
            ),
...
def cli_for(provisioner: DzzyTrialProvisioner, credential) -> DzzyCli:
...
def test_create_is_idempotent_and_isolated(provisioner, manifest):
    ...
    try:
        ...
        with pytest.raises(DzzyCliError, match="private"):
    ...
    provisioner.teardown(handle_a)
    with psycopg.connect(os.environ["DZZY_TESTBED_PG_DSN"]) as conn:
        row = conn.execute(
