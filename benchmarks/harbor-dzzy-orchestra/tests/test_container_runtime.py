...
def test_prompt_hash_and_identity_override_are_fail_closed(tmp_path):
    ...
    endpoint = EndpointLaunchConfig(
        "anthropic", "ANTHROPIC_API_KEY", {"DZZY_ACP_MCP_COMMAND": "evil"}
    )
...
async def test_install_stack_uploads_the_pinned_stack(tmp_path):
    binaries = {}
    for name in ("dzzy-acp", "dzzy-agent", "dzzy-dev-mcp"):
        path = tmp_path / name
        path.write_text("#!binary")
        binaries[name] = str(path)
    rt = runtime(
        tmp_path,
        dzzy_acp_binary=binaries["dzzy-acp"],
        dzzy_agent_binary=binaries["dzzy-agent"],
        dzzy_dev_mcp_binary=binaries["dzzy-dev-mcp"],
    )
    environment = Environment()
    await rt._install_stack(environment)
    assert {target for _, target in environment.uploads} == {
        f"{REMOTE_BIN}/dzzy-acp",
        f"{REMOTE_BIN}/dzzy-agent",
        f"{REMOTE_BIN}/dzzy-dev-mcp",
    }
...
async def test_install_stack_requires_binaries_on_disk(tmp_path):
    rt = runtime(tmp_path, dzzy_acp_binary=str(tmp_path / "missing"))
...
async def test_forwarder_bridges_the_canonical_relay_address(tmp_path):
    from harbor_dzzy_orchestra.container_runtime import FORWARDER
...
async def test_launch_wires_the_desktop_environment(tmp_path, configured, expected):
    ...
    environment = Environment(
        responses={"dzzy-acp": ExecResult(stdout="4242\n", stderr="", return_code=0)}
    )
    ...
    assert f"{REMOTE_BIN}/dzzy-acp" in command
    # The real product wiring: acp spawns dzzy-agent, which gets dzzy-dev-mcp.
    assert env["DZZY_ACP_AGENT_COMMAND"] == f"{REMOTE_BIN}/dzzy-agent"
    assert env["DZZY_ACP_MCP_COMMAND"] == f"{REMOTE_BIN}/dzzy-dev-mcp"
    assert env["DZZY_RELAY_URL"] == trial.relay_ws_url
    assert env["DZZY_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["NOSTR_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["DZZY_AGENT_NO_HINTS"] == "1"
    assert env["DZZY_AGENT_MAX_ROUNDS"] == expected
    assert env["DZZY_ACP_SYSTEM_PROMPT_FILE"].endswith("orch-1.system-prompt.md")
    # The composed prompt was uploaded into the container.
    assert any(
        target == env["DZZY_ACP_SYSTEM_PROMPT_FILE"]
...
async def test_wait_for_agents_ready_requires_every_channel_subscription(tmp_path):
    ...
    from harbor_dzzy_orchestra.container_runtime import _Agent
...
async def test_wait_for_done_requires_orchestrator_authorship(tmp_path, monkeypatch):
    ...
    async def dzzy_json(credential, *args, **kwargs):
        observers.append(credential.agent_id)
        return next(rounds)

    monkeypatch.setattr(rt, "_dzzy_json", dzzy_json)
...
async def test_stop_agents_sweeps_the_uploaded_stack(tmp_path):
    ...
    await DzzyContainerRuntime._stop_agents(environment, agents)
