...
from harbor_dzzy_orchestra import (
    AgentCredential,
    DzzyOrchestraAgent,
    RuntimeResult,
    TrialHandle,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = DzzyOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = DzzyOrchestraAgent(
        ...
        dzzy_acp_binary="/pinned/dzzy-acp",
        dzzy_agent_binary="/pinned/dzzy-agent",
        dzzy_dev_mcp_binary="/pinned/dzzy-dev-mcp",
        dzzy_cli_binary="/pinned/dzzy",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.dzzy_acp_binary == "/pinned/dzzy-acp"
    assert agent.runtime.dzzy_agent_binary == "/pinned/dzzy-agent"
    assert agent.runtime.dzzy_dev_mcp_binary == "/pinned/dzzy-dev-mcp"
    assert agent.runtime.dzzy_cli_binary == "/pinned/dzzy"
